...
| 387 | The R2B pre-open hold window has no tracked evidence set, so its eight published differences cannot be audited | Medium | Medium | Open |
...
### Phase 1 — Statistical Foundation (Issues 1 ✓, 22 ✓, 2 ✓, 3 ✓, 24 ✓, 75 ✓, 157 ✓, 158 ✓, 189 ✓, 186 ✓, 195 ✓, 208 ✓, 331 ✓, 296 ✓, 382 ✓, 384 ✓)
...
UNBLOCKED (start now):
  ...
  #387 The R2B pre-open hold window prints eight paired differences with
       intervals from 50 replications per arm and tracks none of them.
       Invoked as run.R under a parameter override rather than from a driver
       script, so it needs a runner of its own before it needs compute.
       Carries a decision: regenerate at 50 to reproduce the published table,
       or at about 120 to also resolve the forward-surgery row the paper
       labels unresolved. Unblocked by the merge of #384.
  #388 The post-operative intensive care gate, same shape and same reason.
       ...
       about the magnitudes. Carries the decision of whether it shares #387's
       runner module, so the two are best sequenced together. Unblocked by
       the merge of #384.
