...
The table below is `outputs/morris_ranking.csv` from the last full re-run (r=20, 5 reps, 30 days, seed 42; Issue #73 follow-up), ranked by µ\* on the system OT queue, with the since-removed `return_leg_multiplier` row struck out:

| Rank | Parameter | µ\* | σ |
|---|---|---|---|
| 1 | `r1_transport` | 0.0259 | 0.0346 |
| 2 | `ot_hours` | 0.0244 | 0.0320 |
| 3 | `long_resus_mode` | 0.0220 | 0.0242 |
| 4 | `p1_p_max` | 0.0190 | 0.0234 |
| 5 | `surg_mode` | 0.0189 | 0.0159 |
| 6 | `r2b_transport` | 0.0174 | 0.0233 |
| 7 | `in_theatre_rate` | 0.0174 | 0.0243 |
| 8 | `pri1_surg_prob` | 0.0151 | 0.0156 |
| ~~9~~ | ~~`return_leg_multiplier`~~ | ~~0.0132~~ | ~~0.0162~~ |
| 9 | `long_icu_mode` | 0.0116 | 0.0156 |

No parameter dominates on this KPI: the remaining nine parameters' µ\* values span a narrow range (0.0116–0.0259) rather than showing the order-of-magnitude separation that would identify one or two clearly rate-limiting inputs. Every parameter's σ is comparable to or exceeds its µ\* (σ/µ\* ranging 0.84–1.34), indicating nonlinear or interaction-dominated effects throughout rather than the simple linear effects a low-σ, high-µ\* parameter would show — consistent with the branching, resource-contingent routing logic in `R/trajectories.R`, where a parameter's effect depends on which branch a casualty happens to take. The reshuffling elsewhere in this table relative to the Issue #75 version (e.g. `pri1_surg_prob` falling from 1st to 8th, `r1_transport` rising from 5th to 1st) reflects the same RNG-stream-shift effect documented throughout this project for any change that adds or removes random draws (Issues #5, #6, #24, and now #73's follow-up) — each design point's µ\*/σ is computed under a different realised stochastic environment, so some reordering among closely-clustered parameters is expected noise, not necessarily a change in true influence.

**Issue #74 — `return_leg_multiplier` removed after screening, not before:** the finding that motivated this table's most recent full re-run — `return_leg_multiplier` ranking last on system OT queue but becoming the most influential of all ten screened parameters on mean transport utilisation and total DOW count once its scope doubled to four legs — is exactly what prompted its removal (see Limitation L15). The nine remaining rows above are the last full 10-parameter design's values with that row dropped, not a freshly regenerated 9-parameter design; because Morris's one-at-a-time design samples all screened parameters' starting points jointly, dropping a parameter from the design changes the sampling for every other parameter too, so these figures are indicative of the removal's negligible impact on the remaining nine (a full re-run cannot change any of them by more than a rounding-scale amount, since the only code-level change was deleting a `* 1.0` multiplication) rather than a re-derived ground truth. A fresh nine-parameter Morris re-run to regenerate exact figures is tracked as a follow-up (see Further Development).
...
**L14 — R2B/R2E Surgery Duration: Narrative Text Diverges from Shipped `env_data.json` (Low Impact on Documentation Accuracy)**
The [R2B Trajectory](#r2b-trajectory) and [R2E Heavy Trajectory](#r2e-heavy-trajectory) narrative text states the DAMCON surgery duration as a triangular distribution with `min = 41`, `max = 210`, `mode = 95` minutes, citing DCS meta-analyses. The shipped `env_data.json` instead configures `min = 90`, `max = 240`, `mode = 120` minutes for both `r2b.surgery` and `r2eheavy.surgery` — every documented seed-42 baseline result in this README (including the Key Parameters table in `CLAUDE.md`) was generated against the 90/240/120 configuration, not the 41/210/95 figures quoted in the narrative prose. This was found while sourcing citations for the Shiny app's Configure panel tooltips (Issue #14); the tooltip for this field flags the discrepancy directly rather than asserting either value as authoritative. **Impact: Low** — no simulation output is affected (the discrepancy is between two pieces of documentation/config, not between documentation and behaviour), but the narrative text should be corrected to match the shipped configuration, or the configuration re-derived from the cited meta-analyses if the narrative's 41/210/95 figures are the intended target — either resolution is out of scope for this change; tracked as Issue #76.
...
That same finding led directly to `return_leg_multiplier`'s removal from the model (Issue #74; see [Sensitivity Analysis](#sensitivity-analysis) and Limitation L15). The nine-parameter Morris table currently shown is the prior ten-parameter design with the removed row struck out, not a freshly regenerated design — a full nine-parameter re-run (r=20, 5 reps, 30 days, seed 42) to obtain exact current µ\*/σ figures for the remaining parameters is a low-effort follow-up, expected to change no ranking materially given the removal was RNG-neutral.
