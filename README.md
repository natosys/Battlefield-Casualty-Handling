...
Nothing queues for a theatre here, nothing is refused and nothing is pushed back into theatre. This is a requirement the deployed system generates, on the same footing as the bed demand beside it, and the model gives the national support base no theatre establishment for it to fall short of. Two limits attach. The report covers this one population: the repeated procedures a repatriated casualty undergoes over the following weeks, delayed primary closure, washouts, fixation and staged reconstruction, are not represented. A series at one national support base reports 264 operations across 50 combat casualties, 5.2 per patient, over three weeks following the Afghanistan withdrawal [[63]](#references), so what is reported here is a small part of the theatre demand that echelon actually carries (README Further Development L33). And the population is empty at the shipped configuration, the release being disabled, so every figure this project reports carries no Role 4 theatre demand at all.
...
Three aircraft ship, all from the RAAF's account of its aeromedical evacuation capability [[64]](#references), which reports that an AME-configured C-17A "can transport 54 ambulatory and 36 high dependency stretcher patients" and that "the C-130J and C-27J can carry 97 and 21 stretcher patients respectively":
...
| L33 | Every definitive repair is performed in theatre at the shipped configuration, and repeated Role 4 procedures are not represented | Medium |
...
**L33 — Every definitive repair is performed in theatre at the shipped configuration, and repeated Role 4 procedures are not represented.** The model can release a damage control casualty to strategic evacuation with the definitive repair outstanding, when the number waiting to be operated on at R2E reaches `second_surgery.saturation_queue_threshold` (see [R2E Heavy Trajectory](#r2e-heavy-trajectory)), and such a casualty now carries that operation rearward as reported theatre demand rather than losing it at boarding (see [Operating Theatre Demand](#operating-theatre-demand)). The threshold nonetheless ships at zero and every figure this project reports is measured with the release disabled, so the published results carry the same understatement of Role 4 intensive care demand, and the same overstatement of forward theatre demand, that they carried before the lever existed; the coupling to the evacuation policy is unchanged with them, so the policy comparison reported in the replicated analysis understates what shortening the threshold asks of the national support base. The threshold cannot simply be turned on, because the trigger is an informed estimate: no open-access source reports the surgical load at which a theatre begins to evacuate between the two operations, or the share of casualties it does so for. Separately, and for every repatriated casualty rather than that one cohort, the repeated procedures a national support base performs over the following weeks, delayed primary closure, washouts, fixation and staged reconstruction, are not represented at all, so its total theatre demand is understated whatever the threshold is set to; a reported 5.2 operations per casualty [[63]](#references) is the order of what is missing. Closing this means calibrating the threshold against a source that reports the behaviour, and extending the theatre report from the one conserved operation to the procedure stream a repatriated casualty generates, which in turn needs the evacuation decision to carry an expected reconstruction burden rather than being taken on system load and recovery duration alone, the procedures a casualty will require once repatriated being invisible to a threshold on their expected time to being fit for duty.
...
[64] Royal Australian Air Force. *Aeromedical evacuation*. Australian Government, Department of Defence. Retrieved 02 Aug 26, from https://www.airforce.gov.au/our-work/humanitarian-support/aeromedical-evacuation

[65] U.S. Air Force. *C-130 Hercules*. Air Force Fact Sheet. Retrieved 02 Aug 26, from https://www.af.mil/About-Us/Fact-Sheets/Display/Article/1555054/c-130-hercules

[66] Goel, M. K., Khanna, P., & Kishore, J. (2010). Understanding survival analysis: Kaplan-Meier estimate. *International Journal of Ayurveda Research*, *1*(4), 274-278. Retrieved 11 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC3059453/

[67] Royston, P., & Parmar, M. K. B. (2013). Restricted mean survival time: an alternative to the hazard ratio for the design and analysis of randomized trials with a time-to-event outcome. *BMC Medical Research Methodology*, *13*, 152. Retrieved 11 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC3922847/

[68] Hall, A., Graham, B., Hanson, M., & Stern, C. (2023). Surgical capability utilization time for military casualties at role 2 and role 3 facilities. *Military Medicine*, *188*(11-12), e3368-e3370. Retrieved 10 Aug 26, from https://academic.oup.com/milmed/article/188/11-12/e3368/6961509
