...
### PR #294 — Repair and Extend the Single-Run Paper's Revised Register ✓

**Merged:** PR #294, branch `claude/single-run-analysis-review-brj6pw`

No issue was raised for this work, which was taken up directly from a review of
the author's revision to the Abstract and Methods of
`docs/Single_Run_Analysis.md`. It is recorded here under its pull request number
rather than an issue number, and carries no row in the summary table, no phase
sequence item and no dependency graph node, on the precedent PRs #278, #288 and
#292 set.

That revision reframed the paper from a verification report to a planning study,
and in removing the Warm-Up Treatment and Evidence Set and Provenance headings it
left three defects the check suite would have caught: references [3], [4] and [5]
were each cited only in the deleted text and so became uncited entries, the
contents block pointed at two anchors that no longer existed, and the Strategic
Evacuation section linked to a removed anchor for the baseline evidence set. The
first commit repairs all three by restoring the arguments those references
carried, at one to two sentences each rather than the sections they had occupied:
the warm-up justification folded into Model and Run Configuration, and the
verification framing and the single-draw caveat into Scope of a Single Run, which
the Limitations section still cites for the latter. A sentence fragment, a comma
splice, a missing hyphen and a missing space in the newly written prose are fixed
in the same commit.

The second commit carries the revision's editorial position through the remaining
sections. Every section now opens on its finding in the bolded lead sentence the
R2B and R2E sections already used and closes on what that finding means for the
establishment a planner sets; results are stated in the past tense throughout,
matching the Abstract, with the present tense reserved for describing how the
model works. Implementation detail is removed from the narrative on the same
grounds it was removed from Methods, attribute and function names, output file
paths and the assertion holding battle fatigue out of R2B holding being facts
about the code rather than about the modelled system. The hold bed saturation
section is the largest change, its four "Intervention Scenario" blocks and their
`env_data.json` edit recipes rewritten as prose covering the three remedies open
to a planner and what each costs, with the structural derivation, the
15.5-against-10 gap and the two caveats bounding it unchanged. The waiting time
and transport sections now lead with prose ahead of their figure as every other
section does, and the Conclusion drops its restatement of the intensive care
gate's role, which now sits once in R2E Heavy Handling where the mechanism is
explained.

One markdown file changes, +70/−95 across two commits. No figure, count or
citation moves: the citation sequence still runs one to sixteen with every entry
cited, and every published seed-42 value is untouched. All six CI jobs pass,
including the fast suite that runs `check_markdown.R` and `check_references.R`
and the seed-42 byte-for-byte reproduction. The check suite was not run in the
session, R being absent from its container, and the assertions of those two
scripts were verified by hand for the document while the Actions gate ran the
suite itself. No issue becomes ready on this merge. The `chore/` branch deviation
applies to this update as well, the session being directed to a single named
development branch.
