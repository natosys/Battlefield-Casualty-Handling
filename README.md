...
Findings demonstrate that the current system design is capable of managing moderate casualty volumes, historically represented by the Falklands conflict. Following correction of DNBI sub-categorisation, OT-bypass routing, and OT bed scheduling, the primary system constraint is identified as R2B holding bed capacity, which saturates progressively over a 30-day operation due to disease DNBI evacuees occupying hold beds for extended durations. The R2E Heavy operates as the primary surgical node (199 first surgeries in the baseline run) with OT utilisation at 62.8% and 38.0% for the two theatres — elevated but not saturated. The study identifies areas of risk, recommends targeted system refinements, and proposes enhancements to the simulation architecture—including dynamic DOW modelling, pulsed strategic evacuation, and stochastic mass-casualty events.
...
OT rooms are modelled as physical spaces available 24 hours per day. The surgical team operates on a 12-hour shift schedule and is the operative constraint on surgical access. Under seed 42 (30 days), **129 of 200 surgical candidates at R2B were bypassed to R2E** — either because the OT bed was occupied, a queue existed, or the surgical team was off-shift. **71 surgeries** were performed at R2B when both OT bed and team were simultaneously available. R2B OT utilisation was **12.4% against 24-hour room time** and approximately **25% against available team shift time**. The OT queue remained flat at zero throughout the run, confirming the bypass logic is functioning as designed.
...
Following correction of DNBI sub-categorisation (Issue #7), OT-bypass routing (Issues #35 and #37), and 24-hour OT bed availability, the R2E Heavy is now the primary surgical node for the deployed health system. Under seed 42 (30 days), the R2E performed **199 first surgeries**, receiving both direct R1 bypass patients and the 129 R2B bypasses generated by off-shift or occupied R2B OT.

![Alt text](images/r2eheavy_bed_queue_3_teams.png)

**R2E OT utilisation has increased substantially** relative to earlier model versions. OT 1 operated at **62.8%** utilisation against 24-hour room time; OT 2 at **38.0%**. OT queues remain brief and sporadic — reaching a maximum of 2–3 patients on isolated occasions — indicating that while the R2E is now meaningfully loaded, it is not yet saturated under Falklands-equivalent casualty rates. It should be noted that the R2E trajectory currently seizes OT bed resources but does not seize the surgical team directly; the team schedule therefore has no operative effect on R2E surgical timing. Correcting this is part of the individual resource seizure refactor (Issue #4) and will likely increase true R2E OT utilisation figures when implemented.

**ICU queues at the R2E are modest**, with transient peaks of 1–2 patients on isolated days. These are short-lived and do not represent sustained saturation. ICU capacity is adequate for the modelled scenario, though higher casualty rates or a MASCAL injection (Issue #9) would likely expose ICU as the next constraint after R2B holding beds.
...
Following correction of DNBI sub-categorisation (Issue #7), OT-bypass routing (Issue #35), and OT bed scheduling (Issue #37), the primary system bottleneck is identified as **R2B holding bed capacity**. Hold bed queues build progressively over the 30-day period, driven by disease DNBI evacuees in extended holding, and consistently exceed the five-bed establishment at both R2B nodes. OT is not a constraint: R2B OT operates at approximately 25% of available shift time; R2E OT at 62.8% and 38.0% for the two theatres — elevated but not saturated.

The R2E Heavy operates as the primary surgical node, performing 199 first surgeries in the baseline run compared to 71 at R2B. ICU queues are transient; resuscitation and holding beds see limited demand. The mismatch — R2B holding saturation with R2E surgical capacity adequate — suggests that the primary system lever is R2B holding bed expansion or a higher evacuation threshold from R2B holding to strategic evacuation, rather than surgical capacity adjustment.
