...
DOW/WIA rate rises from 0.44% to 0.93%, a factor of 2.1, and the confidence intervals do not overlap. The Falklands-load figure is essentially unchanged from the previous comparison while the surge figure is too, which is the expected result: the surgical pathway split removes an intensive care episode and a theatre episode from half the operated population, and at Okinawa intensity the deaths are driven by treatment queues deep enough that halving one component of the load does not relieve them.
