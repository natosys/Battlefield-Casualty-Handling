...
The cohort ships disabled, `reconstruction_share` being zero, so every figure this project reports carries no reconstruction demand. A share of zero consumes no random draw, which is what keeps the published evidence set exactly as it was.
...
| L33 | Both national support base levers ship disabled, so every published figure carries neither the capacity release nor the reconstruction demand | Medium |
...
**L17 — R2E holding beds carry recovery and evacuation on one pool.** A single pool of 30 beds serves four unrelated demands, and the split between them is measured rather than assumed. Both airlift routes stage in a holding bed: a casualty awaiting the standard pool seizes one at the evacuation decision, and one on the critical pool that is ventilated holds an intensive care bed for its pre-flight period and then steps down into a holding bed for the remainder of its wait, so the route does not decide whether a casualty appears here. Across 50 replications the evacuation wait holds 22.2% of the pool's occupancy (95% CI [19.2%, 25.1%]) at moderate intensity and 36.3% ([34.2%, 38.4%]) at high, the remainder being in-theatre recovery, the post-definitive holding fallback and the post-operative damage control hold. Every lever that delays a sortie raises that share, reaching 47.4% ([41.4%, 53.4%]) at a 40% cancellation rate and 61.6% ([59.5%, 63.6%]) at a 14-day sortie interval, where the evacuation wait becomes the pool's largest single consumer. What the coupling does is push back into intensive care: a ventilated casualty cannot step down from its bounded pre-flight period while the holding pool is full, and that hold runs 24.7 hours at a 3-day interval against 91.2 at 14. Any R2E intensive care or theatre-gating capacity finding must therefore be read alongside the holding pool's occupancy rather than in isolation. Separating the staging pool from the clinical holding beds, or expanding the pool, would each reduce the coupling; the defaults were deliberately not tuned to hide it. The four components do not account for the pool in every campaign: they close exactly in 630 of 650 tracked replications and over-count the rest by up to 4%, never under-counting, for a reason not yet isolated.
...
**L33 — Both national support base levers ship disabled, so every published figure carries neither the capacity release nor the reconstruction demand.** The model can release a damage control casualty to strategic evacuation with the definitive repair outstanding when the number waiting to be operated on at R2E reaches `second_surgery.saturation_queue_threshold`, and can send a casualty whose wounds require staged soft-tissue coverage rearward for the reconstruction sequence under `role4.surgery.reconstruction_share` (see [Operating Theatre Demand](#operating-theatre-demand)). Both carry their work forward as reported Role 4 theatre demand rather than losing it, so neither deletes demand any longer. Both nonetheless ship at zero, and every figure this project reports is measured with them off, because each consumes a random draw once enabled and so moves every published result: the two are intended to be enabled together in one measurement campaign rather than separately, which is the only way the replicated evidence sets are re-run once instead of twice. Until then the published results carry the same understatement of Role 4 demand, and the same overstatement of forward theatre demand, that they carried before either lever existed, and the coupling to the evacuation policy is unchanged with them. Two further limits are properties of the model rather than of the defaults. The saturation threshold is an informed estimate, no open-access source reporting the surgical backlog at which a theatre begins evacuating between the two operations. And the reconstruction cohort is identified by triage priority and whether an operation was performed, the model carrying no anatomical description of a wound, so it reproduces the right proportion of casualties needing reconstruction rather than the right casualties; closing that means giving a casualty an injury pattern at the point of injury, which conditions far more than this echelon.
