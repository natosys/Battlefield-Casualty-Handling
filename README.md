...
A **named scenario profile** overlays a discrete, internally consistent parameter set onto the base configuration, so a given simulation run is scenario-explicit rather than an implicit hybrid. Profiles are named for FORECAS's battle-intensity framing (`moderate_intensity`, `high_intensity`) rather than for a specific historical engagement, since FORECAS [[8]](#References) selects a casualty-rate distribution family by battle intensity and troop type rather than by named conflict; a specific historical battle is retained as each intensity tier's calibration exemplar, not as the scenario's identity.
