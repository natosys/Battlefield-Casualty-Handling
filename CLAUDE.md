...
| `scripts/check_markdown.R` | Maintains the TOC and "Return to Top" links across `README.md`, `docs/Single_Run_Analysis.md`, and `docs/Multi_Run_Analysis.md`, generating each anchor as GitHub does; exits non-zero if any anchor link points at no heading, if any local link or image target does not exist when resolved relative to the document containing it, or if any image carries placeholder or empty alt text. All three checks run across every tracked markdown document, including this one and `docs/BCH_Simulation_Action_Plan.md` (which carry no TOC block and must not be given one). External URLs are out of scope |
