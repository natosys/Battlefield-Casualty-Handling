...
| 337 | Harden the CI system-library install against archive stalls | Low | Low | Open |
...
UNBLOCKED (start now):
  ...
  #337 Give the CI system-library install retries, an apt transport timeout
       and a per-step timeout. A wedged archive mirror currently burns the
       job timeout, up to an hour per affected PR.
