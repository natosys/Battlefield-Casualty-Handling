...
This document checks that the Battlefield Casualty Handling discrete event simulation behaves as designed. It follows one simulated campaign through the health system, echelon by echelon, under the `moderate_intensity` (Falklands 1982-modified) casualty rate baseline. Every figure comes from a single run of the shipped default configuration at seed 42 over 30 simulated days, so one campaign is the unit of analysis throughout. The document has two aims: to show that the model works as specified, and to point to the constraints a reader should then test at statistical scale. The companion document `docs/Multi_Run_Analysis.md` holds every replicated experiment and the methodology behind them, so any figure carrying a confidence interval or a replication count belongs there rather than here.

The system handles the moderate casualty volumes of the Falklands baseline, but two constraints stand out. The first is holding bed capacity at R2B, which fills up over the 30 days as disease DNBI casualties occupy beds for days at a time. Breaking the load down by patient stream shows the shortfall is structural rather than a feature of this run: the configured rates imply about 15.5 beds in use at once against the 10 the establishment holds, an overload of 55%. A two-tier routing policy keeps that load moving, combining an occupancy check made before transport with a three-stage policy applied on arrival, but the structural remedies are more hold beds or an evacuation threshold. The second constraint is the second-shift surgical section at R2E Heavy. Its two operating theatres read as busy for much of the run, but mostly because casualties hold a room while waiting for a surgical team rather than because rooms are scarce. Intensive care at R2E runs hard without saturating, and R2B theatre capacity is not a constraint at all. Whether these findings hold across independent replications, and how the system copes with a much higher casualty rate, is answered in `docs/Multi_Run_Analysis.md`.
