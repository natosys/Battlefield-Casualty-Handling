...
| Post-Operative Intensive Care Gate | 50 reps x 30d x 2 arms | Yes (roughly 2,283 needed on the paired mortality difference) | No | Migrate |
...
**Three experiments were identified as migration candidates on a shared piece of reasoning, and the first one re-run shows that reasoning does not hold for every design it was applied to.** The R2B pre-open hold window, the post-operative intensive care gate and the transport fleet-size sweep share a property none of the retained experiments does: each has a response the design record already flags as under-resolved at 30 days by a wide margin, and each such response is a count of events (operations, admissions, transport legs) rather than a level, so the deficit is a function of how many events a 30-day window contains rather than of the replication count alone. A 360-day window accumulates roughly twelve times the events a 30-day one does at the same arrival rate, which the sustained-operations protocol's own arrival-stability measurement confirms holds across the horizon (see [The Protocol](#the-protocol)), and that was the basis for expecting the migration to buy resolution rather than merely cost more.

**R2B Pre-Open Hold Window, migrated (#405).** The migration was completed and shows a mixed result rather than the general improvement the reasoning above predicted. The forward-surgery row (R2B surgeries) resolves for the first time, +36.73 [+19.19, +54.28], because it is measured at R2B itself and the longer horizon gave it more surgical events to separate the arms by. But the R2E first-surgery row, whose paired standard deviation was 29.5 casualties at 30 days, is 160.2 at 360, and now needs roughly 24,655 replications rather than 838 to resolve to the same half-width; the busy-theatre bypass row moved from 10.4 to 62.2 the same way. [The R2B Pre-Open Hold Window](#the-r2b-pre-open-hold-window) records the reason: this experiment's two arms are not paired on a shared casualty stream past the first hold, so a longer campaign gives the streams more time to drift apart rather than more independent events to average over, and the noise in an absolute count grows with the horizon at least as fast as the count itself does. The "twelve times the events" reasoning above holds for a response measured within one arm's own accumulating count, which is why the R2B-surgeries row resolved, and fails for a response that depends on comparing two arms whose streams diverge, which is why the downstream R2E rows did not; a design's stream-pairing property has to be checked before this reasoning is applied to it, not assumed from its response type alone.

- **Post-Operative Intensive Care Gate.** The paired mortality difference needs roughly 2,283 replications at 30 days for a half-width matching the observed effect. Unlike the hold window, both arms of this comparison run under one control seed without either arm shifting the stream the other draws from, so the two arms stay paired on the same casualty stream for the whole campaign rather than diverging from an early event; the reasoning above is expected to apply here without the hold window's correction, and the migration's outcome is recorded in [The Post-Operative Intensive Care Gate](#the-post-operative-intensive-care-gate).
- **Transport Fleet-Size Sweep.** The design record already states the utilisation estimate is barely pinned down at 10 replications and 30 days. Migrating to the sustained-operations count and horizon, at all nine fleet-size points and both configurations, costs about $30/10 \times 3.8 \approx 11.4$ times the current run, the largest multiple of the three because this sweep already carries nine points and two configurations that the paired two-arm designs above do not; it remains costed rather than executed.
