...
| L32 | The holding establishment is never swept, so its price against the evacuation policy is unknown | Medium |
...
**L32 — The holding establishment is never swept, so its price against the evacuation policy is unknown.** Shortening the evacuation policy and adding R2E holding beds are substitutes: both free forward capacity, the first by retaining fewer casualties and the second by holding more of them, and a planner choosing between them needs their relative cost. Only the policy has been swept. The establishment is fixed at 30 beds in every measurement the project reports, so the sweep can say that a 21-day policy is stable at that establishment and cannot say whether a longer policy would be stable at a larger one, which is the question a force structure review would ask. The two costs are not commensurable either: a shorter policy is paid for in returns to duty and in demand on the national support base, an extra bed in establishment and lift. Closing this requires making the bed counts variable in the same way the transport fleet size already is, then sweeping the establishment against the policy jointly rather than each alone, since the interaction is the quantity of interest.
