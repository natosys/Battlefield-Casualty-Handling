...
Daily casualty treatments at R2B 1 and R2B 2 show saw-tooth cycles in queuing for OT (shown in the plots below), returning to zero in the vast majority of days. Of note, the 12-hour availability window for the single surgical team in each R2B is likely to substantially drive this saw-toothing. R2B OT utilisation across the single-run (seed 42, 30 days) was **6.2%**, reflecting that DNBI sub-categorisation (Issue #7) has removed the artificial surgical demand from battle fatigue and disease cases; OT load is now driven by WIA and NBI casualties only. Only one bypass event was recorded across the 30-day run, confirming that the two-node R2B configuration is adequate for forward surgical demand under these casualty rates.

However, **holding bed queues at R2B are the primary identified bottleneck in this run**. R2B 1 hold bed queues build steadily from approximately Day 15 onward, reaching 8–9 patients waiting by Days 21–22 and remaining elevated through Day 30. R2B 2 shows a distinct pattern — a peak of 9–10 patients queuing around Days 10–12 before resolving — consistent with an early surge of disease DNBI evacuees entering the holding pathway. These queue lengths substantially exceed the five hold beds per R2B unit, indicating that holding capacity is structurally insufficient for the cumulative DNBI and post-surgical holding demand generated over a 30-day operation.

![Alt text](images/r2b_bed_queues.png)

### R2E Heavy Handling

Following DNBI sub-categorisation (Issue #7), the R2E Heavy operates well within its OT and ICU capacity under the baseline casualty scenario. This is a material change from the previous model finding in which undifferentiated DNBI routing inflated surgical demand at R2E.

![Alt text](images/r2eheavy_bed_queue_3_teams.png)

**OT queues at the R2E Heavy are sporadic and brief**, reaching a maximum of two patients waiting on isolated occasions and returning to zero throughout the run. R2E Heavy OT utilisation was **10.8%** (seed 42, 30 days), reflecting genuine traumatic surgical demand from WIA and NBI casualties only. This indicates that OT capacity at the R2E is not a constraint under current baseline conditions. Daily surgical volume at the R2E varies from 0 to 19 cases per day, reflecting stochastic arrival patterns and the concentration of high-acuity WIA cases requiring definitive surgery that cannot be managed at R2B.

**ICU queues at the R2E are also modest**, with transient peaks of 1–2 patients appearing around Days 22–23 and briefly on Day 30. These are short-lived and represent isolated surges rather than sustained saturation. ICU capacity is therefore adequate for the modelled scenario, though these brief peaks indicate that under higher casualty rates, ICU would become constrained before OT.
...
Following correction of DNBI sub-categorisation (Issue #7), the primary system bottleneck is no longer OT availability at the R2E Heavy. OT queues at R2E are sporadic and brief, with overall utilisation of 10.8%; ICU queues are similarly transient. **The dominant constraint identified in this run is R2B holding bed capacity.** Hold bed queues at R2B build progressively over the 30-day period, reaching 8–9 patients queued at R2B 1 by Day 21 and remaining elevated through the end of the run. This reflects the cumulative load of disease DNBI evacuees entering the R2B holding pathway and post-surgical patients awaiting evacuation or recovery. The five hold beds per R2B unit are structurally insufficient to absorb this demand over a sustained operation.

The R2E Heavy, by contrast, is significantly under-utilised relative to its establishment: OT, ICU, resuscitation, and holding beds all show substantial spare capacity. This mismatch — R2B saturation coinciding with R2E under-utilisation — suggests that the current echelon configuration may be front-loaded in holding capacity relative to the true forward surgical demand. Expanding R2B holding capacity, or increasing the evacuation rate from R2B to R2E, represents the most direct lever for improving system throughput under baseline conditions.
