...
Three aircraft ship, all from the RAAF's account of its aeromedical evacuation capability [[63]](#references), which reports that an AME-configured C-17A "can transport 54 ambulatory and 36 high dependency stretcher patients" and that "the C-130J and C-27J can carry 97 and 21 stretcher patients respectively":
...
| L33 | Every definitive repair is performed in theatre at the shipped configuration, and Role 4 surgical activity is not represented | Medium |
...
**L33 — Every definitive repair is performed in theatre at the shipped configuration, and Role 4 surgical activity is not represented.** The model can release a damage control casualty to strategic evacuation with the definitive repair outstanding, when the R2E surgical queue reaches `second_surgery.saturation_queue_threshold` (see [R2E Heavy Trajectory](#r2e-heavy-trajectory)), but that threshold ships at zero and every figure this project reports is measured with the release disabled. Two things follow. The published results carry the same understatement of Role 4 intensive care demand, and the same overstatement of forward theatre demand, that they carried before the lever existed, and the coupling to the evacuation policy is unchanged with them: a shorter policy would move more of this work rearward, so the policy comparison reported in the replicated analysis understates what shortening the threshold asks of the national support base. The threshold cannot simply be turned on to remove this, because the trigger is an informed estimate; no open-access source reports the surgical load at which a theatre begins to evacuate between the two operations, or the share of casualties it does so for. Separately, a released casualty's outstanding repair sets no surgical demand at Role 4, the census reporting only the bed types that echelon is given and an operating theatre not being among them, so the operation such a casualty still needs is invisible in the demand the model publishes. Closing this means calibrating the threshold against a source that reports the behaviour, and giving the Role 4 census a surgical episode for the casualties who arrive owing one. The two are ordered rather than independent, and the census episode comes first: until it exists, enabling the threshold would move surgical work off the deployed hospital and into a report that does not count it, understating total demand rather than redistributing it. A non-zero shipped default belongs with that episode, not before it.
...
[63] Royal Australian Air Force. *Aeromedical evacuation*. Australian Government, Department of Defence. Retrieved 02 Aug 26, from https://www.airforce.gov.au/our-work/humanitarian-support/aeromedical-evacuation

[64] U.S. Air Force. *C-130 Hercules*. Air Force Fact Sheet. Retrieved 02 Aug 26, from https://www.af.mil/About-Us/Fact-Sheets/Display/Article/1555054/c-130-hercules

[65] Goel, M. K., Khanna, P., & Kishore, J. (2010). Understanding survival analysis: Kaplan-Meier estimate. *International Journal of Ayurveda Research*, *1*(4), 274-278. Retrieved 11 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC3059453/

[66] Royston, P., & Parmar, M. K. B. (2013). Restricted mean survival time: an alternative to the hazard ratio for the design and analysis of randomized trials with a time-to-event outcome. *BMC Medical Research Methodology*, *13*, 152. Retrieved 11 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC3922847/

[67] Hall, A., Graham, B., Hanson, M., & Stern, C. (2023). Surgical capability utilization time for military casualties at role 2 and role 3 facilities. *Military Medicine*, *188*(11-12), e3368-e3370. Retrieved 10 Aug 26, from https://academic.oup.com/milmed/article/188/11-12/e3368/6961509
