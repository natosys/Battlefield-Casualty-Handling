...
**What the ranking tells you.** The casualty load dominates. Mass Casualty Event Rate (`mass_casualty_rate`) leads at µ\* 16.27, followed by Priority 1 Surgical Candidacy (`pri1_surg_prob`) at 7.15 and Priority 1 Damage Control Rate (`pri1_dcs_rate`) at 6.50, against a median of 2.37 across the set. Before relying on a run, check that the mass casualty configuration matches the scenario being modelled: an error there shifts surgical queue depth more than an error anywhere else.

Seven of the top ten are Scenario / Casualty Context parameters, so results depend most on assumptions about the casualty load rather than on decisions the health system can make. Two are Policy, the Theatre Evacuation Policy (`evacuation_policy_days`) at rank 7 and OT Shift Length (`ot_hours`) at rank 9, and one is Capacity, the R1 Battle Fatigue Hold Duration (`r1_recovery_mode`) at rank 10. A parameter's category tells you what a high rank means in practice and matters more than its exact position; see "Reading the plots: colour coding" below.

**Effects are not independent.** σ exceeds µ\* for 61 of the 65 parameters, meaning most parameters' influence depends on where the others are set. A parameter that looks inert at the baseline may matter once other values move, so conclusions drawn by changing one input at a time will understate what the model can do. Vary related parameters together, or compare whole scenario profiles, rather than tuning singly.

**Two results to read carefully.** The Reinforcement Demand Cycle ranks 33rd, mid-table. That ranking was screened against a baseline at which reinforcement was disabled, so it shows what reinforcement contributes as it is switched on across 0 to 14 days rather than its influence around the 7-day cycle now shipped; a parameter's elementary effects are local to the baseline they are screened from. Separately, ranks toward the bottom sit very close together: the ten lowest span 0.20 down to zero, and the three lowest are all exactly zero. Treat the lower half of the table as a band rather than an order.

**Influence varies widely inside a single mechanism.** Reinforcement is the clearest case: the demand cycle ranks 33rd while Reinforcement Fulfillment Lag (`fr_fulfillment_lag_days`) ranks 49th and Reinforcement Fill Distribution (`fr_fill_mode_frac`) 55th of 65, so cycle length is the value to get right when configuring it. Mass casualty sizing runs the other way, every part of it ranking high: Event Rate (`mass_casualty_rate`) 1st, Event Size Maximum (`mass_casualty_max_cas`) 4th and Event Size Minimum (`mass_casualty_min_cas`) 5th, so the whole mechanism needs to match the scenario rather than one value within it.
