...
![Stacked bar chart of the casualties occupying an R2B hold bed on each simulation day, each bar decomposed into the disease DNBI, non-battle injury and wounded in action streams, with a dashed reference line at the five-bed per-unit capacity, bars reaching eleven on day 11 and all three streams still present on the closing day](../images/r2b_hold_occupancy.png)

Disease DNBI was the largest stream over the opening week and again over the closing third of the run, averaging 4.6 casualties a day against 3.1 for wounded in action and 1.3 for non-battle injury, and it is the stream that both opens and closes the campaign's occupancy. The series counts a casualty on every day their stay touches, whatever fraction of it they spent in the bed, so it reads above the mean concurrent occupancy the beds themselves record. The dashed line marks the five beds a single R2B unit holds, so any bar above it is a load neither unit could carry alone. The wounded in action stream peaked at 7 in the middle of the run and was still present on the closing day, as were the other two: no stream clears before the campaign ends. Relieving the hold-bed constraint over a sustained campaign is nonetheless a question of managing disease rather than of surgical throughput, disease being the larger stream throughout and the one the arithmetic above shows exceeding capacity on its own. Disease dominating a forward holding pathway matches the campaign record, in which sickness has repeatedly produced admission loads as large as or larger than wounding [[10]](#references).
