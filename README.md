...
**Known gap introduced by this follow-up:** the published Morris screening ([Sensitivity Analysis](#sensitivity-analysis)) ranked `return_leg_multiplier` while it affected only the two R1↔R2B legs; it now also affects the two R2B↔R2E legs, and the screen has not been re-run against this change. Given the parameter ranked low (10th of 10 by µ\*, see the Sensitivity Analysis table) even when it only touched two legs, a material ranking change is not expected — but this is asserted, not verified by re-running the screen. A re-run under this codebase is out of scope for this change; tracked as a follow-up to be picked up alongside any future Morris re-run (see the pattern established by Issue #75).
