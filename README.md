...
| ----------------------- | ---------------------------------- | ------------------------------------------------------------------ |
| Treated-cohort DOW rate | 0.353% (95% CI [0.293%, 0.413%])   | ≤0.46% (3 deaths among over 650 reaching forward surgery [[14]](#references)) |
...
Against that bound the base configuration measures 0.417%, with a 95% confidence interval of 0.354% to 0.480% that sits at it. The ceilings were re-fitted to reach it, from 0.023 and 0.019, once the arrival generators were corrected to deliver the daily rates their configuration names and the resulting larger treated cohort carried mortality above the bound. Settling that took 250 replications: died of wounds averages about one death per replication, and independent 50-replication measurements of the same shipped configuration have returned rates spanning 0.17 percentage points across control seeds, a spread several times the confidence interval any one of them reports. The pre-re-fit configuration measured 0.528% over 150 replications and 0.513% over 250, an overshoot that survived the larger pool rather than dissolving into it, which is what distinguished it from an earlier reported overshoot of 0.617% that no pooled measurement has since reproduced.
