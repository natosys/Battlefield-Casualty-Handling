...
> **Criteria:** C3, C4, C5
> **Computation:** The `queue` column of the simmer resource monitor for the `"ame"` and `"ame_critical"` resources (`plot_ame_queue()`, Issue #109 — factored out of the analysis pipeline into a standalone reusable function, unchanged in behaviour from the original `ame_backlog_plot` inline block), stepped over simulation time; faceted by pool (and by replication when more than one is present), with independent y-axis scales given the pools' very different capacity magnitudes.
