...
**Fixed establishment/capacity counts.** Population sizes, team/bed/vehicle counts (`pop_combat`, `r1_team_count`, `r2b_bed_ot`, `transport_PMVAmb_qty`, etc.) and each selectable AME airframe's fitted critical/standard capacity pair (`airframe_c17a`, `airframe_c130j`, `airframe_c27j`, six fields) represent discrete establishment or hardware decisions a planner sets explicitly, not continuous parameters carrying epistemic uncertainty about a true underlying value.
