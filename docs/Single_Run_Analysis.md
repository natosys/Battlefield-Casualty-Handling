...
**Background.** A deployed health system is sized before the campaign it has to serve, and the casualty volumes anticipated in large scale combat operations exceed those the health systems of the past two decades were built around. The Battlefield Casualty Handling discrete event simulation models that system, moving each casualty rearward through Role 1, Role 2 Basic (R2B) and Role 2 Enhanced Heavy (R2E Heavy) as it claims and releases clinical staff, beds, operating theatres and transport. Before any result the model produces can inform a planning decision, the model has to be shown to behave as its specification describes.

**Objective.** To verify that behaviour by tracing one simulated campaign end to end, and to identify where load gathers, so that a reader knows which constraints are worth testing at statistical scale. Verification of this kind asks whether the code does what the specification says; it does not ask whether that specification represents the real system well, which is a separate question resting on separate evidence.

**Methods.** One run of the shipped default configuration at seed 42 over 30 simulated days, under the `moderate_intensity` (Falklands 1982-modified) casualty rate baseline, examined echelon by echelon. No warm-up period is discarded, the model being a terminating simulation whose opening period is part of what a planner needs to see. One campaign is the unit of analysis throughout, so no figure carries a confidence interval or a replication count; every replicated experiment, and the methodology behind it, sits in the companion document `docs/Multi_Run_Analysis.md`. All figures were produced in the project's pinned development container and reproduce the repository's tracked baseline evidence set byte for byte.

**Results.** The run generated 530 casualties, of which 287 were wounded in action, 171 disease and non-battle injury, and 72 killed in action, arriving unevenly at between 4 and 46 per day. Role 1 handled every casualty without a queue. Two constraints emerged. Holding beds at R2B averaged 7.6 of the 10 available and reached or passed capacity on 10 days, filled by disease casualties staying for days at a time; splitting the load by stream shows the shortfall is structural rather than particular to this run, the configured rates implying about 15.5 beds in use at once against 10 available, an overload of 55%. A two-tier routing policy kept that load moving, diverting 179 casualties to R2E before transport. At R2E Heavy the second-shift surgical section set the pace: its two theatres carried a queue for 46.3% and 34.6% of the run, nearly all of it casualties holding a room while waiting for staff rather than for space. Intensive care ran hard at 85.8% to 93.7% occupancy without saturating, R2B theatre capacity was not a constraint, and transport retained wide headroom. Strategic evacuation was limited by cancelled sorties rather than by seats, two of four scheduled sorties failing to fly and 36 casualties remaining queued at the run's end.

**Conclusion.** The modelled system sustains a single brigade at Falklands-equivalent casualty rates, and the constraints that bind are forward holding capacity and the R2E surgical roster rather than operating theatre space. The levers this points to are more R2B hold beds or an evacuation threshold, a second-shift surgical section at R2E, and more R2E holding beds, which currently carry in-theatre recovery and the strategic evacuation wait on one pool. Because one run illustrates a mechanism rather than estimating an average, none of these figures sizes an effect. Whether the findings hold across independent replications, and how the system copes at a much higher casualty rate, is answered in `docs/Multi_Run_Analysis.md`.
