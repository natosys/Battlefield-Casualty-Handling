...
**Two results to read carefully.** The Reinforcement Demand Cycle ranks 33rd, mid-table, but reinforcement ships disabled (`demand_interval_days = 0`). Its rank comes from the screen varying it across 0 to 14 days, and shows what reinforcement would contribute if switched on, not what it contributes to the shipped baseline. Separately, ranks toward the bottom sit very close together: the ten lowest span 0.20 down to zero, and the three lowest are all exactly zero. Treat the lower half of the table as a band rather than an order.
