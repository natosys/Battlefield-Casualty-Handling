...
| 384 | Most published experiments have no tracked evidence set, so their figures cannot be audited without re-running them | High | Medium | Open |
...
### Phase 1 — Statistical Foundation (Issues 1 ✓, 22 ✓, 2 ✓, 3 ✓, 24 ✓, 75 ✓, 157 ✓, 158 ✓, 189 ✓, 186 ✓, 195 ✓, 208 ✓, 331 ✓, 296 ✓, 382 ✓)
...
UNBLOCKED (start now):
  ...
  #384 Seven of the twelve experiments the companion paper publishes have no
       tracked evidence set at all, so their figures cannot be audited
       without re-running them. The comparative scenario analysis is among
       them: `scripts/run_scenarios.R` writes only to the gitignored
       outputs/ and has no --refresh-baseline flag, so the paper's
       centrepiece exists nowhere in the repository. The cheap half is the
       entry points and the expensive half is the measurement, and the two
       should be sequenced apart. Raised from the #382 audit.
