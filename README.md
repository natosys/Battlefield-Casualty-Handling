...
Calling `load_scenario()` with `scenario = "default"` is a strict no-op, since `resolve_scenario()` returns the parsed JSON unmodified; `load_elms()` reaches the same result more directly, since it never calls `resolve_scenario()` at all. Every entry point that does not name a profile (`run.R`, `scripts/run_warmup.R`, `scripts/run_sensitivity.R`) calls `load_elms()` and so runs the base configuration exactly as it would were the scenario mechanism absent. Profiles are selectable interactively through the Shiny console's Casualty Intensity Profile dropdown, which lists both alongside the base configuration under plainer, conflict-based labels (see [Shiny Application](#shiny-application)), and from the command line through the comparative scenario runner ([Comparative Scenario Runner](#comparative-scenario-runner)).
