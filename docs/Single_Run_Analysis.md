...
The two `high_intensity` rows predate that profile's died-of-wounds calibration and have not been re-measured against it. A casualty who dies of wounds never returns to duty, so the calibrated profile depletes the force somewhat faster than these rows record; the effect steepens the no-reinforcement decline the table is demonstrating rather than working against it, which is why the demonstration stands on the figures as measured (see the README's [Scenario Profiles](../README.md#scenario-profiles)).
...
> **Currency note:** the figures in this section were produced before the model changes that bound the R2E surgical roster to theatre entry and made strategic evacuation a capacity-constrained resource, and they have not been recomputed since. Both the background-only comparison column and the single-run figures below therefore reflect an earlier configuration: the background-only baseline reads 400 casualties where the current shipped configuration produces 530 at this seed, and its post-operative pathway split predates both the strategic evacuation backlog and the theatre evacuation policy that now governs R2E disposition. It also predates the arrival generators being rebuilt around per-day rate draws, which matters here more than elsewhere: the background stream can now itself deliver twenty or more casualties within a two-hour window about once in fifty 30-day runs, so the contrast between background tempo and an injected event is narrower than the table below shows (see the README's [Mass Casualty Event Injection](../README.md#5-mass-casualty-event-injection)). The section is retained because the *mechanism* it demonstrates, an acute surge driving OT and ICU contention well above sustained-tempo levels, is unaffected by those changes; the absolute counts are not current and should not be compared directly against the seed-42 figures reported elsewhere in this document. Regenerating them requires re-running 10 replications under the non-default `rate_per_day = 0.2` configuration. They additionally predate the wounded/killed split described above, so every casualty they count as event-derived is counted as wounded.
