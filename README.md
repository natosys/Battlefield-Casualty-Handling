...
The population term $P$ in the per-minute rate formula (Step 2 above) is not a fixed constant: it is a live, time-varying **effective force size**, read fresh at every simulated minute from a simmer global (`effective_force_combat`/`effective_force_support`) that the running simulation itself updates as casualties occur and return to duty. Casualty production is thereby a function of *combat power available* rather than a fixed rate applied to a static roll strength, consistent with FORECAS's own force-size-dependent projection methodology [[8]](#References) and the return-to-duty-as-force-sustainment-lever literature [[19]](#References).
...
Combat and support effective force size are tracked as two independent globals rather than a single combined scalar: combat and support casualties are already separate generated streams drawn against separate population sizes ([Casualty Generation Rates](#casualty-generation-rates)), and a combat casualty does not reduce support-troop headcount or vice versa, so conflating them into one scalar would misattribute cross-pool effects. Were the two populations in practice fungible (e.g. support personnel cross-employed in a combat role under sustained attrition), this design would understate combat-pool depletion and overstate support-pool depletion relative to a pooled-strength model.

**Mechanism.** Each pool starts at its full establishment strength (`env_data$pops$combat`/`support`) and is updated at three points, all driven by events the simulation itself generates for that specific replication, at the exact simulated instant each occurs rather than being recomputed once per day from accumulated daily totals — a discrete-event simulation can update state exactly when it happens, so polling once daily would introduce an arbitrary lag between an event and its effect on arrival rate with no corresponding gain in accuracy:
...
2. **Credited by 1** the instant a casualty reaches return-to-duty — `credit_rtd()`, applied at each of R1/R2B/R2E's existing `return_day` assignment points (`R/trajectories.R`). `return_day` is set to `now(env)` at the actual completion of each echelon's own timeout-governed recovery/holding period (R1: `r1.recovery`; R2B: `r2b.holding`; R2E: `r2eheavy.holding`), so this credit inherently reflects each echelon's real recovery duration, with no additional delay weighting applied on top of it.
...
Each stream's arrival generator is a stateful closure (`make_ln_arrival_generator()`/`make_exp_arrival_generator()`, `R/environment.R`), called once per arrival, that walks minute-by-minute through the same capped-rate/cumulative-sum-crossing mechanism as Steps 2–3 above, reading the live force-size global at each step so arrival timing responds directly to the replication's own in-run events. Mass casualty event timing (Section 5) is exogenous and pre-computed rather than force-size-reactive, since it represents an imposed external tactical shock (a barrage, strike, or IED) rather than a function of standing force strength the way sustained background attrition is; `wrap_with_mass_casualty()` interleaves it into the `wia_cbt` stream in strict chronological order. Were mass casualty incidence itself scaled to committed force size, this design would over-weight mass casualty risk once the force has depleted materially — a secondary effect expected to be small relative to the primary background-rate mechanism.

The reinforcement demand/fulfillment cycle's shipped defaults (`fill_min_frac = 0.2`, `fill_mode_frac = 0.85`, `fill_max_frac = 1.1`, `fulfillment_lag_days = 7`) give a fill-fraction distribution with a long left tail toward severe under-fill (5th percentile ≈ 0.37 of demand) and a short right tail bounding over-supply (99th percentile ≈ 1.05) — full or over-fulfillment is the *less* likely outcome even though it is the distribution's mode, because the long lower tail pulls the mean well below the mode (≈0.72 vs. 0.85 at the shipped defaults). Demand and the fill fraction are both resolved at submission time, not at fulfillment, modelling the outcome of a reinforcement request as substantially known when it is made, with delivery simply delayed rather than as a second independent source of uncertainty layered on top of delivery timing. `fill_max_frac` is deliberately permitted to exceed 1.0, since a single reinforcement cycle filling slightly more than its own submission-time shortfall estimate is a reasonable real-world outcome; this is safe rather than a source of pool overshoot only because of the credit-time ceiling clamp described above, not because `fill_max_frac` itself is bounded at 1.0. No open-access source was identified that quantifies reinforcement fulfillment rates or lag times for a comparable LSCO reinforcement pipeline, so all five parameters (the demand cycle, the lag, and the three triangular shape parameters) are informed estimates capturing the intended qualitative shape — reinforcement requests are usually under-met, rarely over-met, and never instantaneous — rather than a literature-calibrated pipeline. A shorter lag, or a fill distribution weighted further toward full/over-delivery, would sustain force size, and therefore casualty production, closer to establishment strength than modelled here; the reverse would deepen depletion. The in-flight demand netting and credit-time ceiling clamp described above ensure reinforcement cannot drive either pool above establishment strength under any valid parameter combination, including overlapping demand cycles (`demand_interval_days < fulfillment_lag_days`).
