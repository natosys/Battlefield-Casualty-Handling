...
The ward each category is admitted to is configured rather than fixed in code, as `role4.wards` in `env_data.json`, alongside the set of wards the census reports. A stay can also be split between two wards rather than held in one, which is what `role4.icu_continuation` governs. Where it is enabled, a casualty admitted to intensive care has a Role 4 intensive care phase of the whole post-operative requirement less whatever R2E already served, recorded per casualty as `post_definitive_min`, and the step-down ward carries the remainder of the stay. The phase applies to that cohort alone: a casualty whose category is admitted to a surgical or general ward has no intensive care episode to continue, so keeps the whole stay where it was admitted whether or not theatre operated on it. That is the same contract the model already applies forward, where one stabilisation requirement is drawn once and divided between R2B and R2E so that the total cannot depend on the route (see [R2E Heavy Trajectory](#r2e-heavy-trajectory)); Role 4 extends it one echelon further. Because the remainder is taken as a difference rather than drawn, the two phases sum to the drawn length of stay exactly, and a casualty theatre never operated on has no episode to continue and so keeps the whole stay on its admission ward.

A national support base establishment can also be stated, as `role4.capacity`, giving a bed count per ward. It is a figure the reported demand is measured against rather than a capacity anything respects: no casualty queues for a Role 4 bed, none is refused, and nothing is pushed back into theatre, so a stated establishment changes no simulated outcome and only adds a comparison. Where one is stated the analysis reports, per ward and per replication, the days demand stood above it, the largest overshoot reached and the bed-days of unmet demand. The last is a sum of each day's shortfall rather than a count of the days on which one occurred, because a single day twenty beds short and twenty days one bed short are different planning problems that a day count alone would report identically. A ward stating no establishment is omitted from the comparison rather than reported as having met its demand. Every ward ships with none stated, which reads as unlimited and leaves the output as it was.
