...
Because mass casualty events overlay only the combat WIA stream, immediate KIA and DNBI are not separately generated by a mass casualty event: the acute-surge stress test this feature targets is surgical and ICU throughput, driven by the WIA (survivor) casualty flow, and DNBI is by definition a non-battle event with no causal link to a discrete tactical event. Immediate KIA from a mass casualty event are operationally real but not modelled here, so a materially significant real-world share of immediate KIA would mean the feature understates mass-casualty-driven contention on R1/mortuary transport and KIA-processing resources specifically, while still correctly capturing the OT/ICU surge it targets.

The `"poisson"` mode's shipped defaults trace to Issue #9's Recommended Approach, informed by the compound Poisson parameterisation of Fischer et al. [[35]](#References) and the blast-dominant injury context reported for contemporary LSCO [[37]](#References); no open-access source tabulates event-level (rather than daily-aggregate) mass casualty rate/size distributions for a comparable historical LSCO campaign, so these values remain informed engineering estimates rather than literature-calibrated ones. The `"scheduled"` mode carries no default days or probabilities at all — a planner using it must supply their own scenario-specific values. A materially different event rate or size would change the absolute magnitude of the surge-capacity stress test (queue depth, DOW elevation) reported in [Mass Casualty Event Stress Test](docs/Single_Run_Analysis.md#mass-casualty-event-stress-test), though the qualitative finding that a mass casualty event meaningfully strains OT/ICU capacity relative to background load is expected to be robust in direction even if not in magnitude.
