...
Bound width follows the kind of quantity, not the rule. Durations and rates vary multiplicatively, most often between half and twice the baseline, and narrower where a source constrains the range. Probabilities and efficacy factors vary by an absolute margin, usually 0.15 to 0.25, clipped to stay clinically sensible; that clipping is why several are asymmetric about their baseline. A screened triangular mode must also stay inside its own distribution's fixed minimum and maximum, since `rtriangle()` requires $a \leq c \leq b$ and returns `NA` otherwise: `fr_fill_mode_frac` and `post_op_hold_mode` are bounded accordingly, with the reason recorded in `R/sensitivity.R`.

These bounds are clinical and logistics judgement, not measured uncertainty ranges, so confidence in them is moderate overall and lower for Rule B parameters. Bounds set too narrow understate a parameter's influence; bounds set too wide mix realistic values with unrealistic ones. Because the model responds non-linearly, the ranking can shift with the bounds chosen, though widening every bound would raise µ\* without reordering parameters if responses were monotonic.

The screen runs at r = 5 Morris trajectories rather than the `--r` default of 20, giving 5 × 54 = 270 design points at five replications each. At r = 20 it would need 1,080, four times the compute. A lower r makes each µ\*/σ estimate noisier without biasing it, since the Morris method [[46]](#References) is unbiased at any number of trajectories and only gains precision as more are added. The ranking below should therefore be read as indicating relative influence rather than fixing an exact order, with a re-run at r = 20 a worthwhile follow-up (Limitation L18).
...
On completion, the μ\* vs σ scatter for the R2E ICU queue (`morris_objs$r2e_icu_q`) is rendered as a `ggplot2` object (rather than the base-R `plot.morris` `run_morris()` also saves to `images/`), read directly from the Morris object's elementary-effects matrix (`ee`) rather than recomputed. See [Sensitivity Analysis](#sensitivity-analysis) for what μ\* and σ measure and which response the ranking is built on. The ranked parameter table uses the same plain-English labels as the Configure panel and Sensitivity Calibration table (`MORRIS_LABELS` in `app.R`) rather than raw `morris_params$name` values, and highlights its top 5 rows.
