...
| R2B Pre-Open Hold Window | 50 reps x 30d x 2 arms | Yes, per [The R2B Pre-Open Hold Window](#the-r2b-pre-open-hold-window) (76 to 838 needed) | No | Migrate |
...
**Three experiments are candidates for migration, and none has been re-run to complete it.** The R2B pre-open hold window, the post-operative intensive care gate and the transport fleet-size sweep share a property none of the retained experiments does: each has a response the design record already flags as under-resolved at 30 days by a wide margin, and each such response is a count of events (operations, admissions, transport legs) rather than a level, so the deficit is a function of how many events a 30-day window contains rather than of the replication count alone. A 360-day window accumulates roughly twelve times the events a 30-day one does at the same arrival rate, which the sustained-operations protocol's own arrival-stability measurement confirms holds across the horizon (see [The Protocol](#the-protocol)); migrating these three experiments trades replications for horizon against that same events-per-window budget, rather than requiring the multi-hundred-replication counts [The R2B Pre-Open Hold Window](#the-r2b-pre-open-hold-window) and this section derive at 30 days.

- **R2B Pre-Open Hold Window.** The R2E first-surgery row needs roughly 838 replications at 30 days to resolve to two operations. A 360-day migration at 30 paired replications, the sustained-operations count, is the candidate design, since each replication then carries roughly twelve times the first-surgery events it does now. At the 3.8x per-replication runtime ratio [Whether to Re-Screen at Length](#whether-to-re-screen-at-length) measures, thirty 360-day paired replications cost about $30/50 \times 3.8 \approx 2.3$ times the current 50-replication, two-arm run; whether that resolves the row is a question for the re-run itself, not for this estimate.
- **Post-Operative Intensive Care Gate.** The paired mortality difference needs roughly 2,283 replications at 30 days for a half-width matching the observed effect. Thirty 360-day paired replications cost the same 2.3x multiple of the current run as the hold window above, for the same reason: twelve times more post-operative episodes per replication is what is being bought, and whether it reaches the row's resolution requirement is likewise left to the re-run.
- **Transport Fleet-Size Sweep.** The design record already states the utilisation estimate is barely pinned down at 10 replications and 30 days. Migrating to the sustained-operations count and horizon, at all nine fleet-size points and both configurations, costs about $30/10 \times 3.8 \approx 11.4$ times the current run, the largest multiple of the three because this sweep already carries nine points and two configurations that the paired two-arm designs above do not.
