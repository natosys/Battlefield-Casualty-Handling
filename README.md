...
Both distributions have unbounded right tails, so an implausibly large draw is possible and its cost is worth stating. It is bounded. The closure advances to the new cumulative floor and returns a single arrival (Step 3), so it emits at most one arrival per simulated minute and performs exactly $n_{\text{minutes}}$ iterations across all its calls, whatever any individual draw returns. Execution time is therefore set by the campaign horizon rather than by the tail: a lognormal stream parameterised at 500 casualties per day with a standard deviation of 5,000, two orders of magnitude above anything the model ships, completes its 30-day walk in the same fraction of a second as a shipped one.

That emission rule is itself the one bound on the drawn rate, and it is a property of the closure rather than a parameter. Because the closure emits a single arrival per minute, a minute whose accumulated rate crosses more than one whole casualty discards the remainder without reporting it. At every parameterisation the model ships this is unreachable in practice: a combat stream would need a single draw above 576 casualties per day per 1,000 personnel to reach it, which the shipped WIA parameterisation produces with probability $1.1 \times 10^{-7}$ per minute, giving an expected loss below one casualty in two hundred 30-day runs and far less on every other stream. `scripts/check_arrival_rate_fidelity.R` measures the distance to that ceiling for each shipped stream rather than assuming it, so a re-parameterisation that approaches it fails the check instead of silently under-generating.
