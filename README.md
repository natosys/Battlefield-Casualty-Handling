...
| `R/policy_sweep.R`                 | The evacuation policy trade across replications at the sustained horizon: `reduce_policy_replication()` reduces one replication to the response set inside the forked worker that produced it, `run_policy_measurement()` sweeps a policy at a paired control seed, and `policy_paired_difference()`/`policy_replications_for()` report a difference within replication and the count an unresolved one would need. Forward stability is measured over the campaign's closing days via `policy_pool_state()` |
