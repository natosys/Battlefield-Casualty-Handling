...
Two diagrams sit in a sidebar that stays in view while scrolling. The **Force Structure** diagram beside Health System Architecture draws one node per deployed team at each echelon, with every team at one echelon joined to every team at the next: resources are pooled and taken as available rather than assigned to fixed lanes, and the mesh shows that rather than implying pairings the model does not have. A companion table gives aggregate bed capacity per echelon. Its caption states what it is not, namely a simulated result: it shows structural capacity only and cannot show queues, waits or outcomes, which requires running the model. The **Medevac Chain** diagram beside Medevac draws the transport legs the model actually simulates, with live duration labels; each leg includes an unmodelled-return, or dead-heading, leg, and casualties who die reach the mortuary collocated with R2E.
...
`analyse_replications()` (`R/analysis.R`) is the multi-replication counterpart to `analyse_run()`. It takes the monitoring data `run_replications()` produces, each row carrying its replication number, and returns the same kind of plots but as mean values with 95% confidence intervals rather than single-run traces.

Each queue-depth plot places every resource's queue trace on a shared 240-minute time grid per replication, draws a faint trace for each individual replication so single runs stay visible, and overlays a 95% confidence ribbon around the mean at each time step. Casualty Flow works the same way. Waiting Times instead shows a p10 to p90 band with a median line, pooling every replication's casualties per arrival day, since a per-run scatter does not aggregate meaningfully. Bed & Resource Utilisation has no multi-run equivalent of the single-run Gantt charts, because a chart of individual bed occupancy in one run does not generalise across many; it shows a mean and confidence interval bar chart of utilisation across four resource groups instead (R2B OT, R2E OT, R2E ICU, and the pooled transport fleet). The two Strategic AME plots are shared unchanged between modes, since both already handle multiple replications: the backlog chart separates them and the sortie timeline averages across them.

Every per-casualty breakdown, plot and summary card has a multi-replication form: DOW and return-to-duty by echelon, R2B hold occupancy, OT bypass reasons and routing counts, the R2E post-operative pathway, transport capacity margin and utilisation, time-to-treatment measures, the Role 4 census, unconstrained AME sortie demand, and the mass casualty stress test.

Two of these are pooled across replications rather than averaged: actual AME wait time by route, and the mass casualty DOW rate comparison. Both report a single combined figure, because the underlying counts are usually too small in any one replication for a per-replication rate to mean anything. Group-wise tables count an explicit zero wherever a group does not occur in a replication, so a replication with none of something is counted as zero rather than dropped, which would bias the average upward.

The warm-up period (`WARM_UP_DAYS`, `R/warmup.R`) is applied identically in both modes and ships at 0, since this is a terminating rather than steady-state simulation (see [Warm-up Period Analysis](#warmup-period-analysis)).

Summary cards report the mean with a 95% confidence interval for total casualties, DOW count, R2E ICU peak queue and R2B OT peak queue. Lower bounds are clamped at zero, since all four are counts. The default of 100 replications sits well above the minimum of 30 generally taken as the standard for defensible discrete event simulation output.
