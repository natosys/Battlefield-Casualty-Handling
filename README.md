...
A national support base establishment can also be stated, as `role4.capacity`, giving a bed count per ward. It is drawn on both censuses as a reference line and nothing more. The model sets the demand a deployed trauma system places on the national support base rather than simulating that echelon, so no casualty queues for a Role 4 bed, none is refused, and nothing is pushed back into theatre; a stated establishment accordingly changes no simulated outcome. Reporting how far demand stood above the line, as a count of days or a total of unmet bed-days, would read as a verdict on the adequacy of an echelon this model does not represent, one carrying no case mix, no transfer options and no back-pressure, so that comparison is left to the planner who holds the establishment and the daily census is published for them to make it. Every ward ships with none stated, which reads as unlimited and leaves both plots as they were.
