...
| `scripts/run_long_horizon.R` | CLI entry point for the sustained-operations horizon. Runs the protocol `docs/Multi_Run_Supplement.md` documents at each casualty intensity and writes both the reduced daily series and the per-block means; one long run answers the stability question for every shorter horizon too, the model being causal, so no separate durations are run. `--refresh-baseline` is the only way to write the tracked `data/long_horizon/` |
