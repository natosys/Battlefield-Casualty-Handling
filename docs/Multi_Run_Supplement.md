...
- [The Sustained-Operations Horizon](#the-sustained-operations-horizon)
  ...
  - [The Welch Diagnostic at Length](#the-welch-diagnostic-at-length)
  - [Which Findings Are Horizon-Limited](#which-findings-are-horizon-limited)
  - [Whether to Re-Screen at Length](#whether-to-re-screen-at-length)
  - [Standardising the Other Experiments](#standardising-the-other-experiments)
...
That classification is supported rather than assumed. The Welch graphical diagnostic, run over ten 90-day replications and reported in the system reference [[3]](#references), shows the behaviour a terminating model is expected to show, and `scripts/run_warmup.R` re-runs it on demand. It is also re-derived at a horizon twelve times the experiments' own, which establishes both that a steady state exists for every response at moderate intensity and that it does not for four of them at high; see [Stability, and What It Means for Warm-up](#stability-and-what-it-means-for-warm-up). That re-derivation classifies each response from its 30-day block means rather than from the cumulative moving average the 90-day diagnostic plots, so `scripts/render_long_horizon_warmup.R` runs the same diagnostic directly on the two R2E bed pools closest to the boundary, at daily resolution across the full 360 days; see [The Welch Diagnostic at Length](#the-welch-diagnostic-at-length) for what it adds.
...
### The Welch Diagnostic at Length

The block classification above answers the warm-up question the protocol was written to answer, but it answers it with an estimator the original 90-day diagnostic did not use: a block mean, which forgets the opening transient once it has passed. The cumulative moving average Welch's procedure is usually read from never forgets it, carrying every early day for the whole length of the series, so the two estimators can disagree in a response's favour and neither substitutes for the other. `compute_long_horizon_cma()` (`R/warmup.R`) computes the second estimator directly from the tracked daily series, at the horizon and replication count this section's protocol documents, for the two R2E bed pools the block classification finds closest to the boundary at moderate intensity: intensive care and holding.

At high intensity, the cumulative average shows what the block classification already reports as drifting, without a level for either pool's queue to approach: intensive care runs from a CMA of 30.1 casualties at day 30 to 802.9 at day 360, holding from 38.8 to 290.7, both still climbing at the horizon's end. At moderate intensity the two readings agree in substance but not in shape. The CMA is not flat: it climbs from an empty system to a peak of 1.9 to 2.0 casualties around day 40 to 50, the highest either pool's queue reaches at any resolution in this measurement, then declines and settles into a narrow band, intensive care between 1.1 and 1.3 and holding between 0.7 and 0.9 from roughly day 150 onward, with day 360 reading 1.237 and 0.752. The CMA is therefore still moving at the horizon's end for both pools, by a few hundredths of a casualty per hundred days, at the same time the block classification calls both converged with the opening settled by block one or two. That is not a disagreement: `classify_stability()` asks whether the late blocks have stopped moving relative to their own spread, which they have, while the cumulative average is answering a different and slower-converging question, how far the whole run's history still pulls its own average away from the level it has reached. Both readings are correct for what each measures, and the second is the one Welch's own diagnostic was written to show, so the sustained-horizon warm-up classification now rests on both rather than on the block reading alone. Neither supports discarding an opening period: the CMA never plateaus early enough to mark a transient boundary at moderate intensity, and it has no level to approach at high intensity, so `WARM_UP_DAYS` stays at the value the terminating-simulation argument gives it independent of this measurement.

`scripts/render_long_horizon_warmup.R` renders the diagnostic from the tracked `data/long_horizon/long_horizon_series.csv.gz` alone, running no simulation of its own, and writes the tracked `images/welch_plot_long_horizon.png` and `data/long_horizon/long_horizon_welch_cma.csv` under `--refresh-baseline`. `scripts/check_long_horizon_warmup.R` asserts the cumulative-average arithmetic on a constructed series whose answer is computable by hand, and that the tracked plot's figures reproduce from the tracked series rather than standing as a second and independent claim about it.
...
The sensitivity screens carry the same qualification and it is already recorded: the published Morris and Sobol rankings are 30-day rankings, labelled as such. Whether the rankings themselves would differ at length, as opposed to the responses they rank, is a separate question from the one this section audits, and it is scoped below.

### Whether to Re-Screen at Length

The published Morris and Sobol screens run under the shipped default configuration, not under either scenario profile this protocol measures, so the audit above bears on the decision by analogy rather than by direct measurement. Default sits below moderate intensity in casualty load, at 378 arrivals over 30 days against moderate intensity's 435.7, so a response found to converge within the horizon at moderate intensity is not expected to behave worse at default's lighter load; nothing in this protocol has measured the default configuration at length to confirm that directly.

On that basis, **a re-screen at the sustained horizon is not warranted at this time.** Three considerations together decide it:

- **The configuration the screens run under is the one this protocol finds furthest from the divergent regime.** Every response converges at moderate intensity, and default's lighter arrival stream gives every screened parameter less room to drive a queue past that boundary than moderate intensity already has. The four responses that fail to converge do so only at high intensity, a casualty load roughly two and a half times default's, which the screens do not run under and are not proposed to.
- **The cost is not small.** A 30-day Morris r=20 and Sobol N=200 screen together cost roughly nineteen hours of computation; scaling by the 3.8x runtime ratio a 360-day campaign measures against a 30-day one at the base configuration puts a length-matched re-screen at roughly three days, with a realistic range of three to eight days once congested design points are accounted for, a range recorded when the long-horizon runner was first costed and not narrowed since.
- **Re-screening at length is not free of the coverage gaps tracked separately at #348.** Adding the R2E holding queue response and the two R2E bed counts to the screened set is the change that would let a screen see the constraint this protocol identifies at all, and doing that first, before any re-screen, is what #348 recommends; a re-screen run before it would still be blind to the mechanism this protocol found.

The decision is a scoping one rather than a closure, and two things would change it. If #348's coverage gaps are closed as part of a screen that is running anyway, for whatever reason, extending that screen's horizon costs comparatively little on top of a screen already committed, and running it at length rather than at 30 days should be decided then rather than assumed against. And if a future change to the default configuration moves its casualty load toward moderate or high intensity, the analogy above weakens and the question should be revisited against the configuration as it then stands rather than against the one measured here.

### Standardising the Other Experiments

The audit above covers the sustained-operations protocol's own findings and the sensitivity screens. The remaining nine experiments recorded under [Experimental Designs](#experimental-designs) each choose their own horizon and replication count, and #405 asks the same two questions of each: whether the current count is justified, and whether the 30-day horizon has been checked against the finding that four R2E-adjacent responses do not converge within it. The table below answers both, and the paragraphs after it state the decision for every experiment that is not already at the sustained-operations protocol.

| Experiment | Design | Count justified? | Horizon checked? | Decision |
|---|---|---|---|---|
| Comparative Scenario Analysis | 50 reps x 30d x 2 profiles | No derivation on record | Yes, per [Which Findings Are Horizon-Limited](#which-findings-are-horizon-limited) | Retain (documented deviation) |
| Campaign Time Series | 50 reps x 30d x 2 intensities | Shares the analysis above | Yes, same caveat | Retain (documented deviation) |
| R2B Pre-Open Hold Window | 30 reps x 360d x 2 arms | Yes, per [The R2B Pre-Open Hold Window](#the-r2b-pre-open-hold-window) | Not applicable | Already migrated (#405) |
| Post-Operative Intensive Care Gate | 30 reps x 360d x 2 arms | Yes, per [The Post-Operative Intensive Care Gate](#the-post-operative-intensive-care-gate) | Not applicable | Already migrated (#405) |
| Mass Casualty Event Stress Test | 62 reps x 30d x 2 arms | Yes, per [Mass Casualty Event Stress Test](#mass-casualty-event-stress-test) (derived from resolution) | Yes, by analogy | Retain (documented deviation) |
| Transport Fleet-Size Sweep | 30 reps x 360d x 9 points x 2 configs | No, and the design record already flagged the 10-replication estimate as barely pinned down | Not applicable | Already migrated (#405) |
| Forward ICU Share Decision Frontier | 30 reps x 360d x 5 points | No derivation on record | Not applicable | Already migrated (#405, for consistency) |
| R2B Holding Capacity vs. Evacuation Threshold Sweep | 30 reps x 360d x 15 points | No derivation on record | Not applicable | Already migrated (#405, for consistency) |
| Morris r=20 / Sobol N=200 screens | 5 per point x 30d | Standard screening practice | Yes, per [Whether to Re-Screen at Length](#whether-to-re-screen-at-length) | Retain (already decided) |
| Evacuation Policy + Establishment Sweep | 30 reps x 360d | Not applicable, already at the sustained-operations protocol | Not applicable | Already migrated (#313) |
| Forward Surgical Saturation Release Sweep | 30 reps x 360d | Not applicable | Not applicable | Already migrated |
| Strategic Airlift Collapse Experiment | 30 reps x 360d | Yes (roughly 217 needed for a five-point half-width) | Not applicable | Already migrated |
| Sustained-Operations Horizon Protocol | 30 reps x 360d | Not applicable, this is the reference protocol | Not applicable | Not applicable |

**Two experiments at default-load, sub-boundary configurations were migrated for cross-table consistency rather than on a finding of their own.** The forward ICU share frontier and the R2B holding capacity/evacuation threshold sweep both run under the shipped default configuration, whose casualty load sits below moderate intensity, the only intensity at which every response still converges; on the analogy [Whether to Re-Screen at Length](#whether-to-re-screen-at-length) makes, neither sweep touches a response found to drift, and the audit's original recommendation was to retain both at their existing 30-day, 10- and 20-replication designs. That recommendation is overridden here: a reader comparing three capacity-sweep tables should not have to ask why one runs 360 days and the other two run 30, so all three now share the sustained-operations protocol's horizon and replication count, and the two default-load sweeps carry it for that reason alone rather than because either was under-resolved.

**The comparative scenario analysis and the campaign time series are retained at 30 days by design rather than by oversight.** Both compare the two casualty-intensity profiles at the horizon their own calibration anchors (the Ajax Bay bound, the Okinawa rate) describe, and [Which Findings Are Horizon-Limited](#which-findings-are-horizon-limited) already states, for that same 30-day window, which of their figures are levels and which are points on a trend: every finding at moderate intensity, and every finding not resting on an R2E queue or the evacuation backlog at high intensity. Extending either to 360 days would answer the question [The Protocol](#the-protocol) already answers, not sharpen this one, so both stay at 30 days and defer to the existing caveat rather than needing a new one.

**The mass casualty stress test is retained at 30 days for the same reason, and its replication count is already derived rather than assumed.** [Mass Casualty Event Stress Test](#mass-casualty-event-stress-test) derives 62 from the treated-cohort standard deviation the way [Replication Count and Resolution](#replication-count-and-resolution) sets out, sized to the difference the experiment was reported at before its own re-run; that derivation is the documented justification #405 asks for, and it does not depend on the horizon. The stress test compares an arm with injected events against a background-only arm at the campaign length the rest of the companion paper measures, so lengthening it would move the comparison onto ground the sustained-operations protocol already covers, rather than sharpening this one.

**Three experiments were identified as migration candidates on a shared piece of reasoning, and the two re-run so far show that reasoning does not generalise the way it was first stated.** The R2B pre-open hold window, the post-operative intensive care gate and the transport fleet-size sweep share a property none of the retained experiments does: each has a response the design record already flags as under-resolved at 30 days by a wide margin, and each such response is a count of events (operations, admissions, transport legs) rather than a level, so the deficit is a function of how many events a 30-day window contains rather than of the replication count alone. A 360-day window accumulates roughly twelve times the events a 30-day one does at the same arrival rate, which the sustained-operations protocol's own arrival-stability measurement confirms holds across the horizon (see [The Protocol](#the-protocol)), and that was the basis for expecting the migration to buy resolution rather than merely cost more.

**Both migrated experiments show the same pattern: most count responses get harder to resolve at the new horizon, not easier, and only one response of a different kind improves.** In the R2B pre-open hold window, the paired standard deviation on the R2E first-surgery and busy-theatre bypass rows rose 5- to 6-fold against a 12-fold longer horizon, and their reps-needed figures rose accordingly (838 to roughly 24,655; 104 to roughly 3,712); [The R2B Pre-Open Hold Window](#the-r2b-pre-open-hold-window) attributes this to the two arms drifting onto different casualty streams from the first hold, which a longer campaign gives more time to do. That mechanism does not apply to the intensive care gate, whose two arms share one control seed without an event that shifts the stream the way introducing the hold does, so the expectation going into that migration was that its rows would resolve the way the 12-fold event count implied. [The Post-Operative Intensive Care Gate](#the-post-operative-intensive-care-gate) records that the measurement does not bear that out, and the reading it supports: a count response accumulated over a longer campaign can pick up added variance even without a stream-shifting event, because the intervention's own effect on resource contention is enough to shift which entity's draw is consumed when, and a shared control seed pairs the arms' per-replication seeds without preventing that drift. The one response to improve in either experiment is a time-weighted share rather than an accumulating count (`icu_occupancy`); the R2B-surgeries row in the hold window also resolved, but because its own signal (more R2B surgery under the policy) accumulated faster over the longer campaign than its noise did, not because the noise fell. Neither experiment supports the general claim that a longer horizon buys resolution for an under-resolved count response; it can, but only where the signal accumulates at least as fast as whatever divergence the intervention introduces, and that has to be checked per response rather than assumed from the event-count argument alone.

**Transport Fleet-Size Sweep, migrated (#405).** The design record already stated the utilisation estimate was barely pinned down at 10 replications and 30 days. Unlike the two paired-arm experiments, this sweep is unpaired and its published response, mean queue, is a time-weighted average rather than an accumulating count, so it was expected to behave like the intensive care gate's `icu_occupancy` (which improved on migration) rather than like a raw casualty count (which did not); [The Transport Fleet-Size Sweep](#transport-fleet-size-sweep) and [Option 4](Multi_Run_Analysis.md#option-4-size-the-medical-evacuation-fleet-at-three-ambulances) record whether the measurement bears this out, including whether the previously unreadable utilisation column now resolves.

All three capacity sweeps are executed at the sustained-operations protocol as of this migration. `docs/Multi_Run_Analysis.md`'s Option 4 section and its two sweep design entries above are updated to match.
...
No experiment remains unbacked. The R2B pre-open hold window, the post-operative intensive care gate and the mass casualty stress test no longer belong on that list: `R/hold_window.R`/`scripts/run_hold_window.R`, `R/icu_gate.R`/`scripts/run_icu_gate.R` and `R/mass_casualty.R`/`scripts/run_mass_casualty.R` each give their comparison a runner of its own, `data/hold_window/`, `data/icu_gate/` and `data/mass_casualty/` track their per-replication responses and reductions, and `scripts/check_hold_window_protocol.R`, `scripts/check_icu_gate_protocol.R` and `scripts/check_mass_casualty_protocol.R` each assert the parameters, the tracked set and every figure [The R2B Pre-Open Hold Window](#the-r2b-pre-open-hold-window), [The Post-Operative Intensive Care Gate](#the-post-operative-intensive-care-gate) and [Mass Casualty Events Degrade Care Without Revealing New Constraints](Multi_Run_Analysis.md#mass-casualty-events-degrade-care-without-revealing-new-constraints) above now print from them. `scripts/check_icu_gate_switch.R` and `scripts/check_mass_casualty_kia_split.R` remain alongside their protocol checks, each covering a mechanism rather than magnitudes; neither is subsumed by its protocol check.

The mass casualty stress test was the weakest of the three cases Issue #384 recorded: at ten replications per arm, its own table stated that 13 deaths per arm were too few for a precise figure and labelled the ordinary-casualty finding direction only. Re-run at the 62 replications per arm [Mass Casualty Event Stress Test](#mass-casualty-event-stress-test) above derives, the comparison between event-origin and background casualties resolves; the comparison between the two arms' background-only casualties does not, and is reported as unresolved rather than re-labelled by assumption.
...
<!-- HOLD_WINDOW replications=30 -->
<!-- HOLD_WINDOW days=360 -->
<!-- HOLD_WINDOW seed=42 -->
<!-- HOLD_WINDOW arms=0,60 -->

30 replications of 360 simulated days per arm at control seed 42, under the shipped default configuration with one override, `r2b.surgery.pre_open_window_min` set to 0 in one arm against its shipped 60 in the other; the sustained-operations protocol [The Protocol](#the-protocol) establishes, migrated from the 30-day design this experiment previously ran under (Issue #405). Invoked as:
...
The two arms are not the same realisation. A zero-window run and a 60-minute run at the same control seed share their per-replication seeds, but the first hold shifts `simmer`'s single global stream, and the arrival streams are force-size-reactive closures sampled by thinning [[12]](#references) whose rate the force regeneration loop feeds back from casualty event timing, so the two arms drift into different casualty streams from the first hold onward. Not one of the 30 replication pairs generated the same number of casualties in both arms, and the paired difference in total casualties spans $-139.02$ to $+86.75$. Pairing on the control seed therefore removes none of the between-run variance.

**Migrating this experiment to the sustained horizon resolves one further row rather than the whole table, and the reason corrects the audit's own premise for choosing it.** The forward-surgery effect (R2B surgeries) now clears zero, +36.73 [+19.19, +54.28], where at 30 days it did not. That is the win the migration to #405 was costed against. But the remaining unresolved rows do not follow it: the busy-theatre bypass and R2E first-surgery paired differences, whose standard deviations were 10.4 and 29.5 casualties at 30 days, are now 62.2 and 160.2 at 360, a five- to sixteen-fold rise rather than the fall a longer window of accumulating events was expected to buy. The reason is the same stream divergence the paragraph above states: because the two arms are not paired on a shared casualty stream past the first hold, a longer campaign gives the streams more time to drift apart rather than more independent events to average over, so the noise in an absolute count grows with the horizon at least as fast as the count itself does. Resolving the busy-theatre bypass row to a half-width of two casualties now takes about 3,712 replications rather than 104, and the R2E first-surgery row about 24,655 rather than 838. The evidence set behind this design is measured at 30 replications per arm, reproducing the published table at the new horizon rather than resolving any row the previous design left open; only the forward-surgery row was won, and it was won by the horizon giving that response more surgeries to distinguish the arms by, not by any reduction in the stream-divergence noise the other rows still carry.
...
**Migrating this experiment to the sustained horizon did not resolve the mortality row, and the reps-needed figures moved the same direction the hold window's did.** At a fixed half-width of half a death per campaign, the count resolves to about 471 replications rather than 35 at 30 days, the paired standard deviation on deaths of wounds rising from 1.50 to 5.54; `total_casualties`, `icu_pathway_n` and `hold_pathway_n` move by a similar 11- to 14-fold factor in the same direction. Before this measurement, the expectation recorded above (see [Standardising the Other Experiments](#standardising-the-other-experiments)) was that this experiment would not carry the hold window's stream-divergence correction, both arms sharing one control seed without an event that shifts the stream the way introducing the hold does; the measurement does not bear that out; a count response accumulated over a longer campaign can pick up added variance from a shared seed alone, the intervention's own downstream effect on resource contention being enough to shift which events consume which draw. The one response that improved is `icu_occupancy`, a time-weighted share rather than an accumulating count, whose reps-needed fell from 50 to 6; R2E intensive care utilisation is consequently the only relief this design resolves more precisely at the new horizon than at the old one, and it already resolved at 30 days. The pathway death counts, being cumulative totals over a 12-fold longer window, at least stop returning an exact zero for the intensive care pathway, which is the other genuine gain: 3 of 18,440 rather than 0 of 2,534, a rate rather than an absence.
...
30 replications of 360 simulated days per sweep point at control seed 42, under the shipped default configuration with one override per point: `r2b_icu_share` set to 0, 0.25, 0.5, 0.75 and 1.0 in turn. Point 0 is the shipped default. The horizon and replication count are migrated from 30 days and 20 replications under Issue #405, matching the other two capacity sweeps at the sustained-operations protocol rather than resolving a finding specific to this sweep: it runs under the shipped default configuration, below the casualty load at which any response fails to converge, so nothing here was under-resolved the way the transport sweep's utilisation column was. Run via:
...
Forward intensive care utilisation rises steadily across the swept range, from 1.2% at a zero share, where the beds serve the evacuation wait alone, to 19.2% at a full one. That column therefore reads as the policy being applied rather than as noise. The absolute level moved with the migration to the sustained horizon, from a 30-day range of 12.4% to 24.3%, because the beds' evacuation-wait load (the non-zero share's floor) is itself a function of campaign length; the monotonic shape, which is the property the column is read for, survives unchanged.
...
30 replications of 360 simulated days per grid point at control seed 42, under the shipped default configuration with two overrides per point: R2B holding beds per unit set to 5 (shipped), 7 or 10, and `r2b.holding.evac_threshold` set to disabled (shipped, equivalent to 0), 1, 3, 5 or 7 days, the fifteen-point cross product of both axes. Each replication is drawn independently rather than paired, the same convention the two sweeps above it use. The horizon and replication count are migrated from 30 days and 10 replications under Issue #405, on the same reasoning the forward ICU share frontier's own migration states above: this sweep is retained at the shipped default configuration's casualty load, so the migration matches the other two capacity sweeps' protocol rather than resolving an under-resolution finding of its own.
...
**Why the parameter is swept as well as screened.** `evac_threshold` ships disabled, at zero, and disabling it changes the model qualitatively rather than by degree: the whole drawn convalescence is served forward rather than a bounded share of it. A Morris elementary effect computed across the first grid step, from a lower bound of zero to the next value tested, would measure that on/off transition rather than the threshold's magnitude, and the measured response bears this out rather than merely raising a theoretical concern. At the shipped five beds per unit, moving from disabled to a one-day threshold takes the R2B holding queue from 0.65 casualties [0.61, 0.69] to 0.015 [0.012, 0.017], a single grid step that reaches the queue's lowest swept value on its own (the queue then rises back toward its disabled level as the threshold lengthens, reaching 0.59 [0.56, 0.63] by seven days). Screening the parameter with a lower bound of zero would rank it by this step rather than by where a planner would actually set a threshold. The alternative the issue this sweep answers considered, a lower bound above zero, avoids the discontinuity but only by shipping a non-zero default, which would move every seed-42 figure and every calibration in this project for a screening convenience alone. Sweeping it directly answers the planning question this parameter exists for without either cost. The parameter is nonetheless included in the screened set, at a lower bound of zero, so that every other parameter's elementary effects are taken across configurations with the threshold in force as well as disabled; its own rank is accordingly a measurement of the on/off transition this paragraph describes, and is annotated as such wherever it is reported. `docs/Multi_Run_Analysis.md`'s Option 2 records the same finding against the planning recommendation it supports.
...
The horizon and control seed above are shared with the forward ICU share frontier and the R2B holding threshold sweep, all three now at the sustained-operations protocol under Issue #405 so that a reader comparing their tables is comparing one horizon rather than three, which is why this design states them and the other two do not repeat them. Only this sweep's migration resolves a finding specific to itself; the other two run under the shipped default configuration, below the casualty load at which any response fails to converge, and migrated to the same horizon and replication count for consistency with this one rather than because either had an under-resolved row of its own.

**Why this migration was expected to behave differently from the two paired-arm experiments migrated in the prior pull request.** This sweep is unpaired, each point drawing its own replications rather than sharing a control seed with another arm, so it carries none of the stream-divergence penalty that made the R2B pre-open hold window and the post-operative intensive care gate harder to resolve at 360 days (see [Standardising the Other Experiments](#standardising-the-other-experiments)). And the response the companion paper reads, mean queue, is a time-weighted average over the campaign rather than an accumulating count, the same kind of quantity as the intensive care gate's `icu_occupancy`, the one response in either prior migration that did improve. [Option 4](Multi_Run_Analysis.md#option-4-size-the-medical-evacuation-fleet-at-three-ambulances) records whether the measurement bears this out.

The measurement bears out the expectation above. Mean utilisation across the swept range now falls monotonically with fleet size on both platforms, as a fixed demand spread over more vehicles should, from 36.4% to 7.3% for ambulances and 10.5% to 3.5% for trucks at moderate intensity, and from 92.0% to 18.3% and 25.3% to 6.5% at high intensity; the 10-replication design's utilisation column, which ran the wrong way, is superseded by this one. Utilisation is still not the column a planning decision should rest on, since it describes how busy a vehicle is rather than whether a casualty waits for one, so the companion paper continues to read the queue column for its recommendation; the difference is that utilisation is now available as a supporting reading rather than excluded as noise.
...
The response is not the mean of anything. A campaign is classified collapsed where the R2E holding queue over its closing 90 days averages 20 casualties or more, and the reported quantity is the share of replications that collapse. A threshold classifier is used because the per-replication values are divided rather than spread: each arm carries a mass of campaigns whose closing queue is effectively zero and a separate spread of collapsed ones reaching into the hundreds, and reporting a mean over that population would describe no campaign in it. Across all 180 runs the highest clear campaign averages 17.4 and the lowest collapsed one 20.6, so the counts are unchanged for any threshold between 18 and 20. Outside that band the value does matter, a threshold of 22 returning six collapses at 15% loss rather than seven and one of 40 returning five, which is why the threshold is reported alongside the counts rather than treated as incidental.

Two properties of this experiment bear on how far its table can be read, and both are stated in the companion paper. A campaign's opening carries information about its outcome, which reverses the finding this design returned before the national support base levers were enabled: a logistic fit of collapse on the mean holding queue of a campaign's first 30 days is significant pooled across the four arms in which any campaign collapses ($p = 0.0014$, and $0.0010$ adjusting for the cancellation rate, so it is the opening queue rather than the arm carrying the fit), where the same fit previously distinguished nothing in any arm and returned $0.36$ pooled. Within arms it stays weak and mostly unresolved at 30 replications ($p = 0.13$, $0.10$, $0.18$ and $0.03$ at 10%, 15%, 20% and 25% loss), so the companion paper states it as a tendency rather than a forecast. The daily series the fit is made from is part of the tracked evidence set, so the statement is recomputable rather than resting on a run that is not repeated. And the median holding queue stays below 1 in five of the six arms, so the effect still sits in a tail that a summary of typical performance does not show.
...
The died-of-wounds difference between policies is reported with the replication count a decision would require rather than as a finding, and the measurement now resolves one of the four. Three paired differences against the shipped policy are indistinguishable from zero ($p = 0.94$, $0.78$ and $0.97$ at 15, 45 and 60 days); the fourth is not, the 30-day policy costing $2.77$ further deaths per campaign-year $[0.75, 4.79]$ at $p = 0.009$. That arm is also the one at which both R2E pools saturate and post-operative intensive care access falls to 5.2%, so it is supported by a mechanism the neighbouring arms do not share rather than being the isolated result four comparisons produce by chance. Resolving a difference of one death per campaign-year in the remaining arms would need 88 to 113 replications depending on the arm, derived by the normal approximation this document uses throughout from each comparison's measured paired standard deviation, against the 30 run. The design therefore resolves the forward-stability and force-cost responses and the 30-day mortality cost, and leaves the smaller mortality differences unresolved.
...
Thirty replications of 360 simulated days at each of nine saturation thresholds, at control seed 42 under the shipped default configuration with one override per arm: `r2eheavy.second_surgery.saturation_queue_threshold` at 0, 1, 2, 3, 5, 8, 12, 16 and 24 queued casualties. Zero disables the release and is the arm every other is measured against. Invoked as:

```
Rscript scripts/run_saturation_sweep.R --refresh-baseline
```

The sweep exists because the threshold had no source to set it from. No open-access source reports the surgical backlog at which a deployed hospital begins evacuating casualties between the abbreviated operation and the definitive repair, so the shipped value is an informed estimate under this project's source prioritisation, and the sweep is what that estimate is derived from rather than a check on a value chosen elsewhere. The shipped 8 is the mean closing-window theatre queue an unrelieved campaign carries, which anchors it on a measured property of the model rather than on whichever arm reads best among the responses the sweep itself reports; selecting the threshold on those responses would be choosing the answer from the same data used to measure it.

The range runs past the point at which a higher threshold might stop firing, so that an inert value is identified by measurement rather than assumed. None in the swept range is inert: at a threshold of 24 the release still fires 37 times per campaign-year and still halves the theatre queue against the disabled arm. The relief decays smoothly toward the disabled state rather than switching off, so the choice is where on a continuum to sit rather than which side of a cliff.

The response set is the policy sweep's, extended with four the trade needs and neither sweep previously carried: the forward theatre queue, the casualties released with the repair outstanding, and the operations and theatre-minutes owed at the national support base. The theatre queue is measured over both stages of the two-stage seizure, an operating theatre bed and then a surgical section, because a casualty waiting to be operated on is blocked on exactly one of them and counting either alone counts half the backlog; that is also what `r2e_theatre_queue()` reads live inside the model, so the swept response and the decision the sweep varies measure the same quantity. The arms are paired on one control seed as the policy sweep's are, each arm is checkpointed as it completes, and the sweep writes its own `saturation_sweep*` files. The tracked evidence set is `data/policy/`.

The threshold behaves as a saturation response across horizons, which is what makes a value that fires at length compatible with one that never fires in the published single-run campaign. The R2E theatre queue averages 0.98 casualties over a 30-day campaign at the Falklands-modified rates, 155 at high intensity and 8.2 over 360 days, so the shipped threshold fires constantly at high intensity and at sustained length, and not at all in a short moderate campaign. `scripts/check_definitive_repair_release.R` asserts both halves of that.
...
The count is derived rather than carried over from the ten replications the experiment ran at before Issue #389. [Replication Count and Resolution](Multi_Run_Analysis.md#replication-count-and-resolution) states that separating a died-of-wounds rate by a tenth of a percentage point takes 62 runs, on the treated-cohort standard deviation of 0.0039 measured over 150 replications elsewhere in this project; that is the size of difference this experiment's comparison was reported at before the re-run, so 62 is the smallest replication count in reach of resolving it. `--refresh-baseline` is the only way to write the tracked `data/mass_casualty/`, and it runs the protocol above rather than whatever arguments accompany it.
...
Three limitations are properties of the designs rather than of the model, and each bounds what the companion paper can claim from the experiment it applies to.
...
The transport sweep and the forward intensive care share sweep previously carried a fourth limitation here, their utilisation columns reading as noise at 10 and 20 replications respectively. Both sweeps migrated to the sustained-operations protocol under Issue #405, and utilisation now reads as a clean monotonic trend on both, so the limitation is retired rather than restated; see [Standardising the Other Experiments](#standardising-the-other-experiments) and [The Transport Fleet-Size Sweep](#transport-fleet-size-sweep).
