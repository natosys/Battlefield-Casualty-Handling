...
The Triage Priority Split (Priority 1/2/3, which must always sum to 1) renders as a single two-handle range slider with a colour-coded breakdown bar rather than three independent numeric fields, so the constraint is structural rather than something a planner can violate and then be told about after the fact. `app.R` converts the slider's two breakpoints to the three underlying `env_data.json` values (and back) at the point Quick Run or Save Configuration reads the form; the DNBI Battle Fatigue/Disease/NBI split (Issue #7), which carries the same sum-to-1 constraint, remains three independent fields with the original after-the-fact validation, since it has not yet been converted to the same slider treatment.
