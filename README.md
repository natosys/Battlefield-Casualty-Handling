...
Surgical candidacy is assessed next, behind an ICU availability gate. Priority 1 casualties proceed regardless of ICU status; Priority 2 and below defer entry to the operating theatre while every ICU bed is occupied, polling on a timer and holding no resource in the meantime. R2B surgery does not use ICU beds for post-operative recovery, and the `icu_bed` resources checked here are the same ones the evacuation-wait fallback uses, so the gate rarely fires at baseline load, where R2B ICU use is close to zero.
