...
**Intervention Scenario D — No-queue bypass (Issue #39, implemented).** The R2B No Surgery trajectory now performs a capacity check immediately on entry. If all hold beds across both R2B units are occupied, the arriving patient is routed directly to R2E without queuing (attribute `r2b_hold_bypass = 1`). This prevents infinite holding queues from accumulating and preserves R2E as the receiving echelon for overflow, which is the clinically appropriate disposition. The bypass path is identical to the evacuation-team transport used by surgical patients: the patient seizes an evacuation team, waits a triangular transport delay (`min = 15`, `mode = 30`, `max = 45` minutes), and enters `r2e_treat_wia()`. No hold duration is drawn for bypass patients; `r2b_hold_start` and `r2b_hold_drawn` remain unset. The `r2b_hold_bypass` count is reported in the analysis diagnostics. This intervention does not resolve the structural overload (bypassed patients still enter R2E, increasing R2E load) but eliminates queuing delays at R2B and ensures every patient is dispositioned in finite time.
...
Stream decomposition analysis (Issue #39) confirms that the five hold beds per R2B unit are structurally insufficient: expected concurrent occupancy is approximately 15.5 beds against a 10-bed total capacity across both R2B units (see R2B Hold Bed Saturation section). Four interventions have been analysed: hold duration reduction (insufficient alone), hold bed expansion to 8–10 per unit (structurally resolves the overload), an evacuation threshold policy routing long-duration holders to R2E (implemented; activation requires setting `vars.r2b.holding.evac_threshold` in `env_data.json`), and a no-queue bypass (Issue #39, implemented) that routes any patient arriving when hold beds are full directly to R2E without queuing (`r2b_hold_bypass = 1`). The no-queue bypass eliminates hold queuing delays entirely but transfers overflow load to R2E rather than resolving the structural capacity deficit. **Impact: Medium** — with no-queue bypass active, patients are dispositioned in finite time; the remaining limitation is that overflow increases R2E medical hold load, which will become consequential once time-dependent DOW (Issue #5) is merged. **Impact rises to High once Issue #5 is merged**, at which point hold bypass routing will directly affect modelled mortality pathways.
