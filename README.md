...
To inform the design and implementation of the battlefield casualty simulation, a structured literature review was conducted in two phases. The initial phase used a multi-pronged methodology. First, open-access academic literature and publicly available internet-based resources on battlefield casualty modelling, discrete event simulation (DES), and casualty rate estimation were surveyed. This was complemented by a snowballing technique, recursively exploring the references cited within key papers to identify additional relevant sources. Large language models (LLMs) were then engaged iteratively to identify supplementary resources — algorithmic approaches to casualty distribution modelling, simulation model approaches, and operational insights into trauma system design under combat conditions — surfacing niche publications, technical reports, and grey literature that conventional search techniques did not. Finally, this academic and technical review was supplemented by a survey of publicly available military publications, including field manuals, operational health service support doctrine, and policy documents from allied defence organisations, providing critical context for force structure, casualty flow assumptions, and the operational constraints that shape real-world medical planning. This initial phase established the foundational casualty-rate, DES, distributional, and doctrinal basis summarised in the first four thematic domains below.

The review did not stop there. As the simulation was subsequently extended feature by feature — the three-stage damage control surgery (DCS) model and its treatment-duration and post-operative recovery parameters, the time-dependent died-of-wounds survival function, the Falklands 1982 historical calibration exemplar, the strategic aeromedical evacuation (AME) and Role 4 demand model, the compound Poisson mass casualty event mechanism, and the statistical framework underpinning multi-run replication, warm-up analysis, and Morris/Sobol sensitivity screening — each addition triggered its own deliberate, AI-assisted search for open-access sources addressing that specific clinical, doctrinal, or statistical question, consistent with this project's Academic Standards requirement that every parameter, probability, and distribution be cited to its source at the point it is introduced. This iterative extension is summarised in the further thematic domains that follow the initial four, each tied to the design or parameterisation requirement of a specific subsequent Issue. The literature review is therefore maintained as a living, cumulative component of the project rather than a one-off survey completed before implementation began.
...
Recent doctrinal analyses [[9]](#References), [[2]](#References) and [[5]](#References) emphasized the shift toward prolonged casualty care, contested evacuation, and the need for distributed medical assets. A related analysis [[19]](#References) frames return-to-duty as a direct force-sustainment lever rather than a purely clinical outcome — a framing this project's endogenous force regeneration feedback loop ([Force Regeneration and the Endogenous Feedback Loop](#6-force-regeneration-and-the-endogenous-feedback-loop)) implements directly, coupling casualty production to a live, time-varying effective force size rather than a fixed roll strength. These insights were incorporated into the simulation design to ensure alignment with contemporary operational realities.

#### Damage Control Surgery and Post-Operative Critical Care

The simulation's three-stage damage control surgery (DCS) model — resuscitation, abbreviated surgery, and staged post-operative critical care — is grounded in the clinical DCS literature. Zizzo et al.'s (2020) [[20]](#References) systematic review and Karamarković's (2016) [[21]](#References) account of damage control technique in abdominal surgery establish the three-phase structure that this project's R2B and R2E Heavy trajectories implement. Operative-time data reported for Sohn et al.'s (2018) cohort within Zizzo et al.'s review [[20]](#References) directly sourced the DAMCON surgery duration distribution (triangular, 41–210 minutes, mode 95), corroborated by the rapid-closure operative-window principle central to damage control technique [[22]](#References) and by outcomes literature from an austere-setting DCS series [[23]](#References) and abdominal-trauma DCS practice [[24]](#References). Post-operative critical care requirements — the expectation that DCS is followed by a defined period of ICU-level monitoring rather than an unmonitored recovery area — are established by [[25]](#References), [[22]](#References) and [[24]](#References), and Hardcastle et al.'s (2025) ERAS/IATSIC guidelines [[26]](#References) confirm post-operative ICU or high-dependency care as the recommended standard following major trauma surgery; descriptions of post-DCS stabilisation timeframes (24–36 hours) from this literature [[20]](#References), [[27]](#References), [[24]](#References) informed the ICU length-of-stay parameters at both echelons. Task-time estimation for the R2B/R2E resuscitation phase, where no single published source tabulates an end-to-end duration, drew on the treatment-process optimisation methodology of Hodický et al. (2020) [[28]](#References), constructing an estimate from collated task-duration components rather than a single reported figure. The treatment-efficacy multipliers applied to the died-of-wounds ceiling at each care phase are evidence-based rather than estimated outright: Braverman et al. (2021) [[29]](#References) report a 41% relative mortality reduction from balanced-component damage control resuscitation, and Holcomb et al.'s (2013) PROMMTT study [[30]](#References) provides the haemorrhage-specific mortality basis used to derive the surgical efficacy factor.

#### Preventable Death and Time-Dependent Mortality

The time-dependent died-of-wounds survival function (see [Died of Wounds](#died-of-wounds)) is grounded directly in the preventable-death literature. Eastridge et al. (2012) [[31]](#References) analysed 4,596 battlefield deaths during Operations Enduring Freedom and Iraqi Freedom and found that 87.3% resulted from haemorrhage, predominantly within 30–90 minutes of injury; Kotwal et al. (2011) [[32]](#References) demonstrated that a "golden hour" surgical-capability policy reduced preventable prehospital death from 32% to 3.5% in a Special Operations context. Together these establish the direct empirical link between time-to-care and survivability that motivates modelling died-of-wounds probability as a function of elapsed time since injury, rather than a flat rate independent of system load — the logistic curve's shape parameters are anchored to the mortality time-windows these two studies report.

#### Strategic Aeromedical Evacuation and Role 4 Doctrine

The strategic AME and Role 4 (national support base) demand model (see [Role 4 (National Support Base) Demand Modelling](#role-4-national-support-base-demand-modelling)) draws on NATO's AJP-4.10 allied joint doctrine for medical support [[33]](#References), which establishes the triage framework, echelon functions, and the Casualty Staging Unit/CCATT-CCAST acuity concepts underpinning the model's critical/standard AME pool split. Role 4 ward assignment and length-of-stay parameters follow the general injury-severity length-of-stay gradient described in Nessen et al.'s (2008) *War Surgery in Afghanistan and Iraq* case series [[34]](#References), applied by informed extension since no open-access source tabulates Role 4 ward assignment by this project's exact DNBI/priority category scheme.

#### Mass Casualty Event and MASCAL Simulation

The compound Poisson mass casualty event mechanism (see [Casualty Generation — Mass Casualty Event Injection](#5-mass-casualty-event-injection)) follows the general approach to LSCO casualty-surge simulation described by Fischer et al. (2025) [[35]](#References), and complements the discrete-event mass-casualty-incident stress-testing precedent of SIMEDIS [[36]](#References) — which, unlike either of this project's event-timing modes, injects a fixed, deterministic victim count per scenario rather than a stochastically varying one. Dilday's (2026) [[37]](#References) account of aligning field hospital training with LSCO reality informed the blast-dominant triage priority distribution and high sustained-casualty-tempo context applied to mass-casualty-derived casualties, distinguishing the acute-surge component this feature models from the sustained background tempo the project's lognormal/exponential arrival streams already represent. Fischer et al. [[35]](#References) additionally informed the project's transport dead-heading model: tactical vehicle rate-of-march is governed by terrain, threat, and route conditions rather than payload, so laden and unladen return legs are not doctrinally differentiated by travel time.

#### Statistical Methods for Simulation Verification, Replication, and Sensitivity Analysis

Moving from a single illustrative run to a defensible multi-run analytical framework required its own literature base, distinct from the clinical and doctrinal sources above. Verification and validation followed Sargent's (2010) [[38]](#References) framework and the general DES methodology of Banks et al. [[39]](#References). Warm-up classification used Welch's graphical procedure [[40]](#References) and Law's (2020) [[41]](#References) treatment of terminating-versus-steady-state simulation, with Gafarian, Ancker and Morisaku (1978) [[42]](#References) establishing that warm-up detection methods presuppose a steady state that does not apply to a campaign with a defined finite horizon. Multi-run replication and variance reduction drew on Karl et al.'s (2014) [[43]](#References) demonstration of L'Ecuyer-CMRG parallel random number streams in R, the R Core Team's own documentation of that mechanism [[44]](#References), and Rossetti's (2023) [[45]](#References) treatment of antithetic variate variance reduction. Global sensitivity screening used Morris Elementary Effects [[46]](#References), implemented via R's `sensitivity` package [[47]](#References) and the OpenMOLE documentation of the method [[48]](#References), and Sobol variance-based decomposition following Saltelli et al. (2010) [[49]](#References). Williams et al.'s (2020) [[50]](#References) critical-care DES study, which found stable steady-state estimates required a substantial replication count and reported confidence intervals as standard practice, informed the Shiny application's default replication count and its use of 95% CI bands throughout.

#### Disease and Non-Battle Injury Evidence

Disease DNBI sub-categorisation and its emergency surgical candidacy rate draw on documented evidence of acute surgical disease conditions occurring in deployed populations. Black (2002) [[51]](#References) reports acute appendicitis incidence among deployed Japanese soldiers in Burma, and Weeks et al. (2017) [[52]](#References) document humanitarian surgical care delivered at US military treatment facilities in Afghanistan — together supporting the inclusion of conditions such as appendicitis, cholecystitis, and perforated peptic ulcer within the disease DNBI pathway's surgical candidacy branch, distinct from the battle fatigue and NBI sub-types that do not carry the same surgical risk profile.
...
> **Basis:** Historical data from Iraq and Afghanistan conflicts documents psychiatric and adjustment disorder rates consistent with this proportion (Izaguirre et al., 2025 [[19]](#References)). No open-access ADF-specific figure is available.
> **Uncertainty:** Medium
...
The clinical motivation is the well-documented relationship between time from injury and preventable death. Eastridge et al. (2012) [[31]](#References) analysed 4,596 battlefield deaths during Operations Enduring Freedom and Iraqi Freedom and found that 87.3% resulted from haemorrhage — predominantly junctional and truncal wounds inaccessible to tourniquet — with the majority occurring within 30–90 minutes of injury. Kotwal et al. (2011) [[32]](#References) demonstrated that enforcing a "golden hour" policy mandating surgical capability within 60 minutes of injury reduced preventable prehospital death rates from 32% to 3.5% in a Special Operations context, establishing a direct empirical link between time-to-care and survivability.
...
| -------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1 TCCC                    | 0.83   | Eastridge et al. (2012) [[31]](#References) identify non-compressible haemorrhage (truncal, junctional) as the mechanism in 90% of potentially preventable battlefield deaths — injuries beyond the scope of TCCC intervention. TCCC skills (tourniquet, wound packing, airway management) address the remaining 10%, yielding a modest 17% ceiling reduction. |
| R2B DCR (resus)            | 0.56   | Braverman et al. (2021) [[29]](#References) report that damage control resuscitation with balanced haemostatic products reduces laparotomy mortality from 22% to 13% — a 41% relative reduction — reflecting the haemostatic benefit of early plasma and platelet administration.                                                                              |
| R2B DCS (surgery)          | 0.32   | Holcomb et al. (2013) PROMMTT [[30]](#References) reported a 40% overall mortality rate in massively transfused surgical patients, with exsanguination accounting for 33.3% of deaths — approximately 13% haemorrhage-specific post-DCS mortality. This implies a 68% relative reduction from the pre-DCS ceiling, applied as a factor of 0.32.                |
| R2E DCR (resus)            | 0.56   | Same factor as R2B DCR [[29]](#References); applied only when full resuscitation occurs at R2E (i.e., the casualty bypassed R2B). Casualties pre-resuscitated at R2B receive a short resus at R2E; this factor is not re-applied, avoiding double-counting the DCR effect.                                                                                     |
| R2E DCS 1st op             | 0.25   | Post-operative mortality in optimally resuscitated DCS patients is approximately 3–5% at 30 days — a 75% relative reduction from the pre-first-DCS ceiling [[29]](#References).                                                                                                                                                                                |
...
> **MODEL ASSUMPTION — DOW LOGISTIC PARAMETERS:** The parameters *p_base*, *p_max*, *k*, and *t_mid* are calibrated to the Falklands War 1982 (Operation CORPORATE) historical outcome rather than empirically fitted to per-minute individual-level survival curves, which no published dataset provides. Payne (1983) [[13]](#References) reports three post-operative deaths among 233 casualties treated at the Ajax Bay Advanced Surgical Centre and forward stations. Jolly (2018) [[14]](#References) confirms that of 580 British soldiers and marines wounded in action, only three died of wounds — a DOW/WIA rate of 0.52%. The ceiling values (p1_p_max = 0.023, p2_p_max = 0.019) were iteratively calibrated until 50-replication Monte Carlo output produced a mean DOW/run of approximately 0.70 (0.45% of 154 baseline WIA), with a 95% CI spanning the 0.52% historical target. The shape parameters (k, t_mid) are anchored to aggregate mortality time-window analysis in Eastridge et al. (2012) [[31]](#References) and Kotwal et al. (2011) [[32]](#References); the logistic form is a standard S-shaped approximation for time-dependent failure processes (Law, 2020 [[41]](#References)).
> **Basis:** Payne (1983) [[13]](#References); Jolly (2018) [[14]](#References); Eastridge et al. (2012) [[31]](#References); Kotwal et al. (2011) [[32]](#References).
...
> **Basis:** The clinical trade-off is described in Turner & Wilson (2024) [[25]](#References) and Remondelli et al. (2023) [[2]](#References); the standard of post-operative ICU/HDU care against which the "hold" pathway is a departure is set out in Hardcastle et al. (2025) [[26]](#References). The specific 3.0× penalty multiplier is an informed estimate — no open-access source quantifies a ward-vs-ICU mortality ratio for post-DCS trauma patients specifically — chosen to produce a materially higher, but not overwhelming, realised DOW rate for the hold pathway relative to ICU.
...
> **Basis:** DCR factor (0.56) anchored to Braverman et al. (2021) [[29]](#References); DCS factor (0.32) anchored to Holcomb et al. (2013) PROMMTT [[30]](#References); TCCC factor (0.83) derived from Eastridge et al. (2012) [[31]](#References) non-compressible haemorrhage analysis. The R2E DCS second-operation factor (0.57) is an informed estimate with no direct literature anchor.
...
| ------------------------ | ------------------ | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1 TCCC                  | 0.83               | 1.0                  | TCCC is a post-1990s doctrine [[31]](#References); no equivalent tourniquet-forward/haemostatic-dressing prehospital doctrine is documented for 1982 British forces in the available sources. No additional ceiling reduction is attributed to this checkpoint.                                                                                                   |
| R2B / R2E resuscitation  | 0.56               | 0.90                 | Braverman et al.'s (2021) [[29]](#References) 0.56 factor is specific to balanced-component damage control resuscitation. A modest benefit from whole-blood/crystalloid resuscitation (available in 1982) is retained; the specific balanced-ratio benefit is not.                                                                                                |
| R2B DCS / R2E DCS 1st op | 0.32 / 0.25        | 0.55                 | Payne (1983) [[13]](#References) records near-zero post-operative mortality among casualties who reached the Ajax Bay Advanced Surgical Centre, so definitive surgical intervention itself is retained as materially protective; the more aggressive modern factors reflect additional staged damage-control/haemostatic-adjunct technique not available in 1982. |
...
> **MODEL ASSUMPTION — FALKLANDS-ERA TREATMENT EFFICACY:** The `moderate_intensity` treatment efficacy factors are informed estimates, not literature-derived values — no open-access source quantifies 1982 British field-surgical efficacy in the same multiplicative-ceiling terms used by this model. They were constructed by reasoning from the absence of specific modern techniques (TCCC, balanced DCR, staged DCS) documented in [[31]](#References), [[29]](#References), [[30]](#References), while preserving Payne's (1983) [[13]](#References) and Jolly's (2018) [[14]](#References) evidence that 1982 field surgery itself was highly effective for casualties who reached it.
> **Basis:** Payne (1983) [[13]](#References); Jolly (2018) [[14]](#References); absence of a documented equivalent to [[31]](#References), [[29]](#References), [[30]](#References) for the 1982 conflict.
...
> 
> - **Rule A (citation-anchored, moderate uncertainty):** approximately baseline ±40%, applied to parameters whose baseline value traces to a specific open-access source already cited elsewhere in this README (e.g. the DOW logistic shape parameters to Eastridge et al. (2012) [[31]](#References) and Kotwal et al. (2011) [[32]](#References); the treatment-efficacy factors with a named clinical-trial basis to Braverman et al. (2021) [[29]](#References) or Holcomb et al. (2013) [[30]](#References); the casualty generation rates to FORECAS [[8]](#References)).
...
> **Basis:** Informed estimation (Source Prioritisation level 5), directionally consistent with the Role 4/CONUS length-of-stay patterns for combat casualties by injury category described in Nessen et al. (2008) [[34]](#References). This source has not been used to extract exact per-category day counts for this model; a direct extraction of tabulated LoS data from it is identified as further development work.
> **Uncertainty:** High
...
DOW probability is now modelled as a time-dependent shifted logistic function of elapsed time since injury, calibrated to [[31]](#References) and [[32]](#References). The conditional increment formulation across echelons ensures that queue saturation and evacuation delay produce measurable increases in modelled mortality. DOW count and rate by echelon are now sensitive to system load. See the [Died of Wounds](#died-of-wounds) section for parameter details.
