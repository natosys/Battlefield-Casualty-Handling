...
Mean DNBI per run falls under `high_intensity`, from 169.9 to 157.3, even though the profile overrides only the WIA and KIA streams and leaves DNBI generation parameters untouched. This is a consequence of the endogenous force feedback loop (see [Force Regeneration and the Endogenous Feedback Loop](#6-force-regeneration-and-the-endogenous-feedback-loop)) rather than an artefact of the profile. Every stream's arrival rate is expressed per 1,000 troops and multiplied by the effective force size read fresh at each minute step, so the heavier battle attrition this profile produces depletes the exposed population faster and generates proportionally fewer disease and non-battle injury casualties. Higher-intensity warfighting therefore raises total casualty load while modestly reducing the absolute DNBI contribution to it, because fewer troops remain in theatre to fall sick.
