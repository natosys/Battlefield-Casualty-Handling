...
One caution bounds how far this table should be read, and one earlier caution no longer holds. A campaign's opening does carry information about its outcome, which is a change from the measurement made before the national support base levers were enabled: fitting collapse on the mean holding queue of a campaign's first 30 days is significant pooled across the four arms that collapse at all (p = 0.0014, and 0.0010 adjusting for the cancellation rate, so it is the opening queue rather than the arm doing the work). Within arms it is weaker and mostly unresolved at 30 replications (p = 0.13, 0.10, 0.18 and 0.03 at 10%, 15%, 20% and 25% loss), so this is a tendency rather than a usable forecast: a campaign that opens with a filling holding pool is likelier to end collapsed, but no single campaign's first month settles its year. And the median holding queue stays below 1 in five of the six arms, so a reader watching typical performance would see nothing wrong anywhere across that range. The entire effect sits in the tail, which is why the variance-based sensitivity screen in the README ranks this parameter 50th of 65 on the system theatre queue: those methods measure a parameter's effect on the mean of a response, and this one acts on how often a campaign fails altogether. The same screen ranks it 6th of 65 on mean Role 4 occupancy and 12th on the standard-route evacuation backlog, so the parameter is not weak, it is weak on the response the published ranking happens to report.
