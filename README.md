...
3. **Reinforcement** — every `force_regeneration.reinforcement.demand_interval_days` (`env_data.json`), each pool independently submits a demand equal to its current shortfall against establishment strength, net both of any shortfall already claimed by an earlier, still-pending cycle and of any reinforcement already delivered but not yet absorbed (`initial − current − pending − carry`, floored at 0), then, after a configurable `fulfillment_lag_days`, delivers a fraction of that demand drawn from a Triangular(`fill_min_frac`, `fill_mode_frac`, `fill_max_frac`) distribution. A pool absorbs only as much of a delivery as it has room for, so it can never rise above `initial`; whatever arrives beyond that room is held as carry and absorbed on a later cycle rather than discarded (`build_reinforcement_trajectory()`, `R/trajectories.R`). Netting the carry out of demand is what keeps a pool from requesting the same reinforcement twice, and so bounds the carry. This is the one genuinely periodic mechanism in the model; it ships disabled (`demand_interval_days = 0`) so no generator is added and no RNG draws are consumed.
...
Because a delivery beyond the room available is carried rather than dropped, the fraction the model applies is the fraction the distribution drew. Measured over 489 delivered cycles in each pool, the realised mean fill fraction is 0.715 (standard error 0.009) for the combat pool and 0.710 (0.009) for the support pool, against the distribution's own mean of 0.7167. About 4.4% of draws exceed the demand they were drawn against, and 1.8% of delivered cycles found less room in the pool than they carried, that second figure depending on how fast the pool is depleting during the fulfillment lag rather than on the distribution alone. Discarding the excess instead would lower the realised mean to 0.7151 in the case where room and demand coincide, and would leave `fill_max_frac` naming an over-delivery the model could never apply. `scripts/check_lever_realisation.R` asserts that everything delivered is either absorbed or carried.
