...
Across 50 replications of each scenario (30 simulated days, seed 42), the comparison confirms that the current establishment's adequacy conclusion does not extrapolate from Falklands to Okinawa intensity: mean total casualties per run rise 2.30-fold, the R2E Operating Theatre mean queue rises approximately 180-fold, the R2E Intensive Care Unit mean queue rises approximately 4.7-fold from a low base, the R2E Holding bed mean queue rises approximately 5.2-fold from a base already materially non-zero, and the R2B Holding bed mean queue rises approximately 7.1-fold, while R2B OT queue remains at zero in both scenarios — not because R2B absorbs any of the surge, but because the model's existing bypass routing diverts all surgical overflow to an already-saturated R2E. Died-of-wounds rate as a proportion of WIA rises from 0.55% to 0.79%. Transport (PMV Ambulance / HX240M) remains the one echelon with genuine headroom at both intensities.
...
The system's resilience to surge is directly quantified rather than inferred. This multi-run comparison confirms that neither R2B nor R2E can absorb Okinawa-intensity casualty rates without fundamental redesign: R2E OT mean queue rises approximately 180-fold and R2E Hold mean queue approximately 5-fold relative to the Falklands-modified baseline, R2B Hold queue rises approximately 7-fold, and DOW/WIA rate rises by a factor of roughly 1.4 — all while R2B OT queue remains at zero only because the existing bypass routing shunts all surgical overflow onto an already-saturated R2E rather than R2B absorbing any of the surge itself. Effective LSCO medical support at Okinawa intensity would require scalable holding capacity at forward echelons, a deeper surgical roster at R2E, and dynamic load-balancing between R2B and R2E — capabilities the current static establishment does not provide.

These figures were regenerated at 50 replications per scenario after the per-minute rate cap was removed from the arrival generators, and they supersede the comparison made while that cap was holding realised generation below the rates the configuration named (see the README's [Casualty Generation](../README.md#casualty-generation)). Both profiles moved, and the interesting point is that they moved by different amounts, which is what compresses every ratio in the tables above. `moderate_intensity` draws all six streams from lognormals, whose WIA and KIA arms had been realising only about 79% of their configured means, so its totals rise sharply: total casualties from 381.1 to 441.6 per run and WIA from 151.1 to 189.8. `high_intensity` overrides its WIA and KIA streams to exponential, and an exponential stream had been realising 95% of its mean whatever that mean was, so it gains only what that 5% and its lognormal DNBI streams give it: total casualties from 972.5 to 1,015.6. The surge arm therefore barely moves while the baseline arm rises by a sixth, and the headline R2E theatre queue ratio falls from roughly 430 to roughly 180 largely because its denominator grew rather than because the surge eased: the `high_intensity` mean theatre queue itself moves only from 39.3 to 40.8.

The died-of-wounds rows are the exception to that reading, and should not be interpreted as a movement at all. Each is a single 50-replication measurement of a response averaging one death per replication at Falklands load, which the README's [Further Development](../README.md#further-development) entry L22 shows is resolved only to roughly a tenth of a percentage point at that replication count. The `moderate_intensity` DOW/WIA rate reads 0.55% here against 0.44% in the previous measurement, a gap well inside that resolution, while the 150-replication pooled treated-cohort figure for the same profile moved only from 0.279% to 0.353%. The queue and occupancy rows above are time-weighted over far more events per replication and are correspondingly better resolved.

The `high_intensity` rows were re-measured again after the replication framework was corrected so that a measurement is a function of its control seed alone (see the README's [Multi-run Replication Framework](../README.md#multi-run-replication-framework)). Every `moderate_intensity` figure is unchanged to the precision reported here, that profile being measured first in the previous run and so already drawn under the generator every measurement now uses. `high_intensity` moved because it now runs on the same per-replication seeds as the profile ahead of it rather than on wherever the parent stream had reached by the time its turn came, which makes the pair a comparison on common random numbers and makes each profile reproducible on its own. The movements are the size that replication-to-replication variation at 50 replications produces: total casualties 1,015.7 to 1,015.6, the R2E theatre mean queue 39.2 to 40.8, and died of wounds 5.18 to 5.42 deaths per run.

The qualitative conclusion is unchanged and the quantitative one is now measured against generation rates the configuration actually delivers. The run was made in an unpinned R 4.3.3 sandbox rather than the project's pinned Dev Container, under the same caveat as the other figures refreshed alongside it (see `CLAUDE.md`'s Key Parameters provenance caveats). `images/scenario_comparison.png` was regenerated alongside the tables by the command given above. A comparable Vietnam-intensity comparison remains unavailable pending a genuine FORECAS-sourced Vietnam combat-troop WIA/KIA table.
