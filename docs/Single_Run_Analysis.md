...
Every bed type the echelon fields appears here, and all four panels share one vertical scale, so the depths are comparable between pools rather than each filling its own panel. The theatre queue is the deep one, rising and falling in waves that reached fourteen casualties on day 28 and twelve on day 27 across only two rooms. The holding and intensive care queues sit close to the axis by comparison, neither exceeding a single casualty on any one bed, but they are shallow in different ways: the intensive care queue formed behind three of its four beds, while the holding queue appeared behind 19 of the thirty at one time or another, which is a pool absorbing a broad low-level demand rather than one turning casualties away. The four resuscitation beds never queued at all. Read together, the panels place the pressure on theatre access rather than on beds, which is the reading the rest of this section develops.
...
![Stacked bar chart of daily Role 4 bed occupancy split between intensive care and holding beds, flat at zero until day 8, rising to about a hundred and twenty concurrent patients on day 30, then decaying to near zero by day 68, with holding beds about three quarters of the total throughout and intensive care the remainder, and a dotted line marking the end of the 30-day engagement window](../images/role4_census.png)
