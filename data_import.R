...
#       resource_vector <- c()
#       
#       # ── Case: structured sub_elms ──
#       if (!is.null(elm$sub_elms) && !identical(elm$sub_elms, 0)) {
#         for (sub in elm$sub_elms) {
#           if (is.null(sub$sub_elm)) {
#             for (res in sub$resources) {
#               res_type <- res$type
#               res_name <- if ("name" %in% names(res)) res$name else res$resource
#               qty <- if (!is.null(res$qty)) res$qty else 0
#               for (j in seq_len(qty)) {
#                 res_id <- paste("c", elm_name, res_type, res_name, j, paste0("t", i), sep = "_")
#                 resource_vector <- c(resource_vector, res_id)
#               }
#             }
...
#           } else {
#             sub_name <- sub$sub_elm
#             sub_qty <- if (!is.null(sub$qty)) max(1, sub$qty) else 1
#             sub_teams <- vector("list", sub_qty)
#             
#             for (team_index in seq_len(sub_qty)) {
#               sub_vector <- c()
...
#                 }
#               }
#               sub_teams[[team_index]] <- as.character(sub_vector)
#             }
#             
#             instance[[sub_name]] <- sub_teams
#           }
...
#         
#         # ── Case: sub_elms == 0 ──
#       } else if (identical(elm$sub_elms, 1)) {
#         if (!is.null(elm$resources)) {
#           for (res in elm$resources) {
#             res_type <- res$type
#             res_name <- if ("name" %in% names(res)) res$name else res$resource
#             qty <- if (!is.null(res$qty)) res$qty else 0
#             for (j in seq_len(qty)) {
...
#       
#       # ── Assign instance ──
#       if (elm_name == "r1" || identical(elm$sub_elms, 0)) {
#         elm_instances[[i]] <- as.character(resource_vector)
#       } else {
#         elm_instances[[i]] <- instance
#       }
#       
...
#           bed_type <- bed$name
#           bed_ids <- paste0("b_", elm_name, "_", bed_type, "_", seq_len(bed$qty), "_t", i)
#           elm_instances[[i]][[paste0(bed_type, "_bed")]] <- bed_ids
#         }
#       }
#     }
#     
