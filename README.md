...
With these weaker factors, `dow.params` was re-calibrated by the same iterative Monte Carlo procedure used for the base configuration, reaching the historical target at `p1_p_max` = 0.0089 and `p2_p_max` = 0.0074, down from the base 0.023 and 0.019. A lower ceiling is required to compensate for the weaker factors' smaller ceiling reduction. The floors (`p1_p_base`, `p2_p_base`), the shape parameters ($k$, $t_{mid}$ for both priorities), and the P3 flat rate (`p3_flat`) are unchanged from base under this profile; only the two ceilings are re-calibrated. Each of these terms is defined by the shifted logistic curve given in [Survival Function](#survival-function). A 50-replication run (30 days, `seed = NULL`) of `moderate_intensity` produced:

| Metric                  | `moderate_intensity` (50-rep)      | Historical comparator                                              |
| ----------------------- | ---------------------------------- | ------------------------------------------------------------------ |
| Treated-cohort DOW rate | 0.610% (95% CI [0.500%, 0.730%])   | ≈0.46% (3 deaths among over 650 reaching forward surgery [[14]](#References)) |
| Mean DOW/run            | 1.35 (95% CI [1.10, 1.60])         | No directly comparable published figure                            |
| DOW/WIA rate            | 0.910% (95% CI [0.740%, 1.070%])   | Not historically constrained (see Further Development, L22)        |
| KIA:WIA ratio           | 0.451                              | 0.328 (255 killed : 777 injured [[53]](#References), [[54]](#References)) |

The treated-cohort rate, measured over the casualties that reach an R2B or R2E facility, now sits above the historical comparator rather than spanning it: its 95% confidence interval runs from 0.500% to 0.730% against the Ajax Bay figure of approximately 0.46%. The base configuration sits in the same place by the different mechanistic route described in [Parameter Calibration](#parameter-calibration), giving 0.617% with a 95% confidence interval of [0.503%, 0.732%] over 50 replications. Both profiles were within the target before intensive care after the definitive repair was modelled as a separate episode, and neither has been re-fitted since; see [Further Development](#further-development), L22. The KIA:WIA ratio does not match, but this is a characteristic of the base casualty generation rates that both profiles share rather than something this profile introduced, since the profile overrides only the DOW ceiling and treatment efficacy factors (see Further Development).
...
| R2B forward ICU penalty             | `r2b_icu_penalty`                       | 1.31×    | 1.09   | 1.59  | A    |
...
Bounds are derived by transforming a compositional range rather than stated in coordinate space, where a number would have no intuitive meaning. The first coordinate's bounds transform a plausible range for the leading part's share, holding the ratio of the two trailing parts at baseline, which is the sub-composition the first balance leaves untouched: Priority 1 spanning 0.45 to 0.80 for the triage split, disease spanning 0.40 to 0.75 for the DNBI composition, and mass casualty Priority 1 spanning 0.55 to 0.85. Each range brackets the observed variation the corresponding baseline was drawn from, the triage and DNBI ranges against the Falklands and FORECAS sources cited for those baselines [[8]](#References), and is wider than either rule would give, since a composition assumed rather than measured deserves the wider bound. The second coordinate applies Rule B multiplicatively to the ratio of the two trailing parts (×0.5 to ×2.0), which in coordinate space is the symmetric interval $\pm\sqrt{1/2}\ln 2$ around the baseline, a balance being a scaled log ratio.
...
For any group that screens as influential, a confirmatory decomposition treats the composition as a single object. `run_sobol()` samples whole compositions from a Dirichlet centred on the group's baseline, transforms them to coordinates, and computes first-order and total-order Sobol indices [[49]](#References) over the same design as the other selected parameters, which reports how much output variance the composition explains rather than how much each contrast does. Both coordinates of a group enter together whenever either is selected, since an index on one coordinate of a composition varied a coordinate at a time would not be an index on the composition. The Dirichlet concentration is set per group so that two standard deviations of the leading part match the half-width of the same compositional range the Morris bounds use, giving 28.7, 30.8 and 36.3 respectively, rather than a single figure chosen for no stated reason.
...
**L9 — Antithetic variates applied to arrivals only.** Variance reduction is applied to arrival generation. Service times and routing probabilities are drawn inside simmer's engine from the global random number stream and cannot be paired without deep trajectory instrumentation, so the confidence-interval narrowing is smaller than full antithetic pairing would give.
