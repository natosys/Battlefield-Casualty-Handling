...
| 148 | Lognormal casualty generator uses a fixed absolute rate cap, not a mean-relative one | Medium | Low | **Merged (PR #202)** |
...
### Issue 148 — Lognormal Casualty Generator Uses a Fixed Absolute Rate Cap ✓

**Merged:** PR #202, branch `claude/issue-148-iqwyi5`

`make_ln_arrival_generator()` capped every per-minute lognormal draw at one fixed absolute value of 5, applied identically to every lognormal stream whatever its own mean. `make_exp_arrival_generator()` was corrected to a mean-relative cap during Issue #54; the fix was never back-ported. The cap is now `cap_multiplier × mean_daily` at the same default multiplier of 3 in both generators. Truncation probability across the four shipped lognormal parameterisations narrows from a spread of roughly 150-fold (0.05% to 7.32%) to under 6-fold (1.16% to 6.71%); it does not become uniform, because a lognormal's tail above a multiple of its mean also depends on its coefficient of variation, which genuinely differs between streams, and the roxygen and README say so rather than claiming the exact invariance the exponential case has. The latent failure the issue was raised against is closed: a lognormal stream entered at FORECAS's high-intensity WIA mean of 6.86 would have had 49.3% of its draws truncated and realised 57% of the mean asked for, against 94.6% under the mean-relative cap.

The shipped baseline moved more than the issue anticipated, which judged the default profile not materially affected. Unlike the RNG-stream shifts recorded above, this changes casualty generation itself rather than only the draw order downstream of it, and the direction differs by stream because the retired cap sat at a different multiple of each stream's mean: 7.4 times the KIA streams' mean of 0.68, where it trimmed 1.4% of draws, against 2.8 times the WIA streams' mean of 1.77, where it trimmed 7.3%. Equalising the multiple therefore tightens the KIA cap and loosens the WIA one. A consequence worth recording is that KIA now realises 79% of its nominal FORECAS mean where it previously realised 92%: cross-stream consistency improves while fidelity to the cited KIA rate falls, a trade inherent to any mean-relative multiplier and recorded as README Further Development L27.

Because generation rates changed rather than only their ordering, every downstream published measurement was re-taken: the 50-replication comparative scenario tables, the 150-replication died-of-wounds calibration for both shipped profiles, and the 30-replication scenario casualty mix. Both profiles still pass the Ajax Bay treated-cohort check, the base configuration at 0.436% (95% CI [0.361%, 0.511%]) on an interval that now spans the 0.46% bound where it previously sat below, and `moderate_intensity` at 0.271% (95% CI [0.211%, 0.330%]); neither configuration's ceilings were adjusted, the target being one-sided. All six regression check scripts pass. The published Morris ranking was not re-run and is flagged in README as predating a change that moves all six casualty-generation parameters.

**Seed-42 baseline (30 days, single run):** Total casualties 386→382; WIA 149→151; KIA 67→57 (the largest single-stream movement); DNBI 170→174; DOW count 1→0; priority split P1 195/P2 67/P3 57/KIA 67 → 206/67/52/57; R2B surgeries 50→54; R2E first-op surgeries 87→98, second-op 22→21; `surgery_deferred` 12→17; R2E post-op stabilisation icu/hold 45/34→37/49 and post-definitive icu/hold 55/45→44/56; R2E ICU per-bed utilisation 93.3/88.4/83.6/73.0→91.7/92.7/88.4/85.4 with queue depth reaching the third bed; realised in-theatre share 32.4%→36.7%; Role 4 94 decisions/75 boarded→88/67. Full detail, including rows explicitly flagged as not recomputed in this refresh, is in `CLAUDE.md`'s Key Parameters table.
...
### Phase 1 — Statistical Foundation (Issues 1 ✓, 22 ✓, 2 ✓, 3 ✓, 24 ✓, 75 ✓, 157 ✓, 195 ✓)
...
COMPLETE (merged to main):
  ...
  #148 Lognormal arrival generator's fixed absolute rate cap of 5 replaced
       by the mean-relative cap the exponential generator has used since
       Issue #54, cap_multiplier x mean_daily at the same default
       multiplier of 3. Cross-stream truncation spread narrows from
       roughly 150-fold to under 6-fold; residual spread is coefficient-
       of-variation dependent and documented as such rather than claimed
       away. Changes casualty generation itself, not only the draw order:
       seed-42 KIA 67 to 57, WIA 149 to 151, total 386 to 382. Every
       downstream published measurement re-taken (50-rep scenario tables,
       150-rep DOW calibration both profiles, 30-rep casualty mix); both
       profiles still pass the Ajax Bay bound. Cap-induced shortfall
       against nominal means recorded as README Further Development L27,
       not closed. Morris ranking not re-run (PR #202)
...
*Prepared June 2026. Updated 12 August 2026 to reflect: completion of Issue #148 (lognormal casualty generator uses a fixed absolute rate cap rather than a mean-relative one, PR #202) — one cap of 5 applied to every lognormal stream whatever its own mean, where the exponential generator has scaled its cap by the stream's mean since Issue #54; the cap is now `cap_multiplier × mean_daily` at the same multiplier of 3 in both, narrowing the cross-stream truncation spread from roughly 150-fold to under 6-fold and closing the latent failure a planner would have hit on entering a higher mean, where a stream at FORECAS's high-intensity WIA rate would have lost half its draws and realised 57% of the rate asked for. The residual spread is coefficient-of-variation dependent, which the roxygen and README state rather than claiming the exact invariance the exponential case has. Unlike the RNG-stream shifts recorded below this moves casualty generation itself, and not uniformly: the retired cap sat at 7.4 times the KIA streams' mean but 2.8 times the WIA streams', so equalising the multiple cut seed-42 KIA from 67 to 57 while raising WIA from 149 to 151. Every downstream published measurement was re-taken accordingly and both shipped profiles still pass the Ajax Bay treated-cohort bound. The resulting shortfall against each stream's nominal mean is recorded as README Further Development L27 rather than closed, and the Morris ranking now predates a change reaching all six casualty-generation parameters. Also updated 11 August 2026 to reflect: completion of Issue #153 (`scripts/run_scenarios.R` fails at the plotting step outside a UTF-8 locale, PR #199) — the comparative runner aborted after its replications had finished, matching an em dash written as a source literal against a scenario label `jsonlite` flags UTF-8, which a C locale cannot translate; axis labels now come from the scenario identifier, the plot title's em dash from an escape, and `check_scenario_labels.R` asserts the rendered plot is byte-identical across locales. The em-dash TOC anchor defect recorded alongside it was fixed in the same PR and found to be the smaller half of a larger one: `check_markdown.R` also stripped the hyphen-minus GitHub keeps and collapsed the spaces GitHub replaces one for one, so every hyphenated heading carried a broken anchor in every locale. Anchors are now generated by GitHub's own algorithm, verified against the ids GitHub produces, correcting 45 TOC anchors, 15 body cross-references and the 216 citation links, with a link check added to prevent recurrence. No model code changed, so no baseline value moves. Also updated 11 August 2026 to reflect: completion of Issue #195 (`surg_mode` screened over a range sitting almost entirely above the baseline it should surround, PR #197) — the bounds spanned 90 to 150 minutes against a shipped mode of 95, leaving the baseline 0.083 of the way along its own range, so on a four-level grid three of the four sampled values ran a longer operation than the model performs and, Morris drawing each trajectory's base point from the whole parameter box, the inflated theatre occupancy reached the rank of every parameter acting through theatre contention rather than this row's alone. Now 57 to 133, Rule A around the shipped mode, baseline at 0.500. The original pair traces to an "approximately ±25–50%" duration rule in the earliest README, applied to the 120-minute mode Issue #76 superseded; Rule A was not merely unused on this row but unavailable, since before Issue #76 the distribution minimum was 90 and Rule A around 120 would have given 72, below it. The lower endpoint was read against the source, which reports first-look operative times for six included studies with central estimates of 85 to 100 minutes rather than the single study the registry names. An audit of the other sixty-three rows found none with a baseline near an edge of its own range, and recorded that twenty-eight do not follow their declared width literally, in five patterns all explained by convention or clipping. `morris_params` reaches no ordinary run, so no seed-42 value moves. Theatre occupancy severity invariance is recorded as README Further Development L26 and not closed. Also updated 10 August 2026 to reflect: completion of Issue #186 (two screened parameters carry a baseline outside their own bounds, PR #194) — three `morris_params` baselines did not describe the shipped model: `evacuation_policy_days` at 0.10 against 30 days, `r2e_hold_mode` at 12,960 against 38,880, and `surg_mode` at 120 against 95, the last found by the audit and invisible to a bounds check because it sat inside its own range. Morris never reads that column; `run_sobol()` holds every unselected parameter there and the Shiny Calibration tab shows it as `Baseline`. Bounds are now asserted at source time, and `scripts/check_morris_baseline.R` asserts that applying the whole baseline vector through `apply_params()` leaves `env_data.json` unchanged, deriving its parameter-to-path mapping from `apply_params()` rather than restating it. No RNG stream moves, so the seed-42 run reproduces bit-identically against the tracked log. `surg_mode`'s bounds were left off centre and raised as Issue #195, re-deriving them being a change to the design rather than to a held-fixed baseline. Also updated 9 August 2026 to reflect: completion of Issue #189 (replication confidence intervals treat antithetically paired runs as independent, PR #192) — `run_replications()` paired replications on a shared seed while every interval divided by the replication count, so the point estimates stood and the widths did not. Each replication now draws its own seed. The pairing was withdrawn rather than extended: the negation cannot reach the service-time draws simmer takes inside its own event loop, and over 75 pairs it bought -0.04 on total casualties, the one response it did reach. The +0.38 correlation on died-of-wounds count that the issue reported does not replicate, measuring -0.005 over the same 75 pairs; the defect fixed is the unit-of-analysis error, which is structural. `scripts/check_replication_independence.R` asserts independence structurally, after a correlation-based version of it failed on a signal that proved identical under prescheduled and one-fork-per-job execution and under the withdrawn pairing. Every multi-replication interval in all four documents was re-measured and the conflicting `moderate_intensity` figures reconciled; both shipped configurations now sit below the Ajax Bay bound rather than the base spanning it, which the one-sided comparison treats as agreement, so neither ceiling moved. No model change, so the seed-42 run reproduces bit-identically. Also updated 8 August 2026 to reflect: completion of Issue #178 (died-of-wounds ceilings overshoot the Ajax Bay calibration target, PR #190) — the issue asked for both mortality ceilings to be re-fitted to close an overshoot against the treated-cohort bound, and closes with neither ceiling moved, because the overshoot does not reproduce. Three independent 50-replication measurements of the unchanged base configuration return 0.348%, 0.400% and 0.499%, pooling to 0.416% (95% CI [0.346%, 0.485%]) over 150 replications, an interval spanning the ~0.46% bound; the reported 0.617% is one draw from that spread. `moderate_intensity` sits below the bound at 0.274% and is left there, the comparator being an upper bound rather than a point estimate, so raising ceilings to meet it would add deaths no source evidences. Two measurement faults are recorded rather than absorbed: a response averaging one death per replication is not resolved by a single 50-replication measurement, and replications are antithetically paired with the negation reaching only the arrival generators, so partners correlate +0.38 on mortality while every interval divides by the replication count. `scripts/check_dow_calibration.R` now asserts the comparison that had been made by hand at each recalibration. No model change, so no baseline figure changes and no tracked artifact was regenerated; Issue #189 was raised from the work. Also updated 8 August 2026 to reflect: completion of Issue #158 (nine simplex-constrained composition parameters unscreened, PR #187) — the R1 triage priority split, the DNBI sub-type composition and the mass casualty priority split are each constrained to sum to one and were held out of the Morris screen, because a one-at-a-time design cannot vary a member of such a group without leaving the simplex or applying a renormalisation that shapes the result. The two highest-ranked parameters in the published screen are both conditional on a casualty being Priority 1, while the parameter setting the Priority 1 share carried no evidence at all. The nine are now screened as six balance coordinates through an isometric log-ratio transform, which satisfies the constraint by construction, taking the screened set from fifty-eight parameters to sixty-four; bounds are derived by transforming a compositional range, and `run_sobol()` gains a Dirichlet whole-composition path for confirmation. No model change, so no baseline figure changes; the production ranking is deferred to Issue #155, whose scope was extended to record it, and three of the issue's acceptance criteria remain open until that run. No issue was newly unblocked by this merge, and Issue #186 was raised from the work. Also updated 7 August 2026 to reflect: completion of Issue #157 (Morris screening evaluated seven KPIs selected by availability, PR #184) — the screen ranked fifty-eight parameters against seven response variables that were the measures reachable from `summarise_replications()` plus a death count, while the Model Outputs section separately defines seventeen KPIs chosen against five documented criteria. Only one of the seven mapped cleanly onto a documented KPI, so no time-to-care, return-to-duty, bypass or strategic evacuation response was screened at all, and a parameter delaying time to first surgery without lengthening a queue was invisible. The response set is now the Model Outputs set, reduced to thirty-six scalar responses, each carrying its criteria mapping and its scalar reduction in a `morris_kpis` registry, with a ranking CSV per response and degenerate responses flagged rather than reported as zero influence. No model change, so no baseline figure changes; the production screening run is deferred to Issue #155, and the published ranking table is left as it stands with its staleness note extended. No issue was newly unblocked by this merge. Also updated 6 August 2026 to reflect: completion of Issue #180 (`check_r2e_surgery_seizure.R` structural check inert, PR #182) — the regression check guarding R2E surgical section seizure had been locating its blocks by a trajectory label PR #176 renamed, so its structural half skipped every assertion while reporting a failure that blamed the model. The name now lives once in `R/trajectories.R` and the check derives its pattern from it, check-integrity failures are reported separately from model failures, and a section-coverage assertion was added. A test-harness fix, verified RNG-neutral, so no baseline figure changes. No issue was newly unblocked by this merge. Also updated 6 August 2026 to reflect: completion of Issue #147 (`ot_hours` missing from `env_data.json`, PR #179) — the operating theatre shift length, the highest-ranked controllable lever in the Morris screen, was the only simulation parameter held as a hardcoded function default rather than a configuration value, duplicated across six files and absent from the Configure panel's parameter registry. It now lives at `vars.surgical_roster.shift.ot_hours` and is read through a single accessor, with every call site defaulting to the configured value; the Run tab's duplicate slider was removed, since it overrode the configuration without being captured by it. A refactor, so no baseline figure changes. Issue #180 was raised from this work, recording that `scripts/check_r2e_surgery_seizure.R` has been failing on main since PR #176 renamed the trajectory its structural half matches on. No issue was newly unblocked by this merge. Previously updated 5 August 2026 to reflect: completion of Issue #173 (every casualty requiring surgery routed through the damage control sequence, PR #176) — the surgical population is now split between the staged damage control pathway and a single-stage definitive procedure, keyed to triage priority by a `pri*_dcs_rate` family, so a single-stage casualty takes one theatre episode and one intensive care episode instead of two of each. R2E second surgeries fell from 58 to 22, deferred surgeries from 22 to 12, and post-definitive care in an intensive care bed rose from 34% to 55%; the 50-replication treated-cohort DOW rate fell from 0.876% to 0.617% against the ~0.46% Ajax Bay target, recovering most of the overshoot Issue #159 had introduced. Setting every rate to 1.0 reproduces the previous model bit-identically. Further Development L25 was deleted and its citations repaired. No issue was newly unblocked by this merge. Previously updated 3 August 2026 to reflect: completion of Issue #159 (post-operative intensive care time not conserved across treatment location, and the post-definitive episode absent from the model entirely, PR #174) — the requirement is now drawn once and split between echelons by a configurable forward-holding policy, and the two clinically distinct episodes damage control requires are modelled separately, which surfaced that four R2E intensive care beds cannot cover both. Issue #173 was raised from that work, since the model routes every operated casualty through the damage control sequence where the literature puts it at roughly a quarter of emergent trauma laparotomies, inflating the intensive care constraint the merge reports. Previously updated 2 August 2026 to reflect: completion of Issue #160 (AME sortie configurations understating C-17A capacity, PR #171) — the two strategic AME aircraft configurations shipped at 2 critical with 8 standard places, or 0 critical with 20 standard, against the RAAF's published AME fit for the C-17A of 36 high dependency stretcher and 54 ambulatory patients, so the pool governing the model's evacuation findings was provisioned eighteen times below the airframe it was meant to represent. Both configurations and the rule selecting between them were replaced by a single named airframe holding its published capacity, with the C-130J-30 and C-27J selectable alongside the shipped C-17A; retiring the two-configuration abstraction was recorded as a design decision, since the source describes one aircraft carrying both categories on the same sortie and Configuration B had no counterpart anywhere. The seed-42 evacuation backlog at day 30 fell from 58 casualties to 16, the critical-pool mean wait from 6.0 days to 1.1, and Role 4 peak occupancy rose from 17.0 to 73.0, meaning the earlier figure understated the national support base's bed requirement roughly fourfold. Limitation L17 was rewritten rather than refreshed: the R2E holding pool remains heavily loaded at 88% occupancy over 90 days, but evacuation wait now accounts for only about a third of it, so the constraint is the bed count rather than airlift. The Welch warm-up analysis and the 50-replication comparative scenario analysis were both re-run, and every 50-replication row in `CLAUDE.md` refreshed, clearing pending flags some had carried since Issue #73. The Morris screen was not re-run, with the airframe capacities' continued exclusion from it recorded; `images/scenario_comparison.png` remains blocked on Issue #153. No issue newly unblocked. Previously updated 2 August 2026 to reflect: completion of Issue #156 (R2E disposition drawn independently of severity, and AME evacuees re-occupying ICU beds, PR #169) — Phase 5's disposition was an unconditioned Bernoulli draw reading no casualty attribute, and Priority 1 surgical evacuees then held an ICU bed for the whole evacuation wait, a mean of roughly 12.8 days against a documented 24-hour norm, which together made the project's headline ICU-constraint finding substantially an artefact of bed accounting. Both were fixed by one mechanism: a severity-scaled recovery-to-duty duration compared against a configurable `evacuation_policy_days`, shipped at the doctrinal 30 days, with AME-awaiting casualties staging in holding beds apart from a bounded pre-flight ICU period for a ventilated 15% of the critical pool. The realised in-theatre share became an output (30.3% at seed 42; 28.9%, 95% CI [27.7%, 30.1%] at 50 replications) validated against the 7.6%-42.1% historical range rather than asserted as an input, and evacuation is now demonstrably severity-conditioned, at 0%/80%/100%/100% by recovery-duration quartile. Three findings changed qualitatively: R2E ICU is no longer the binding constraint (the second-shift surgical section is, and R2E OT queue rises 143-fold under Okinawa intensity because a casualty holds a theatre while waiting for staff), the post-operative pathway split reverses to icu=72/hold=47, and evacuation policy becomes a planning lever. Two weaknesses were recorded rather than absorbed: the four severity factors are informed estimates tuned against a wide range (L23), and the residual Welch ICU-queue rise is bed-blocking from a saturated holding pool rather than the original defect (L17, rewritten). The Morris screen was not re-run and now names two entries stale in definition; `images/scenario_comparison.png` was not regenerated, blocked on Issue #153. No issue newly unblocked; Issue #155's scope was extended. Previously updated 2 August 2026 to reflect: completion of Issue #152 (DOW calibration target of 0.52% not supported by its cited sources, PR #167) — the target "3 DOW / 580 WIA ≈ 0.52%" appeared in neither cited reference, described a British wounded-in-action cohort where the source covers casualties from both sides, and expressed a treated-cohort mortality rate rather than a DOW/WIA rate; reference [13] was additionally attributed to the wrong author. Resolution route (b) was taken, reframing the target as the treated-cohort rate it actually is: three deaths among the "over 650" casualties reaching the Ajax Bay Advanced Surgical Centre (≈0.46%, Westphalen 2018), compared against the model's own cohort of casualties reaching an R2B or R2E facility. Neither ceiling changed, both shipped configurations already spanning the corrected target at 50 replications (base 0.633%, 95% CI [0.392%, 0.874%]; `moderate_intensity` 0.491%, 95% CI [0.310%, 0.673%]), so the seed-42 baseline is unchanged and was reproduced exactly as a validation step. The alternative treated cohort of Jackson et al. (3 of 233 operated on, 1.29%) was rejected on measured grounds, reaching it requiring ceilings that drive whole-of-WIA DOW to roughly 3.6%. The 255:777 campaign totals were sourced to Hansard as references [53] and [54], and Limitation L22 added for what the corrected target still cannot constrain. One long-pending multi-run row was refreshed, mean DOW/run moving from ~0.70 to 1.34 as the accumulated effect of earlier RNG-stream shifts was finally measured. No issue newly unblocked. Previously updated 2 August 2026 to reflect: completion of Issue #154 (tracked seed-42 baseline could drift out of sync, PR #165) — `run.R` wrote a different artifact set on each of its two paths, so any invocation at all, including a one-day smoke test, rewrote the tracked `images/` plots while `logs/logs.txt` and `data/` were rewritten only by single runs, allowing the seed-42 regression evidence to describe a mixture of runs with nothing in the diff to signal it. Every run now writes beneath the gitignored `outputs/` tree alone, and the tracked set is written only under a `--refresh-baseline` flag that requires `--iterations 1`, so a partial refresh cannot be expressed; the single-run `sink()` gained an `on.exit()` guard, and `replication_summary.csv` is now emitted from both run modes. A per-file audit found the drift had already occurred, the tracked set spanning six commits and four model configurations, and it was regenerated from one run and committed together. A second commit refreshed `docs/Single_Run_Analysis.md`, which Issue #161's PR had left self-contradictory (a Conclusion citing 50 to 76% ICU utilisation against an R2E section reporting 98.8 to 100.0%, and casualty tables still describing the pre-Issue-18 model), and corrected its R2B holding narrative: the beds run near-full with no queue because capacity-aware routing diverts casualties upstream to R2E, exporting the structural shortfall rather than absorbing it. Seed-42 baseline unchanged (artifact destinations only; no RNG draws consumed). No issue newly unblocked. Previously updated 1 August 2026 to reflect: completion of Issue #161 (R2E surgery seized no surgical team, PR #162) — a regression of the fix merged as Issue #8, in which the R2E trajectory's `surg_team`/`surg_teams` variables were computed and then discarded, leaving R2E surgical throughput bounded only by its two theatre beds and the R2E shift roster with no effect at any hour. Both R2E surgery branches now seize a surgical section as a block around the procedure, with per-casualty section selection so that concurrency tracks the rostered establishment (two by day, one by night) rather than a build-time pick of one section in three; `scripts/check_r2e_surgery_seizure.R` was added to guard structurally and behaviourally against a third reintroduction. Seed-42 baseline shifted (see "Recently Merged Issues" above for the full before/after table); `CLAUDE.md`'s Key Parameters table refreshed in the same PR, in an unpinned R 4.3.3 sandbox whose fidelity was established by a control run reproducing every documented post-Issue-23 figure exactly. No issue newly unblocked. Previously updated 18 July 2026 to reflect: completion of Issue #115 (add an in-app Getting Started guide, PR #143) — a new `docs/Getting_Started.md` covering the app's purpose, the Configure → Run → Analyse workflow, key parameters worth adjusting first, and how to read each Analyse-tab graph, surfaced inside `app.R` as a default-landing tab rather than only linked as a GitHub file, with a cross-link from the Analyse tab; a same-PR follow-up added a fourth "Sensitivity Screening" workflow subsection and a fuller Morris/Sobol explanation after an owner review found the guide's original single-clause mention of the Sensitivity Calibration tab insufficient. Verified live via Playwright/headless Chromium against R 4.3.3 installed directly in the host sandbox — a pinned-Dev-Container build got as far as `renv::restore()` before hitting a container-networking/proxy gap to CRAN, then hung for hours once the Docker daemon itself became unresponsive mid-retry, and was abandoned in favour of installing just the ~20 packages `app.R` imports directly on the host. No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked — no open issue lists Issue #115 as a dependency and none currently carries `status: blocked`. Previously updated 18 July 2026 to reflect: completion of Issue #116 (repo cleanup — audit and resolve orphaned objects and file/folder structure, PR #141) — a two-pass audit removed 14 orphaned files and 16 transitively-orphaned `renv.lock` packages, relocated `STYLE_GUIDE.md` to `docs/`, and rewrote `CLAUDE.md`/README's structure tables to match; see "Recently Merged Issues" above for full detail. No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Previously updated 17 July 2026 to reflect: completion of Issue #114 (revise in-app help text for clarity and remove internal issue-number references, PR #139) — see "Recently Merged Issues" above for full detail; no issue newly unblocked. Previously updated 17 July 2026 to reflect: a follow-up correction to Issue #112 (PR #137) — a user review of the three-way category split found `icu_defer_check_interval`/`ame_dow_check_interval` were not genuine screening candidates (both are `timeout()`+`rollback()` polling-loop intervals approximating continuous monitoring, not a real standing order a health system could issue) and removed them from screening entirely, and found `post_surgery_prob` miscategorised as Policy when it is in fact a clinical-severity fact (Context), inconsistent with its sibling `in_theatre_rate` which correctly stayed Policy. `fr_fulfillment_lag_days`/`fr_fill_mode_frac` were reviewed and kept Policy, flagged as a closer call. The screen settled at 53 parameters (36 Context / 10 Capacity / 7 Policy) and was re-run in full (r=5, 1,350 simulation runs, ~108 minutes, zero NA/failed KPIs) rather than patched, since `morris()`'s trajectory sampling depends on the factor count; the README ranking table and all seven `images/morris_*.png` plots were regenerated from real results, with the largest rank shifts between the two designs called out explicitly. No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Previously updated 17 July 2026 to reflect: completion of Issue #112 (verify sensitivity screening covers all relevant model parameters, PR #135) — a full audit of `env_data.json`'s numeric parameter surface against the Morris screening set (`R/sensitivity.R`), combining `R/app_params.R`'s 269-field Shiny registry with a direct read of the DOW logistic/treatment-efficacy constants, found and closed 44 gaps against the 11 parameters previously screened (55 total), spanning R1/R2B/R2E durations and probabilities, the DOW logistic curve and treatment-efficacy factors, casualty generation rates, and the force regeneration (Issue #18) and strategic AME (Issue #23) subsystems named in the issue as motivating examples; the remainder were excluded with a documented, category-specific rationale in a new README "Parameters Excluded from Screening" subsection. The full Morris re-run (r=5, reduced from the project's prior r=20 convention given the 5× parameter-count increase; disclosed as new README Limitation L18) surfaced and fixed two genuine, previously-unexercised latent bugs as a positive collateral finding — an out-of-envelope `rtriangle()` bound (`fr_fill_mode_frac`) and a missing `n_days` guard on the reinforcement demand scheduler (`R/replication.R`) — before completing cleanly with valid µ*/σ for all 55 parameters and zero failed design points. Three owner-requested same-PR follow-ups addressed plot readability (`ggrepel::geom_text_repel()` labels, replacing base R `plot.morris()`'s overlap-prone placement), added a colourblind-safe three-way Context/Capacity/Policy category colour split to both the CLI and Shiny scatter plots and the Shiny calibration table (owner-corrected mid-PR to reclassify `r1_transport`/`r2b_transport` from Capacity to Context), and added a Variable-to-plain-English lookup table to both the Shiny app and a new README "Parameter Name Reference" subsection. No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked — no open issue lists Issue #112 as a dependency and none currently carries `status: blocked`. Previously updated 17 July 2026 to reflect: completion of Issue #117 (audit and complete Shiny app Analysis tab coverage of simulation outcomes, PR #133) — a systematic audit cross-checked every element `analyse_run()`/`analyse_replications()` (`R/analysis.R`) return against what `app.R`'s Analyse tab actually rendered, closing 15+ Quick Run outputs with zero UI wiring (R2B hold bed occupancy and OT bypass reason, R2E post-operative pathway and OT utilisation, the transport capacity margin plot and per-platform utilisation, time-to-treatment/dwell/transit KPIs, DOW/RTD-by-echelon breakdowns, Role 4 census and unconstrained AME sortie demand, actual AME wait time by route, and the mass casualty event stress-test reconstruction) via a new Transport tab, Return to Duty & DOW tab, Mass Casualty Events tab, and extensions to the existing Bed & Resource Utilisation and Strategic AME tabs, then extending `analyse_replications()` itself (`build_attributes_wide()`/`ci_by_group()` helpers) to compute a mean ± 95% CI equivalent of every one of those outputs for Full Analysis mode rather than leaving it with an explanatory placeholder. Verified by direct pipeline execution plus `shiny::testServer()` driving the real `app.R` server through a live Quick Run — the latter reproduced production's real package-attach order and caught a genuine pre-existing bug predating this issue (from Issue #23): `assign_role4_los()`'s unqualified `select()` resolved to `simmer::select()` rather than `dplyr::select()`, silently breaking every Role 4 census computation; fixed by qualifying it. Also fixed a latent Full Analysis CSV-download bug (Bed & Resource Utilisation's download referenced a Quick-Run-only field, silently downloading an empty file). No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Previously updated 16 July 2026 to reflect: completion of Issue #128 (graph of hold beds used in absence of ICU not surfaced in the Shiny app, PR #131) — `R/analysis.R::analyse_run()` has computed `r2e_icu_gating_plot`/`r2e_icu_gating_daily` since Issue #43, but `app.R` never rendered it; a new Quick-Run-only "R2E OT-ICU Gating" panel was added to the Analyse tab's Bed & Resource Utilisation nav panel, following the exact `shrink_to_fit_plot_ui()`/`new_shrink_to_fit_plot()` (Issue #121) and PNG/PDF/CSV download conventions already used by every other panel in that tab. No new computation; UI wiring only. Verified live: R 4.3.3 and every required package installed ad hoc in-session (no Docker daemon available for the pinned Dev Container), the underlying pipeline run directly at seed 42/30 days confirmed `r2e_icu_gating_plot`/`r2e_icu_gating_daily` populated exactly matching `CLAUDE.md`'s documented post-op pathway baseline (hold=104, icu=4), and the app was driven end-to-end with Playwright/headless Chromium confirming the panel renders, its Expand modal works, and its CSV download produces the correct data. No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Previously updated 16 July 2026 to reflect: completion of Issue #124 (force reinforcement can credit effective force size above initial establishment strength, PR #129) — `build_reinforcement_trajectory()`'s `credit_fn()` (`R/trajectories.R`) now clamps the credited value to `min(initial, current + fill)` rather than adding the submission-time fill amount unconditionally, and a new per-pool "pending" global (`reinf_combat_pending`/`reinf_support_pending`, `R/replication.R`) lets `demand_fn()` net out shortfall already claimed by an in-flight (submitted, not-yet-credited) cycle, closing an overlapping-cycle (`demand_interval_days < fulfillment_lag_days`) double-claim identified in a follow-up comment on top of the original unconditional-credit report. Verified against the real `simmer`-based mechanism, not a design-only argument: R 4.3.3 plus the four packages the reinforcement code path actually requires (`jsonlite`, `triangle`, `simmer`, `simmer.bricks`) were installed ad hoc in-session (no Docker daemon available for the pinned Dev Container); an aggressive overlapping-cycle stress configuration (`demand_interval_days=2` vs `fulfillment_lag_days=10`, `fill_max_frac=1.5`) produced 0 ceiling violations on the fixed code across 2,220 recorded global-write events, and the identical configuration reliably overshot when re-run against pre-fix `main` (`effective_force_combat` max 2624 of initial 2500; `effective_force_support` max 1313 of initial 1250), confirming the stress test genuinely exercises the bug. No `env_data.json` change; seed-42 baseline unaffected (reinforcement only adds a generator, and now the pending globals, when `demand_interval_days > 0` — the shipped default is `0`; direct re-run confirmed total casualties 386, matching `CLAUDE.md`'s documented baseline exactly). No issue newly unblocked. Previously updated 16 July 2026 to reflect: completion of Issue #109 (add AME repatriation and queue visualisation to the analysis tab, PR #126) — `plot_ame_queue()` (factored out of `analyse_run()`'s pre-existing inline `ame_backlog_plot` block) and new `compute_ame_sorties()`/`plot_ame_sortie()` (reconstructing every scheduled AME sortie opportunity's outcome — configuration selected, seats used vs. capacity — from the `"ame"`/`"ame_critical"` resource monitor against the schedule's own deterministic firing times, since `build_ame_sortie_trajectory()` keeps no sortie log of its own) are wired into both `analyse_run()` and `analyse_replications()` and into a new "Strategic AME" Analyse tab in `app.R`. Verified by direct execution rather than code inspection alone: R 4.3.3 and every required package installed from source ad hoc in-session (no Docker daemon available for the pinned Dev Container), `run.R --seed 42 --days 30 --iterations 1` run directly. This surfaced a real, pre-existing bug predating this issue (from the Issue #23 follow-up): `ame_wait_and_board()` (`R/trajectories.R`) uses a manual `timeout()`/`rollback()` polling loop rather than simmer's native queueing, so a waiting casualty never registers in the `"ame"`/`"ame_critical"` resources' own queue tracking — the resource monitor's `queue` column is structurally always 0, and the extracted `plot_ame_queue()` always rendered a flat zero backlog line regardless of the true state. Fixed with a new `compute_ame_backlog()` reconstructing the true backlog from per-casualty `r2e_departure_time`/`ame_departure_time`/`ame_route` event timestamps instead; `compute_ame_sorties()`'s `seats_used` had the same class of bug (an instantaneous pre-sortie queue snapshot, also always 0), fixed by measuring the resource's `server`-count change across each sortie's window (to the next scheduled sortie, or end of run for the last one) instead. The corrected backlog's peak values (critical pool 89, standard pool cyclical to 9) match figures already documented in the README's "Strategic Evacuation and Role 4 Demand" section, confirming the fix — that prose had always been derived correctly from `ame_wait_time_summary`, which is how the plot bug went unnoticed since Issue #23 merged. No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected (total casualties 386, matching the documented post-Issue-23 baseline exactly); no issue newly unblocked. Previously updated 16 July 2026 to reflect: completion of Issue #121 (graphs in Shiny app exceed available window space instead of scaling to fit, PR #123) — every Analyse-tab plot output is now wrapped in a "shrink-to-fit" container (`shrink_to_fit_script()`/`bch_shrink_to_fit_css()`, `app.R`): a client-side script shrinks the container's CSS height to fit the viewport minus a fixed chrome allowance, `renderPlot()` keeps an explicit natural height (never `"auto"`) so Shiny never re-renders in response to the container's displayed size, and a paired stylesheet makes the plot image track that height (`height: 100%`, `width: auto`, `margin: 0 auto`) so the browser scales the already-rendered, full-detail image down losslessly and centred; an "Expand to full size" modal link is provided per plot. Two earlier designs (auto-sized `renderPlot()` redrawing to fill a JS-shrunk container; a CSS `transform: scale()` on a full-size render) were tried and dropped after live testing found each reintroduced real defects — respectively, label/row overlap at small sizes, and a compounding-shrink bug from Shiny's resize-sensing measuring a transformed container's on-screen size. Two owner-directed follow-ups extended scope within the same PR: Queue Depths and Quick Run's Bed & Resource Utilisation, each previously one combined `patchwork` image, were split into individually-sized panels (a "group" shared-budget mechanism was tried first, then reverted once established that scrolling *between* panels is acceptable — only a single panel exceeding the viewport is not); and R2B Treatment (itself a 3-panel `patchwork` stack) was found sharing R2E Surgery's 400px height convention rather than Casualty Flow's 700px (the same 3-panel shape), causing y-axis-title overlap between its stacked sub-panels, fixed by giving it 700px. Verified live via Playwright/headless Chromium against an ad hoc R 4.3.3 sandbox (`renv::restore()` from the project's own `renv.lock`; no Docker daemon available for the pinned Dev Container) across all four commits, including the direct Issue #111 regression check (the Expand modal shows every Gantt row with no overlap). No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Previously updated 15 July 2026 to reflect: completion of Issue #111 (Bed & Resource Utilisation Gantt chart rows overlap due to insufficient vertical space, PR #120) — the Quick Run "Bed & Resource Utilisation" tab's fixed `plotOutput(height = "1400px")` squeezed individual R2B/R2E bed rows past legibility as an echelon's bed count grew (the default `env_data.json` alone has 40 R2E bed slots in one unfaceted Gantt panel); a new `utilisation_plot_height()` reactive (`app.R`) instead computes the container/render height from the distinct resource-row count in `r2b_gantt$data` (summed per R2B team facet) and `r2e_gantt$data`, at 25px/row with a 150px floor per section, leaving Full Analysis mode's static 500px bar-chart height untouched. Verified live via Playwright/headless Chromium against a from-scratch R 4.3.3 environment (no Docker daemon available for the pinned Dev Container): Quick Run computed 1675px (vs. the old 1400px) with every bed row legible, and a before/after comparison against the pre-fix code at the identical seed/config reproduced the exact reported row-overlap symptom, confirming the fix. No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Previously updated 15 July 2026 to reflect: completion of Issue #110 (queue-depth plots use fixed y-axis scale and clip data, PR #118) — audited every queue-depth plot function in `R/analysis.R` and found three affected in the Quick Run single-run path (`p_r1_queues`, `p_r2b_bed_queues`, `p_r2e_bed_queues`), each hardcoding `scale_y_continuous(limits = c(0, 10))`; replaced with `limits = c(0, NA)` + `expansion(mult = c(0, 0.05))`, matching the existing utilisation plot's pattern. The Full Analysis (multi-run CI ribbon) equivalents were already dynamic and needed no change. Verified by direct execution rather than code inspection alone: R 4.3.3 and required packages installed ad hoc in-session (no Docker daemon available for the pinned Dev Container), `run.R --seed 42 --days 30 --iterations 1` run directly, console KPIs matching the documented baseline exactly, and the rendered R2E Heavy Bed queue plot confirmed the ICU queue climbs past 20 by day 30 at that exact baseline — the fixed 0–10 scale was clipping real, currently-documented output, not just a hypothetical stress-test scenario. No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Previously updated 14 July 2026 to reflect: completion of Issue #23 (strategic evacuation demand — Role 4 occupancy and AME sortie requirements, PR #107) — `compute_role4_census()`/`compute_ame_demand()` (`R/analysis.R`) close the outbound half of the causal chain Issue #18 opened on the inbound side. Four owner-directed follow-ups substantially extended scope within the same PR: strategic AME became a real scheduled, capacity-limited, failure-prone simmer resource (fixing a fixed-vs-additive capacity bug caught during verification); AJP-4.10(B) research grounded a two-pool critical/standard split, then a further redesign to two planner-named aircraft configurations selected each sortie by a backlog-minimising rule (`select_ame_configuration()`); the default sortie interval changed from 3 to 7 days to match the reinforcement mechanism's cadence; and a periodic wait-time DOW poll (`ame_dow_poll()`, new `dow_echelon = 5`) closed the model's only previously-unbounded, risk-free wait, using the same conditional-increment logistic formula and `timeout()`-then-`rollback()` polling pattern already established elsewhere. All four follow-ups are exposed in a new Shiny "Strategic AME" Configure subgroup and on the Medevac Chain sidebar diagram. Seed-42 baseline shifted (not RNG-stream-neutral — AME wait duration changes R2E bed-release timing, and the sortie generator and DOW poll each consume new `runif()` draws): total casualties 386; 133 strategic evacuation decisions, 40 boarded by day 30, critical-pool mean wait 12.8 days, Role 4 peak occupancy 19.0 (day 22); see the Key Parameters table in `CLAUDE.md` for full detail. No issue newly unblocked — no open issue currently carries `status: blocked`. Phase 4 Scenario Expansion now complete. Previously updated 13 July 2026 to reflect: completion of Issue #18 (endogenous casualty generation — force regeneration feedback loop, PR #105) — `effective_force_combat`/`effective_force_support` live simmer globals, debited at each casualty's `injury_time` and credited at each RTD event (`R/trajectories.R`), give `in_theatre_rate` a real mechanical effect on arrival rate for the first time, closing the gap identified in Issue #3's sensitivity screening; the six background casualty streams switch from pre-computed `at()` arrival vectors to force-size-reactive generator closures (`R/environment.R`). Reinforcement was redesigned mid-PR, per owner feedback, from a fixed periodic size to a demand/fulfillment-lag/triangular-fill model (`demand_interval_days`/`fulfillment_lag_days`/`fill_min_frac`/`fill_mode_frac`/`fill_max_frac`) — self-limiting since demand is the pool's actual shortfall rather than a fixed size, producing a materially more realistic demonstration (arrests decline to a statistically flat trend rather than overshooting into growth). Ships disabled by default, confirmed byte-identical to the pre-redesign baseline with zero RNG draws consumed when disabled. Adds a Shiny "Force Regeneration" Analyse tab and a "Reinforcement Demand & Fulfillment" Configure subgroup, verified end-to-end against a live running app via headless-browser testing — catching two real bugs (a missing multi-run plot path, a key-naming mismatch) before merge. Seed-42 baseline shifted (RNG-stream-shifting in a structurally larger way than prior such merges — the draw *order* changes, not just values drawn from an unchanged order): total casualties 400→386; see the Key Parameters table in `CLAUDE.md` for full detail, including several rows explicitly flagged as not yet re-derived in this refresh (unpinned-sandbox provenance caveat, no Docker access in the development environment). Unblocks Issue #23 (`status: blocked` → `status: ready`). Previously updated 13 July 2026 to reflect: completion of Issue #57 (transport fleet-size capacity margin sweep, PR #103) — implements `plot_transport_capacity_margin_by_fleet_size()` (`R/analysis.R`), replacing the Issue #6-era stub; sweeps PMV Ambulance (1–5) and HX240M (1–4) fleet size directly, reusing `run_replications()` (the same engine Issue #10's comparative scenario runner uses) rather than duplicating replication/aggregation logic; writes `outputs/transport_capacity_by_fleet_size.csv` and `images/transport_capacity_margin_by_fleet_size.png`; new `scripts/run_transport_sweep.R` CLI entry point. A follow-up commit within the same PR, made at owner request beyond the issue's original scope, integrates the sweep into the Shiny app's Sensitivity Calibration tab alongside Morris/Sobol, dispatched via the existing `scripts/shiny_worker.R` subprocess mechanism, followed by three further owner-requested UX passes — a plain-language panel description in place of developer-facing prose, human-readable facet titles plus a proper plot legend for the ribbon/mean-line/reference-line (previously explained only in prose), y-axis labels moved to their conventional left-hand position, and the "how to read this" interpretation text relocated above the plot and shortened to one instruction — verified live via Playwright driving headless Chromium against the actual running app. No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Previously updated 12 July 2026 to reflect: completion of Issue #77 (Configure panel eager-render race, PR #101) — reverted the `group_ui_*` panel outputs, 19 curve-preview plots, and 2 sticky-sidebar diagrams from Issue #14's eager-render override back to Shiny's default `suspendWhenHidden = TRUE`, so a panel's fields bind only once actually opened, eliminating both the ~18-20s startup cost and a race where a widget's own late-arriving initial-value message could silently overwrite an edit made in the first several seconds after load or a scenario switch. Added `fill_missing_defaults()` so `validate_config()` still treats a never-opened panel's fields as their correct scenario default rather than an error-worthy NULL. Also closes the duplicate Issue #98 (a slider-specific report of the same underlying mechanism; its own proposed fix, PR #100, was investigated and found not to address the actual defect once tested live). Verified via headless-browser WebSocket testing against the real running app, not just reasoned about: the silent-revert race reproduced 2/2 times before the fix and 0/2 after; a regression the fix itself introduced (Quick Run failing validation with non-default panels closed) was caught in testing and fixed before merge. No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Previously updated 11 July 2026 to reflect: completion of Issue #15 (Shiny app — Full Analysis mode and Sensitivity Screening, PR #97) — activates the two modes Issue #14 left as disabled placeholders (Full Analysis via `run_replications()`, mean ± 95% CI on four KPI cards and ribbon/band result tabs; Sensitivity Screening via `run_morris()`/`run_sobol()`, real per-design-point progress). Live testing on a local Docker Desktop dev container and a GitHub Codespace found and fixed several reliability gaps beyond the initial UI wiring: `detect_safe_cores()` bounds `mclapply` concurrency from real-time cgroup memory headroom, recomputed fresh before each run; `run_replications()`/`run_morris()`/`run_sobol()` gained `max_cores` caps, killed-worker detection, and an inter-iteration `gc()` against sequential-screen memory growth; a previously-latent `simmer::select()`/`dplyr::select()` masking bug in `R/analysis.R` (present since Issue #14, never triggered by the CLI path's different package-attach order) was fixed by qualifying every ambiguous call; and, most significantly, Full Analysis/Morris/Sobol no longer call these functions in-process inside a `future()` body — both `multisession` and `multicore` proved unsafe for a future body that itself calls `mclapply()` (a corrupted control socket and a real deadlock respectively, the latter confirmed via `ps`), so all three now shell out via `system2()` (`run_shiny_worker()`/`scripts/shiny_worker.R`) to the same code path `run.R` already runs successfully with full parallelism. `.devcontainer/devcontainer.json` also gained `hostRequirements` and a second forwarded port for the Shiny app. No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Previously updated 11 July 2026 to reflect: completion of Issue #9 (mass casualty stochastic injection — compound Poisson process, PR #92) — `generate_mass_casualty_events()` (`R/environment.R`) overlays Poisson-distributed mass casualty surge events onto the background lognormal generation, plus a second scheduled-day mode (owner feedback) giving each planner-specified day its own occurrence probability, casualty-count bounds, and priority split; Configure panel exposure (owner feedback) added a "Mass Casualty" accordion panel including a dynamic `+`/`−` scheduled-event list (capped at 20 slots, later fixed to remove a dead-space layout bug); "MASCAL" acronym renamed to "mass casualty" throughout (owner feedback). Ships disabled by default (`rate_per_day = 0`, empty schedule); seed-42 baseline unaffected, re-verified against the merged `main` tip. No issue newly unblocked. Previously updated 11 July 2026 to reflect: completion of Issue #93 (Dev Container build failure — missing `libuv` runtime + `renv.lock` R version mismatch, PR #94) — corrected `renv.lock`'s `R.Version` field from `4.3.3` to `4.4.2` (verified via `renv::snapshot()` as package-version-neutral) and added `libuv1t64` + `curl` to `.devcontainer/Dockerfile`, fixing the actual build failure (`fs` package's prebuilt binary failing `dyn.load()` on a missing `libuv.so.1`). No `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Also backfills the previously-missing post-merge update for Issue #72 (adopt `renv` for reproducible R package dependency pinning, PR #91) — `renv.lock` (116 packages) committed, Dockerfile switched to `renv::restore()` with a pre-warmed cache; no `env_data.json` or trajectory-logic change; seed-42 baseline unaffected; no issue newly unblocked. Previously updated 10 July 2026 to reflect: completion of Issue #76 (R2B/R2E surgery duration narrative vs. shipped `env_data.json` divergence, PR #89) — reconciled to 41/210/95 minutes (Sohn et al. 2018 within Zizzo et al. 2020), matching the README narrative; `env_data.json` had shipped an unsourced 90/240/120 placeholder since the project's initial commit. Seed-42 baseline shifted (RNG-stream-shifting change, same class as Issues #5, #6, #43, #73) — see the Key Parameters table in `CLAUDE.md`. No issue newly unblocked. Previously updated 10 July 2026 to reflect: completion of Issue #85 (`check_env_data_summary.R` crash on R1's integer `sub_elm`, PR #87) — hardened via a new `resolve_sub_elm()` helper that treats any non-character `sub_elm` the same as a missing one, fixing both the `purrr::map_chr()` crash and a masked defect it had been hiding (R1's ENV SUMMARY column header rendering as a bare `"1"` instead of `"Base"`). No `env_data.json` schema change; no issue newly unblocked. Previously updated 10 July 2026 to reflect: completion of Issue #74 (remove the dead-heading return leg multiplier, PR #83) — `return_leg_multiplier` removed entirely from `env_data.json`, `R/trajectories.R`, `R/sensitivity.R`, and the Shiny Configure panel, despite Issue #73's Morris re-run having just found it highly influential on transport utilisation and DOW count; removal proceeded on owner-directed operational grounds (no doctrinal basis for departing from a symmetric round trip) rather than the issue's original since-invalidated "no detectable effect" rationale. RNG-stream-neutral (confirmed by a before/after worktree diff); no issue newly unblocked. Previously updated 10 July 2026 to reflect: completion of Issue #73 (R2B → R2E WIA dead-heading return leg configured but never applied, PR #81) — resolved in three stages within one PR: dead-code removal (formalising the R2B organic-asset design), a subsequent owner-directed reversal adding a working dead-heading return leg to that same organic resource plus real R2B→R2E road-move mortuary transport for KIA/DOW, and a verified (not merely asserted) Morris sensitivity re-run that found `return_leg_multiplier` — after its scope doubled from two legs to four — is now the most influential screened parameter on mean transport utilisation and near-top on total DOW count, correcting an initial unverified claim to the contrary. No issue newly unblocked; Issue #74's removal proposal was flagged (via comment) as resting on a now-outdated premise. Seed-42 baseline shifted (RNG-stream-shifting change, same class as Issue #6) — see the Key Parameters table in `CLAUDE.md`. Previously updated 09 July 2026 to reflect: completion of Issue #75 (stale `p1_p_max` Morris screening bounds — re-derived bounds, full Morris re-run, PR #79), plus a follow-on DOW survival-function curve added to the Shiny Configure panel during the same PR. Seed-42 baseline unaffected (Morris design table and Shiny UI code only; no `env_data.json` or trajectory-logic change). No issue was newly unblocked by this merge. Previously updated 09 July 2026 to reflect: completion of Issue #14 (Shiny app — parameter editor, Quick Run mode, single-run output display, PR #71), including the `R/analysis.R` ggplot-object refactor that was the sole gating dependency for Issue #15 — unblocking Issue #15 (`status: blocked` → `status: ready`). Three pre-existing gaps found (not introduced) during Issue #14 were raised as new issues: #75 (stale Morris screening bounds for `p1_p_max`), #76 (R2B/R2E surgery duration narrative vs. shipped `env_data.json` divergence), and #77 (Configure-panel eager-render/`suspendWhenHidden` race); a fourth (#73, R2B→R2E dead-heading return leg never applied) was already open, and a related owner-raised follow-up (#74, remove `return_leg_multiplier` entirely) is also open. Previously updated 07 July 2026 to reflect: completion of Issue #10 (comparative scenario runner, PR #69) — compares `moderate_intensity`/`high_intensity` (not the uncited Vietnam/Okinawa figures originally in the issue body), unblocking Issue #57 (`status: blocked` → `status: ready`). Previously updated 06 July 2026 to reflect: completion of Issues #19 (PR #21), #1 (PR #16), #8, #22 (PR #26), #2 (PR #20), #3 (PR #30), #24 (PR #32), #7 (PR #34), #35 (PR #36), #37 (PR #38), #44 (PR #47), #39 (PR #48), #5 (PR #53), #6 (PR #56), #43 (PR #59), #60 (PR #62), #54 (PR #67), and partial completion of #40 (bypass-reason diagnostic, PR #64); addition of new Issues #43 (OT–ICU gating), #44 (RTD KPI annotation), #57 (fleet-size capacity margin sweep), and #60 (bed/resource `qty: 0` silently creates one unit instead of zero — discovered during Issue #43 testing); and reclassification of Issues #4 and #40 (remaining Scenario A/B scope) as `status: backlog` — both are unblocked but deprioritised, not currently planned. Phase 1 Statistical Foundation complete. Phase 2 Model Fidelity complete except Issue #40's deferred Scenario A/B scope — Issues #8, #35, #37, #44, #5, #6, #43, and #14 all merged; Issue #9 unblocked; Issue #18 unblocked. Phase 4 Scenario Expansion in progress — Issues #54 and #10 merged; Issue #57 unblocked. Phase 5 Interface in progress — Issue #14 merged; Issue #15 unblocked.*
