...
# Assign colors to each role-team combination
role_team_keys <- unlist(lapply(roles, function(role) {
  paste0(role, "_t", 1:team_count)
}))

names(role_team_colors) <- role_team_keys

# Then update the scale_color_manual() call:
scale_color_manual(values = role_team_colors)
