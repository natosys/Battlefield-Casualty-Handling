  ...
  output$analyse_body <- renderUI({
    ...
    navset_tab(
      ...
      nav_panel("Sensitivity Calibration",
        ...
        p(class = "text-muted small",
          "Sweeps PMV Ambulance and HX240M fleet size directly (Issue #57), rather than only casualty ",
          "rate or transport duration, to locate the fleet size at which the transport layer becomes a ",
          "binding constraint. Each fleet size runs its own multi-replication block at the current ",
          "Configure panel settings, reusing the \"Replications per Point\" value set above for Morris; ",
          "wall-clock time scales with the total number of vehicle-count points swept x ",
          "replications-per-point x simulation duration. See README Shiny Application — Sensitivity Panel."),
        layout_column_wrap(
          width = "280px",
