...
| `scripts/run_policy_sweep.R` | CLI entry point for the evacuation policy sweep at the sustained horizon. Sweeps `r2eheavy.recovery.evacuation_policy_days` across the range its doctrinal source states is a command decision, pairing the arms on one control seed each so a difference between policies is measured within replication, and reports each response with a 95% confidence interval alongside the paired difference against the shipped policy and the replication count each unresolved difference would need. `--refresh-baseline` is the only way to write the tracked `data/policy/` |
