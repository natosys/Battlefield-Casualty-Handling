...
| 339 | Re-screen the four responses whose definition changed after the tracked Morris screen | Medium | High | Open |
...
UNBLOCKED (start now):
  ...
  #339 Re-screen r2b_dwell_mean, r2e_dwell_mean, ame_backlog_mean and
       ...
       rather than the monitoring data behind them, so this is a full screen
       of roughly nineteen hours. Raised from #331 and widened by #362, whose
       correction supersedes the two backlog responses and settles the
       issue's own question in favour of re-screening the whole set. #335 is
       settled and does not add a parameter to that design: evac_threshold
       stays outside the screened set on evidence rather than reasoning.
  #348 The Morris screen has no holding bed queue response and never varies
       ...
       remaining coverage gaps against #339 is the substance, #335 having
       settled without adding a parameter: a response added to a re-screen
       that is happening anyway is nearly free. Raised from the #312 audit.
