...
| 331 | The dwell-time KPIs drop every casualty still in the echelon when the run ends | High | Medium | Open |
...
UNBLOCKED (start now):
  #331 The R2B and R2E dwell-time KPIs drop every casualty still in the
       echelon at the run's end, 21.6% and 18.0% at seed 42, always the
       longest stays; both are screened Morris responses, so a parameter
       that lengthens dwell pushes casualties out of its own response.
       Raised from the audit #327 asked for.
