...
|---|---|---|---|
| Total casualties/run | 385.6 (p10–p90: 384.0–387.0) | 968.4 (p10–p90: 965.0–972.1) | 2.51× |
| WIA/run | 149.0 (148.0–150.0) | 656.0 (653.0–659.0) | 4.40× |
| DOW/run | 0.66 (0–2.0) | 6.12 (2.0–10.0) | 9.27× |
| DOW/WIA rate | 0.444% (95% CI [0.286%, 0.601%]) | 0.933% (95% CI [0.811%, 1.055%]) | 2.10× |

The two died-of-wounds rows carry a wider uncertainty than the intervals shown. Both are single 50-replication measurements of a response averaging well under one death per replication at Falklands load, and the interval is computed as though the replications were independent when they are antithetically paired, which narrows it (see the README's [Parameter Calibration](../README.md#parameter-calibration) and Further Development entry L22). Repeating the `moderate_intensity` measurement at other control seeds returns DOW/WIA rates from 0.309% to 0.471%, so the ratio between the two profiles should be read as approximate. The `moderate_intensity` treated-cohort mortality rate pooled over 150 replications, which is the quantity the historical comparator constrains, is reported in the README's [Scenario Profiles](../README.md#scenario-profiles). The queue rows below are not affected in the same way: they are time-weighted occupancy measures with far more events behind each replication.
...
The system's resilience to surge is directly quantified rather than inferred. This multi-run comparison confirms that neither R2B nor R2E can absorb Okinawa-intensity casualty rates without fundamental redesign: R2E OT mean queue rises approximately 760-fold and R2E Hold mean queue approximately 25-fold relative to the Falklands-modified baseline, R2B Hold queue rises approximately 12-fold, and DOW/WIA rate roughly doubles — all while R2B OT queue remains at zero only because the existing bypass routing shunts all surgical overflow onto an already-saturated R2E rather than R2B absorbing any of the surge itself. Effective LSCO medical support at Okinawa intensity would require scalable holding capacity at forward echelons, a deeper surgical roster at R2E, and dynamic load-balancing between R2B and R2E — capabilities the current static establishment does not provide.

These figures were regenerated at 50 replications per scenario after the surgical population was split between the staged damage control pathway and a single-stage definitive procedure, and they supersede the comparison made when every operated casualty took the staged pathway. That change removes one theatre episode and one intensive care episode from roughly half of operated casualties, which is why the R2E OT and R2E ICU rows fall at both loads; the R2E Hold row rises at surge because a holding bed is where a casualty recovers when no intensive care bed is free, so relieving theatre pressure moves the residual congestion onto beds. The run was made in an unpinned R 4.3.3 sandbox rather than the project's pinned Dev Container, under the same caveat as the other figures refreshed alongside it (see `CLAUDE.md`'s Key Parameters provenance caveats). `images/scenario_comparison.png` was regenerated alongside the tables, the plotting stage having been run under an explicit UTF-8 locale; it fails outside one, a defect tracked separately. A comparable Vietnam-intensity comparison remains unavailable pending a genuine FORECAS-sourced Vietnam combat-troop WIA/KIA table.
