...
#
# Terminal:
#   Rscript scripts/run_sensitivity.R                    # full Morris (r=20)
#   Rscript scripts/run_sensitivity.R --quick            # smoke test (r=3)
#   Rscript scripts/run_sensitivity.R --sobol            # Morris then Sobol
#
# RStudio Console:
#   source("R/sensitivity.R")                            # loads helpers
#   morris_result <- run_morris(r=3, n_rep=3, n_days=5)  # smoke test
#   run_sobol(morris_result$ranking$parameter[1:5])      # Sobol on top 5
...
message(sprintf(
  "Sensitivity config: r=%d, reps=%d, days=%d, levels=%d",
  opt$r, opt$reps, opt$days, opt$levels
))
