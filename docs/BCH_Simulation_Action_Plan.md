...
| 227 | Delete the twelve `wip/*` checkpoint refs once the evidence set is on main | Low | Low | **Closed as completed (under #237)** |
...
| 233 | The analysis pipeline consumes RNG, so `analyse_run()` is not idempotent | Medium | Medium | **Merged (PR #255)** |
| 234 | Rewrite `docs/STYLE_GUIDE.md` as an enforceable R code standard | High | Medium | **Merged (PR #249)** |
| 236 | Global configuration save/restore is not exception-safe; `R/analysis.R` lacks input validation | Medium | Low | **Merged (PR #257)** |
| 237 | Housekeeping — delete the ten `wip/*` branches, reconcile the Further Development scan table, close `CLAUDE.md` drift | Low | Low | **Merged (PR #259)** |
...
| 241 | Apply the code standard — decompose the oversized functions behind `testServer` and Playwright verification | High | High | **Merged (PR #269)** — seven PRs (#261, #263, #264, #265, #266, #267, #269); closed with one acceptance criterion unmet, taken up as Issue 270 |
| 262 | `check_markdown.R`'s scan table check aborts outside a UTF-8 locale | Low | Low | **Merged (PR #268)** |
| 270 | Split the five simulation-logic functions still over the length limit, verified byte for byte | Medium | High | **Merged (PR #276)** — five PRs (#272, #273, #274, #275, #276), one per function |
| 238 | Re-cut the analysis papers by method — move the replicated experiments out of the single-run paper | High | Medium | **Merged (PR #280)** |
| 239 | Bring the single-run analysis paper to publication standard | High | High | **Merged (PR #282)** |
| 240 | Bring the multi-run analysis paper to publication standard | High | High | **Merged (PR #284)** |
| 295 | Replicated analysis of national support base demand and strategic evacuation reliability | High | Medium | **Merged (PR #356)** |
| 296 | Re-run the post-operative intensive care gate under independent replications | Medium | Medium | **Merged (PR #350)** |
| 297 | Render the multi-run paper's result tables as figures | Medium | Low | **Merged (PR #298)** |
| 300 | Sweep the transport fleet at high casualty intensity | Medium | Low | **Merged (PR #402)** |
...
| 316 | A run producing no return to duty crashes the analysis pipeline | Low | Low | **Merged (PR #326)** |
| 320 | A replication lost to memory pressure is dropped silently from the published count | High | Low | **Merged (PR #322)** |
| 327 | The R2B hold occupancy figure omits every episode still running when the run ends | Medium | Low | **Merged (PR #332)** |
| 331 | The dwell-time KPIs drop every casualty still in the echelon when the run ends | High | Medium | **Merged (PR #340)** |
| 333 | Resolve the two R2B holding thresholds, one name for two levers | Medium | Low | **Merged (PR #334)** |
| 335 | Sweep R2B holding capacity against the evacuation threshold | Medium | Medium | **Merged (PR #400)** |
...
| 373 | Role 4 staged reconstruction is unrepresented, and the evacuation decision carries no reconstruction burden | Medium | Medium | **Merged (PR #377)** |
| 376 | Both national support base levers ship disabled, so the forward theatre never sheds load | High | High | **Merged (PR #379)** |
| 382 | The strategic evacuation section's figures are gated by no check, and had drifted from their own evidence set | Medium | Medium | **Merged (PR #385)** |
| 384 | Most published experiments have no tracked evidence set, so their figures cannot be audited without re-running them | High | Medium | **Merged (PR #390)** |
| 387 | The R2B pre-open hold window has no tracked evidence set, so its eight published differences cannot be audited | Medium | Medium | **Merged (PR #392)** |
| 388 | The post-operative intensive care gate has no tracked evidence set, so its published magnitudes cannot be audited | Medium | Medium | **Merged (PR #394)** |
| 389 | The mass casualty stress test has no tracked evidence set, and its replication count may not be worth backing as it stands | Low | Medium | **Merged (PR #396)** |
| 405 | Standardise every replicated experiment's horizon and replication count | Medium | High | **Merged (PR #415)** |
| 410 | Issue #348's two coverage gaps, a holding bed queue response at each echelon and the R2E bed counts, were not added when the #339 re-screen ran | Medium | Medium | **Merged (PR #417)** |
...
### Issue 410 — The Two Coverage Gaps Issue #348 Left Orphaned, Closed ✓

**Merged:** PR #417, branch `feature/issue-410-morris-coverage-gaps`

Issue #348 raised two Morris screening coverage gaps alongside its cache-schema defect; PR #398 fixed only the defect, leaving the gaps untracked until this issue was raised to hold them. Both are closed. `r2b_hold_q` and `r2e_hold_q` are added as screened responses, each the time-weighted mean queue length of its echelon's holding bed pool on the same `safe_q()` convention every other queue response uses. `r2e_icu_beds` and `r2e_hold_beds` are added as screened parameters, needing a new `apply_bed_establishment_params()` (`R/sensitivity.R`) rather than a `vars`-tree write, because the establishment lives in `build_element_resources()`'s already-expanded resource-identifier vectors by the time `apply_params()` receives it: the function regenerates each R2E team instance's `icu_bed`/`hold_bed` vector at the rounded length Morris draws, round-to-nearest over a continuous one-at-a-time step being the discretisation decision #348 asked to have recorded.

The full r = 20 Morris screen is re-run at the resulting 80-parameter, 1,620-point design and published to `data/sensitivity/morris_r20/`, replacing the 78-parameter screen #339 published. Both additions land where the audit that raised them expected: `r2e_icu_beds` leads its own `r2e_icu_q` response at µ\* = 1.51, the only response in the tracked set where a fixed establishment count outranks every casualty-load or clinical-probability parameter, and `r2e_hold_beds` ranks 21st of 80 on the primary system queue ranking. `README.md`'s current-ranking narrative, table and captions are rewritten from the new screen's measured values, and `data/sensitivity/README.md` records the re-screen's decisions and that the Sobol decomposition's parameter selection has moved further from the current ranking (two of its five selected parameters still lead, one has fallen from 2nd to 7th, one from 4th to 16th, while a fourth rises from 25th to 4th).

**Seed-42 baseline (30 days, single run):** unchanged. No trajectory, `env_data.json` value or tracked seed-42 artifact is touched; `scripts/check_baseline_reproduction.R` passes.

**Unblocked by this merge:** No new issues unblocked. #408 (re-run the Sobol decomposition on the new selection) was already `status: ready` from #339's merge and remains so; its target ranking has moved again, which `data/sensitivity/README.md` now records.

### Issue 405 — Standardise Every Replicated Experiment's Horizon and Replication Count ✓

**Merged:** PR #415, branch `claude/405-migrate-transport-sweep`

The transport fleet-size sweep, the third migration candidate PR #411 costed and the one PR #413 left unexecuted, is migrated to the sustained-operations protocol, and during review the decision widened: rather than leaving the transport sweep alone at 30 replications over 360 days while the forward ICU share frontier and the R2B holding threshold sweep stayed at their own 30-day, 10/20-replication designs, all three capacity sweeps now share one protocol, so a reader comparing their tables compares one horizon rather than three. `TRANSPORT_SWEEP_REPLICATIONS`, `ICU_SHARE_SWEEP_REPLICATIONS` and `HOLD_THRESHOLD_SWEEP_REPLICATIONS` move to 30 and the shared `CAPACITY_SWEEP_DAYS` to 360 (`R/analysis.R`), and all three sweeps are re-run under `--refresh-baseline` (the transport sweep at both the shipped and `high_intensity` configurations). Every dependent artifact is updated: `data/sweeps/` (four tracked files), the four tracked sweep images, `docs/Multi_Run_Analysis.md`'s transport, forward ICU share and R2B holding threshold sections, all three design sections in `docs/Multi_Run_Supplement.md`, and `CLAUDE.md`.

Two qualifications the smaller designs carried are genuinely resolved by the migration, not just restated at a new horizon. Transport utilisation ran the wrong way on both platforms at 10 replications and was excluded from the paper's reading entirely; at 30 replications over 360 days it falls monotonically with fleet size on both platforms, at both casualty intensities (36.4% to 7.3% for ambulances at moderate intensity, 92.0% to 18.3% at high intensity). The R2B holding threshold sweep's returns-to-duty and died-of-wounds responses did not separate across the grid at 10 replications; at 30 replications the one-day-threshold arm's returns to duty (2,615.7 [2,579.6, 2,651.8]) and deaths of wounds (11.90 [10.41, 13.39]) are both clear of the disabled arm's (2,118.8 [2,092.7, 2,145.0] and 15.40 [14.14, 16.66]), a policy-relevant finding the smaller design could not report. The forward ICU share frontier and R2B holding threshold sweep were migrated for cross-table consistency rather than because either was itself under-resolved, both running under the shipped default configuration below the casualty load at which any response fails to converge; this is documented explicitly rather than left implicit, so a future reader does not mistake the migration for a finding. A stale limitation in `docs/Multi_Run_Supplement.md`'s "Limitations of the Designs Recorded Here", naming the transport and ICU share sweeps' utilisation columns as undetermined, is deleted rather than marked resolved, along with two other passages that still described the pre-migration reading as current.

This completes all three migration candidates PR #411's audit costed, closing #405.

**Seed-42 baseline (30 days, single run):** unchanged. No trajectory, `env_data.json` value or tracked seed-42 artifact is touched; `scripts/check_baseline_reproduction.R` passes.

**Unblocked by this merge:** No new issues unblocked. #405's audit and every costed migration candidate are now complete.

### Issue 405 — Two of Three Migration Candidates Executed (Partial)

**Advanced:** PR #413, branch `claude/405-migrate-hold-window-icu-gate`

**Reference is deliberately non-closing.** PR #411 costed three migration candidates without executing any of them; this pull request executes two, the R2B pre-open hold window and the post-operative intensive care gate, leaving the transport fleet-size sweep and #405's remaining audit tasks open.

`R/hold_window.R` and `R/icu_gate.R`'s protocol constants move from 50 replications x 30 days to 30 x 360, both experiments re-run under `--refresh-baseline`, and every dependent artifact updated: `data/hold_window/`, `data/icu_gate/`, `docs/Multi_Run_Analysis.md`'s Option 3 table and forest plot and its Post-Operative Intensive Care Gate section, and both design sections and the Standardising the Other Experiments table in `docs/Multi_Run_Supplement.md`. The measurement does not bear out the migration's own rationale, that a 360-day window's twelvefold event count would buy resolution for under-resolved rows. Instead, most count-based responses in both experiments become harder to resolve: the R2E first-surgery row's replications-needed rises from 838 to roughly 24,655 and the ICU gate mortality row's from 35 to roughly 471, a longer campaign giving the two arms' event orderings more time to diverge from a shared seed rather than giving the comparison more independent events to average over. This holds even for the ICU gate, whose arms were expected to carry none of the hold window's stream-shifting penalty, since both run under one control seed with no event that shifts the stream the way introducing the hold does; the intervention's own effect on resource contention turns out to be enough on its own. The one response type to improve in either experiment is a time-weighted share (`icu_occupancy`, reps-needed 50 to 6) rather than an accumulating count, and the R2B-surgeries row also resolves (+36.73 [+19.19, +54.28]), but because its own signal accumulated faster than its noise rather than because the noise fell. The Standardising the Other Experiments text is rewritten to report this measured pattern rather than the pre-registered prediction, narrowing the general claim to responses whose signal accumulates at least as fast as whatever divergence the intervention introduces.

Real relief remains established in both experiments at the new horizon: R2B keeps 80.03 casualties forward per campaign against none at a zero window, and R2E intensive care utilisation falls 7.06 percentage points under the gate, larger than the 5.25 points the 30-day design measured. `scripts/check_hold_window_protocol.R` and `scripts/check_icu_gate_protocol.R` both pass against the regenerated evidence, as do the two mechanism checks the migration does not touch (`scripts/check_icu_gate_switch.R`, `scripts/check_pre_open_window.R`); 49 of 50 fast checks pass overall, the one failure (`check_lint.R`) being a pre-existing environment issue in the unpinned sandbox this measurement was made in (`lintr` is not pinned in `renv.lock`), unrelated to this diff.

**Seed-42 baseline (30 days, single run):** unchanged. No trajectory, `env_data.json` value or tracked seed-42 artifact is touched; `scripts/check_baseline_reproduction.R` passes.

**Unblocked by this merge:** No new issues unblocked. The transport fleet-size sweep remains the one uncosted-to-executed migration candidate under #405, now flagged for re-costing against a measured rather than an assumed variance before it is attempted.

### Issue 405 — Standardise Every Replicated Experiment's Horizon and Replication Count (Partial)

**Advanced:** PR #411, branch `claude/405-heartbeat-update-9gvvi8`

**Reference is deliberately non-closing.** The issue asks for an audit of all thirteen replicated experiments' horizon and replication count against the sustained-operations protocol #312 established, a decision on which should migrate, costing each migration, and then the re-runs and document updates themselves. This pull request completes the audit and the decision alone, which the issue's own recommended approach treats as the first task and expects to precede any re-run; the re-runs, the corresponding updates to `docs/Multi_Run_Analysis.md`, and the drift reporting for any newly migrated experiment remain. #405 stays open.

A new "Standardising the Other Experiments" section in `docs/Multi_Run_Supplement.md` continues the audit the existing "Which Findings Are Horizon-Limited" and "Whether to Re-Screen at Length" subsections already carry out for the sustained-operations protocol's own findings and the sensitivity screens. For the remaining nine experiments it records whether the current replication count has a documented derivation, whether the 30-day horizon has been checked against the finding that four R2E-adjacent responses do not converge within 360 days, and a retain-or-migrate decision. Three experiments (the R2B pre-open hold window, the post-operative intensive care gate, and the transport fleet-size sweep) are decided as migration candidates, each already flagged in its own design entry as under-resolved at 30 days on an event count a 360-day horizon would directly address, and each is costed against the measured 3.8x runtime ratio (roughly 2.3x, 2.3x and 11.4x the current run respectively). The remaining six non-sustained-horizon experiments are retained, with the reasoning recorded per experiment: two run under the shipped default configuration, whose casualty load sits below the boundary at which any response fails to converge, and three compare the two casualty-intensity profiles at the horizon their own calibration anchors describe, so lengthening them would answer a different question rather than sharpen the one they ask.

**Seed-42 baseline (30 days, single run):** unchanged. No trajectory, `env_data.json` value or tracked seed-42 artifact is touched, and no tracked evidence set is re-run.

**Unblocked by this merge:** No new issues unblocked. The three migration candidates remain open follow-up work under #405 itself.

### Issue 339 — Re-screen the Responses Whose Definition Changed ✓

**Merged:** PR #407, branch `claude/heartbeat-progress-updates-hguuez`

The Morris screen is re-run at r = 20, 5 replications and 30 days over all thirty-six responses, replacing the rankings of the four responses whose definitions had changed since the tracked screen (the two dwell times under #331's Kaplan-Meier restricted mean, and the two backlog responses #362 corrected). Before the run, an audit of every numeric leaf in `env_data.json` against `morris_params` and the README's exclusion list found thirteen parameters screened by neither, and `apply_role4_and_threshold_params()` (`R/sensitivity.R`) adds them, taking the set from 65 to 78 parameters and the design from 1,320 to 1,580 points: the Role 4 reconstruction share, return interval and post-reconstruction return rate, the four Role 4 length-of-stay modes, the Role 4 intensive care continuation duration, the R2E pre-flight critical hold share and duration, the forward theatre saturation release threshold, the R2B holding evacuation threshold, and the mass casualty wounded/killed split. The run took about fourteen hours on four cores in an unpinned R 4.3.3 environment at commit `a3dc41a`, resuming once from its design point cache after a host restart. Across the sixty-five parameters both screens share, the dwell rankings' order broadly held (rank correlation 0.89 at R2B, 0.88 at R2E). On the primary ranking no parameter separates from the one below it at this trajectory count, and L18 is rewritten to say so.

Two departures from the plan are recorded rather than hidden. The audit missed that #335 had settled `evac_threshold` outside the screened set: it ships disabled, so its first grid step measures switching it on. It was screened anyway and, by the owner's decision, kept, with its ranks (first on R2B dwell, both forward return-to-duty rates and p90 time to surgery) annotated throughout as that on/off transition, and the Multi-Run Analysis and Supplement updated to match. And #348's two coverage gaps, a holding bed queue response at either echelon and the R2E bed counts as screened parameters, were not added, although this re-screen was the occasion they were sequenced against; adding them now needs a screen of its own. Three of the added parameters (`role4_return_interval_mode`, `role4_post_reconstruction_return_rate`, `role4_icu_continuation_mode`) measure µ\* of zero on every response, none of the thirty-six measuring what they change. The Sobol decomposition's five parameters no longer match the ranking, `mass_casualty_min_cas` falling to 12th and `pri1_dcs_rate` to 25th; Issue #408 is raised to re-run it on the new selection.

**Seed-42 baseline (30 days, single run):** unchanged. No trajectory, `env_data.json` value or tracked seed-42 artifact is touched.

**Unblocked by this merge:** #408 (re-run the Sobol decomposition on the new selection) moves from `status: blocked` to `status: ready`. #348 was already closed (PR #398, Task 1 only); its two remaining coverage gaps, a holding bed queue response at each echelon and the R2E bed counts as screened parameters, were not added to this re-screen; Issue #410 now tracks them.
...
**Merged:** PR #402, branch `claude/work-on-300-kldi34`

Option 4's three-ambulance recommendation rested on a fleet-size sweep run at moderate intensity only, and the paper named this as the finding most likely to change under surge. `plot_transport_capacity_margin_by_fleet_size()` (`R/analysis.R`) and `scripts/run_transport_sweep.R` gained a `scenario` parameter, resolved against the parsed configuration before the per-point fleet-size override is applied, so the same design runs under any named profile; a non-default scenario writes its own `_<scenario>`-suffixed CSV and figure. The sweep was re-run under `high_intensity` at the documented protocol (10 replications x 30 days per point, PMV Ambulance 1-5, HX240M 1-4, seed 42), and the tracked result is added to `docs/Multi_Run_Analysis.md`'s Option 4 alongside the existing moderate-intensity table.

The boundary the sweep locates does not move. One vehicle is far more inadequate at high intensity (43.63 casualties queued, 95% CI [19.68, 67.57], against 3.21 at moderate intensity), but the queue at two vehicles is close to the moderate-intensity reading at the same fleet size, and at three, the shipped establishment, it is smaller in absolute terms (0.069 against 0.131) though now resolved away from zero (CI [0.0345, 0.1031]) rather than spanning it, the higher casualty volume giving the sweep more transport events to pin the estimate down. The truck fleet shows the same pattern at a smaller scale. Three ambulances and four trucks remain sufficient, so the qualification that Option 4's evidence rests on one casualty rate is removed, the corresponding Further Development item is retired (renumbering the two after it), and README Further Development entry L19 is narrowed to the mass-casualty-injection half of the gap that remains. `scripts/check_capacity_sweep_protocol.R` gained checks for the new tracked evidence set and its published table, on the same shape as the existing moderate-intensity checks.

**Seed-42 baseline (30 days, single run):** unchanged. No trajectory, arrival process or `env_data.json` value was touched.

**Unblocked by this merge:** No new issues unblocked.

### Issue 335 — R2B Holding Capacity vs. the Evacuation Threshold, Swept Jointly ✓

**Merged:** PR #400, branch `claude/work-on-335-smwtny`

Option 2 named two substitute remedies for the R2B holding shortfall, capacity and an evacuation threshold, and labelled both untested; this closes that gap and the sequencing question it left open for #348. `plot_r2b_hold_threshold_sweep()` (`R/analysis.R`) and `scripts/run_hold_threshold_sweep.R` sweep R2B holding beds per unit (5 shipped, 7, 10) against `r2b.holding.evac_threshold` (disabled/shipped, 1, 3, 5, 7 days) as a fifteen-point cross product, ten replications each, on the shape the existing transport and forward ICU share sweeps already use. Ten beds per unit clears the forward queue (0.93 casualties to 0.03) with no measured cost to R2E holding or intensive care; the threshold only reaches comparable relief at an aggressive one-day setting, and does so by measurably raising R2E holding utilisation (74.2% to 84.6%) rather than removing the load. `docs/Multi_Run_Analysis.md`'s Option 2 is updated with the tracked result and `docs/Multi_Run_Supplement.md` gains the design.

The sweep also settles the Morris screening question the issue raised without a re-screen: `evac_threshold` stays outside the screened set. The measured response confirms the discontinuity the issue predicted, most of the swept range's movement occurring in the single step from disabled to one day, which a screening elementary effect across that step would measure as the on/off transition rather than the parameter's magnitude.

**Seed-42 baseline (30 days, single run):** unchanged. No trajectory, arrival process or `env_data.json` value was touched.

**Unblocked by this merge:** No new issues unblocked. #348's two remaining coverage gaps are still gated on #339 as well as #335, and #335 alone does not trigger the re-screen; #348 stays open pending that decision.

### Issue 348 — The Sensitivity Cache Cannot Detect a Response Added After It Exists (Partial)

**Advanced:** PR #398, branch `claude/work-on-348-m9o6sa`

**Reference is deliberately non-closing.** Issue #348 raises three separable pieces: a cache trap that corrupts or silently empties a design point cache when a response is added after it exists, two coverage gaps in the screened response and parameter sets (no holding bed queue response at either echelon, and the R2E bed counts excluded from screening), and the sequencing risk of doing any of it out of step with #335 and #339, which touch the same design. This pull request closes the cache trap alone, which the issue names as ready now and dependent on nothing; the two coverage gaps remain, gated on whichever of #335 or #339 next triggers a re-screen, since a Morris design is a function of its parameter count and adding parameters separately would triple the compute cost of adding them together. #348 stays open.

`cache_append()` (`R/sensitivity.R`) wrote a design point's response vector under whatever header `points.csv` already carried, so a response added to `morris_kpis` or `SOBOL_RESPONSES` after a cache existed was invisible to it: the first re-evaluated point wrote a wider row under the file's original, narrower header, and every later read either errored on the ragged row or, had the missing column been backfilled with `NA` instead, would have read every point as cached with the new response silently and permanently empty, indistinguishable from a response legitimately undefined at that design point. `cache_check_schema()` checks a cache's header against the response set the evaluation loop is about to look up before that loop starts, archiving a cache that does not carry all of it under a `.stale-<timestamp>` suffix rather than resuming it, so every point recomputes instead of reading as cached with the gap unfilled. `run_morris()` and `run_sobol()` both call it before their evaluation loops. `scripts/check_screen_cache.R` gained a sixth assertion covering the new behaviour, and `data/sensitivity/README.md` now records the response and parameter set every tracked ranking was screened against.

**Seed-42 baseline (30 days, single run):** unchanged. No trajectory, arrival process or `env_data.json` value was touched, and the tracked evidence set's cache files already carry every response and parameter this change checks for, so `cache_check_schema()` is a no-op against them.

**Unblocked by this merge:** No new issues unblocked. #335 and #339 remain the gates on the remaining two tasks.

### Issue 389 — The Mass Casualty Stress Test, Backed by a Tracked Evidence Set ✓

**Merged:** PR #396, branch `claude/gallant-thompson-ci6i8q`

Mass Casualty Events Degrade Care Without Revealing New Constraints prints a four-row table from replicated arms, and it was the last of the three unbacked experiments Issue #384 raised, and the weakest case of the three: the section's own table labelled its central comparison direction only, thirteen deaths per arm being too few for a precise figure, and #384 recorded that a tracked evidence set at the same ten replications would defend a comparison the paper already declined to read precisely. Rather than back it as it stood, the issue first derived the replication count that would resolve it: separating a died-of-wounds rate by a tenth of a percentage point, the size of the difference the table reported, is the exact case `docs/Multi_Run_Analysis.md`'s Replication Count and Resolution already states needs 62 runs. `R/mass_casualty.R` and `scripts/run_mass_casualty.R` give the comparison a runner of its own, on `R/hold_window.R`'s arrangement but unpaired, the two arms running under their own control seeds rather than a shared one; `data/mass_casualty/` tracks both arms' per-replication responses, the total-casualty and event-count summary, and the died-of-wounds rate pooled across replications with an exact binomial interval, plus the illustrative single run behind `images/mass_casualty_events.png`, the one tracked image `run.R --refresh-baseline` cannot produce. `scripts/check_mass_casualty_protocol.R` asserts the protocol constants against the supplement's marker comments, the tracked set against those constants, and every figure the table prints against the tracked measurement, closing with a hand-computable check on the pooled rate and its interval; it was validated by fault injection, a perturbed figure failing it on that exact cell. `scripts/check_mass_casualty_kia_split.R` is unchanged and continues to cover the injection mechanism rather than the magnitudes.

Re-run at 62 replications, the event-versus-ordinary died-of-wounds comparison resolves for the first time: 140 deaths among event casualties against 91 among ordinary casualties in the same arm, a rate of 0.91% [0.76%, 1.07%] against 0.33% [0.27%, 0.41%], intervals that no longer overlap, upgrading the finding from direction only to measured. What the wider count does not resolve is whether concurrent injection itself raises the ordinary-casualty rate: 0.30% [0.24%, 0.37%] without injection against 0.33% [0.27%, 0.41%] with it is a movement inside both arms' intervals, so that comparison is reported as unresolved rather than upgraded by assumption. The supplement's evidence audit table now records every experiment as backed.

**Seed-42 baseline (30 days, single run):** Unchanged. This PR alters no trajectory, no arrival process and no `env_data.json` value, so no RNG stream shifts and no tracked seed-42 artifact was regenerated.

**Unblocked by this merge:** No new issues unblocked. #389 was the last of the three unbacked experiments Issue #384 raised, and no other open issue was gated on it.

### Issue 388 — The Post-Operative Intensive Care Gate, Backed by a Tracked Evidence Set ✓

**Merged:** PR #394, branch `claude/work-on-388-oq0yvd`

The Post-Operative Intensive Care Gate section prints the shipped rationing rule's effect on R2E intensive care utilisation and on deaths of wounds, and none of those numbers existed in a tracked file: the experiment's entry point was `run.R` under a parameter override rather than a driver script, on the same reading Issue #387 closed for the pre-open window. `R/icu_gate.R` and `scripts/run_icu_gate.R` give it a runner of its own, on `R/hold_window.R`'s arrangement for a paired two-arm comparison rather than sharing that issue's module, the two differing in what each arm overrides and what each reduces to; the decision to keep them apart is recorded in the supplement. `data/icu_gate/` tracks both arms' per-replication responses and their paired differences at 50 replications per arm, and `scripts/check_icu_gate_protocol.R` asserts the protocol constants against the supplement's marker comments, the tracked set against those constants, and every figure the section prints against the tracked measurement, closing with a hand-computable check on the interval construction; it was validated by fault injection, a perturbed figure failing it on that exact cell. `scripts/check_icu_gate_switch.R` is unchanged and continues to cover the mechanism rather than the magnitudes.

Regenerating the evidence set moved every published figure in the section: R2E intensive care utilisation falls from 89.1% [87.5%, 90.6%] without the rule to 83.8% [82.2%, 85.5%] with it, a paired difference of 5.25 points [3.20, 7.30] against the previous 6.80 [5.13, 8.47]. The mortality point estimate changes direction, deaths of wounds reading 1.14 [0.76, 1.52] without the rule against 1.36 [1.02, 1.70] with it, a paired difference of 0.220 [-0.207, +0.647]; the rule now costs a fraction of a life rather than saving one, which is what the design predicts rather than the opposite of it, though the interval still spans zero and the replication count the effect would need falls from roughly 2,283 to 180. Within the enabled arm, 0 of 2,534 casualty-replications recovering in intensive care died of wounds against 3 of 2,120 recovering in a holding bed (0.14%).

**Seed-42 baseline (30 days, single run):** Unchanged. This PR alters no trajectory, no arrival process and no `env_data.json` value, so no RNG stream shifts and no tracked seed-42 artifact was regenerated.

**Unblocked by this merge:** No new issues unblocked. #389, the one remaining unbacked experiment Issue #384 raised, was already unblocked by that merge.

### Issue 387 — The R2B Pre-Open Hold Window, Backed by a Tracked Evidence Set ✓

**Merged:** PR #392, branch `claude/work-on-387-bzdb4w`

Option 3 of the companion paper prints eight paired differences comparing the shipped 60-minute R2B pre-open hold window against a window of zero, and none of the twenty-four numbers behind the table existed in a tracked file: the experiment's entry point was `run.R` under a parameter override rather than a driver script, so it needed a runner of its own before it needed compute, which is why it did not close alongside the other two Issue #384 raised. `R/hold_window.R` and `scripts/run_hold_window.R` give it one, on `R/policy_sweep.R`'s arrangement for a paired two-arm comparison, and track both arms' per-replication responses and their paired differences in `data/hold_window/` at the documented 50 replications per arm. `scripts/check_hold_window_protocol.R` asserts the protocol constants against the supplement's marker comments, the tracked set against those constants, and every cell of the published table against the tracked measurement, closing with a hand-computable check on the reduction functions; it was validated by fault injection, a perturbed table figure failing it on that exact cell.

Regenerating the evidence set moved the published figures rather than merely backing them: the paired differences replicate today are computed on a different sample than the untracked run that produced the previous table, and three of the eight measures the old table read as established are two on the new one, the R2E theatre-entry-deferred effect no longer excluding zero. The section's headline claim is softened to match, and the supplement's replications-needed figures for the unresolved rows are restated from the tracked data (about 76 replications per arm for the forward-surgery effect, 838 for R2E first surgeries) rather than left as the earlier estimate. The issue's own open decision, whether to regenerate at 50 to reproduce the published table's shape or at roughly 120 to also resolve the forward-surgery row, was resolved for 50: closing the audit gap was the acceptance criterion, and the compute for 120 buys one row rather than the whole table.

**Seed-42 baseline (30 days, single run):** unchanged. This PR alters no trajectory, no arrival process and no `env_data.json` value, so no RNG stream shifts and no tracked seed-42 artifact was regenerated; the one cross-reference in `CLAUDE.md`'s Key Parameters table to a 50-replication pre-open hold figure is updated to cite the newly tracked `data/hold_window/` measurement (6.76 held per run, 95% CI [6.10, 7.42]) in place of the untracked one it replaces. The measurements were made in an unpinned R 4.3.3 sandbox; CI reproduced the tracked seed-42 set byte for byte in the pinned container on the same pull request, and `check_replication_memory.R` failed once on an unrelated NA peak-memory reading from the RSS poller and passed clean on a re-run.

**Unblocked by this merge:** No new issues unblocked. #388 and #389, the two remaining unbacked experiments Issue #384 raised, were already unblocked by that merge; #388's own entry notes it may share this issue's runner module, which is now available to build on but not a gating dependency.

### Issue 384 — The Paper's Centrepiece, Backed by a Tracked Evidence Set ✓

**Merged:** PR #390, branch `claude/work-on-384-wdcnnk`

Most of the experiments the companion paper publishes wrote their results only to the gitignored `outputs/`, so no tracked file held the numbers the paper printed and there was nothing a reader or a check could compare them against. This closes the first three. `scripts/run_scenarios.R`, `scripts/run_transport_sweep.R` and `scripts/run_icu_share_sweep.R` gain `--refresh-baseline` on the convention `scripts/run_airlift_sweep.R` established, writing the tracked `data/scenarios/` and `data/sweeps/`; the flag fixes each protocol's replication count, horizon, seed and swept range rather than accepting whichever arguments accompany it, so a mistyped argument cannot leave the tracked set describing a different experiment. Two protocol checks, `scripts/check_scenario_protocol.R` and `scripts/check_capacity_sweep_protocol.R`, assert the code's constants against the supplement's markers, the tracked set against those constants, and every published figure against the tracked measurement, each closing with an assertion on a hand-computable input so that agreement is not two copies of one error. Both were validated by fault injection rather than by inspection.

The issue predicted that a section with no tracked set could drift with nothing to notice, and the re-measurement found exactly that. The comparative scenario queue table mixed per-bed averages with pool totals, printing R2E intensive care at 0.078 against a measured 1.229 and R2E holding at 0.056 against 1.656. The paper's own abstract already carried the correct ratios, so the body table had been stale and unauditable at the same time. It is now the pool total throughout, recovered by the same estimator the campaign time series uses, and the surrounding prose, the conclusion and `images/paper_queue_growth.png` are reconciled to it. One claim was withdrawn as now false, that every queue grows faster than casualty volume: the transport fleets grow 2.29-fold against volume's 2.40 and are the one pool that does not. `scripts/render_paper_figures.R` had a hardcoded `CASUALTY_RATIO <- 2.33` whose own roxygen claimed it came from the table; it now reads the ratio from the table, so it cannot drift again.

The supplement gains a section recording the rule deciding which published figures require a tracked evidence set, the audit of every experiment against it, the three that remain unbacked with the reason each is still owed one, the three exempt under the rule, and the decision not to share a markdown table parser between the protocol checks. The two capacity sweeps keep a mean and an interval per point rather than the replications behind them, so their check cannot recompute the half-width; that limit is recorded rather than left to be inferred.

**Seed-42 baseline (30 days, single run):** unchanged. This PR alters no trajectory, no arrival process and no `env_data.json` value, so no RNG stream shifts. The comparative scenario casualty totals reproduced the published table exactly and `images/scenario_comparison.png` came back byte-identical, which is what makes the queue table's disagreement a finding rather than an environment difference. The measurements were made in an unpinned R 4.3.3 sandbox; CI reproduced the tracked seed-42 set byte for byte in the pinned container, but no check re-runs the three new evidence sets, so a maintainer re-run remains the standard before their figures are treated as canonical.

**Unblocked by this merge:** #387, #388 and #389, the three remaining unbacked experiments, each raised from this issue's audit and each gated on the rule, the audit table and the check pattern this merge establishes.

### Issue 382 — The Strategic Evacuation Figures, Asserted Against Their Evidence Set ✓

**Merged:** PR #385, branch `feature/issue-382-airlift-protocol-check`

Four replicated experiments had a protocol check asserting that the figures the companion paper prints agree with the tracked evidence set behind them, and this one had none: `data/airlift/`'s baseline and sweep summaries were read by no check at all, the collapse check covering only the collapse files. That is the section that drifted between PR #379 and PR #381 with nothing reporting it, printing 26.96 casualties still waiting at high intensity against a tracked 108.28.

`scripts/check_airlift_protocol.R` closes it on the pattern the siblings establish, in 74 assertions and ten seconds: every protocol parameter in `R/airlift.R` equals the value the supplement documents in a marker comment, the tracked responses carry that replication count across the documented thirteen configurations, every figure the paper's three tables print is the tracked summary's own, and `summarise_airlift()`'s mean and interval are right on an input whose answer is computable by hand, so a table agreeing with the summary is not two copies of one error. A missing table, row or column fails rather than passing quietly. Validated by fault injection rather than by inspection: perturbing a printed figure, deleting a table marker, renaming a row, dropping a column and changing a documented parameter each fail it, the first reproducing the historical drift exactly.

**The open question the issue asked be settled was settled in the prose's favour.** Parsing figures out of sentences fails on a stylistic reword rather than on a stale number, so the section's figures move into tables instead: the baselines and the cancellation arms become two new tables, the interval table gains the ventilated hold row, and the prose narrates them rather than restating them, which is also what this project's one-home rule asks. No figure changes value. The supplement's account of the holding occupancy split is corrected in the same PR, it having still described the two-component estimator that PR #363 replaced and PR #381 corrected again.

**Seed-42 baseline (30 days, single run):** No metric moves. No model code changes and no tracked evidence set is regenerated.

**Unblocked by this merge:** No new issues unblocked. #384 is raised from the audit this issue's last task called for.

### Issue 362 — The Strategic Evacuation Stay, Bounded by Its Release ✓

**Merged:** PR #381, branch `claude/issue-362-h0dhli`

The over-count had one cause, and it is the reason the residual was always an over-count and never the reverse. `holding_occupancy_split()` bounded a casualty's staging stay by `ame_departure_time`, which only boarding sets. A casualty who dies during `r2e_ame_dow_poll()` releases its staging holding bed at death and never departs, so the split could not tell it from a casualty still waiting when the window closed and charged it the whole unserved remainder of the campaign. In the check's own shipped arm one casualty, staged at minute 5,056 and dead one poll interval later, accounted for 25.4885 bed-days of a 25.5-bed-day residual. The release is now recorded on the casualty as `ame_hold_end`, which both routes out of the pool set, and the stay is bounded by it.

**The same defect was found and fixed in a second reconstruction.** `compute_ame_backlog()` added a casualty to the strategic evacuation queue at the evacuation decision and removed it on departure, so a casualty who died waiting inflated the published series for the rest of the campaign. A second defect in the same function is fixed with it: the step function was extended to the window's close by copying the first rather than the last of the events sharing the campaign's final instant, so a pool cleared by a sortie that also received an arrival at that instant read one casualty high, which is the common case. At seed 42 the standard route's series now peaks at 18 rather than 19 and returns to zero, which is what that figure's alt text and the single-run paper's reading of it have always said and what the superseded reconstruction did not show.

Both fixes are analysis-side and `set_attribute()` consumes no random draw, so the model is untouched: the seed-42 evidence set reproduces byte for byte and `images/ame_backlog.png` is the only tracked artifact the refresh moves. `data/airlift/` was regenerated at 650 replications; `hold_total_bed_days` and every decision, boarding, wait and Role 4 response are identical to the superseded copy, and only the three split columns move. Every replication now closes exactly, against 619 of 650, and the worst over-count of 6.43% of a campaign's pool is gone. `scripts/check_holding_occupancy_split.R` is tightened rather than relaxed, its population bound falling from 8% to the arithmetic tolerance and its exact-closure share from 80% to every replication, and the new `scripts/check_ame_backlog_exit.R` holds the backlog's two properties against a constructed monitor and against the reconstruction it replaces.

**Two figure corrections came with it, neither caused by this fix.** `CLAUDE.md`'s AME wait-time DOW poll row claimed no seed-42 run had ever produced such a death, contradicting the DOW count row two lines above it; the run produces one, and it is the casualty the issue turns on. And the national support base section of `docs/Multi_Run_Analysis.md` had not been refreshed since PR #379, disagreeing with its own tracked evidence set in quantities this fix does not touch, worst among them the casualties left waiting at high intensity, printed as 26.96 against a tracked 108.28. Both are corrected against the regenerated set, and the gap that let the second happen is raised as #382.

**Seed-42 baseline (30 days, single run):** No metric moves. The model is unchanged and the tracked logs and arrival diagnostics reproduce byte for byte.

**Unblocked by this merge:** No new issues unblocked. #339 gains two more superseded responses and is retitled accordingly, #348 gains a fourth driver against its sequencing table, and #382 is raised.

### Issue 376 — Both National Support Base Levers, Enabled and Re-measured ✓

**Merged:** PR #379, branch `claude/nsb-casualty-360-day-run-yje6gh`

The capacity release and the reconstruction cohort both ship in force, and every replicated evidence set is re-measured once under them rather than twice, which is the reason the two flips were held for one issue. `role4.surgery.reconstruction_share` is set to the 0.2 its calibration already supported. `r2eheavy.second_surgery.saturation_queue_threshold` had no source to set it from, so `scripts/run_saturation_sweep.R` was written to sweep it at the sustained horizon and it ships at 8, the mean closing-window theatre queue an unrelieved campaign carries. Anchoring on that measured property rather than on whichever arm reads best avoids choosing the answer from the same data used to measure it.

**The sweep decides against the alternatives rather than merely describing them.** A threshold of 1 costs 2.8 further died of wounds per campaign-year (p = 0.025) and 5.9 points of post-operative intensive care access, a released casualty holding an intensive care bed through the whole pre-flight wait, so releasing eagerly is measurably harmful. From 2 upward the forward relief is statistically identical at every arm, so the threshold sets how much work moves rearward rather than how much stability it buys forward. No threshold in the swept range of 0 to 24 is inert, the relief decaying smoothly toward the disabled state rather than switching off. The threshold is a saturation response and behaves as one across horizons: the R2E theatre queue averages 0.98 over a 30-day Falklands-rate campaign, 155 at high intensity and 8.2 over 360 days, so the shipped value fires at length and at intensity and never fires in the published single-run campaign.

**Five published findings changed in substance rather than in their digits**, and each is recorded where it appears. The 30-day evacuation policy now carries a resolved mortality cost of 2.77 deaths per campaign-year (p = 0.009) at the same setting where both R2E pools saturate, where the sweep previously resolved no mortality effect anywhere in the range. The airlift collapse cliff moves from 15% to 10% sortie cancellation, the reconstruction cohort putting more casualties on the standard route so the same proportional loss strands more of them. The claim that a campaign's outcome cannot be predicted from its opening is withdrawn, an opening-window fit now being significant pooled across the collapsing arms (p = 0.0014). R2E intensive care and holding queues now scale about 25-fold and 24-fold from moderate to high intensity against 6.4 and 5.3, so all three R2E pools scale together rather than the theatre queue standing apart. And the R2B holding structural overload falls from 60% to 37%.

**Three regression checks failed on code rather than on stale documentation.** One assertion added with the reconstruction cohort was simply wrong. The censored-interval check compared a seven-day restricted mean against a thirty-day capped one, so the ordering it asserted was never a theorem; four uncensored stays reproduce the failure with no censoring at all, and both are now capped at the restriction horizon. The third is a real degradation and is recorded as one: the holding occupancy split's four components over-count by 25.5 bed-days of 389 where they previously closed to within a thousandth of a bed-day, the same defect #362 tracks biting more often now that more casualties wait on the standard route. Its bounds are raised to 8% with the measurement and the cause stated at each, deliberately rather than quietly, and the ratchet still fails on any further worsening.

**Seed-42 baseline (30 days, single run):**

| Metric | Before | After |
|---|---|---|
| Total casualties | 495 | 378 |
| WIA / KIA / DNBI | 212 / 64 / 219 | 136 / 78 / 164 |
| Total return to duty | 172 (34.7%) | 134 (35.4%) |
| Role 4 peak occupancy | 120.0 | 76.0 |
| Role 4 bed-days (ICU / hold) | 906 / 2,660 | 529 / 1,776 |
| Role 4 operations owed | 0 | 35 across 27 days |

Almost all of that movement is stream shift rather than behaviour: the reconstruction cohort draws once per Priority 1 operated casualty at disposition. The saturation threshold contributes nothing at this horizon.

**Unblocked by this merge:** No new issues unblocked. #362 is unchanged in status but now carries a larger measured residual, recorded in the check that bounds it.

---

### Issue 373 — Reconstruction as an Interval, Not a Count ✓

**Merged:** PR #377, branch `feature/issue-373-role4-reconstruction`

A repatriated casualty whose wounds need staged soft-tissue coverage undergoes a sequence of operations, not one, and #368 had closed only the single conserved repair. The model now represents the sequence, and represents the reason it happens as a reason to evacuate in its own right rather than as a filter applied to casualties already evacuated for something else. `role4.surgery.reconstruction_share` selects the cohort on the wound, at 18.8% of 5,227 combat casualties with an Injury Severity Score of 9 or more who survived through all levels of care (D'Souza et al., 2022), applied to Priority 1 casualties operated on in theatre.

**The blocker recorded against this issue was real, but it was not the one the issue stated.** The scoping the earlier entries called for was a conditioned rate, and no open-access source reports one. What actually blocked the work was that the model cannot identify the cohort: it carries no anatomical description of a wound, only triage priority, whether an operation was performed, and the damage control pathway. Selecting on expected recovery, as a first design did, reads the same severity classification the evacuation decision already reads, so it picks the severe tail twice rather than finding the casualties whose wounds need coverage. Selecting on the wound proportion reproduces the right share of casualties rather than the right casualties, which is the limit now recorded under Further Development L33 instead of a calibration claim.

The sequence is interval-driven rather than count-driven. The Royal Centre for Defence Medicine's protocol is a return to theatre every 48 to 72 hours until no further sign of necrosis or infection (Evriviades et al., 2011), so the interval is the parameter and the count is its consequence, following from how long the debridement phase runs. Lengthening the interval reduces the procedure count while the reconstruction itself survives, which is what the check asserts to show the count is derived. After the reconstruction the model reports nothing, reported flap failure rates of 0% to 1.4% supporting that (Connolly et al., 2016); the residual is representable through `post_reconstruction_return_rate` and ships at zero. Reconstruction joins the evacuation policy and the forward saturation release as one of three reasons a casualty leaves theatre, `evacuation_reason` recording which decided each disposition, and it changes the outcome rather than restating it: at a 60-day policy four of the seed-42 cohort would be retained on recovery alone.

**Seed-42 baseline (30 days, single run):** unchanged. `logs/logs.txt` and every `data/` file reproduce byte for byte, a share of zero consuming no draw. This was verified rather than assumed: the mechanism was built shipping active first, and at a share of 0.2 seed 42 produces 378 arrivals against 495, returning to exactly 495 at zero. The whole movement is the stream shift from one extra draw per Priority 1 operated casualty at disposition, with no behavioural change to any other casualty. The default was then returned to zero, so the merge carries the verification without carrying the movement.

**Unblocked by this merge:** No new issues unblocked. #376 was raised before this merge and is already `status: ready`; it enables both national support base levers together and re-measures every replicated evidence set once, rather than twice.

---

### Issue 368 — The Operation a Released Casualty Carries Rearward ✓

**Merged:** PR #374, branch `feature/issue-368-role4-surgical-demand`

A casualty released to strategic evacuation with the definitive repair outstanding, which PR #371 made possible, lost that operation at the moment of boarding: the Role 4 census reports bed types and that echelon is given no operating theatre, so work taken off the deployed hospital arrived nowhere. Enabling the release would have deleted demand rather than redistributed it. `compute_role4_surgical_demand()` now reports it, in operations and theatre-minutes under the `ot` label the deployed echelons use, beside the bed census rather than inside it: a theatre is a throughput resource and a bed an occupancy one, and an operation of ninety-five minutes occupies no bed for a day.

The operation is conserved rather than re-estimated, which is the property the issue turned on. The releasing theatre draws it, from the distribution its own second procedures come from, and the casualty carries that draw rearward as `definitive_repair_minutes`; the analysis reads the attribute and takes no draw at all, so the report is a function of the run rather than of how many times it is analysed, and total surgical time across the two echelons is exactly what a casualty operated on entirely in theatre would have consumed. The requirement is assigned and never seized: no capacity, no queue and no shortfall is introduced at Role 4, and `scripts/check_role4_surgical_demand.R` asserts that as an absence alongside the twenty-two positive assertions.

A defect in the preceding merge was found and fixed on the way. A released casualty carries no `post_definitive_min`, theatre having served none of the post-operative episode, so `add_role4_icu_days()` owed them nothing and they held an intensive care ward for a whole stay with no intensive care phase in it. They are now owed the whole requirement, and take the operated Priority 1 ward and length of stay whatever their triage priority, on the same reasoning that puts them on the critical airlift route.

**Scope was narrowed deliberately, and the reason is a dependency rather than effort.** The repeated procedures a repatriated casualty undergoes over the following weeks are not represented, and an open-access series puts the gap at 5.2 operations per casualty against the single one reported here. Representing them needs the aeromedical evacuation decision to carry an expected reconstruction burden, and it currently selects on system load and recovery duration alone, which say nothing about what a casualty will require once repatriated. #373 was raised for that work and is blocked on the scoping.

**Seed-42 baseline (30 days, single run):** unchanged. `logs/logs.txt` and every `data/` file reproduce byte for byte, and `images/role4_census.png` is byte-identical to a fresh render, which is the artifact the ward-category change could have moved. The released cohort is empty at the shipped threshold of zero, so the new category branch never fires and no random draw is taken anywhere reachable. The Key Parameters bed composition row is reworded to record that `ot` demand now exists as a reported quantity while remaining zero here.

**Unblocked by this merge:** No new issues unblocked. #365's non-zero shipped default is no longer blocked on the demand being lost, that half being closed, but it remains a separate decision with its own evidence; #373 is raised `status: blocked` on the evacuation decision scoping.

---

### Issue 365 — Evacuation Before the Definitive Repair, Built and Shipped Disabled ✓

**Merged:** PR #371, branch `feature/issue-365-dcs-evacuation-before-definitive-repair`

A damage control casualty operated on at R2E returned to that theatre for the definitive repair unconditionally, evacuation being the last phase of the trajectory, so the model had no way to represent the casualty flown out with the abbreviated operation still the only one they have had. `r2eheavy.second_surgery.saturation_queue_threshold` sets the number of casualties waiting to be operated on at or above which such a casualty is released to strategic evacuation instead. The decision reads a queue and draws nothing, and the threshold ships at zero, which disables the release; the check asserts that a zero threshold and a configuration with the field absent altogether produce the same campaign, so the lever costs the published model nothing. A released casualty carries `definitive_repair_outstanding`, serves no post-definitive episode, there being no repair for one to follow, and is not retained in theatre whatever their drawn recovery.

The issue's premise turned out weaker than it read, and that is worth recording. The model already evacuates damage control casualties before their definitive repair at R2B, 77 of them at seed 42, the R2B to R2E leg being exactly that movement. What was missing was the narrower R2E to Role 4 variant, which affects the 55 casualties per run who take their second operation in theatre. The trigger is theatre saturation rather than a drawn share, so it fires when forward surgical capacity is the binding constraint, which is what stops a sweep over that capacity from being self-limiting.

**Two defects in the first commit were found by the owner reading the design rather than by the check.** The trigger counted the surgical section queues alone, but theatre entry is a two-stage seizure, an operating theatre bed and then a section, so a casualty waiting for theatre is blocked on one or the other; counting sections alone saw only those already holding one of the two theatre beds, capping the measurable queue at two and making any larger threshold unreachable. `r2e_theatre_queue()` now sums both, which moved the seed-42 release at a threshold of one from 9 casualties to 17. And a released casualty waited for their flight in a holding bed, a Priority 2 one unconditionally and a Priority 1 one at the ventilated share; such a casualty is by construction not stable enough for a holding bed, and now takes the critical airlift route holding an intensive care bed for the whole pre-flight period. The release therefore costs forward intensive care rather than being free at the forward end.

**The threshold cannot ship at a value that fires until #368 lands, and that ordering is now recorded rather than implied.** A released casualty's outstanding operation currently leaves the model at boarding: the Role 4 census reports only the bed types that echelon is given and an operating theatre is not among them. Enabling the release today would take surgical work off the deployed hospital's figures and deliver it nowhere, understating total demand rather than redistributing it. #368 was rewritten to carry the non-zero default as one of its implementation tasks, along with the requirement that a Role 4 theatre requirement be assigned rather than seized, with no queue, capacity or contention, and that a released casualty's repair be carried rearward on the duration R2E already draws so surgical time is conserved end to end.

**Seed-42 baseline (30 days, single run):** unchanged. `logs/logs.txt` and every `data/` file reproduce byte for byte, the release branch being unreachable at the shipped threshold and consuming no random draw when it is not taken. No tracked image moves and no row of the Key Parameters table changes.

**Unblocked by this merge:** No new issues unblocked. #368 is unchanged in status but is now the explicit blocker on this issue's shipped default, and the two are sequenced with #368 first.

---

### Issue 314 — The Role 4 Boundary, Held ✓

**Merged:** PR #369, branch `feature/issue-314-role4-capacity-reporting`

This completes #314 by withdrawing four of its tasks rather than delivering them, and by aligning the census onto the bed types the deployed echelons already field. Both changes narrow what the model claims about the national support base to what it represents: the demand a land-based trauma system places on that echelon, and nothing about the echelon itself.

The `role4.capacity` block, its validation, its reader, the shortfall function and the reference lines on both censuses are removed. The model sets that demand and does not simulate Role 4, so nothing queues for a bed there, nothing is refused and nothing is pushed back into theatre; a stated establishment is a statement about an echelon carrying no case mix, no transfer options and no back-pressure, and a shortfall reported against one is arithmetic on a demand signal wearing the clothes of a capacity finding. PR #366's description had claimed the analysis reported that comparison, which no code path ever called; rather than wire it up, the boundary is now asserted. `scripts/check_role4_ward_phases.R` fails if any of the four removed symbols reappears in `R/analysis.R` or if a capacity block reappears in `env_data.json`, so an absence that was a convention is now a gate.

The census also reported occupancy under `ICU`, `Surgical Ward` and `General Ward`. The middle label named a specialty the model does not represent: evacuation is the last phase of the R2E trajectory, so every casualty reaching Role 4 has had their definitive repair in theatre and no surgery happens there, and that label carried 58.3% of the bed-days. The census now reports `icu` and `hold`, which is what R2B and R2E field. Two of the four bed types carry no Role 4 demand and are absent rather than reported as zero: there is no reception episode, so no `resus`, and no outstanding repair, so no `ot`. That second absence is a property of the model rather than of a national support base, which operates on repatriated casualties repeatedly; #368 carries what remains of that question.

**Seed-42 baseline (30 days, single run):** intensive care is unchanged at 906 bed-days (25.4%), and the 2,079 surgical ward and 581 general ward bed-days collapse into 2,660 holding bed-days (74.6%). The total of 3,566 and the peak of 120.0 on day 30 are unmoved, both being properties of arrivals and length of stay rather than of bed type. The collapsed distinction rested on a mapping the README recorded as having no open-access source, so it removed an uncited split rather than information the model could support. `images/role4_census.png` is the only tracked artifact that moves; `logs/logs.txt` and every `data/` file reproduce byte for byte.

**One task is left undone and is arguably moot.** #314 asked for the issue's own evidence table, the Role 4 occupancy comparison between a 30-day and a 21-day evacuation policy, to be re-measured at the current shipped defaults. That table was written to motivate the capacity work, arguing that the model could not say whether a national support base able to hold 165 concurrent patients exists. With capacity out of scope on model-boundary grounds, re-measuring it serves no remaining purpose in this issue; the underlying figures live in the policy sweep #313 delivered. The issue is recorded as complete on that basis rather than left open on a task whose premise has been withdrawn.

**Unblocked by this merge:** No new issues unblocked. #368 was raised from this work and is narrowed by it, the labelling half being resolved and the representation half remaining; #365 is unaffected.

---

### Issue 314 — The Role 4 Ward Composition, Calibrated (Partial)

**Advanced:** PR #366, branch `feature/issue-314-role4-ward-composition`

The category-to-ward mapping moves out of a hard-coded `case_when` in `assign_role4_los()` into `role4.wards`, alongside the ward levels the census and both plots report; those levels had been three separate literal vectors, so a ward added to the mapping would have appeared in neither plot. `role4.icu_continuation` then splits a stay between an intensive care phase and a step-down ward. The split is built on the conservation contract the model already applies forward rather than on a new fraction: `draw_stabilisation_icu()` draws one requirement and divides it between R2B and R2E so the total cannot depend on the route, and Role 4 extends that one echelon further, serving the post-operative requirement less whatever R2E already delivered, read from the `post_definitive_min` attribute. The step-down ward takes the remainder as a difference rather than a second draw, which makes conservation structural.

The mechanism ships enabled and calibrated. An earlier revision of the branch shipped it disabled on the grounds that no source supported a value, and that reasoning was reversed during review: disabling it did not avoid publishing an estimate, it published every Priority 1 surgical casualty in an intensive care bed for the whole of a 10 to 45 day stay, which has no source at all and is known to be wrong. Johnston, Henning and Harrison report a mean Level 3 intensive care stay of 7.6 days across 570 admissions of 527 UK service personnel repatriated to a Role 4 facility, which is the Role 4 portion of the episode directly; theatre serves a mean 1.08 days beforehand, so the shipped total is 1/4/21 days and the model reproduces the source figure after the deduction. Seed-42 composition moves from 75.6% intensive care to 25.4%.

Two defects were found by rendering the census rather than by the check written to hold it. The requirement was drawn for every casualty carrying a post-operative episode rather than only those admitted to intensive care, and the step-down was then applied whether or not an intensive care phase preceded it, so a casualty admitted to a general ward was renamed into a surgical ward and 333 bed-days moved between two wards on the strength of a phase that never happened. All 43 assertions passed throughout, because every one of them was blind to bed-days moving between wards while the total conserved exactly. The assertion that closes that gap now exists and earned its place immediately, failing on the first fix and exposing the second site.

**Seed-42 baseline (30 days, single run):** `logs/logs.txt` and every `data/` file reproduce byte for byte, the change being analysis-layer. `images/role4_census.png` is the only tracked artifact that moves. No numeric published figure changes: the Role 4 peak of 120.0 on day 30 and the decay below one around day 68 are properties of arrivals and stay length rather than of ward, and the requirement is drawn after the length-of-stay draws so those are unmoved. The Key Parameters table gains a ward composition row.

**Reference is deliberately non-closing.** Three tasks remain on #314, and the first is a gap the pull request's own description overstated: `role4_capacity_shortfall()` computes days above establishment, peak overshoot and unmet bed-days per replication and `scripts/check_role4_ward_phases.R` exercises it, but nothing in the analysis pipeline calls it, so a planner running the model never sees the comparison. The capacity line is drawn on the single-run census plot and not on the multi-run one. And the evidence table in the issue has not been re-measured at the current shipped defaults. #314 stays open.

**Unblocked by this merge:** No new issues unblocked. #365 was raised from this work and is tabled as open.

---

### Issue 313 — The Holding Establishment Sweep, and the Defaults Both Levers Now Rest On ✓

**Merged:** PR #363, branch `feature/issue-313-holding-establishment`

This closes #313 by completing the three tasks PR #360 left open. The R2E holding occupancy split is closed exactly rather than by residual: four attributes were added to `R/trajectories.R` recording when each holding episode begins, and `holding_occupancy_split()` was rewritten to reduce the four components independently rather than deriving the evacuation wait as what the others leave over. Each attribute is set after the `seize_selected()` it follows rather than before it, which is what keeps it stream-neutral, a seize being able to block so that a draw moved across it reorders against the arrival draws taken during the wait; the seed-42 evidence set reproduces byte for byte.

Measuring the corrected split withdrew a published claim. `docs/Multi_Run_Analysis.md` stated that only the standard evacuation route stages in holding beds, the critical route holding an intensive care bed instead. The critical route does stage in holding beds, and the section is retitled and re-measured accordingly: the evacuation share of holding occupancy moves from 10% to 22.2% at moderate intensity, 29% to 36.3% at high, and 39% to 61.6% at a 14-day sortie interval. Waits, sorties and the Role 4 census reproduce to the digit across the re-measurement, which is the consistency check that the change is confined to the split.

The holding establishment is then made sweepable, `set_hold_establishment()` varying the bed count the way the transport fleet size is varied, and swept at 30, 45, 60 and 90 beds at the shipped 21-day policy, 30 replications over 360 days per arm. The frontier is flat above 45: the closing-window holding queue falls from 1.68 at the shipped 30 beds to 0.01 at 45 and 0.00 beyond, occupancy from 57.4% to 44.4% and then nowhere, and no health outcome separates the four arms, post-definitive intensive care access reading 35.1% at every establishment. The establishment therefore cannot be traded against the policy at the shipped policy, because it has no effect there to trade away. Whether a large enough pool would make a longer policy stable is a property of the joint grid, which a one-dimensional sweep does not visit, and README Further Development L32 is rewritten from "never swept" to that narrowed gap, costed at twenty cells and identified as compute rather than method.

**The recorded decision on the defaults.** Neither shipped default moves, and both now rest on a sweep rather than on a single comparison. 21 days is the only value in the doctrinal 15 to 60 day range that is both admissible against the 7.6% to 42.1% historical in-theatre envelope and stable at sustained-campaign length; at that policy the 30-bed establishment is within 15 beds of everything a larger pool could offer, at the cost of a residual queue that forms in a minority of campaigns. The issue's acceptance criterion asking the README to document that the shipped combination is over-subscribed at length was written when the default was 30 days and its premise did not survive PR #319; the README documents the interaction and why a 30-day policy saturates the pool while a 21-day one does not, which is the truthful form of what the criterion asked for.

Two limitations are recorded rather than closed. The died-of-wounds effect stays unresolved across both sweeps, reported with the 135 to 221 replications a decision would need, which the issue's third acceptance criterion permits. And the issue's own third reason for not moving the default stands unaddressed by design: the 21-day policy moves the bottleneck to a national support base the model treats as unlimited demand, so the model can say the policy relieves the echelon it constrains and cannot say the policy is feasible. The issue scopes that out explicitly, and it is #314.

`scripts/check_policy_sweep_protocol.R` gained the establishment half: the axis reaches the resources it claims to at counts deliberately not multiples of the shipped 30, a configuration whose pool cannot be found fails rather than silently measuring the shipped establishment at every point, and both published tables are parsed out of the paper and compared against their tracked summaries. `scripts/check_holding_occupancy_split.R` was rewritten to assert closure across the 650-replication tracked population rather than in one campaign, which is the honest form of the property: closure held at seed 42 and a small minority of replications over-count by up to about 4%, raised as #362.

**Seed-42 baseline (30 days, single run):** unchanged. No `env_data.json` field moves and no distribution, probability or resource count is touched; `scripts/check_baseline_reproduction.R` reports the tracked evidence set reproducing byte for byte in `logs/logs.txt`, all seven `data/arrivals_*.txt` diagnostics and `data/mass_casualty_events.csv`. The one red check on the first local run was the lint ratchet, three indentation findings from the generalised table check, fixed rather than absorbed.

**Unblocked by this merge:** No new issues unblocked. #314 is not gated on this merge; it was already open and carries the national support base capacity question this issue scoped out.

---

### Issue 313 — The Evacuation Policy Sweep, and the Default It Justifies (Partial)

**Advanced:** PR #360, branch `feature/issue-313-evacuation-policy-sweep`

This issue asked for a sweep of `r2eheavy.recovery.evacuation_policy_days` before any default moved, and recorded as an acceptance criterion that no default be changed on the evidence of the single 21-day comparison it contained. PR #319 moved it from 30 to 21 on exactly that comparison. Two of the three gates the issue named were satisfied in the interim, #310 for a documented command and #312 for the horizon protocol, and two were not: the sweep, and the died-of-wounds effect at $p = 0.068$. The sweep is therefore delivered as the missing justification for a shipped default rather than as a precondition for a decision, which is the more urgent of the two framings.

`R/policy_sweep.R` and `scripts/run_policy_sweep.R` sweep the policy across 15, 21, 30, 45 and 60 days, the range the doctrinal source states is a command decision, at 30 replications over 360 days per arm under the shipped configuration at control seed 42. Each replication is reduced to one response row inside the forked worker that produced it, on `R/airlift.R`'s arrangement, and forward stability is measured over the campaign's closing 90 days rather than its whole length. The arms are paired on one control seed each and differences are taken within replication, which is what makes 30 replications sufficient: each arm's own spread across campaigns is large enough to hide the effects being measured. The runner checkpoints each arm as it completes and resumes from any complete arm file, so an interruption costs the arm in flight rather than all 150 replication-years, the protection `scripts/screen_cache.sh` gives the sensitivity screen.

The result vindicates the shipped value on stronger grounds than it shipped on, and the historical envelope does most of the work. The realised in-theatre share is an output of the policy, validated against the 7.6% to 42.1% range of historical campaigns: 15 days retains 4.7% and falls below it, 45 and 60 days retain 60.9% and 82.3% and sit far above it. Only 21 and 30 days produce a theatre resembling the campaigns the model is calibrated against, and of those only 21 is stable, 30 leaving both R2E pools at full occupancy through the closing quarter with a holding queue of 601, 301 casualties never evacuated and post-definitive intensive care access down from 35.1% to 5.5%.

Two of the issue's own premises did not survive the sweep. Returns to duty are not monotone in the policy: they peak at 30 days (2,276 per campaign-year) and fall at 45 and 60 (2,205 and 2,172), because past saturation a retained casualty waits rather than convalesces, so lengthening the threshold buys retention without buying recovery. The force cost of the shipped policy against the returns-maximising one is real at 116.4 fewer per campaign-year [69.2, 163.6], corroborating the issue's own 120.1 measured under different defaults, but it is the price of stability rather than a loss recoverable by lengthening. And the mortality effect that gated the decision is undetectable across the whole range, four of five paired differences being indistinguishable from zero with no monotone pattern; it is reported as unresolved with the 135 to 221 replications a decision would require, which the issue's third acceptance criterion permits.

One defect was found by asserting the new reduction against the analysis pipeline rather than by inspection: `post_definitive_pathway` is a simmer attribute and therefore numeric, so a comparison against the string `"icu"` reported an intensive care access share of zero at every policy. `scripts/check_policy_sweep_protocol.R` now holds that agreement, the reduction's returns to duty, died of wounds, disposition count and in-theatre share all matching the pipeline's at seed 42.

**Seed-42 baseline (30 days, single run):** unchanged. No model code, `env_data.json` field or tracked seed-42 artifact is touched; `R/policy_sweep.R` is a read-only reduction over monitoring data, and `scripts/check_baseline_reproduction.R` confirms the evidence set reproduces byte for byte.

**Reference is deliberately non-closing.** Three tasks remain on #313: the R2E holding establishment sweep, recorded as README Further Development L32 because establishment and policy are substitutes whose relative price is unmeasured; the missing duration attribute on the post-operative damage control hold arm, so the holding occupancy split closes exactly rather than by residual; and any further decision on the default. #313 stays open.

**Unblocked by this merge:** No new issues unblocked.

### Issue 312 — The Strategic Airlift Collapse Experiment Gains an Entry Point (Partial)

**Advanced:** PR #358, branch `feature/issue-312-airlift-collapse-runner`

`docs/Multi_Run_Supplement.md` recorded one experiment as not re-executable from a tracked command: the strategic airlift reliability sweep at a 360-day horizon, whose response is a collapse classification rather than a mean. Its two halves existed separately, `scripts/run_long_horizon.R` running a reducing 360-day replicated campaign without a sweep and `scripts/run_airlift_sweep.R` sweeping sortie cancellation at a 30-day horizon and reporting means. `scripts/run_airlift_collapse.R` is the two together plus the response, classifying a campaign as collapsed where its R2E holding queue over the closing 90 days averages twenty casualties or more, and reporting the share that collapse with a Clopper-Pearson interval, the zero-count arms needing an interval a normal approximation would give zero width. `scripts/check_airlift_collapse_protocol.R` gates it, and the supplement's fifth design limitation is deleted rather than narrowed.

Re-executing the experiment moved two of its six arms and withdrew one published finding. Three of thirty campaigns collapse at 15% sortie loss rather than the five previously published, and twenty-one rather than twelve at 25%. The previous text read 36.7% and 40.0% as the risk saturating above 20% loss; the measured 36.7% and 70.0% have intervals that barely overlap, and the median closing holding queue moves from below 2 to 88.7 across the same step, so collapse stops being the unlucky campaign's outcome and becomes the ordinary one. The two other findings stand in substance: losses to 10% cost nothing, the worst campaign reaching 16.0 as under perfect lift, and the transition is a cliff rather than a slope. The superseded arms read 0, 0, 0, 5, 11 and 12 collapsed of thirty with worst closing queues of 17.9, 17.9, 17.9, 161.0, 298.9 and 247.3.

Two claims the previous text made did not survive re-measurement and are corrected rather than carried. The separation the classifier rests on is narrower than stated: the highest clear campaign averages 19.5 and the lowest collapsed one 21.8, against the 17.9 and 84 previously claimed, so the counts hold for any threshold between 18 and 22 but a threshold of 40 would change the 15% arm, which is now said rather than dismissed as not a tuning choice. And the early-campaign predictor claim rested on a run nobody repeated; the classified pool's daily series is now tracked, which makes it recomputable and confirms it across the range rather than at one arm (p = 0.42, 0.43 and 0.54 at 15%, 20% and 25% loss, 0.36 pooling them). The check reclassifies that tracked series and reproduces all 180 published responses, so the paper's table is auditable without re-running 180 replication-years.

**Seed-42 baseline (30 days, single run):** unchanged. No model code, `env_data.json` or tracked seed-42 artifact is touched; `R/airlift.R`'s additions are read-only reductions over monitoring data, and `scripts/check_baseline_reproduction.R` confirms the evidence set reproduces byte for byte.

**Reference is deliberately non-closing.** This pull request discharges the airlift entry point that Issue #295's merge had narrowed rather than closed. Two tasks remain on #312: scoping whether a sensitivity re-screen at the sustained horizon is warranted, and the `WARM_UP_DAYS` classification at that horizon, which rests on the block means alone. #312 stays open.

**Unblocked by this merge:** No new issues unblocked. Issue #313 stays gated on #312, which remains open.

### Issue 295 — Replicated National Support Base Demand and Strategic Evacuation ✓

**Merged:** PR #356, branch `feature/issue-295-role4-replicated`

Every published finding about strategic aeromedical evacuation and the national support base came from one campaign, and the section carrying them was the only one of the multi-run paper whose figures had no confidence interval. `R/airlift.R` measures the response set across replications and sweeps the two settings the single-campaign evidence pointed at, reducing each replication to one row inside the forked worker that produced it so that thirteen configurations at fifty replications hold the responses rather than the monitoring data behind them. `scripts/run_airlift_sweep.R` is its entry point.

The measurement refines the single-campaign reading in three places rather than confirming it. Timing matters more than airframe-days: two sorties at a 10-day interval give a mean wait of 2.30 days [2.05, 2.55] and two at a 14-day interval give 5.51 [5.12, 5.90], the same count and more than double the wait, so a planner buys how early and how regularly aircraft come rather than how many fly. The interval is accordingly the stronger of the two levers, spanning 0.18 to 5.51 days where the whole cancellation range spans 0.81 to 4.17. And national support base demand peaks 1.36 days before the campaign ends [0.83, 1.89] rather than after it, the single campaign having seen the long tail decay rather than the peak arrive late.

The holding-bed coupling is measured and turns out to apply to one route only. A casualty awaiting the standard airlift pool holds an R2E holding bed for its whole wait; one awaiting the critical pool holds an intensive care bed already seized upstream. At a 40% cancellation rate the standard route accounts for 91.1 holding bed-days against the 340.4 the critical route would add if charged to the same pool, so the issue's premise that every waiting casualty holds a holding bed would have overstated the coupling more than fourfold. `scripts/check_holding_occupancy_split.R` asserts the component that is not free by construction, including a casualty-by-casualty recount taken independently of the function under test. README Further Development L17 now cites the measured split.

Two measurement defects were found and fixed rather than shipped. The Role 4 peak was not reproducible: that length of stay is drawn by the analysis rather than the simulation, so the same finished campaign gave a peak of 117, 119 or 121 from three different stream positions, and it is now drawn under the replication's own seed. And a sortie scheduled at exactly the horizon was counted as cancelled, its capacity bump never resolving, which reported cancellation rates of 10%, 17% and 33% at a configured probability of zero purely because 30 divides by 3, 5 and 10; such a sortie is censored rather than observed and is now dropped from both counts.

**Seed-42 baseline (30 days, single run):** unchanged. No model code, `env_data.json` or tracked seed-42 artifact is touched, and `scripts/check_baseline_reproduction.R` confirms the evidence set reproduces byte for byte in the pinned container. The reduction was validated against the Key Parameters table before anything was swept, reproducing 193 decisions, 193 boarded, none queued, four sorties scheduled and flown, a 1.0-day mean wait at a p90 of 4.0, and 17 ventilated holds at 24.9 hours.

**Unblocked by this merge:** No new issues unblocked. Issue #312's remaining airlift task is narrowed rather than discharged: this merge gives the reliability sweep a tracked command at a 30-day horizon, while the 360-day collapse experiment that issue names still has none.

### Issue 312 — The Sustained-Operations Horizon: Protocol, Measurement and Warm-up Re-derivation (Partial)

**Advanced:** PR #354, branch `feature/issue-312-long-horizon-protocol`

**Reference is deliberately non-closing.** Issue #312 asks for a long-horizon protocol and a list of things resting on it. This pull request discharges the protocol itself, its measurement, the duration pilot, the warm-up re-derivation, the per-response steady-state statement and the audit of horizon-dependent findings. Two tasks remain: the strategic airlift sweep entry point, which belongs to Issue #295 and is recorded there, and scoping whether a sensitivity re-screen at length is warranted. #312 stays open.

`R/long_horizon.R` runs the model over a 360-day horizon at 30 replications per casualty intensity and reduces each replication to a daily series inside the forked worker that produced it, discarding the monitoring data there. The parent process holds the series rather than the monitors, so peak memory is set by one replication rather than by their sum: a full protocol run holds at about 2.7 GB across the process tree. `R/queue_series.R` extracts the pool-queue step-function estimators added under Issue #301 into a base-R module that `R/analysis.R` and the new runner both source, on the `R/censoring.R` precedent, so the campaign figures and the block series measure the same quantity by the same estimator; the refactor is behaviour-preserving, both tracked Issue #301 figures re-rendering byte for byte.

The measurement splits by intensity, and it corrects this issue's own premise. At moderate intensity every response converges, most within two 30-day blocks, so the 30-day window every experiment in this project uses is a representative one and no published moderate-intensity finding is horizon-limited. At high intensity four responses do not: the R2E theatre queue runs 85.2 in block one to 1,725.9 in block twelve and is still growing at 11.3% per block over the second half, R2E holding beds 33.3 to 816.5, R2E intensive care 2.5 to 228.2, and the strategic evacuation backlog 24.1 to 548.7, with all three R2E pools at 1.00 occupancy from block two. Casualty arrivals are flat at 34.9 against 35.2, so this is sustained load rather than force consumption, which is the confound the issue named and which the shipped 7-day reinforcement cycle controls. The issue's own 360-day figures had `moderate_intensity` diverging; they predate the default changes of PR #319, as the issue flagged, and at the shipped defaults the divergence is a high-intensity phenomenon alone.

The warm-up classification is re-derived against that horizon. `WARM_UP_DAYS` remains 0 and its stated basis is unchanged, but the classification is now correct for the right reason at both intensities: at moderate a steady state exists and every converged response settles by block seven, most by block one or two; at high none exists for the R2E pools, so those four are reported as having no steady state rather than a warm-up period of zero, which is a different claim.

Two claims made in drafting were disproved by the new regression check rather than by inspection, and both corrections are in the same pull request. The reduction clamped events beyond the horizon into the final day, which made a day's count depend on what happened after it. And the protocol claimed that block one of a long run reproduces a standalone 30-day run at the same seed; it does not, the arrival stream being drawn over the horizon a run requests, so two runs of different length at one control seed diverge from their first arrival. `scripts/check_long_horizon_protocol.R` now asserts the weaker property that does hold, that each day's reduction uses that day's events alone, together with the falsity of the stronger one so it cannot be reinstated.

**Seed-42 baseline (30 days, single run):** unchanged. No model code, `env_data.json` or tracked seed-42 artifact is touched, and `scripts/check_baseline_reproduction.R` confirms the evidence set reproduces byte for byte in the pinned container.

**Unblocked by this merge:** No new issues unblocked. Issue #295 gains the long-horizon runner its sweep entry point can be built on, and is noted accordingly.

### Issue 301 — Campaign Time Series for Queue Length and Degraded Care ✓

**Merged:** PR #352, branch `feature/issue-301-time-series-figures`

Every figure the analysis pipeline produced collapsed a campaign to a scalar, which cannot tell a queue that recurs in peaks and clears between them from one that never clears. Those are different resourcing problems, the first answered by surge capability that can be brought to bear on the heavy day and stood down afterwards, the second only by permanent establishment, and the paper's first planning option assumed the second without establishing it. Queue length by resource pool and the rate of degraded post-operative care are now reported as time series across 50 replications per casualty intensity, as the median with an interquartile band rather than a mean with a confidence interval, the question being what a campaign looks like rather than how precisely its average is known.

A pool's total queue is not recorded by the simulation and cannot be read off any single monitor row, each bed being monitored separately, so it is recovered by differencing each bed's own series into changes and accumulating those changes in time order, which is exact rather than interpolated. The binned value is the time-weighted mean of that step function computed from its cumulative integral, so a peak falling entirely inside a bin is carried rather than missed as an edge sample would miss it, and the two clearance statistics the paper quotes are computed from the unbinned series so neither depends on the bin width. `scripts/render_time_series_figures.R` separates measurement from rendering: `--run` executes the model and writes three aggregated series to `data/time_series/`, and an invocation without it renders from those files and nothing else, so both figures are a function of tracked data and re-rendering reproduces them byte for byte.

The measurement answers the question the issue was raised to ask, and the two intensities differ in kind rather than only in scale. At high intensity the R2E theatre queue stands empty for 5% of the campaign, runs unbroken for 26.9 of the 30 days, and grows through the whole campaign to a median of 160.5 casualties on day 29 without settling. At moderate intensity the same pool stands empty for 80% of the campaign, its median is zero throughout, and its longest unbroken busy run is 3.0 days. Option 1 is therefore a standing establishment measure at high intensity and a surge measure at moderate. Two further results came out of the same measurement: the R2E holding queue oscillates at high intensity on a period of about seven days matching the strategic sortie interval, so that queue is set by when the aircraft come rather than by the bed count, and the degraded care rate is a standing condition rather than an episodic one, the median high-intensity campaign delivering no post-definitive care in an intensive care bed at all from day 19 onward.

`scripts/check_time_series_figures.R` defends the arrangement in 22 assertions. Three of its four groups compare derived artifacts against one another and would pass on two copies of one error, so the fourth exercises the step-function estimators on inputs whose answers are computable by hand; it caught a wrong expectation in its own first draft. Fault injection confirmed it fails on an edge-sampling bin mean, on a clearance statistic that sums spells rather than taking the longest, and on a clearance percentage the paper states that the data does not support.

**Seed-42 baseline (30 days, single run):** unchanged. No model code, `env_data.json` or tracked seed-42 artifact is touched, and `scripts/check_baseline_reproduction.R` confirms the evidence set reproduces byte for byte in the pinned container.

**Unblocked by this merge:** No new issues unblocked.

### Issue 296 — The Post-Operative Intensive Care Gate, Re-Measured Under Replication ✓

**Merged:** PR #350, branch `feature/issue-296-icu-gate-independent-reps`

The pre-theatre intensive care gate was published on the strength of a before-and-after pair of single runs, a design that cannot separate the gate's effect from the seed's. It is now measured at 50 replications per arm, paired under common random numbers so that each replication's two arms share an arrival stream and the difference is estimated within replication rather than between runs. The two halves of the published finding part company under that design. The load finding holds and strengthens: the R2E intensive care pool runs at 91.4% utilisation (95% CI [90.1%, 92.8%]) with the gate disabled and 84.6% ([83.2%, 86.0%]) with it in force, a paired difference of 6.80 percentage points ([5.13, 8.47]) that does not span zero. The mortality finding does not survive: died-of-wounds counts are 1.30 per run ([0.97, 1.63]) disabled against 1.24 ([0.96, 1.52]) enabled, a paired difference of 0.060 ([-0.356, +0.476]) whose interval spans zero and whose point estimate carries the opposite sign to the one the gate's design predicts. At the observed paired standard deviation of 1.46 deaths, resolving that difference at the current half-width would take roughly 2,283 replications, so `docs/Multi_Run_Analysis.md` now labels the gate *load measured, mortality unresolved* rather than *direction only*. The previously published figures (a 74.1% to 60.2% stabilisation intensive care share, 0.84 to 1.00 died of wounds) are superseded rather than reproduced; they predate the default changes PR #319 shipped and no longer describe the model.

Making the comparison possible required giving the gate an off switch. `icu_gating.enabled` is added to the `r2b` and `r2eheavy` blocks of `env_data.json`, shipping at 1, and read through a new `icu_gate_enabled()` accessor in `R/trajectories.R` that validates the field and fails naming it. With the switch off, `r2b_surgery_gate()` and `r2e_surgical_branch()` return their pre-gate arms, so the model as it stood before the gate existed is reachable from configuration rather than only from version history. `scripts/check_icu_gate_switch.R` asserts that reproduction in fourteen assertions: that no casualty is deferred and none takes the degraded holding-bed recovery with the gate disabled, that both pathways are reachable with it in force so the first assertion is not vacuous, that the shipped value is the enabled one, and that a malformed value is rejected with a message naming the field. The R2B half is asserted at a forward intensive care share of 0.8, the shipped share of zero leaving that gate close to inert; fault injection confirmed the check fails on each of three independently broken versions.

**Seed-42 baseline (30 days, single run):** unchanged. `icu_gating.enabled` ships at the value that reproduces the previous behaviour, and `scripts/check_baseline_reproduction.R` confirms the tracked evidence set reproduces byte for byte.

**Unblocked by this merge:** No new issues unblocked.

### Issue 312 — The Long-Horizon Protocol: Blocking Task Discharged (Partial)

**Advanced:** PR #347, branch `feature/issue-312-long-horizon-runner`

**Reference is deliberately non-closing.** Issue #312 asks for a long-horizon protocol: a replicated runner that reduces as it goes, the protocol documented in `docs/Multi_Run_Supplement.md`, a re-derived warm-up classification, a per-response steady-state statement, an audit of horizon-dependent findings and a regression check on the protocol's parameters. This pull request discharges the task that issue names as gating all of them, and nothing else on it. #312 stays open.

Building the casualty trajectory retained roughly 175 MB that neither releasing it nor calling garbage collection reclaimed, which is why a replicated long-horizon run exhausted memory before completing. The cause is now diagnosed, and it is not where the issue expected. It is a per-activity cost in simmer rather than in this project's code: a minimal trajectory of nothing but timeouts, built and released with no project code loaded, retains a constant 6.0 R cons cells per activity, linear from 250 activities to 8,000. The casualty trajectory carries 12,612 activities at 58.4 cells each, the higher rate because its closures capture builder frames rather than a trivial one, and that product is the 68 MB one build costs. The issue's earlier reading that a bare simmer loop plateaus came from a trajectory small enough for the same effect to look like noise, and its hypothesis that the global `env` binding was the retainer is ruled out: dropping it releases about 10% and dropping `env_data` another 11%, leaving 3.9M cells live with no trajectory referenced.

The remedy is the process isolation the issue anticipated, now with a reason rather than as a workaround. A fork that runs one replication and exits returns its retention to the operating system; a fork that runs a batch accumulates it once per replication. `mclapply`'s default pre-divides jobs into one batch per core, so peak memory grew with the replication count, and `dispatch_replications()` now asks for one fork per job unconditionally, which the capped Shiny path already did for an unrelated blast-radius reason.

**Seed-42 baseline (30 days, single run):** No published value moves. The dispatch arrangement decides which process a replication runs in, not what it computes, and the tracked evidence set reproduces byte for byte. The figures below are peak resident memory across the process tree, at 30 days on two cores.

| Replications | Batched | One fork per job |
|---|---|---|
| 4 | 1,340 MB | 884 MB |
| 8 | 2,084 MB | 796 MB |

Growing against flat. Extrapolating the batched arrangement to the shipped 50-replication comparative analysis is where the memory exhaustion had been coming from.

`scripts/check_replication_memory.R` asserts both the structural arrangement and the behavioural consequence, the latter against a worker retaining a known amount in a process global so that it measures the dispatch rather than the model and runs in seconds. Validated by fault injection: restoring the batched dispatch fails three of its six assertions, peak rising from 533 MB at four jobs to 1,327 MB at sixteen against a flat 427 and 483 on the fix. Two defects in the check were found and fixed while writing it, both of which would have made it pass vacuously: its sampler runs in a forked child, so reading the process id there measured the sampler's own empty tree, and it originally passed a core cap, which selects the path that already forked per job and so never exercised the broken arrangement.

`docs/Multi_Run_Supplement.md` loses a claim this change falsifies. It argued the replication-loss bias is small partly because losses arrive in batches mixing heavy campaigns with light ones; losses are now exactly one replication, so that argument is gone and the case rests on the fixed cost alone, which the paragraph now states rather than dropping silently.

**Unblocked by this merge:** No issue becomes ready, none having declared a dependency on this task. Issue #295's airlift sweep is materially easier, a 30-replication 360-day arm now being executable through the ordinary framework rather than needing a driver script, and that is recorded on it. Issue #348 was raised from the audit that followed, carrying the two Morris coverage gaps and the cache trap out of #312's comments so their sequencing against #335 and #339 is visible. Issue #313 stays blocked, gated on #312, which remains open.

### Issue 337 — Harden the CI System-Library Install Against Archive Stalls ✓

**Merged:** PR #345, branch `feature/issue-337-ci-apt-hardening`

The workflow's `Install system libraries` step ran `apt-get` inside the pinned container with no retry configuration, no transport timeout and no per-step timeout, so a stalled Ubuntu archive mirror hung it until the job timeout fired, 60 minutes for three of these jobs and 180 for the slow suite, and reported a failure naming nothing useful. One run was observed wedged past 25 minutes and had to be cancelled and re-dispatched by hand, against a 7m39s worst case that did complete. A transient mirror fault therefore presented as a red gate on a sound pull request, which under this project's own rule blocks review of unrelated work.

Each of the four copies now writes an `apt.conf.d` fragment before `apt-get update`, setting `Acquire::Retries` to 5 and both transport timeouts to 30 seconds, and carries `timeout-minutes: 15`. Writing the fragment before the update rather than passing options to the install is what makes the guards cover both commands. Nothing about what is installed changes. The options were verified against the Debian manpages rather than assumed, and the fragment was written locally and read back with `apt-config dump`, which returns all three.

The guards are applied inline in all four copies rather than factored into a composite action, because `timeout-minutes` is not among the step properties a composite action supports, so the per-step timeout could not live inside one. A call-site timeout over a composite step was not relied on either, an unenforced timeout being worse than none because it looks like protection. That leaves four copies which must not drift, and drift here is silent: three jobs hardened and one missed looks exactly like four hardened until the unhardened one wedges.

**Seed-42 baseline (30 days, single run):** No simulation output is in scope. The merge changes the workflow, adds one check script and documents the failure mode; it touches no R module, no configuration the model reads and no tracked artifact.

`scripts/check_ci_apt_guards.R` is what makes the duplication safe, asserting that the four steps exist, that each carries a per-step timeout, that each sets the retries and both transport timeouts before `apt-get update`, and that the four bodies stay byte-identical, dropping only the `if:` condition `browser-checks` legitimately carries. Validated by fault injection: removing one job's timeout, removing one job's retry setting, and moving the guards to after the update each fail two assertions, the specific rule and the drift rule, the first being the three-of-four case exactly. The hardened step was confirmed in CI on the pull request itself, completing successfully in all three container jobs, which is the one acceptance criterion that could only be tested there. `docs/Continuous_Integration.md` gains a section naming the failure mode and why a second timeout on one pull request is a workflow issue rather than a third re-run.

**Unblocked by this merge:** No new issues unblocked.

### Issue 342 — Nothing Asserts That the Two Structure Tables List Every Module and Script They Claim To ✓

**Merged:** PR #343, branch `feature/issue-342-structure-table-completeness`

Two tables claim to be complete indexes of the codebase. `CLAUDE.md`'s Repository Structure table says it covers the repository as a whole, and the README's Codebase Structure table is what that table points a reader at for detail on each `R/` module. Nothing checked either claim and both had drifted. `R/cli.R` had no row in the README table from the day it was created, while all eleven other modules had one, and the file is named in the README's own prose, so a reader meeting it there and turning to the table concluded it did not exist. That row is added here.

It was the second instance of the same failure in as many merges: PR #340 added `R/censoring.R` and `scripts/check_censored_interval_estimation.R` and gave neither a row, corrected by hand in PR #341. A convention enforced only by a reviewer noticing keeps failing that way, and it fails silently, a missing row breaking nothing that runs.

`scripts/check_structure_tables.R` asserts the five rules the tables promise: every `R/` module has a row in both tables, every `scripts/check_*.R` and every other `scripts/` entry point has a row in `CLAUDE.md`'s, and neither table names a path that no longer exists, so a rename cannot leave a row pointing at nothing. The file set comes from `git ls-files` rather than a filesystem glob, so an untracked scratch file cannot fail it, and every backticked path in a row's first cell is collected rather than one path per row being assumed, `CLAUDE.md` grouping the Playwright toolchain and the `renv` files into single rows.

**Seed-42 baseline (30 days, single run):** No simulation output is in scope. The merge adds one check script and one table row to each of two documents, touching no R module, no configuration and no tracked artifact.

The check is non-vacuous on the day it was written: against `main` at `1b51abf` it failed on rule 1 naming `R/cli.R` and passed the other four. It also covers its own registration, rule 3 counting 31 check scripts before the new one was staged and 32 after, which is why `scripts/check_structure_tables.R` has a `CLAUDE.md` row of its own. Validated by fault injection: deleting the check's own row fails rule 3 naming it, renaming `R/warmup.R` in the table fails twice over on coverage and on staleness, and deleting the `R/censoring.R` row from the README table fails rule 1. The fast suite passes 31 of 31, and the roxygen ratchet holds at its baseline, two file-scope constants having been given the R3 headers they needed rather than the baseline being refreshed.

The rule set is deliberately narrow and is stated in the check's banner: it does not require a row for everything under `docs/`, `tests/`, `data/` or the repository root, those being listed today but a rule forcing it generating churn over files nobody looks up. The check verifies presence rather than accuracy, so a row whose description has gone stale still passes.

**Unblocked by this merge:** No new issues unblocked.

### Issue 331 — The Dwell-Time KPIs Drop Every Casualty Still in the Echelon When the Run Ends ✓

**Merged:** PR #340, branch `feature/issue-331-dwell-censoring`

`compute_treatment_interval_kpis()` differenced a start attribute against an end attribute and kept the casualties carrying both, so a casualty still at the echelon when the run ended was dropped. That is right-censoring, and the dropped are not a random subset: they are the ones still present, which is both the late arrivals and the long stayers. At seed 42 over thirty days it dropped 21.6% of R2B stays and 18.0% of R2E stays. The magnitude is worse than those shares suggest: the dropped R2B stays had already run a mean of 15.9 days against a completed-case mean of 0.71 days, so the excluded were already twenty-two times longer than the included before counting the time they still had to serve, and of the forty-six censored R2E stays none carried the evacuation flag, all being casualties serving convalescence of whom only sixteen of sixty-two were counted at all.

The three intervals whose cohort at risk is known are now estimated by the Kaplan-Meier product-limit estimator over everyone who entered them, reported as a restricted mean with the censored count and share beside it. Time to first surgery is excluded because its cohort is not identifiable, a casualty with no surgery time being either one still waiting or one who never needed an operation, and it reports the completed-case mean with a count that bounds the censoring from above instead. The estimators live in a new `R/censoring.R` sourced by both `R/analysis.R` and `R/sensitivity.R`, so a KPI and the screened response of the same name cannot drift apart.

Two findings came out of the work that the issue had not anticipated, and both shaped the fix. The censored share is configuration-dependent rather than a constant offset, reaching 55.5% of R2E stays under `high_intensity`, so the figure was least trustworthy in exactly the congested regime it is consulted for, and a screening design had points differing in how much of the cohort the old response could see at all. And the obvious restriction horizon is wrong: restricting to the thirty-day window would put 63% of the reported R2B dwell beyond the last observed completion at day nine, where the curve plateaus at 0.202 and never moves again, which is the flat-tail convention rather than a measurement. The horizon is seven days, inside the observed support of both dwells and the planning cycle the reinforcement demand interval and the strategic sortie interval already run on, and a `tail_share` column reports how much of each mean rests on the tail.

**Seed-42 baseline (30 days, single run):** No tracked artifact moves and the evidence set reproduces byte for byte, the dwell KPIs being written to no tracked file. The figures below are of series the pipeline previously mis-measured.

| KPI | Before | After | Censored |
|---|---|---|---|
| R2B dwell, mean | 1,022 min | 2,920 min | 52 of 241 (21.6%) |
| R2B dwell, p90 | 3,880 min | not locatable | |
| R2E dwell, mean | 3,505 min | 3,788 min | 46 of 255 (18.0%) |
| R2E dwell, p90 | 5,442 min | 19,770 min | |
| R2B to R2E transit, mean | 29.08788 min | 29.08788 min | 0 of 189 |

The transit mean is unchanged to five decimal places, which is the estimator degenerating to the arithmetic mean on real data as it does analytically. The R2B p90 becomes unavailable because the curve falls only to 0.202: nine in ten stays cannot be bounded from a thirty-day run, and 3,880 was the p90 of the stays that happened to close, a different and shorter-staying population.

`scripts/check_censored_interval_estimation.R` asserts the degeneracy, recovery of a known restricted mean from injected censoring, cohort coverage, the disclosure, the zero tail share and the unreached quantile. Validated by fault injection: restoring the completed-case mean fails seven of its twenty assertions and breaking the product-limit risk set fails nine, in different groups. Both ratchets hold at their baselines with no refresh, three `<<-` findings and one long line having been fixed rather than absorbed.

The tracked Morris rankings for both dwell responses rest on the superseded definition and cannot be re-derived, the cache holding the thirty-six scalar responses per design point rather than the monitoring data behind them. They are marked as such in `data/sensitivity/README.md` and the re-screen is raised as Issue 339.

**Unblocked by this merge:** No new issues unblocked. Issue #339 was raised from this work; it is ready to start but was not gated on anything before it existed.

### Issue 333 — Resolve the Two R2B Holding Thresholds, One Name for Two Levers ✓

**Merged:** PR #334, branch `feature/issue-333-r2b-threshold-collision`

The R2B holding pathway has two thresholds, and their names had converged onto one identifier. `hold_threshold` is an occupancy fraction read at Role 1, before transport: a casualty is sent to an R2B unit only while that unit's holding beds sit below it. `evac_threshold` is a duration read at R2B: how long a casualty may occupy a holding bed before moving to R2E to serve the remainder of the same convalescence there. Different units, different echelons, different questions. The identifiers did not say so, the Morris parameter named `r2b_hold_threshold` setting the first while a function named `r2b_hold_threshold()` read the second, so a reader tracing the published ranking name into the source landed on a function about the other lever. Every read and write was checked and none was wired to the wrong field, so nothing was computed wrongly; what the collision cost was the next change, which could have crossed them silently. The function is now `r2b_evac_threshold()`, named for the field it reads, and its header states what the other threshold is and which one the Morris name refers to. The Morris name itself does not move, appearing as it does in the tracked `morris_r20` rankings and the README's ranking table, where renaming it would invalidate an evidence set for no modelling gain.

`evac_threshold` was absent from `env_data.json` entirely, so the only way to set it was to edit the parsed tree in R. That is not a dormant feature: the analysis documents name an evacuation threshold as Option 2's cheapest remedy for the R2B holding shortfall and list a joint sweep of it against holding capacity as the most tractable item of further analysis in the paper, and neither could be run through any tracked entry point. It now has a field in `env_data.json` shipping at zero, and a Shiny registry entry beside the reroute threshold a planner setting it needs to see. Zero is the disabled value, following the convention `force_regeneration.reinforcement.demand_interval_days` already sets; it cannot mean a zero-minute forward stay, a casualty held forward for no time being one who was never held, which `hold_threshold` decides upstream.

**Seed-42 baseline (30 days, single run):** No published value moves. The shipped configuration leaves the branch disabled exactly as its absence did, and the tracked evidence set reproduces byte for byte. Setting the new field alone now moves 46 of 65 holding casualties onto the early-evacuation branch, against none at the shipped setting, which is the reachability the merge establishes.

`scripts/check_lever_realisation.R` gains two assertions and a correction. It had asserted that no casualty is evacuated under "a threshold that is not configured", which is no longer the state being tested; it now asserts that the shipped value is the disabled one and that the branch stays unreachable at it. A new structural assertion reads which holding field each accessor actually mentions, so the collision cannot return by either accessor being pointed at the other's field. Validated by fault injection: cross-wiring `r2b_evac_threshold()` to `hold_threshold` fails three assertions, one of them naming the two fields read.

**Unblocked by this merge:** Issue #335, the joint sweep of R2B holding capacity against the evacuation threshold, which was gated on this issue giving the threshold a configuration entry.

### Issue 327 — The R2B Hold Occupancy Figure Omits Every Episode Still Running When the Run Ends ✓

**Merged:** PR #332, branch `feature/issue-327-hold-episode-censoring`

`summarise_r2b_hold_occupancy()` reconstructed every holding episode as `r2b_hold_start` to `return_day`. A casualty leaves an R2B holding bed by one of three routes and only one of them sets `return_day`, so the reconstruction mis-measured the other two, in opposite directions. A casualty still holding a bed when the run ends sets neither attribute and was dropped, which is the defect the issue names. A casualty sent on under the evacuation threshold releases the bed at once and sets `r2b_hold_served`, while `return_day` is set later and at another echelon, so charging the bed to it held them at R2B for the whole of a stay they spent at R2E. Both are fixed by one shared `r2b_hold_episodes()`, which also removes a duplication between the single-run and replication paths whose two copies had already drifted in their guard.

The second error was the larger and was not known when the issue was raised. Under the 4320-minute threshold the analysis documents recommend as a planning lever, the old reconstruction reported 303.2 holding bed-days against a true 172.9, overstating by 76%: a planner testing that lever was shown it barely working when it removes about a third of the load. At the shipped configuration the omission runs the other way, understating by 13%.

**Seed-42 baseline (30 days, single run):** No published value moves. The figures below are of a series the figure previously mis-measured, validated against the resource monitor, which counts bed occupancy directly and owes nothing to the attributes.

| Configuration | Old reconstruction | Fixed | Resource monitor |
|---|---|---|---|
| Shipped, no threshold | 196.6 bed-days | 225.4 | 225.8 |
| `evac_threshold` 4320 min | 303.2 bed-days | 169.8 | 172.9 |

Two further defects in the same figure were fixed with it. The y axis read "Mean Concurrent Patients in Hold" and does not measure that, the series counting a casualty on every day their stay touches, so it now says what it counts; whether the series should instead be time-weighted is left open. And the capacity line was drawn at one unit's five beds while the bars carried the load of both, so a bar could sit above the line with neither unit over its own establishment; each unit now has its own panel and its own line, read from the establishment rather than from a literal in the source. `docs/Single_Run_Analysis.md` loses a claim that was a pure artifact of the censoring, the wounded in action stream having appeared to clear by the closing days.

`scripts/check_hold_episode_reconstruction.R` asserts each exit route bounds its own episode and cross-checks the bed-days against the monitor at both configurations, which is what would catch a fourth exit route added later. Validated by fault injection: six of its nine assertions fail against the previous reconstruction. `scripts/check_absent_attribute_columns.R` had asserted that stripping `return_day` leaves the holding summary empty, which was a consequence of this defect rather than a property worth keeping, and is rewritten.

**Unblocked by this merge:** No new issues unblocked. Issues #331, #333 and #335 were raised from this work and the review that followed it.

### Issue 315 — The R2E Bed Queue Figure Omits the Holding Beds ✓

**Merged:** PR #329, branch `feature/issue-315-r2e-bed-queue-holding`

`plot_r2e_bed_queues()` named the bed types it plotted, `"ot"` and `"icu"`, so the holding pool was excluded by construction and no amount of data could put it in the figure. `plot_r2b_bed_queues()` already selected on a wildcard and derived the type from the resource name, so one pipeline plotted the two echelons to different rules with nothing stating why. R2E now follows R2B's rule, and all four pools appear, one panel per bed type, each panel labelled with the size of its pool.

The panels share one vertical scale, which is not what the issue recommended. Its reasoning, that a shared axis across thirty holding beds and two theatres would flatten the smaller series, does not survive measurement: the shallow pools never exceed one casualty and the theatre queue reaches fourteen, so on a shared axis they sit about a fourteenth of the way up the panel, legible as a distinct band. A free scale would instead draw every pool filling its own panel whatever it queued, so a pool never exceeding one casualty would appear exactly as prominent as one reaching fourteen. That is the same harm as the omission the issue exists to fix, reached by a different route. The figure was rendered both ways and the repository owner settled it on inspection.

**Seed-42 baseline (30 days, single run):** No published value moves; the figures below are new measurements of a series that was previously not plotted at all, reported as a time-weighted mean queue summed across each pool's beds.

| R2E pool | Beds | Pool mean queue | Max on one bed | Beds that ever queued |
|---|---|---|---|---|
| Holding | 30 | 1.73 | 1 | 19 |
| Theatres | 2 | 2.29 | 14 | 2 |
| Intensive care | 4 | 0.464 | 1 | 3 |
| Resuscitation | 4 | 0 | 0 | 0 |

The omitted series was therefore comparable to the theatre queue and roughly 3.7 times the intensive care queue that was plotted, though shallow across many beds rather than deep on few. The measurement method reproduces the paper's published per-theatre queue shares exactly, 27.2% and 20.2% of the run, which is what validates it.

`scripts/check_bed_queue_coverage.R` asserts the selection rule rather than today's establishment, injecting a bed type no configuration ships and requiring it to appear in each echelon's figure without a code change. It was validated by fault injection, the previous named-type selection failing two of its assertions and naming the omitted pools. Two further defects surfaced during the work and are fixed in the same merge: the R2B wildcard covers a fourth bed type, intensive care, which a two-day run does not exercise and which nothing had asserted; and a discrete colour scale stops assigning colours past its palette's eighth and drops the surplus levels silently, so a ninth bed type would have reintroduced this issue's own omission one type further along, in the figure just repaired for it.

Only the tracked image moves. `logs/logs.txt` and every `data/` file are byte identical, which is what a plotting change requires, and `scripts/check_baseline_reproduction.R` passes.

**Unblocked by this merge:** No new issues unblocked. Nothing in Phase 7 was gated on #315.

### Issue 316 — A Run Producing No Return to Duty Crashes the Analysis Pipeline ✓

**Merged:** PR #326, branch `feature/issue-316-absent-attribute-columns`

The attributes monitor records what casualties did, so the wide pivot of it carried a column only where at least one casualty set that attribute, and the shape of the frame therefore depended on what the run happened to produce. Analysis code reading a column directly failed with an object-not-found error on any run that produced none of it: a one-day campaign ran to completion, wrote its KPI summary, printed "Simulation complete" and then died in `summarise_r2b_hold_occupancy()` on `object 'return_day' not found`. The fix is a guarantee rather than a guard at each of the roughly 150 call sites. `MODEL_ATTRIBUTE_KEYS` in `R/constants.R` lists all 77 keys the model can set and `build_attributes_wide()` adds each as an all-NA column where the run set none, so the frame has one shape whatever the run did. `summarise_r2b_hold_occupancy()` also gains a real guard, returning its existing empty result where no hold episode has both a start and an end rather than a series of zeroes, which would read as nobody having occupied a bed.

The hand-written list the guarantee replaces had already proved incomplete, and the merge records that rather than only replacing it: `R/sensitivity.R` carried a second list of fourteen further keys, added for the same reason at a different call site, and it is deleted here. All 36 sensitivity responses are identical to `main`'s on a one-day run, which is the check that the deletion is behaviour-preserving rather than merely untested.

The issue's own boundary table no longer described `main` by the time the work started. It recorded one-day and two-day runs failing and three-day runs completing; at the sustained-campaign defaults of PR #319 only the one-day run fails. That is the issue's own point, duration being a proxy for the condition rather than the condition itself, so the regression check strips the attribute from a real run's monitoring data instead of relying on a run being short enough. `scripts/check_absent_attribute_columns.R` asserts the key list against the `set_attribute()` calls in `R/trajectories.R`, the two dynamic ones included, and was validated by fault injection: four of its ten assertions fail against the previous key list and one against the previous guard.

The shortest supported run is settled as the one day the entry point already accepted, and README's Running the simulation section says so. The merge also registers `scripts/check_replication_loss_reporting.R`, which PR #322 added without an entry in `CLAUDE.md`'s structure table, `scripts/README.md` or `scripts/check_runtimes.csv`.

No simulation output changed: the seed-42 evidence set reproduces byte for byte and a 30-day run matches `main` output for output across the whole of `outputs/` but for the row order of arrivals still in theatre in `mon_arrivals.csv`, which differs between two runs of identical code and is the pre-existing nondeterminism recorded under Issue #270. No baseline refresh was required. The fast suite passes 27 of 27, the roxygen ratchet holds at 171 and the lint baseline falls by one with the removed loop.

Issue #327 was raised from this work and is not addressed by it: the same function reconstructs each hold episode from its start and its end, so a casualty still holding a bed when the run ends is dropped entirely, 9 of 50 episodes at seed 42 over 30 days, which reaches `images/r2b_hold_occupancy.png` and the single-run paper's reading of it.

**Unblocked by this merge:** No new issues unblocked. Nothing in Phase 6 or Phase 7 was gated on #316.

### Issue 311 — Two Inferences in the Single-Run Paper Do Not Survive a Longer Horizon ✓

**Merged:** PR #324, branch `feature/issue-311-rescope-single-run-inferences`

`docs/Single_Run_Analysis.md` inferred from one 30-day run that intensive care saturation is a standing condition of the four-bed establishment rather than something the campaign works its way into. That is a claim about how the system behaves over time, and a 30-day window cannot separate a standing shortfall from the opening phase of a slow accumulation. The observation the run does support, that degraded care within the window tracks the day's surgical load rather than an accumulating backlog, is retained; the inference beyond it is replaced by a statement of what the horizon leaves open, cross-referencing the replicated evidence on the rationing rule. The Scope of a Single Run section gains the general form of the same caveat, and the evacuation policy table gains a sentence naming the dimension it does not report, the R2E holding pool from which a longer policy's theatre bed-days are drawn.

Half the issue was resolved before work on it began, and the merge records that rather than leaving a reader hunting for text that no longer exists. The issue's second claim, that the planner's lever at the strategic evacuation decision is schedule resilience rather than cabin fit, was removed under PR #319 when the shipped sortie cancellation probability changed to zero and the recommendation resting on varying cancellation draws went with it; the "busy but not saturated" heading claim went at the same time and now describes the observed occupancy without the adequacy reading. Both were verified absent rather than assumed absent, and the finding is recorded as a comment on the issue.

One task is left undone deliberately. The issue asks for cross-references to the long-horizon evidence, and no long-horizon measurement is yet reproducible by a documented command, so the rescoped text states what the horizon leaves open rather than citing a figure it cannot point at. Citing the sandbox measurement that motivated the issue would put a number in the project's published output that no tracked command reproduces, which is the failure the rescoping exists to prevent.

The merge touches one document and no R source, configuration or tracked artifact, so no figure and no published value moves. The fast suite passed 26 of 26.

**Unblocked by this merge:** No new issues unblocked. Nothing in Phase 7 was gated on #311.

### Issue 320 — A Replication Lost to Memory Pressure Is Dropped Silently from the Published Count ✓

**Merged:** PR #322, branch `feature/issue-320-realised-replication-count`

`run_replications()` tolerated a replication whose worker process was killed, continuing on the survivors, and published the count it had asked for while computing the metrics from the survivors. A run that lost workers therefore reported a replication count it never achieved, and every interval beside it was read against that number. The framework now returns `n_replications`, the count that contributed, alongside `n_requested`; `run_scenario()` returns both, since the requested count describes the experiment and the realised count belongs anywhere a result is labelled with its sample size; and `run_welch_analysis()` passes the realised count to `plot_welch()`, whose subtitle could otherwise claim ten replications when eight completed.

The larger change is that a run losing any replication now stops. `MAX_REPLICATION_LOSS` ships at zero. An earlier revision of the pull request set it at a tenth, on the reasoning that losing a few of fifty costs precision alone, the survivors remaining independent draws; the repository owner challenged that in review and it does not hold. It assumes the replications that die are a random subset of those dispatched, which is not established: a worker is killed because the host exhausted its memory, the operating system takes the largest process, and a replication generating more casualties carries more monitoring data, so the losses skew toward the heavier campaigns and the survivors are biased low on queue depth, occupancy and mortality, the responses the papers report. Two considerations bound the bias and are recorded rather than dropped, the response-dependent share of a replication's footprint being small against a fixed per-call cost and prescheduled losses arriving as batches that mix heavy campaigns with light, but neither makes it zero and it has not been measured. Prescheduling also empties the band the tolerance created: `mclapply` pre-divides replications among workers, so a killed fork loses roughly a quarter of a 50-replication run rather than one replication, which is what the 12-of-50 loss observed during Issue #312's work was. The threshold remains a parameter a caller may raise deliberately at the call site for a run whose cost justifies it.

The issue's fifth task, recording the realised count in the outputs the papers are built from, needed no code and none was written: `replication_summary.csv`, `scenario_comparison_totals.csv` and `scripts/check_dow_calibration.R` already derive their counts from the data through `n()`, `length(x)` and `length(pooled)`. `scripts/check_replication_loss_reporting.R` asserts the accounting in thirteen places, injecting failure by handing the dropper the NULL entries `mclapply` returns for a killed worker rather than by exhausting memory, and was validated by fault injection rather than by passing: removing the realised count from the return value fails three assertions, restoring the requested count to the Welch call fails two, and restoring the tenth-part threshold fails one. `docs/Multi_Run_Supplement.md` gains a section stating how a lost replication is handled and why the threshold is zero.

No simulation output changed, the seed-42 evidence set reproducing byte for byte, so no baseline refresh was required. The lint baseline rose by four `assignment_linter` findings and was refreshed, all four being `<<-` required by the code standard: one in the `fail()` helper every check script carries and three assigning the execution model's globals. The roxygen ratchet held at 171 throughout.

**Unblocked by this merge:** No issue becomes ready. The measurement integrity this restores is a prerequisite for the replicated re-measurement queued in #295, #296, #300, #228 and #312 rather than a gate any of them declares.

### PR #319 — Ship Sustained-Campaign Defaults and Refresh Every Document That Reports Them ✓

**Merged:** PR #319, branch `feature/issue-312-sustained-campaign-defaults`

Changes three shipped defaults in `env_data.json` and brings every document reporting a figure measured under the old ones back into agreement with the model. `r2eheavy.recovery.evacuation_policy_days` moves from 30 to 21, the convalescent load at 30 days being large enough that the R2E holding pool no longer settles over a sustained campaign and takes the intensive care beds behind it down with it; the doctrinal source names 30 days as a worked example and is explicit that the threshold is a command decision spanning at least 15 to 60 days. `force_regeneration.reinforcement.demand_interval_days` moves from 0 to 7, a disabled cycle modelling a force that is never replaced, so a long run measures a force being consumed rather than a health system under sustained load. `role4.ame.failure_probability` moves from 0.15 to 0, on the reasoning that this model's subject is the land-based trauma system and its business with the strategic echelon is to state the demand that system generates rather than to simulate the reliability of the airlift meeting it, which is the treatment Role 4 itself already receives; the value was never sourced. The Morris screen was re-run at the parameter's new bounds of 0 to 0.30 and its tracked evidence set, its seven tracked scatter plots, the README ranking table and Further Development L18 were all republished from it. `docs/Multi_Run_Analysis.md` gains a strategic airlift reliability sweep, `docs/Multi_Run_Supplement.md` the design of that experiment, and the README names the AME capacity banking assumption that makes a cancellation a permanent loss of lift rather than a deferral.

**Reference is deliberately non-closing.** Issue #312 asks for a long-horizon protocol: a duration pilot, the protocol documented in `docs/Multi_Run_Supplement.md`, a re-derived warm-up classification, a per-response steady-state statement, an audit of horizon-dependent findings and a regression check. This pull request ships the defaults #312's own evidence motivated, which is a prerequisite for the protocol rather than the protocol itself, and closes one of its tasks, labelling the published sensitivity rankings with the horizon they were screened at. #312 stays open.

**Seed-42 baseline (30 days, single run):**

| Metric | Before | After |
|---|---|---|
| Total casualties | 530 | 495 |
| Strategic evacuation decisions | 135 | 193 |
| Boarded within the campaign | 99, with 36 still queued | 193, none queued |
| Mean evacuation wait | 10.1 days | 1.0 days |
| Role 4 peak occupancy | 90.0 on day 30 | 120.0 on day 30 |
| Ventilated pre-flight ICU hold | 109.4 h mean against a 102.8 h median | 24.9 h against 22.2 h |
| Effective force, end of run | combat 2225, support 1162 | combat 2407, support 1208 |

All three died-of-wounds calibrations pass at the new defaults on clean 150-replication samples, so no ceiling was re-fitted: `default` 0.464% (95% CI [0.397%, 0.531%]) and `moderate_intensity` 0.353% ([0.294%, 0.411%]) against the Ajax Bay bound of 0.46%, and `high_intensity` 3.498% ([3.378%, 3.618%]) against the Okinawa target of 3.40%.

**New finding.** The airlift reliability sweep, 30 replications of a 360-day campaign at each of six cancellation probabilities paired across one seed vector, measures what the zero-cancellation default assumes away. Losses up to 10% cost nothing at all, the worst campaign at 10% reaching the same holding queue as under perfect lift; between 10% and 15% the collapse rate goes from zero to 16.7% and the worst queue from 17.9 to 161; above 20% the risk saturates. The mechanism is capacity banking, sortie capacity accumulating and never being released, so an early run of cancellations consumes a margin later sorties cannot rebuild. The effect sits entirely in the tail, the median holding queue staying below 2 in five of the six arms, and is not predictable from a campaign's first 30 days.

**Unblocked by this merge:** No issue becomes ready. Issue #313 stays blocked, gated on #312, which remains open. Issue #320 was raised from work following this merge and is tabled as open.

### Issue 310 — Complete `run.R`'s Command Line Surface and Validate It at the Boundary ✓

**Merged:** PR #317, branch `feature/issue-310-cli-switches`

`run.R` could not express configurations the model supports and validated none of its own arguments. It gains `--scenario`, `--mode`, `--images-dir` and `--max-cores`; `scripts/run_warmup.R` gains `--seed`, `--output-dir`, `--images-dir` and `--refresh-baseline`. A scenario profile was previously reachable only through the comparative multi-replication runner or the Shiny console, so a single run under a named profile could not be expressed at all, which is what the issue was raised from. `--mode` states which of the two execution paths, and so which artifact set, a run produces; it is inferred from `--iterations` when omitted, so every documented invocation behaves as before, and a contradiction between the two is rejected. A baseline refresh now requires both single-run mode and the default scenario, each refusal naming its own reason, the second closing a route by which a profile could have left the tracked evidence set describing a configuration the repository does not ship.

The validation rules live in a new `R/cli.R` rather than inside `run.R`'s CLI block, so `scripts/check_input_validation.R` can source them alone and assert every one in milliseconds. The first implementation tested them by spawning a process per case, which took that check from a tracked 16 seconds to over two minutes, a cost the per-PR gate would have paid on every pull request; one spawned invocation is retained to show the rules are wired into the entry point rather than merely present in the module. The check gains twenty assertions and now runs in 26 seconds.

The same pull request closes an artifact-policy leak found while adding the directory switches: `images/welch_plot_icu_queue.png` is tracked, and `scripts/run_warmup.R` overwrote it on every default invocation with no flag, which is the failure mode Issue #154 closed for `run.R` and which remained open at the other entry point. It now writes under `outputs/` unless `--refresh-baseline` is passed.

Two defects found in testing are recorded rather than fixed here. `Rscript run.R --days 1` and `--days 2` crash in the analysis pipeline on an absent `return_day` column, which is pre-existing on `main` and is raised as Issue #316; the trigger is that no casualty returned to duty rather than the run length itself, so it is reachable by parameter choice and not only by a short run. An unknown `--scenario` is rejected by `resolve_scenario()` rather than by `R/cli.R`, so its message does not match the format of the other rejections; duplicating the list of valid profiles into the validation module would have created a second authority on what scenarios exist, which was judged the worse trade.

No simulation output changes. The tracked seed-42 evidence set reproduces byte for byte, which is what demonstrates that replacing `load_elms()` with `load_scenario("env_data.json", "default")` is a no-op rather than merely asserting it; the lint ratchet holds at 1,163 findings and the roxygen ratchet at 171, both exactly their baselines, and all twenty-five fast checks pass.

**Unblocked by this merge:** Issue #312's stated dependency on #310 is satisfied, a long-horizon protocol needing runs expressible as a documented command; it was already carrying `status: ready` when raised, so no label changes. Issue #313 stays blocked, gated on #312 rather than on #310.

### PR #308 — Evaluate Options for a User-Adaptable Treatment Flow ✓

**Merged:** PR #308, branch `claude/treatment-flow-customization-8a95k4`

No issue was raised for this work, which was taken up directly from a request to
evaluate how far the `env_data.json` parameter mechanism could be extended to let
a user adapt the treatment flow itself rather than only its magnitudes. It is
recorded here under its pull request number, on the precedent PRs #278, #288,
#292, #298, #302, #304 and #305 set, and carries no row in the summary table, no
phase sequence item and no dependency graph node. The document's purpose is to
precede an issue set rather than to close one, so the issues it recommends are
left to be raised against the stage they belong to.

`docs/Treatment_Flow_Configurability.md` records three properties of the current
code that make the extension tractable and were not designed for it. Leaf values
are passed through uninterpreted by `build_variable_tree()`, and the shipped file
already carries arrays and strings alongside its numbers, so a specification of
arbitrary shape fits inside the existing `vars` tree with no schema change and
inherits the scenario overlay, the console's save and load handlers and the
structural validator. The trajectory file is already decomposed into roughly
eighty named single-purpose builders taking their resources as arguments, and the
two composition roots are the only functions resolving an establishment into
resource vectors, which makes them the point at which a specification would be
consumed. Selecting behaviour rather than magnitude from configuration is
established practice rather than a departure, the R2E surgical branch already
choosing an arm from a configured priority threshold and the `high_intensity`
profile already changing a distribution family.

Four options follow. Widening the parameter surface over seven identified
literals is incremental and leaves the topology untouched, and each new field is
continuous and therefore screenable. A curated set of structural policy switches
read at build time gives real flow adaptation over a vocabulary a clinician can be
asked to review, at the cost that a switch is not continuous and so cannot be
screened, its evidence coming from replicated comparison runs instead. A
declarative pathway specification with a compiler is the only option under which a
user can add, remove or reorder an echelon, and its cost is roughly double what
the trajectory file alone suggests: the analysis pipeline is keyed to the current
clinical vocabulary in fifty-four places, the simmer selection identifiers are
allocated by convention rather than by code at some thirty call sites, and the
configuration globals recorded as Further Development L31 become a precondition
rather than a tidiness improvement. An R override file sourced from the command
line is kept as a research-only facility, being unavailable to the console and
unreproducible from a configuration file alone. The recommended sequence takes
the first three in order, gating the third behind L31 and Issue 4, which rewrites
the same trajectory internals.

Two constraints carried from project precedent apply to every stage: a new
structure must reproduce the seed-42 baseline byte for byte at its default
setting, by the degenerate-default technique Issue #173 and the pre-open window
check demonstrate, and each switch requires its own regression check asserting
that its non-default settings reach the model.

Two incidental observations are recorded in the document without being acted on,
neither confirmed as a defect: two different selection policies are applied to the
same R2B holding pool on its queued and unqueued paths, and
`validate_env_data_json()` is reached only from the Shiny console, every
command-line entry point parsing `env_data.json` without it.

One markdown document is added and one `CLAUDE.md` Repository Structure row
indexes it. No published value moves, no R source, configuration or tracked
artifact is touched, and the Actions gate passed. The check suite was not run in
the session, R being absent from its container; the assertions
`scripts/check_markdown.R` makes about the new document, six relative links all
resolving and no anchors, images or TOC block, were verified by hand while the
gate ran the suite itself.

**Unblocked by this merge:** No new issues unblocked.

### PR #305 — Point the README at the Multi-Run Supplement ✓

**Merged:** PR #305, branch `claude/multi-run-supplement-paper-n6fk7o`

No issue was raised for this work, which follows directly from PR #304 below and
is recorded here under its pull request number, on the precedent PRs #278, #288,
#292, #298 and #302 set. It carries no row in the summary table, no phase
sequence item and no dependency graph node.

The README introduces its companion analysis documents in two places, the
abstract's list of what the system reference does not contain and the
conclusion's statement of where results are published, and both named the
single-run and multi-run papers alone. `docs/Multi_Run_Supplement.md` now
appears in each, as a third bullet with the same shape of one-line description
and as a third title link, the abstract's count reading three companion
documents rather than two. The Multi-run Replication Framework section gains a
cross-reference to the supplement's Experimental Designs and Replication Count
and Resolution sections, which is where a reader goes from the framework itself
to how it was used and how many replications a response needs before its
interval resolves anything.

Three prose lines change in one file. No published value moves, no R source,
configuration or tracked artifact is touched, and all checks pass.

**Unblocked by this merge:** No new issues unblocked.

### PR #304 — Make the Multi-Run Supplement a Standalone Academic Paper ✓

**Merged:** PR #304, branch `claude/multi-run-supplement-paper-n6fk7o`

No issue was raised for this work, which was taken up directly from a request to
bring `docs/Multi_Run_Supplement.md` to the standard of the two analysis papers
it complements. It is recorded here under its pull request number, on the
precedent the pull requests above set, and carries no summary table row, phase
sequence item or dependency graph node.

The supplement was created by PR #298 as a holding place for the designs and
derivations the multi-run paper shed on its way to journal length, and read as
one: a purpose note, seven experiment designs and a provenance paragraph, with
no abstract, no introduction, no conclusion and no reference list of its own.
It is now a standalone paper. It gains a title naming its subject, a structured
abstract, an introduction locating it against the other three documents, a
limitations section stating what its designs cannot support, a conclusion, and
its own independently-numbered list of thirteen open-access references cited in
order of first appearance. The existing content is retained in full and expanded
where the design detail was implicit: the parallel dispatch and generator
restoration that make replications independent, each scenario profile's
calibration and what it overrides, the conventions common to every design, the
replication counts a given half-width requires as a table, and the four
regression checks that defend the framework's properties. The force regeneration
comparison is promoted to its own section, being the one experiment the
supplement reports in full, and now states what its result implies for the
designs around it: casualty volume at high intensity is not stationary across a
campaign, and every experiment here is run without reinforcement.

The document is brought under `scripts/check_markdown.R` and
`scripts/check_references.R` alongside the other three academic documents, so
its table of contents, its links and its reference list are checked on every
pull request rather than by hand. `CLAUDE.md`'s academic-output rules widen from
three documents to four throughout, with the supplement's update triggers added
to the per-PR table, and its em dash, reference list and source removal rules
extended to it. Two stale document titles are corrected in passing: the
multi-run paper's citation of the single-run paper, and the README's conclusion
link text for it, both still carrying the title PR #292 superseded.

The check suite was not run in the session, R being absent from its container as
it was for PR #288, and the two checks whose scope the pull request changes were
emulated instead: the table of contents block matches the one GitHub's own
anchor rules generate, every local link and anchor across the tracked documents
resolves, and the reference list is numbered one to thirteen with every entry
cited, uniquely sourced and dated. The Actions gate ran the real suite and
passed. No published value moves and no R source, configuration or tracked
artifact is touched.

**Unblocked by this merge:** No new issues unblocked.

### PR #302 — Withdraw the Moderate-Intensity Adequacy Claim ✓

**Merged:** PR #302, branch `claude/multi-run-analysis-style-ir8sc3`

No issue was raised for this work, which was taken up directly from the third
review round of `docs/Multi_Run_Analysis.md`. It is recorded here under its pull
request number, on the precedent PRs #278, #288, #292 and #298 set. Two issues
were raised from the review and both remain open, each with a summary table row.

Two corrections of substance came out of the review, and neither moves a
measured value. The paper had opened by calling this "the establishment that
copes at moderate intensity" while arguing, sixty lines later, that theatre
contention is present at moderate intensity and is a standing rather than a
contingency constraint. Those cannot both hold. The claim of adequacy is
withdrawn: the lead now records that the establishment is already under strain
at the lower of the two rates tested, and the later passage states that
casualties wait for surgery there too, the load being absorbed rather than
comfortably accommodated. Separately, the mortality difference between the
holding and intensive care pathways, 0.16% against 0.06%, had been written as
though the simulation discovered it. It is built in, reduced care selecting a
different died-of-wounds curve, so the figures count how many casualties the
rule sends down the higher-risk curve rather than establishing that the curve is
right. The text now says so.

Three changes of voice run the length of the document. Direct address to the
reader is removed throughout, an audience not needing to be told that a paper
written for it is written for it. "Binding constraint" becomes "principal
constraint", and compute time is described as a limitation rather than a binding
constraint. Editorialising is removed from the abstract's background.

The paper is retitled "Surgical Hours, Not Operating Theatres: Sizing the
Land-Based Trauma System for Large Scale Combat Operations", naming its finding
rather than its method, with the README link text following. This replaces the
shared stem PR #292 gave both analysis papers for the multi-run paper only, so
the pairing is broken in one direction; retitling `docs/Single_Run_Analysis.md`
to match was left out of the pull request rather than made unasked.

Smaller changes: a worked example added to the confidence interval explanation,
the single-campaign diversion figure dropped from an argument about system
design, the multi-run rationale rewritten to lead with why one campaign measures
the campaign rather than the system, and the transport recommendation restated to
name its moderate-intensity-only evidence as a shortfall.

Two review requests needed per-timestep simulation output the repository does not
track and were raised rather than half-met: #300, sweeping the transport fleet at
high intensity, and #301, queue length and degraded care rate as time series.
Both are added to the paper's Further Development table. Body prose stands at
5,617 words. No published value moves, no file under `R/` or `env_data.json` is
touched, and all checks pass.

**Unblocked by this merge:** No new issues unblocked. #300 and #301 were both
raised ready.

### PR #298 — Reframe the Multi-Run Paper Around Planning Decisions ✓

**Merged:** PR #298, branch `claude/multi-run-analysis-style-ir8sc3`

No issue was raised for the reframe itself, which was taken up directly from a
review of `docs/Multi_Run_Analysis.md` conducted over three rounds of comments
on a rendered draft. It is recorded here under its pull request number, on the
precedent PRs #278, #288 and #292 set. The pull request also closes Issue #297,
which was raised from the review and carries its own summary table row.

The paper is reorganised around the decisions a planner makes rather than the
order its experiments were run, and cut from 10,726 words to 5,603 words of body
prose against a journal length of roughly 5,000. It now runs: where the trauma
system fails first, four planning options in priority order, the system design
features that shape how the results read, demand on the national support base,
the effects the simulation could not resolve, and a development agenda. Every
experiment is retained beneath the decision it informs. Each option carries one
of four evidence labels, measured, direction only, unresolved or untested, so a
diagnosis that is established is never read as a remedy that is not; the paper's
first-ranked option is one whose diagnosis is measured and whose remedy the
simulation cannot yet evaluate, and the labels are what let it be ranked first
without overclaiming.

Three changes of voice run the length of the document. R1, R2B and R2E are named
directly rather than as forward and rearward facilities; moderate and high
intensity replace the Falklands and Okinawa campaign names outside the methods;
and every unresolved effect is framed as constrained by compute time, states the
run count that would settle it, and appears in the development agenda rather
than dead-ending.

Two structural changes have consequences beyond the paper. The reinforcement
experiment leaves it as force generation rather than health system performance,
its design and results moving to the new `docs/Multi_Run_Supplement.md`, with
the two external references to that section repointed there from `README.md` and
`docs/Single_Run_Analysis.md`. And the full experimental designs, the
replication-independence argument, the withdrawn antithetic pairing and its
measurement, the interval construction and the replication-count derivation move
to that same supplement, which is what allows the paper to reach journal length
without losing the record `CLAUDE.md` requires it to keep.

`scripts/render_paper_figures.R` is added with the three figures it writes,
closing Issue #297. It parses its values out of the paper's own markdown tables
rather than holding a copy of them, so a figure cannot drift from the table it
illustrates, and it fails rather than writing where a table has moved or no
longer parses. The queue figure plots growth against a reference line at the
2.33-fold rise in casualty volume and sizes each point by its absolute queue,
after a first version ranked the transport fleets second on growth alone and so
contradicted the paper's own finding that transport holds margin.

The first CI run failed on `check_roxygen.R`, `missing_constant_header` rising
from 171 to 178, the seven findings being the new script's file-scope constants;
rule R3 was met for its functions but not its constants. Fixed in the same pull
request and verified back at baseline. No published value moves, no file under
`R/` or `env_data.json` is touched, and the seed-42 reproduction passes.

Two issues were raised from the review and remain open: #295, a replicated
analysis of national support base demand, which would replace the single-campaign
section the paper now labels as such; and #296, a re-run of the post-operative
intensive care gate under independent replications, which would retire the
narrow-interval caveat that section still carries.

**Unblocked by this merge:** No new issues unblocked. #295 and #296 were both
raised ready.

### PR #292 — Retitle the Two Analysis Papers ✓

**Merged:** PR #292, branch `claude/review-paper-titles-fy0lj2`

No issue was raised for this work, which was taken up directly from a review of
the three documents' titles. It is recorded here under its pull request number
rather than an issue number, and carries no row in the summary table, no phase
sequence item and no dependency graph node, on the precedent PRs #278 and #288
set.

Both analysis papers now open with the method and the system it is applied to.
`docs/Single_Run_Analysis.md` becomes "Applying Discrete Event Simulation to the
Land-Based Trauma System: Model Verification and Baseline Behaviour" and
`docs/Multi_Run_Analysis.md` "Applying Discrete Event Simulation to the
Land-Based Trauma System: Replicated Experiments and Comparative Scenario
Analysis". The shared stem replaces the "Battlefield Casualty Handling" one both
titles carried from Issues #238 and #240, which named the repository rather than
the system modelled, and each subtitle is left carrying the unit of analysis
that separates one paper from the other. `README.md`'s own title is unchanged,
a bare project name being the right heading for a repository landing page.

The title review that preceded the change also found the README's Conclusion
naming the multi-run paper "Multi-Run Analysis", a title superseded under Issue
#240, while naming its companion correctly in the same sentence. It is brought
into agreement in the same commit, both links now using the subtitles rather
than the full titles, the stem being common to the two.

Three lines change across three files and no heading below a title moves, so no
table of contents anchor and no cross-reference between the documents is
affected. No seed-42 baseline value moves and no R source, configuration or
tracked artifact is touched. No issue becomes ready on this merge.

The check suite was not run. R is not installed in the session container, so
`Rscript` was unavailable; neither retitled line is a heading any anchor points
at, the `<!-- TOC START -->` blocks beginning below it in both documents, and
the one edited README link resolves to a file that exists. The GitHub Actions
gate ran the suite itself on the pull request.

### PR #288 — Simplify the Single-Run Analysis Prose ✓

**Merged:** PR #288, branch `claude/single-run-analysis-prose-xc40d5`

No issue was raised for this work, which was taken up directly from the
observation that `docs/Single_Run_Analysis.md` read stiffly against the standard
its own README abstract sets. It is recorded here under its pull request number
rather than an issue number, and carries no row in the summary table, no phase
sequence item and no dependency graph node for the same reason.

The document's narrative prose is rewritten so that it stays an academic paper
while reading clearly for the operational planners and military staff it is
written for. Three patterns carried most of the stiffness and are addressed
throughout: openers that framed a claim before stating it, of which "From a
health system planning perspective, this data implies a need for scalable
treatment capacity" is representative; sentences chaining two or three claims
through hinges such as "which is what" and "being too coarse to", now split; and
nominalised process language where a verb was available. Repeated restatements
of the run configuration are dropped where the Methods table already fixes it
for the whole document. Ninety prose lines are replaced one for one, and the
four `Intervention Scenario A/B/C/D` labels in the R2B section are deliberately
left as they were, this document citing Scenario C by name.

Nothing but prose changed, which was verified rather than asserted: the numeric
tokens of the document before and after differ only in how many times a figure
is repeated, never in value and with none introduced; the heading and image
lines diff empty, so every anchor the README and `docs/Multi_Run_Analysis.md`
link to still resolves and no alt text moved; and the citation sequence still
runs one to sixteen in order of first appearance with every entry cited. No
seed-42 baseline value moves, no R source, configuration or tracked artifact is
touched, and no issue becomes ready on this merge.

The check suite was not run. R is not installed in the session container, so
`Rscript` was unavailable and the assertions of `scripts/check_markdown.R` and
`scripts/check_references.R` were instead checked by hand for this document:
every anchor resolves to a heading in the file, every local link and image
target exists relative to `docs/`, no image carries placeholder alt text, and
the reference list is numbered from one in order of first appearance with each
entry carrying a URL and a retrieval date. The GitHub Actions gate ran the suite
itself on the pull request.

### Issue 240 — Multi-Run Analysis Paper to Publication Standard ✓

**Merged:** PR #284, branch `claude/config-output-style-2qd8aq`

`docs/Multi_Run_Analysis.md` carried the project's strongest quantitative
argument in a form that would not have survived review. Large parts of its
Interpretation and Conclusion existed to set a current figure beside a
superseded one, so a reader had to track which numbers were live; the
confidence intervals were reported throughout without the method ever being
stated; the two results tables reported uncertainty differently from one
another, the queue table carrying the paper's central finding with no
interval at all; and the abstract described a design of n≥30 that no
experiment in the document used. The document is now retitled Replicated
Experiments and Comparative Scenario Analysis, to name what PR #280 gave it,
and gains an Introduction stating its two research questions, a Methods
section and a Limitations section. Methods sets out the replication as the
unit of analysis, states the replication-independence argument in the
document's own text rather than by cross-reference, gives the interval method
explicitly as a Student *t* interval with the truncation convention it uses,
and carries the replication-count power analysis moved out of `CLAUDE.md`,
which now cross-references it. Limitations names four classes, the first
three specific to this document's designs and the fourth a table of the
README's `L` entries by identifier. The supersession narrative is removed
throughout: where such a paragraph also carried a live mechanism, that
mechanism is restated in the present tense, and the withdrawn forward-surgery
claim in the pre-open hold window section becomes a limitation carrying its
own power calculation rather than a retraction. Both remaining `Issue #N`
references and all nine em dashes are cleared, and the reference list goes
from two entries to fourteen.

Both results tables now report uncertainty consistently. The casualty and
mortality table gains a 95% interval and a p10-p90 range on every row, and
the resource queue table gains intervals it never carried, computed from a
fresh 50-replication measurement of both profiles at seed 42. That
measurement reproduced every published casualty, mortality and queue figure
exactly, which is what establishes that the intervals belong to the numbers
already published rather than to a new run. Two queue cells move, and for a
reason the table now states rather than for a change in the model: the
aggregation convention is that a resource idle throughout a replication
contributes a zero rather than dropping out of that replication's average,
which takes `moderate_intensity` R2E Hold from 0.622 to 0.598 and Transport
from 0.00386 to 0.0038. The R2E Hold ratio moves from 4.33× to 4.51×, and
the abstract, the Interpretation and the Conclusion are updated with it.

The measurement was taken in an unpinned R 4.3.3 sandbox, no Docker daemon
being available in this session; the exact reproduction of every published
figure, and `scripts/check_baseline_reproduction.R` passing byte for byte in
the same sandbox, are the evidence that the environment did not affect it.
The stale `n≥30` design claim was corrected in the three other places it had
been repeated, and a cross-reference in `docs/Single_Run_Analysis.md` that
pointed here for the Welch diagnostic, which the README reports, is repaired.
All twenty-four fast regression checks pass. No seed-42 baseline value moves.

**Unblocked by this merge:** No new issues unblocked. Issue #240 was the last
open item in Phase 7, which is now complete.

### Issue 239 — Single-Run Analysis Paper to Publication Standard ✓

**Merged:** PR #282, branch `claude/config-output-style-7n24ms`

`docs/Single_Run_Analysis.md` read as an internal record of how the model had
changed rather than as a paper. Twenty-two `Issue #N` references and five
blockquoted maintainer notes are removed, the live content among them promoted
rather than deleted: the warm-up rationale and the pinned-container provenance
become subsections of a new Methods section, and the R2B hold-bed MODEL
ASSUMPTION block becomes prose closing the intervention analysis, cross-
referencing README entry L4 instead of restating it. The apparatus the issue
asked for is added, a Methods section giving the run configuration, the warm-up
treatment, the distinction between verification and validation and the
provenance of the evidence set, and a Limitations section cross-referencing
README entries L1, L3, L4, L11, L16, L17, L23 and L26 by identifier. The
reference list grows from three entries to sixteen, all open access and all
drawn from or consistent with the vetted set already in `README.md`. Spelling is
settled to British throughout, thirty em dashes are cleared, six placeholder alt
texts are replaced, and the R2B saturation heading is re-punctuated with its
inbound anchor in `docs/Multi_Run_Analysis.md` repaired to match.

The figure-verification pass was executed rather than reasoned about, and this
is where the merge found more than the issue anticipated. The pinned Dev
Container was built and the model run at seed 42:
`scripts/check_baseline_reproduction.R` passes, and `run.R --refresh-baseline`
reproduced all thirty-one
tracked figures byte for byte, so no artifact needed regenerating and none is
committed. Every numeric claim reconciles against the run's own output, and the
arrival-generation figures were additionally recomputed from `logs/logs.txt`
independently of the model. Reading each figure against the text that describes
it then found eight descriptions that disagreed with their own plots. Three were
substantive: the Role 4 census was described as still rising at the run's end and
not decaying within the window shown, when its axis runs to day 69 and it decays
to near zero; the AME backlog caption claimed the critical pool ends the run
above its pre-departure level, when it ends at 24 against about 35; and the
intensive care gating figure was described as carrying two pathways and
clustering late, when it carries three and shows degraded care from the first day
onward. Two further descriptions called bar charts area charts and put a
reference line at the wrong quantity. Three prose claims that the current
baseline contradicts were removed with them, among them an assertion that both
evacuation pools clear within days, against a measured 10.1-day mean wait and 36
casualties still queued at day 30. The lesson is the one the issue's own
recommended approach anticipated: a verification pass that is a by-product of
editing does not find these, and one that is executed does.

**Seed-42 baseline (30 days, single run):** No metric changed. The tracked
evidence set reproduces byte for byte in `logs/logs.txt`, in all eight arrival
diagnostics under `data/`, and in all thirty-one figures under `images/`. The
merge touches two markdown files and no R source, configuration or tracked
artifact.

**Unblocked by this merge:** No issues become ready. #240 was already ready, #238
having been its gate, and nothing else in Phase 7 was gated on #239.

### Issue 238 — Re-cut the Analysis Papers by Method ✓

**Merged:** PR #280, branch `claude/issue-238-ty4ubc`

The boundary between the two analysis documents was drawn by title rather than
by method. `docs/Single_Run_Analysis.md` was scoped in its own abstract as an
illustrative single-run analysis while reporting replicated experiments with
confidence intervals, and the interval methodology those sections depended on
was described in the other document, so a reader of either paper alone got an
incomplete methods picture. The split is now by unit of analysis: a result from
one run belongs to the first document, a result from many to the second.

Five replicated experiments move to `docs/Multi_Run_Analysis.md`, where the
replication framework and interval methodology already live: the 50-replication
post-operative intensive care gate validation, the forward ICU share decision
frontier, the transport fleet-size sweep, the force regeneration reinforcement
trend table and the mass casualty stress test. The issue named four; the force
regeneration trend table was the fifth, a 15/12-replication comparison under a
non-default reinforcement configuration that the issue's own acceptance
criterion reached even though its table did not list it. Each migrated section
now opens with a design statement naming its replication count, its seed and
any parameter set away from the shipped default, which promotes the mass
casualty section's non-default configuration out of the inline blockquote it
had been flagged in. The Return to Duty section's incidental 50-replication
in-theatre share was deleted rather than migrated, the text itself having
recorded that the figure was not recomputed for the change that preceded it.

Two things deliberately stayed. The evacuation-policy comparison table is three
single runs rather than a replicated experiment, so moving it would have
breached the receiving document's scope instead of honouring it. And
`docs/Multi_Run_Analysis.md` keeps its title, which is now narrower than its
contents; its abstract, methodology and conclusion were broadened to describe
the six experiments that sit alongside the scenario comparison, and the retitle
is left to Issue #240.

`docs/Single_Run_Analysis.md` is retitled "Model Verification and Baseline
Behaviour", the previous title having named a method rather than a subject.
Cross-references were repaired in both documents, `README.md`, `CLAUDE.md` and
`.github/pull_request_template.md`; `CLAUDE.md`'s Repository Structure and
README Maintenance tables now state each document's scope and the boundary rule
between them. `scripts/check_markdown.R` regenerates both tables of contents
and confirms every anchor, local link and image target resolves, and
`scripts/check_references.R` confirms all three reference lists remain sound,
the migrated prose carrying no numbered citation and so needing no renumbering.

**Seed-42 baseline (30 days, single run):** Unaffected. The merge touches five
markdown files and no R source, configuration or tracked artifact, and no
figure in `CLAUDE.md`'s Key Parameters table is quoted differently or moved.

**Unblocked by this merge:** Issues #239 and #240 both become ready, #238
having been the gate that settled what each document contains. Issue #230, the
other gate on both, merged as PR #243.

### PR #278 — Run the Fast Check Suite Several Checks at a Time ✓

**Merged:** PR #278, branch `claude/check-performance-optimization-eh2o1v`

No issue was raised for this work, which was taken up directly from the
observation that the pull request gate had grown to a quarter of an hour. It is
recorded here under its pull request number rather than an issue number, and
carries no row in the summary table, no phase sequence item and no dependency
graph node for the same reason.

`scripts/run_all_checks.R` gains `--jobs`, which runs several checks at once as
separate `Rscript` processes, dispatched longest first from the runtimes now
tracked in `scripts/check_runtimes.csv` and refreshed by `--refresh-runtimes`.
The two checks that regenerate a tracked document are run first and on their
own, their failure signal being a repository-wide `git status` comparison that
cannot attribute a change to one of two concurrent writers. Each concurrent
child is given an `MC_CORES` of the machine divided by the job count or half the
machine, whichever is larger, which the `parallel` package turns into the
`mc.cores` default the replication framework dispatches on. The workflow's fast
suite step runs `--jobs auto`.

Measured in the pinned container on the four-core runner, the fast suite falls
from 16 min 51 s to 7 min 27 s, all twenty-four checks passing in each
arrangement and the working tree left clean by each. An intermediate
arrangement dividing the cores exactly between the jobs measured 8 min 53 s and
is recorded in `scripts/README.md` alongside the other two, because it is what
set the final core share: four cores between four jobs left
`check_measurement_reproducibility.R` with one, where it took 8 min 44 s against
2 min 55 s on four, and the suite spent its last four minutes running that one
check on a quarter of the machine. The final figure is at the machine's limit
rather than the scheduler's, 29 minutes of check time across four cores, so the
remaining levers are a larger runner or less work in the checks themselves.
Sharding the suite across a matrix of jobs was considered and rejected: it would
shorten the gate further but pay a container initialisation and a library
restore per shard, which is a cost in billed minutes rather than in latency.

No simulation output moves, this pull request changing no model code, no
configuration and no seed, and the seed-42 evidence set reproduces byte for
byte. `README.md`, `docs/Continuous_Integration.md`, `scripts/README.md` and
`CLAUDE.md` are updated; the check count at the head of `scripts/README.md` is
corrected from twenty-one to twenty-five in passing.

**Unblocked by this merge:** No new issues unblocked.

### Issue 270 — Split the Five Simulation-Logic Functions Still Over the Length Limit ✓

**Merged:** PRs #272, #273, #274, #275 and #276, branch `claude/config-output-style-7tz4x0`

The five functions Issue 241 closed without splitting are split, one pull
request each, in the order the issue set: `run_once` (147 lines) into four named
assembly steps; `build_casualty_trajectory` (334) into two attribute segments
and the R1 pathway over its three DNBI sub-type arms; `r2b_treat_wia` (560) into
one builder per step of the R2B pathway; `r2e_treat_wia` (766), the longest
function in the repository, into fourteen lifted locals and six per-phase
segments; and `run_replications` (116) into a captured/restored generator state,
the dispatch and the killed-worker filter. Every lifted builder takes the
resources it uses as arguments rather than closing over a parent's locals, which
was the actual obstacle the issue identified, and each carries a roxygen header.
The lint ratchet's `function_length` count falls from nine to four, and the four
remaining are exactly those `docs/STYLE_GUIDE.md`'s D1 conformance entry records
as deliberate: the three orchestrators and `extract_kpis`. That entry no longer
carries a simulation-logic group at all.

Three deduplications came out of the reading rather than being sought: the R1
mortuary branch stood verbatim in both the killed-in-action arm and the
died-of-wounds arm, the R1 convalescence in both the disease arm and the
post-treatment arm, and the R2B to R2E movement in four places. Each is now one
function called from each site.

The verification is what makes the sequence defensible. Every pull request was
gated on `scripts/check_baseline_reproduction.R` passing byte for byte, and each
was additionally checked by running the model at seed 42 on the branch and on
`main` and comparing the whole of `outputs/` file by file, which is a stronger
statement than the tracked evidence set alone: every CSV, markdown table and
plot the analysis pipeline writes is identical. That comparison surfaced one
difference, in the row order of `outputs/mon_arrivals.csv` for arrivals still in
theatre at a run's end, which two runs of identical code reproduce between
themselves, so it is pre-existing nondeterminism in simmer's monitor output
rather than an effect of any split; sorted, the files are byte-identical and
every analysis output derived from them agrees. For `run_replications` the
comparison was made at three replications rather than one, `run.R --iterations 1`
calling `run_once()` directly and never reaching that function.

The judgement recorded in `r2e_treat_wia`'s own header, that it should not be
decomposed because `join()` copies a segment rather than continuing the same
object, is answered rather than set aside: a phase appended to the chain adds to
the same object, and the splits introduced no `join()` the undivided bodies did
not already make. The header proposed that whoever attempted the split do it
against the reproduction check, one branch at a time, which is what the five
pull requests did.

Two faults were caught by the gate rather than by review, which is the case for
the sequencing the issue chose. A regular expression collapsing the shared R2B
evacuation segment rewrote that function's own body into a call to itself, which
parsed cleanly and failed at trajectory-construction time with a C stack
overflow; and two revisions raised `indentation_linter` above its baseline on
newly written continuation lines. Both were fixed before the pull request they
belonged to was opened.

**Seed-42 baseline (30 days, single run):** unchanged, and byte-identically so
on all five merges.

**Unblocked by this merge:** No new issues unblocked. Nothing in Phase 6 or
Phase 7 was gated on Issue 270.

### Issue 241 — Apply the Code Standard: Decompose the Oversized Functions ✓

**Merged:** PRs #261, #263, #264, #265, #266, #267 and #269, branch `claude/config-output-style-82tzf4`

Recorded as one entry because it is one issue. Six of its seven acceptance
criteria are met; the seventh, that no function exceed the length limit, is not,
the count standing at nine functions against the eighteen it started from. The
issue was closed on that basis rather than held open, the residual being a
distinct piece of work with its own approach and its own verification, raised as
Issue 270. The seven pull requests were sequenced deliberately rather than
delivered as one, the whole being a three-thousand-line refactor whose diff would
otherwise have been too large to review as a unit, which is the fault the issue
exists to correct.

**PR #261 — the verification scaffolding.** Committed first, on the reasoning
that a test which only exists on the branch that does a refactor proves nothing
about the refactor. The console's coverage is split by what each half can see:
`tests/testthat` drives the reactive state machine through `shiny::testServer()`
in process, with no browser, and runs inside the fast suite as
`scripts/check_testthat.R`; `tests/playwright` drives a running app in headless
Chromium as its own continuous integration job. Playwright rather than a
snapshot-based R harness was a deliberate choice, not a default: it keeps
browser automation out of `renv.lock` entirely and drives the app over HTTP, so
it is indifferent to how `server` is structured internally, which is the
property that made it survive the decomposition unchanged. The cost is a Node
toolchain alongside `renv.lock` in an otherwise pure-R repository.

**PRs #263 and #264 — the decompositions the issue names.** `analyse_run()`
went from 1,418 lines to 398 and `analyse_replications()` from 810 to 229,
behind 48 named functions; `server` went from 2,293 lines to 195 behind roughly
sixty, one per Configure group, per asynchronous run and per Analyse tab. Each
function's parameters and return value were derived from the parse tree rather
than read off by eye. Both were verified by artifact comparison: 89 outputs,
byte-identical.

**PR #265 — the incidental tasks.** The literal `1440` appeared 122 times rather
than the 93 the issue counted and now appears once, as `DAY_MIN` in a new
`R/constants.R`. The constant is not the `day_min` the issue names, for two
reasons: `day_min` is a runtime global an entry point sets, so it is not in
scope for a check calling into a module directly, and a parameter named
`day_min` cannot default to a global of its own name. The aliases the issue
records as dead were not dead once the pipeline was decomposed, having become
the parameter names some ninety sites depend on; the smaller correct fix was
made instead. `r2e_treat_wia()` is assessed and left alone, with the reason
recorded in its own header.

**PRs #266 and #267 — the length limit where it and readability agree.**
`render_group_body`, `build_param_registry`, `generate_env_summary_section`,
`build_environment`, `apply_params` and `plot_r2b_icu_share_frontier` split at
seams they already carried, taking the count from eighteen to eleven.

**PR #269 — the screen drivers, behind an invariant designed for them.**
`run_morris()` (212 lines) and `run_sobol()` (240) were the two extractions the
earlier pull requests could not derive from the parse tree, and
`scripts/check_screen_order.R` was written before either was touched. It stubs
the model evaluation and drives both over their real designs, asserting that a
design repeats at one control seed and differs at another, that every point is
evaluated exactly once in index order with the parameters the design row claims,
that a resumed screen re-evaluates nothing, and that a response vector carries
the names the drivers index it by. It was validated by fault injection rather
than by passing: reversing the evaluation order and moving the cache by one both
fail it. `extract_kpis()` fell from 205 lines to 126 behind
`prepare_kpi_frames()` and `extract_role4_kpis()`, verified by comparing all 36
responses over one captured monitoring set, the function being a pure reduction
and so decidable by identity. The check costs 2 minutes 58 seconds and joins the
fast suite; it was 6 minutes 23 seconds until `ggsave()` was shadowed alongside
the evaluation stub, the drivers having rendered thirty-six labelled scatter
plots per call that the check asserts nothing about.

**What the artifact comparison could not see, and what was added because of it.**
Three classes of fault surfaced that no byte comparison would have caught, and
each produced a check rather than only a repair. Decomposing the analysis
pipeline created three stages returning a name bound only inside a conditional,
which fail whenever that conditional does not fire; two reproduced byte for byte
at four replications and failed at one.
`scripts/check_analysis_decomposition.R` reads the binding structure instead, so
it covers every path at once. Decomposing `server` left one panel reading a
local of another, which all 171 R assertions passed over and the browser suite
caught on one tab of ten; `scripts/check_console_bindings.R` now asserts the
property across all sixty-odd panels, including those no browser test opens, and
caught the same fault class again in PR #266. Removing an alias in PR #265
exposed `attributes` having shadowed `base::attributes`.

**Where the remaining nine stand.** Three are orchestrators whose bodies are a
sequence of named calls, recorded in the style guide's conformance table with the
argument rather than split, splitting one into sub-orchestrators trading an
explicit dependency list for a smaller number. `extract_kpis()` is the fourth,
at 126 lines after the two pieces that lift out whole were lifted; what remains
is one reduction per response domain over a shared set of frames, and threading
the rest into the response assembly would move the coupling into a parameter list
rather than remove it. The other five are simulation logic, and Issue 270 takes
them up. The deferral recorded here through PR #267, that a split shifts the
random number stream and so moves every published figure, reads differently now
that the phase's verification exists: `scripts/check_baseline_reproduction.R`
settles the question byte for byte in 44 seconds and gates every pull request, so
a stream shift fails loudly rather than passing silently. What actually stops
those five is interface design, the same obstacle the screen drivers posed, and
PR #269 established the method for it.

The distinction between an interface that can be derived and one that must be
designed was established by attempting the screen-driver split during PR #266 and
reverting it, not by predicting it. A screen is an eleven-hour run, and a fault
in evaluation order would leave a cache resuming into wrong responses with
nothing reporting it, which is why that split waited for a check built for it.

**Seed-42 baseline:** unchanged throughout, and verified rather than assumed at
every step. No published value moves; `scripts/check_baseline_reproduction.R`
reports the tracked evidence set reproducing byte for byte on each of the seven
merges. The lint ratchet ends below where it started, 1,217 findings to 1,209,
with `function_length` at nine against a baseline of eighteen.

**Raised by this work:** Issue #262, `scripts/check_markdown.R` aborting outside
a UTF-8 locale, found when the suite was run in a bare shell and fixed under
PR #268. Continuous integration was unaffected, the pinned container setting a
UTF-8 locale, so it reached a contributor running the suite locally. Issue #270
carries the unmet length criterion forward, covering the five simulation-logic
functions with the sequencing and the verification the residual needs.

**Unblocked by this merge:** No new issues unblocked. Nothing in Phase 6 or
Phase 7 was gated on #241.

---

### Issue 262 — `check_markdown.R` Aborts Outside a UTF-8 Locale ✓

**Merged:** PR #268, branch `claude/config-output-style-82tzf4`

`scripts/check_markdown.R` exited non-zero rather than reporting whenever the
session's `LC_CTYPE` was not UTF-8. The pattern matching the Further Development
entry headings carries a literal em dash, because that is the character the
headings use, and outside a UTF-8 locale R could not translate it, so
`regexec()` raised before reaching any document content. The cost was not the
false failure but what accompanied it: the check exited having asserted nothing
about the scan table, and a contributor reading the failure as environmental,
which it was, learned nothing about whether the table and its entries still
agreed. Both `regexec()` calls now match byte-wise, the treatment the same file
already applied to its table-of-contents writer, and the check asserts that its
own entry-heading match is byte-wise, so a future non-ASCII pattern cannot
reintroduce the fault silently. Continuous integration was never affected, the
pinned container setting a UTF-8 locale.

This entry was omitted when PR #268 recorded the six Issue 241 merges, the pull
request having carried both changes and recorded only one; it is added here
...
### Issue 237 — Housekeeping: Scan Table Reconciliation, Branch Cleanup and Documentation Drift ✓

**Merged:** PR #259, branch `claude/config-output-style-y8l4pj`

Four unrelated inconsistencies, grouped so that none consumed a branch of its own. The substantive one was the README's Further Development scan table, which is derived from the entries beneath it and had drifted from three of them: L17, L18 and L22 each named a gap differently in the table from the heading of the entry the row points at, and the identifier column carried no header. The table is now reconciled to the entry headings, which are the fuller statements and the authoritative ones, and `scripts/check_markdown.R` gained an assertion so the drift cannot recur: it reads the table's rows and the bolded entry lead-ins below the Further Development heading and reports four ways they can disagree, a row naming no entry, an entry carried by no row, a title mismatch, and an impact differing from the group heading the entry sits under. Entry L22 was four paragraphs where every neighbour is one, which `CLAUDE.md`'s own length rule identifies as a sign that something is being explained twice; it is now one paragraph, the replication-count derivation it carried having a single home in `CLAUDE.md`'s Key Parameters table and the measured rates it restated being already reported in the README's Parameter Calibration section and in the multi-run comparative analysis. The stale "Issues In Review" cross-reference in this document is repaired, that section now being correctly empty.

The twelve `wip/*` checkpoint refs were verified redundant and deleted, which closes Issue #227 as well. Verification compared each ref blob by blob against the tracked evidence set: all 38 files on `wip/issue-155-morris-r20-results` are byte-identical to `data/sensitivity/morris_r20/`, all four on `wip/issue-155-sobol-results-r20top5` to the `sobol_*.csv` under `sobol_n200/`, and the point caches on the three `screen-cache-*` refs that carry published designs to the tracked `points.csv` in `morris_r20/`, `sobol_n200/` and `noise_floor/`. The remaining seven carried only superseded intermediates: the two n=64 designs, two partial caches, and an eleven-row `screen-cache-fake-test`. The remote held twelve such refs rather than the ten the issue enumerated. The session's git credentials refuse a ref deletion with HTTP 403 while permitting an ordinary push, so the deletion itself was performed by the repository owner through the GitHub web interface. The fourth item, the `CLAUDE.md` Repository Structure omissions, was already closed: both files the issue named are present, and every file under `scripts/` and `docs/` was checked against the table with none missing.

**Seed-42 baseline:** unchanged. The PR touches no file the model executes, and `scripts/check_baseline_reproduction.R` reports the tracked evidence set reproducing byte for byte. The first CI run was red on the lint ratchet rather than on any check: the two lines added for the new assertion printed the `⚠️` and `✓` characters the rest of `check_markdown.R` uses, raising `pictographic_character` from 130 to 133. They now print the `[PASS]`/`[FAIL]` markers the rest of the check suite uses, which the ratchet does not count and which match the convention the style guide sets for a check script, and the rule is back at its baseline.

**Unblocked by this merge:** No new issues unblocked. Nothing in Phase 6 or Phase 7 was gated on #237. Issue #227 is closed as completed rather than unblocked, this merge having carried out its work.

---

### Issue 236 — Make Global Configuration Save and Restore Exception-Safe ✓

**Merged:** PR #257, branch `claude/config-output-style-5naq0a`

The model's configuration lives in three globals, `env_data`, `day_min` and `counts`, because `run_once()`, `build_env()` and the trajectory closures resolve them from the global environment rather than receiving them as arguments. Every entry point that varies the configuration therefore overwrote them and was responsible for putting them back, and four of the five did so by an assignment at the foot of the function, which the error path never reaches; `run_scenario()` never restored them at all. The failure was silent and consequential: a sweep that errored partway through left the session on the swept configuration, so a baseline run made next in the same session was computed against the wrong parameters and reported as the baseline. `capture_config_globals()` and `restore_config_globals()` (`R/environment.R`) now snapshot and restore the set, registered with `on.exit(..., add = TRUE)` at the point of save in both capacity sweeps, `run_morris()`, `run_sobol()` and `run_scenario()`. The existing explicit restores are kept rather than replaced, since the CSV and plot each sweep writes after its loop belong to the caller's configuration and not the last sweep point's. The snapshot records an unbound name as unbound, so a caller that never set the globals, which is how `scripts/run_scenarios.R` reaches `run_scenario()`, gets them removed rather than left on the last scenario.

The same PR added the input validation the issue asked for. `R/analysis.R` had no `stop()` across its three thousand lines and consumed monitoring data frames assuming column presence throughout; `analyse_run()`, `analyse_replications()` and the two sweeps now assert the shape of what they receive and name the missing element, column or argument, and `app.R` validates at both boundaries where a configuration enters, the startup load and the upload handler, so a malformed `env_data.json` names its offending field and reports every fault rather than only the first. Interior helpers are left assuming well-formed input, which is the intended division and is now recorded as such in the style guide's conformance table. Two regression checks hold both behaviours: `scripts/check_config_restore.R`, whose seven assertions all fail with the `on.exit()` calls removed, and `scripts/check_input_validation.R` with twenty. The argument-threading alternative, which would end the arrangement rather than make it safe, is recorded as Further Development L31 rather than implemented, since it touches every trajectory closure that reads the configuration and Issue #4 revisits that area anyway.

**Seed-42 baseline:** unchanged. These are error-path and validation changes, so the success path takes the same branches in the same order and consumes the random number stream identically; `scripts/check_baseline_reproduction.R` reports the tracked evidence set reproducing byte for byte, and the full fast suite passes 20 of 20. The lint ratchet rises by thirteen `assignment_linter` findings and its baseline is refreshed accordingly, every one of the thirteen a `<<-` the standard permits: three accumulator closures under G1 and the configuration globals set from the two new check scripts under G2. The `indentation_linter` and `function_length` rises the first draft produced were repaired rather than absorbed.

**Unblocked by this merge:** No new issues unblocked. Nothing in Phase 6 or Phase 7 was gated on #236.

---

### Issue 233 — Make the Analysis Pipeline Stream-Neutral and Idempotent ✓

**Merged:** PR #255, branch `claude/config-output-style-d5f84d`

The analysis pipeline is a report over monitoring data the simulation has already produced, so analysing one run twice has to give one answer, and it did not. `assign_role4_los()` drew each evacuated casualty's Role 4 length of stay from a triangular distribution at analysis time without restoring the random number stream, so a second call over identical monitoring data continued the stream and returned a different Role 4 census; the published figures were reproducible only because `run.R` calls `set.seed()`, `run_once()` and `analyse_run()` in a fixed order, which is a property of the calling sequence rather than of the seed. `with_preserved_rng()` (`R/analysis.R`) now saves `.Random.seed` and restores it on exit around the draw, so the pipeline consumes no net randomness and a second analysis resumes from the position the first started at. No seed is set around the draw, which is the decisive property: the draw stays at exactly the stream position it already occupied, so no published figure moves and no baseline refresh was needed. The pooled mass casualty timeline's render-time jitter is given an explicit seed through `position_jitter()` for the same reason, that site demonstrating a second consumer was not hypothetical.

`scripts/check_analysis_idempotence.R` is new and asserts idempotence for both entry points, over the returned Role 4 census and over every file each writes, plus the stream neutrality both rest on. It runs a 30-day single run and two 30-day replications rather than the shorter run the pipeline alone would need, because the length-of-stay draw is made only for a casualty who has boarded a sortie and at seed 42 the first two scheduled sorties are cancelled; it fails rather than passing vacuously when either census comes back empty. Verified against the unfixed pipeline, where six of its eight assertions fail. The move-into-simulation alternative, drawing the length of stay as a casualty attribute set at evacuation, was recorded as Further Development L30 rather than implemented, since it shifts the whole seed-42 baseline for an output that produces no queue, no mortality and no feedback into the land trauma system.

**Seed-42 baseline:** unchanged. `scripts/check_baseline_reproduction.R` reports the tracked evidence set reproducing byte for byte, and a full 30-day seed-42 run before and after the change is identical in every output, `role4_census_daily.csv` and `images/role4_census.png` included. The only difference the comparison surfaced, the row order of ongoing arrivals in `mon_arrivals.csv`, is pre-existing process-to-process nondeterminism: two runs of unchanged code differ the same way and the sorted contents agree.

**Unblocked by this merge:** No new issues unblocked. Issue #241 was never gated on #233, though landing it first removes a source of confusing byte-comparison failures there.
...
**Merged:** PR #247, branch `claude/output-style-config-oi6ijp`

`README.md`'s reference list carried sixty-seven entries and five distinct defects, the most damaging of which was a misattribution. References `[9]` and `[19]` were the same *Military Review* article, the second listed under an author list that does not belong to it; `[19]` is deleted and its three citations redirected to `[9]`. References `[41]` and `[63]` were the same Law (2020) Winter Simulation Conference paper, merged to one entry.

Two entries violated the project's own open-access rule, and both are resolved by re-sourcing rather than by softening the claim, an open-access substitute already present in the list being available in each case. The general discrete event simulation methodology claim moves from Banks, Carson, Nelson and Nicol, a textbook with no URL, to Rossetti's open-access KSL book, which the document already cited for antithetic variates and which is now listed at its root URL so it can carry both. The claim that warm-up detection presupposes a steady state moves from Gafarian, Ancker and Morisaku, a paywalled *Naval Research Logistics Quarterly* article with no URL, to Rossetti's *Simulation Modeling and Arena* chapter 5, whose title is "Statistical Analysis for Infinite Horizon Simulation Models" and which places warm-up deletion inside infinite-horizon analysis exactly as the claim requires. The uncited Puy et al. (2022) entry on total-order estimators is cited where `scripts/compare_sobol_estimators.R`'s alternative pick-freeze estimators are described, which is what that paper compares, and the simmer JSS entry gains the retrieval date the rules require. The list is then renumbered in order of first appearance, sixty-seven entries to sixty-three, with every `[[n]]` in the text updated.

The URL verification the issue also asked for found three further paywalled entries the issue had not identified. Two are fixed as URL swaps to freely readable copies of the same articles, changing no claim: Howard et al. (2019) moves from the JAMA Network paywall to `PMC6583837`, and Zizzo et al. (2020) from Lippincott, which returns HTTP 402, to `PMC7710165`. A stray closing parenthesis inside the Kemple and Lacy DTIC URL is removed.

`scripts/check_references.R` is added to hold the list to these properties, all of which a hand renumber breaks silently and none of which any existing check tested: every `[[n]]` resolves to an entry, every entry is cited at least once, the entries are numbered from one without gaps in order of first appearance, no two entries share a URL, and each entry carries a URL and a retrieval date. It runs over all three academic documents; the two analysis documents were already sound, so for them it is a guard against regression. Whether a URL is open access is a judgement no script can make and stays a manual step at the point a reference is added, and reachability is deliberately not tested either, several unambiguously public sources among the sixty-three returning 403 to an automated request.

Two limitations are recorded rather than closed. The R checks were not executed in the authoring session, R being unavailable in the container and its installation declined, so `check_references.R` was verified by porting its seven assertions to Python and running them both ways: the repaired documents return no violation, and the pre-merge `README.md` returns exactly the defects the issue lists. And one paywalled entry remains, Hall et al. (2023) in *Military Medicine*, which has no open-access copy in PubMed Central, Europe PMC or the Oxford Academic advance-article PDF and is the sole source for the trauma registry operative-time figures in Further Development L26; removing it would delete quantitative detail that entry depends on, so it is left standing for its own issue. The Economist entry was reviewed and retained at the repository owner's direction.

**Seed-42 baseline:** unchanged. No model code, `env_data.json`, tracked diagnostic, log or image is modified.

**Unblocked by this merge:** No new issues unblocked. Nothing in Phase 6 or Phase 7 was gated on #231.
...
### Phase 1 — Statistical Foundation (Issues 1 ✓, 22 ✓, 2 ✓, 3 ✓, 24 ✓, 75 ✓, 157 ✓, 158 ✓, 189 ✓, 186 ✓, 195 ✓, 208 ✓, 331 ✓, 296 ✓, 382 ✓, 384 ✓, 387 ✓, 388 ✓, 312 ✓, 339 ✓, 405 ✓, 410 ✓)
...
6g. ~~**Issue 331** — The dwell-time KPIs drop every casualty still in the echelon when the run ends. Both dwells differenced a start attribute against an end attribute and kept the casualties carrying both, so a stay still running at the window's close was dropped; the dropped are the casualties still present, which is both the late arrivals and the long stayers. At seed 42 that was 21.6% of R2B stays and 18.0% of R2E stays, and the dropped R2B stays had already run 22 times longer than the completed mean. Both are screened Morris responses, so a parameter that lengthened dwell removed casualties from the response meant to measure it. The three intervals with an identifiable cohort are now estimated by Kaplan-Meier over everyone who entered them, reported as a restricted mean with the censored share beside it. The audit found the share is configuration-dependent rather than a constant offset, reaching 55.5% of R2E stays under `high_intensity`, and that the obvious restriction horizon is wrong: the full window would put 63% of the R2B figure beyond the last observed completion. Raised Issue 339.~~ — **Merged PR #340.**
6h. **Issue 312, partially advanced** — The blocking task on the long-horizon protocol is discharged: building the casualty trajectory retained roughly 175 MB per call that neither releasing it nor garbage collection reclaimed, which is why a replicated long-horizon run could not complete. The cause is a per-activity cost in simmer rather than in this project's code, 6.0 R cons cells per activity measured on a trajectory of nothing but timeouts and linear from 250 activities to 8,000, against 12,612 activities in the casualty trajectory. `dispatch_replications()` now gives each replication its own forked process, which returns the retention on exit and holds peak memory flat in the replication count. The protocol itself remains open. — **PR #347, non-closing.**
6i. ~~**Issue 296** — The post-operative intensive care gate was published from a before-and-after pair of single runs, which cannot separate the gate from the seed. It is re-measured at 50 paired replications per arm under common random numbers, and the two halves of the published finding part company: the gate relieves the R2E intensive care pool by 6.80 percentage points (95% CI [5.13, 8.47]), which is resolved, while its effect on died-of-wounds counts is 0.060 per run ([-0.356, +0.476]), which is not, and would need roughly 2,283 replications to resolve at the observed paired standard deviation. The gate also gains an `icu_gating.enabled` switch at both echelons, so the pre-gate model is reachable from configuration rather than only from history, with a check asserting it reproduces that model. **Merged PR #350**~~
6j. **Issue 312, partially advanced a second time** — The sustained-operations horizon now exists, is measured and is documented. `R/long_horizon.R` runs the model over the protocol's horizon and reduces each replication to a daily series inside the forked worker that produced it, so peak memory is set by one replication rather than by their sum; a full 30-replication 360-day run at both intensities holds at about 2.7 GB. The measurement splits by intensity, which corrects the issue's own premise: at moderate intensity every response converges, most within two 30-day blocks, so the 30-day window every experiment here uses is representative; at high intensity four do not, the R2E theatre queue running 85.2 in block one to 1,725.9 in block twelve and still growing at 11.3% per block over the second half, with all three R2E pools at 1.00 occupancy from block two. Casualty arrivals are flat, so this is sustained load rather than force consumption. The warm-up classification is re-derived on that basis and `WARM_UP_DAYS` remains 0, but the four drifting responses are now reported as having no steady state rather than a warm-up period of zero. Two claims made in drafting were disproved by the new check rather than by inspection and corrected in the same PR: the reduction clamped events beyond the horizon into the final day, and block one of a long run does not reproduce a standalone 30-day run at the same seed, the arrival stream being drawn over the horizon requested. The airlift sweep entry point stays with Issue #295 and a re-screen at length is unscoped, so #312 remains open. — **PR #354, non-closing.**
6k. **Issue 312, partially advanced a third time** — The last experiment the companion supplement recorded as not re-executable from a tracked command now has one. `scripts/run_airlift_collapse.R` sweeps sortie cancellation across a replicated 360-day campaign and classifies each replication rather than averaging it, the two halves of the experiment having existed separately in the long-horizon runner (the horizon, no sweep) and the airlift sweep (the sweep, a 30-day horizon, means). Re-executing it moved two of the six arms and withdrew one published finding: three of thirty campaigns collapse at 15% sortie loss rather than five and twenty-one rather than twelve at 25%, so the previous reading of 36.7% and 40.0% as the risk saturating above 20% loss is replaced by 36.7% and 70.0% on intervals that barely overlap, with the median closing holding queue moving from below 2 to 88.7 across the same step. The classifier's sensitivity is stated rather than asserted away, the highest clear campaign averaging 19.5 against the lowest collapsed one at 21.8 where the previous text claimed 17.9 against 84, and the counts hold for any threshold between 18 and 22. The classified pool's daily series is tracked, which makes the early-campaign predictor claim recomputable and confirms it across the range rather than at one arm (p = 0.42, 0.43 and 0.54 at 15%, 20% and 25% loss, 0.36 pooled). The supplement's fifth design limitation is deleted rather than narrowed. A sensitivity re-screen at length remains unscoped and the warm-up classification at that horizon rests on the block means alone, so #312 remains open. — **PR #358, non-closing.**
6l. **Issue 313, partially advanced** — The shipped 21-day evacuation policy had no sweep behind it, PR #319 having moved it from 30 on the single paired comparison this issue recorded as insufficient. `scripts/run_policy_sweep.R` sweeps the parameter across the 15 to 60 day range its doctrinal source states is a command decision, at 30 replications over 360 days per arm, pairing the arms on one control seed each so a difference is measured within replication; each arm's own spread across campaigns is large enough to hide the effects, which is what makes 30 replications sufficient. The result vindicates the shipped value on stronger grounds than it shipped on: the 7.6% to 42.1% historical in-theatre envelope excludes 15 days (4.7%), 45 (60.9%) and 60 (82.3%), and of the two admissible values only 21 is stable, 30 leaving both R2E pools at full occupancy through the closing quarter with a holding queue of 601, 301 casualties never evacuated and post-definitive intensive care access down from 35.1% to 5.5%. Two of the issue's own premises were corrected: returns to duty are not monotone in the policy, peaking at 30 days and falling at 45 and 60 because past saturation a retained casualty waits rather than convalesces, and the mortality effect that gated the decision is undetectable across the whole range, four of five paired differences being indistinguishable from zero, reported as unresolved with the 135 to 221 replications a decision would need. The runner checkpoints each arm so an interruption costs one arm rather than 150 replication-years. The holding establishment sweep, the missing hold-duration attribute and any further decision on the default remain, so #313 stays open. — **PR #360, non-closing.**

6m. ~~**Issue 382** — Assert the strategic evacuation experiment's published figures against `data/airlift/`, the four sibling experiments each having a protocol check and this one having none, which is why its section drifted from its own evidence set undetected. `scripts/check_airlift_protocol.R` checks the parameters against the supplement's marker comments, the tracked responses against the documented configurations, every figure of the paper's three tables against the tracked summary, and the interval construction against a hand-computable input; a missing table, row or column fails rather than passing quietly. The section's figures move out of prose into tables, which is what makes them checkable and the question the issue asked be settled. Validated by fault injection, the historical drift among the five faults injected. Raised #384.~~ — **Merged PR #385.**

6n. ~~**Issue 384** — Most of the twelve experiments the companion paper publishes had no tracked evidence set at all, the comparative scenario analysis among them, so their figures could be audited only by re-running them. The first three are closed: `scripts/run_scenarios.R`, `scripts/run_transport_sweep.R` and `scripts/run_icu_share_sweep.R` gain `--refresh-baseline` writing the tracked `data/scenarios/` and `data/sweeps/`, each flag fixing its protocol rather than accepting the caller's arguments, and two protocol checks assert the published tables against them, both validated by fault injection. The audit the issue asked for is recorded in the supplement along with the rule deciding which figures need a set, the three experiments still owed one, the three exempt, and the reasoning for not sharing a table parser between the checks. Re-measurement found the predicted drift: the scenario queue table had mixed per-bed averages with pool totals, printing R2E intensive care at 0.078 against a measured 1.229, while the paper's own abstract already carried the correct ratios. Raised #387, #388 and #389 for the three that remain.~~ — **Merged PR #390.**

6o. ~~**Issue 387** — The R2B pre-open hold window's eight-row table had no tracked evidence set, its entry point being `run.R` under a parameter override rather than a driver script. `R/hold_window.R` and `scripts/run_hold_window.R` give it one, tracking both arms' per-replication responses and their paired differences in `data/hold_window/` at 50 replications per arm, and `scripts/check_hold_window_protocol.R` asserts the parameters, the tracked set and every published figure against the measurement, validated by fault injection. Regenerating the evidence set moved the figures: two of the eight paired differences clear zero on the fresh sample rather than three, the R2E theatre-entry-deferred effect no longer among them, and the section's headline claim and the supplement's replications-needed figures are restated from the tracked data. 50 replications reproduces the published table's shape rather than the roughly 120 that would also resolve the forward-surgery row, closing the audit gap the issue was raised for.~~ — **Merged PR #392.**
...
6t. **Issue 405, partially advanced** — The audit task #405 asks for first: every one of the thirteen replicated experiments' current replication count and 30-day horizon is checked against the sustained-operations protocol's finding that four R2E-adjacent responses do not converge within it, and a new section of `docs/Multi_Run_Supplement.md` records the result for the nine experiments the sustained-operations protocol and the sensitivity re-screen audit had not already covered. Three (the R2B pre-open hold window, the post-operative intensive care gate, the transport fleet-size sweep) are decided migration candidates, each already flagged as under-resolved at 30 days on an event count a 360-day horizon would directly address, and each is costed at roughly 2.3x, 2.3x and 11.4x the current run against the measured 3.8x per-replication runtime ratio; the remaining six are retained with the reasoning recorded per experiment. None of the three migrations is executed, and the re-runs and the corresponding `docs/Multi_Run_Analysis.md` updates remain, so #405 stays open. — **PR #411, non-closing.**
6u. **Issue 405, partially advanced a second time** — Two of the three migration candidates #411 costed are executed: `R/hold_window.R` and `R/icu_gate.R`'s protocol constants move from 50 replications x 30 days to 30 x 360, both re-run under `--refresh-baseline`, and `docs/Multi_Run_Analysis.md`'s Option 3 table and Post-Operative Intensive Care Gate section, both design entries and the Standardising the Other Experiments table in `docs/Multi_Run_Supplement.md` are updated to match. The measurement does not bear out the migration's own rationale: most count-based responses in both experiments become harder to resolve at the longer horizon rather than easier, the R2E first-surgery row's replications-needed rising from 838 to roughly 24,655 and the ICU gate mortality row's from 35 to roughly 471, because a longer campaign gives two arms' event orderings more time to diverge from a shared seed rather than giving the comparison more independent events to average over; this holds even for the ICU gate, whose arms were expected to carry none of the hold window's stream-shifting penalty. The one response type to improve in either experiment is a time-weighted share (`icu_occupancy`, reps-needed 50 to 6) rather than an accumulating count, and the R2B-surgeries row also resolves, but because its own signal accumulated faster than its noise rather than because the noise fell. The Standardising the Other Experiments text is rewritten to report this measured pattern rather than the pre-registered prediction. The transport fleet-size sweep, the third candidate, remains costed but unexecuted, now flagged for re-costing against a measured rather than an assumed variance before it is attempted. #405 stays open. — **PR #413, non-closing.**
6v. ~~**Issue 405** — The transport fleet-size sweep, the third migration candidate #411 costed, is executed, and during review the decision widened: rather than leaving it alone at the sustained-operations protocol while the forward ICU share frontier and the R2B holding threshold sweep stayed at their own 30-day designs, all three capacity sweeps are migrated together so a reader comparing their tables compares one horizon rather than three. `TRANSPORT_SWEEP_REPLICATIONS`, `ICU_SHARE_SWEEP_REPLICATIONS` and `HOLD_THRESHOLD_SWEEP_REPLICATIONS` move to 30 and the shared `CAPACITY_SWEEP_DAYS` to 360 (`R/analysis.R`), all three sweeps re-run under `--refresh-baseline` at both the shipped and `high_intensity` configurations where applicable, and every dependent artifact updated: `data/sweeps/`, the four tracked sweep images, `docs/Multi_Run_Analysis.md`'s transport, forward ICU share and R2B holding threshold sections, all three design sections in `docs/Multi_Run_Supplement.md`, and `CLAUDE.md`. Two qualifications the smaller designs carried are genuinely resolved rather than merely restated at a new horizon: transport utilisation, which ran the wrong way on both platforms at 10 replications and was excluded from the paper's reading entirely, now falls monotonically with fleet size at both casualty intensities (36.4% to 7.3% for ambulances at moderate intensity, 92.0% to 18.3% at high intensity); and the R2B holding threshold sweep's returns-to-duty and died-of-wounds responses, neither of which separated across the grid at 10 replications, now both clear their neighbours at the one-day threshold against the disabled arm. The forward ICU share frontier and the R2B holding threshold sweep were migrated for cross-table consistency rather than because either was itself under-resolved, which is documented explicitly rather than left implicit. A stale limitation naming the transport and ICU share sweeps' utilisation columns as undetermined is retired from `docs/Multi_Run_Supplement.md` rather than marked resolved, along with two other passages that still described the pre-migration reading as current. All three costed migration candidates from #411's audit are now executed, closing #405.~~ — **Merged PR #415.**
6w. ~~**Issue 410** — The two coverage gaps Issue #348 raised alongside its cache-schema defect, and left orphaned when PR #398 fixed the defect alone, are closed: `r2b_hold_q`/`r2e_hold_q` join the screened responses, and `r2e_icu_beds`/`r2e_hold_beds` join the screened parameters via a new `apply_bed_establishment_params()` that regenerates the R2E team's bed-identifier resource vectors rather than writing the `vars` tree every other parameter uses. The full r = 20 Morris screen is re-run at the resulting 80-parameter, 1,620-point design and published to `data/sensitivity/morris_r20/`, replacing #339's 78-parameter screen; `r2e_icu_beds` leads its own `r2e_icu_q` response, the only response in the tracked set where a fixed establishment count outranks every casualty-load or clinical-probability parameter.~~ — **Merged PR #417.**
...
15f. ~~**Issue 365** — Allow strategic evacuation between the abbreviated operation and the definitive repair at R2E. `r2eheavy.second_surgery.saturation_queue_threshold` releases a casualty awaiting their definitive repair to strategic evacuation once the number waiting to be operated on reaches it, the trigger being theatre saturation rather than a drawn share. The decision draws nothing and ships at zero, which disables it, so the seed-42 evidence set reproduces byte for byte; a released casualty holds an intensive care bed on the critical airlift route, not a holding bed. The non-zero default is blocked on #368, without which the released casualty's outstanding operation leaves the model at boarding.~~ — **Merged PR #371.**

15g. ~~**Issue 376** — Enable both national support base levers and re-measure the replicated evidence. The reconstruction cohort ships at its calibrated 0.2 and the saturation threshold at 8, swept before it was set because no source names one; six replicated evidence sets re-measured once rather than twice, and five published findings changed in substance. Added `scripts/run_saturation_sweep.R`. Requires Issues 365, 368 and 373.~~ — **Merged PR #379.**

15h. ~~**Issue 362** — Bound a casualty's strategic evacuation staging stay by the release of its bed rather than by its departure. Only boarding sets a departure time, so a casualty who died awaiting a sortie was charged the whole unserved remainder of the campaign, which is why the residual was always an over-count. `ame_hold_end` is set on both routes out of the wait, and `compute_ame_backlog()` is corrected for the same defect and for a terminal-extension error found with it. Analysis-side and draw-free, so seed 42 reproduces byte for byte; `data/airlift/` regenerated, all 650 replications now closing exactly. Added `scripts/check_ame_backlog_exit.R`; raised #382. Requires Issue 313.~~ — **Merged PR #381.**
...
14a. ~~**Issue 313** — Review the R2E holding establishment and evacuation policy defaults, and the README sections resting on them. Raised because long-horizon measurement indicated the shipped configuration was over-subscribed at length and deliberately proposed no value. Delivered as two sweeps rather than a defaults change: the evacuation policy across the doctrinal 15 to 60 day range, and the holding establishment at 30, 45, 60 and 90 beds, both at 30 replications over 360 days per arm with the arms paired on one control seed. Neither default moves, and both now rest on a sweep: 21 days is the only setting in the range both admissible against the historical in-theatre envelope and stable at length, and at that policy the pool stops binding 15 beds above its 30-bed establishment. The holding occupancy split was closed exactly on the way, which withdrew the published claim that only one evacuation route stages in holding beds. Mortality stays unresolved with the replication count a decision would need, and the national support base capacity question the issue scoped out remains with #314.~~ — **Merged PRs #360 and #363.**

### Phase 4 — Scenario Expansion (Issues 54 ✓, 9 ✓, 10 ✓, 57 ✓, 18 ✓, 23 ✓, 124 ✓, 160 ✓, 153 ✓, 148 ✓, 203 ✓, 206 ✓, 149 ✓, 151 ✓, 207 ✓, 295 ✓, 314 ✓, 368 ✓, 373 ✓, 300 ✓)
...
15j. ~~**Issue 295** — Every published finding about strategic aeromedical evacuation and the national support base came from one campaign, the only section of the multi-run paper whose figures carried no confidence interval. The response set is now measured at 50 replications in each of thirteen configurations, two baselines and eleven swept values, and the measurement refines the single-campaign reading rather than confirming it. Timing matters more than airframe-days: two sorties at a 10-day interval give a mean wait of 2.30 days and two at a 14-day interval give 5.51, so the interval is a stronger lever than reliability. The evacuation wait consumes a holding bed on the standard route alone, the critical route holding an intensive care bed instead, so charging both to the holding pool would overstate the coupling more than fourfold. And national support base demand peaks 1.36 days before the campaign ends rather than after it, the single campaign having seen the tail decay rather than the peak arrive late. **Merged PR #356**~~

16a. ~~**Issue 314** — Make Role 4 demand configurable and reportable against a stated capacity, without modelling the echelon. Delivered as a narrowing rather than an addition. The ward mapping is configured and the stay steps down from intensive care on the conservation contract the model already applies forward, calibrated to a Role 4 source rather than assumed. The capacity half is withdrawn: the model sets the demand a deployed trauma system places on the national support base and does not simulate that echelon, so a stated establishment and any shortfall against it are removed and their absence asserted by the gate. The census reports the bed types the echelons field, `icu` and `hold`, two of the four carrying no Role 4 demand and being absent rather than zero. Stage 2, Role 4 as a seized resource with back-pressure into theatre, was never in scope.~~ — **Merged PRs #366 and #369.**

16b. ~~**Issue 368** — Role 4 surgical activity is not represented, so its demand is understated. The operation a casualty released with the definitive repair outstanding carries rearward is now reported in operations and theatre-minutes under the `ot` label, conserved from the draw the releasing theatre took rather than re-estimated, and assigned rather than seized so the echelon stays unsimulated. Fixed the intensive care such a casualty was owed and never given. Scoped to that cohort; the staged reconstruction a repatriated casualty undergoes is split to #373, blocked on the evacuation decision carrying a reconstruction burden.~~ — **Merged PR #374.**

16c. ~~**Issue 373** — Role 4 staged reconstruction is unrepresented, and the evacuation decision carries no reconstruction burden. The sequence is now interval-driven, on the Royal Centre for Defence Medicine's 48 to 72 hour return protocol, so the procedure count follows from the debridement phase rather than being configured; the cohort is selected on the wound at 18.8% of the open wounds injury profile rather than on expected recovery, which would read the severity classification the evacuation decision already reads; and reconstruction becomes a third reason to evacuate, recorded in `evacuation_reason`. Ships disabled, both national support base levers being flipped together in #376.~~ — **Merged PR #377.**

16d. ~~**Issue 300** — Sweep the transport fleet-size at high casualty intensity, on `plot_transport_capacity_margin_by_fleet_size()`'s existing moderate-intensity design. The boundary between an inadequate and an adequate fleet does not move under surge; three ambulances and four trucks remain sufficient, so the qualification that Option 4's evidence rests on one casualty rate is removed.~~ — **Merged PR #402.**

### Phase 5 — Interface (Issues 72 ✓, 93 ✓, 15 ✓, 77 ✓, 110 ✓, 111 ✓, 121 ✓, 109 ✓, 128 ✓, 117 ✓, 112 ✓, 114 ✓, 116 ✓, 115 ✓, 154 ✓, 152 ✓, 147 ✓, 201 ✓, 155 ✓, 337 ✓, 301 ✓)
...
30c. ~~**Issue 337** — Harden the CI system-library install against archive stalls. The step ran `apt-get` with no retry configuration, no transport timeout and no per-step timeout, so a stalled Ubuntu archive mirror hung it until the job timeout fired, 60 minutes for three of these jobs and 180 for the slow suite, reporting a failure naming nothing useful; one run was observed wedged past 25 minutes and cancelled by hand. Each of the four copies now writes an `apt.conf.d` fragment before `apt-get update` and carries a 15 minute step timeout. The guards are inline rather than in a composite action because `timeout-minutes` is not a step property a composite action supports, so `scripts/check_ci_apt_guards.R` asserts the four copies stay identical and each guards the update as well as the install.~~ — **Merged PR #345.**
30d. ~~**Issue 301** — Every figure in the analysis pipeline collapsed a campaign to a scalar, which cannot tell a queue that recurs in peaks and clears between them from one that never clears; those are different resourcing problems, and the paper's first recommendation assumed the second without establishing it. Queue length by resource pool and the degraded post-operative care rate are now reported as time series across 50 replications per casualty intensity, as the median with an interquartile band. At high intensity the R2E theatre queue stands empty for 5% of the campaign, runs unbroken for 26.9 of the 30 days and is still growing on day 29; at moderate intensity it stands empty for 80% with a longest unbroken run of 3.0 days. Measurement and rendering are separated, an ordinary invocation rendering from the tracked series alone, so both figures reproduce byte for byte without running the model. **Merged PR #352.**~~

### Phase 6 — Code Quality and Verification (Issues 230 ✓, 231 ✓, 232 ✓, 233 ✓, 234 ✓, 236 ✓, 237 ✓, 235 ✓, 252 ✓, 241 ✓, 262 ✓, 270 ✓, 310 ✓, 316 ✓, 342 ✓)
...
4. ~~**Issue 233** — Make `analyse_run()` stream-neutral, the analysis pipeline currently consuming random draws and so not being idempotent. Not a prerequisite for Issue 241, though landing it first removes a source of confusing byte-comparison failures there. The stream is saved and restored around the draw rather than reseeded, so the draw stays where it was and no published figure moves; `scripts/check_analysis_idempotence.R` holds both entry points to it, and the post-hoc placement of the draw is recorded as Further Development L30.~~ — **Merged PR #255.**
...
6. ~~**Issue 236** — Make global configuration save and restore exception-safe, and add input validation to `R/analysis.R`. The restore is registered with `on.exit(..., add = TRUE)` at the point of save in both capacity sweeps, both screens and `run_scenario()`, which had no restore at all, so a run that fails partway can no longer leave the session on the swept configuration and report the next run as the baseline. Validation is added at the four analysis entry points and at `app.R`'s two configuration-loading boundaries, naming the missing column or offending field. Two regression checks hold both; the argument-threading alternative is recorded as Further Development L31 rather than implemented.~~ — **Merged PR #257.**
7. ~~**Issue 237** — Housekeeping: delete the ten `wip/*` branches, reconcile the Further Development scan table against its entries, and close the drift between `CLAUDE.md` and the repository it describes. The scan table disagreed with three of its own entries and is reconciled to them, `scripts/check_markdown.R` now asserting the correspondence so the drift cannot recur; entry L22 is trimmed from four paragraphs to the one-paragraph length of its neighbours. The remote held twelve `wip/*` refs rather than ten, all verified redundant against `data/sensitivity/` and all deleted, which closes Issue #227 with it. The `CLAUDE.md` omissions were already closed.~~ — **Merged PR #259.**
...
9. ~~**Issue 241** — Apply the code standard: decompose `server`, `analyse_run` and `analyse_replications` behind `testServer` and Playwright verification, verify the analysis pipeline byte for byte against the tracked seed-42 baseline, replace the ninety-three literal `1440`s with the named constant, and assess `r2e_treat_wia` for a provably stream-neutral decomposition. Requires Issues 234, 235 and 230. Seven pull requests, six of seven acceptance criteria met: the two console suites exist and gate every PR, the two analysis entry points, `server` and both screen drivers are decomposed and verified, the literal `1440` is gone, and `r2e_treat_wia` is assessed and recorded. The seventh is unmet, nine functions exceeding the limit against eighteen at the start, and the issue was closed on that basis rather than held open; Issue 270 takes the residual up. Raised Issues 262 and 270 along the way.~~ — **Merged PRs #261, #263, #264, #265, #266, #267 and #269.**
9a. ~~**Issue 262** — `scripts/check_markdown.R` aborting outside a UTF-8 locale, its Further Development scan table pattern carrying a literal em dash that R cannot translate in a C locale, so the check exited non-zero having asserted nothing rather than reporting. Both `regexec()` calls now match byte-wise, the treatment the same file already applied to its table-of-contents writer, and the check asserts its own entry-heading match is byte-wise so the property cannot regress silently. Continuous integration was never affected, the pinned container setting a UTF-8 locale; it reached a contributor running the suite in a bare shell, which is how it was found.~~ — **Merged PR #268.**
9b. ~~**Issue 270** — Split the five simulation-logic functions still over the length limit: `r2e_treat_wia` (766 lines), `r2b_treat_wia` (560), `build_casualty_trajectory` (334), `run_once` (147) and `run_replications` (116). The deferral Issue 241 recorded, that a split shifts the random number stream and so moves every published figure, no longer holds as a reason to wait: `scripts/check_baseline_reproduction.R` settles it byte for byte in 44 seconds and gates every pull request. What actually stops them is interface design, a lifted sub-trajectory no longer closing over its parent's locals, which is the obstacle the screen drivers posed and which PR #269 established a method for. Four of the five already carry their decomposition as named locals or as a chain of composable segments. One pull request per function, cheapest interface first. All five are split, the `function_length` count falling from nine to four, which is the count D1's conformance entry records as deliberate; every pull request reproduces the seed-42 evidence set byte for byte and matches `main` output for output.~~ — **Merged PRs #272, #273, #274, #275 and #276.**

9c. ~~**Issue 310** — Complete `run.R`'s command line surface and validate it at the boundary. The entry point could not express a scenario profile at all, a single run under one being reachable only through the comparative multi-replication runner or the Shiny console, and it validated none of its own arguments: `--days 0`, `--iterations 0` and a `--warm-up` at or beyond the run length were accepted, a full simulation attempted, and the failure raised inside the analysis pipeline naming a monitoring data frame the user never referred to. Adds `--scenario`, `--mode`, `--images-dir` and `--max-cores`, and `--seed` and the directory switches to `scripts/run_warmup.R`, whose measurement had no way to be reproduced from the command line. The rules live in a new `R/cli.R` so the regression check can assert them without spawning a process per case, which was measured at over two minutes against that check's 16 seconds. Also closes an artifact-policy leak, `scripts/run_warmup.R` having overwritten the tracked Welch figure by default. The seed-42 evidence set reproduces byte for byte and both ratchets hold at their baselines.~~ — **Merged PR #317.**

9d. ~~**Issue 316** — A run producing no return to duty crashes the analysis pipeline on an absent attribute column. The wide pivot of the attributes monitor carried a column only where some casualty set that attribute, so the frame's shape depended on what the run produced and analysis code reading a column directly failed with an object-not-found error where it produced none. Fixed as a guarantee rather than as a guard at each of roughly 150 call sites: `MODEL_ATTRIBUTE_KEYS` lists all 77 keys the model can set and `build_attributes_wide()` adds each as an all-NA column where the run set none. The hand-written list it replaces had already proved incomplete, a second list of fourteen further keys standing in `R/sensitivity.R` for the same reason, and that one is deleted with the 36 sensitivity responses unchanged. Validated by fault injection, four of the new check's ten assertions failing against the previous key list. No simulation output moves and no baseline refresh was required. Raised Issue 327.~~ — **Merged PR #326.**
9e. ~~**Issue 342** — Nothing asserted that the two tables claiming to index the codebase actually do. `CLAUDE.md`'s Repository Structure table says it covers the repository as a whole and the README's Codebase Structure table is what it points a reader at for each `R/` module, and both had drifted: `R/cli.R` had never had a README row despite being named in the README's own prose, and PR #340 had added two files without rows, corrected by hand afterwards. A missing row breaks nothing that runs, so only a reviewer noticing ever caught one. The new check asserts every `R/` module has a row in both tables, every `scripts/check_*.R` and other `scripts/` entry point has one in `CLAUDE.md`'s, and neither table names a path that no longer exists; it failed on `R/cli.R` the day it was written, and covers its own registration.~~ — **Merged PR #343.**

### Phase 7 — Publication (Issues 238 ✓, 239 ✓, 240 ✓, 311 ✓, 315 ✓, 327 ✓, 333 ✓, 335 ✓)
...
1. ~~**Issue 238** — Re-cut the two analysis papers by method, moving the replicated experiments out of the single-run paper so each document reports one class of evidence. Settles what each of the other two issues is polishing, so it gates both. Five experiments move rather than the four the issue enumerated, each gaining a design statement, and the single-run paper is retitled to name its subject rather than its method.~~ — **Merged PR #280.**
2. ~~**Issue 239** — Bring `docs/Single_Run_Analysis.md` to publication standard: remove the twenty-two issue-number references and three blockquoted maintainer notes, add a methods section and a limitations section cross-referencing the README's `L` entries, verify every numeric claim against the current baseline, and clear the presentation defects. The figure-verification pass was executed rather than reasoned about, and found eight figure descriptions disagreeing with the plots they describe alongside the changelog voice the issue enumerated.~~ — **Merged PR #282.**
3. ~~**Issue 240** — Bring `docs/Multi_Run_Analysis.md` to publication standard: complete the statistical specification and remove the supersession narrative, and settle whether its title should name the wider scope PR #280 gave it. The title is settled as Replicated Experiments and Comparative Scenario Analysis, the queue table gains the intervals it never carried from a re-measurement that reproduces every published figure, and the power analysis moves out of `CLAUDE.md` into the paper's methods.~~ — **Merged PR #284.**
5. ~~**Issue 315** — The R2E bed queue figure omits the holding beds, the one pool the papers identify as saturated. `plot_r2e_bed_queues()` named the two bed types it plotted, so a third was excluded by construction while `plot_r2b_bed_queues()` selected on a wildcard, one pipeline plotting the two echelons to different rules. R2E now follows R2B's rule and all four pools appear. The panels share one vertical scale rather than the free scales the issue recommended, that recommendation's premise not surviving measurement: a free scale draws a pool never exceeding one casualty exactly as prominently as one reaching fourteen, which is the omission's own harm by another route. A new check asserts the rule against a bed type no configuration ships, and found two further defects on the way, an unasserted fourth R2B bed type and a colour scale that stops drawing past its palette's eighth. Only the tracked image moves.~~ — **Merged PR #329.**
6. ~~**Issue 327** — The R2B hold occupancy figure omits every episode still running when the run ends. A casualty leaves an R2B holding bed by three routes and only one sets `return_day`, so the reconstruction dropped the episodes still open and charged the evacuated ones to a day set later at another echelon. Both are fixed by one shared reconstruction, replacing two copies that had already drifted. The second error was the larger and unknown: under the evacuation threshold the papers recommend, the figure reported 303.2 holding bed-days against a true 172.9. The corrected reconstruction agrees with the resource monitor at both configurations, which is the independent measurement the new check cross-references. The figure's axis label and its capacity line were corrected with it, each R2B unit now carrying its own panel. Raised Issues 331, 333 and 335.~~ — **Merged PR #332.**
6a. ~~**Issue 333** — Resolve the two R2B holding thresholds, one name for two levers. `hold_threshold` is an occupancy fraction read at Role 1 deciding whether a casualty is sent to an R2B unit at all; `evac_threshold` is a duration read at R2B deciding how long they may stay once there. A function named `r2b_hold_threshold()` read the second while the Morris parameter of that name sets the first, so a reader tracing the published ranking name into the source landed on the other lever. Nothing was computed wrongly; the cost was the next change. The function is renamed for the field it reads, and `evac_threshold`, which had no `env_data.json` entry at all and so could not be set through any tracked entry point, gains one shipping at the disabled value and a Shiny registry entry beside it. Setting it alone moves 46 of 65 holding casualties onto the early-evacuation branch. Unblocks Issue 335.~~ — **Merged PR #334.**
6b. ~~**Issue 335** — Sweep R2B holding capacity jointly against the evacuation threshold #333 gave a configuration entry, and decide whether that threshold joins the Morris screen. Ten beds per unit clears the forward R2B holding queue with no measured cost to R2E holding or intensive care; the threshold only matches that at an aggressive one-day setting, and does so by loading R2E holding instead. `evac_threshold` stays outside the screened set, the swept response confirming the discontinuity at its disabled default a screening elementary effect would otherwise measure as an on/off switch.~~ — **Merged PR #400.**
4. ~~**Issue 311** — Scope the single-run paper's inferences to the horizon that supports them. The paper drew two conclusions about the system's behaviour over time from one 30-day run: that intensive care saturation is a standing property of the establishment rather than something the campaign develops into, and that the planner's lever at the strategic evacuation decision is schedule resilience rather than cabin fit. A rescoping rather than a recomputation, no figure being in question. The second claim and the "busy but not saturated" heading had already gone with PR #319's rewrite of the sortie reliability passage, which is recorded on the issue rather than assumed; the first is replaced by a statement of what the horizon leaves open, cross-referencing the replicated evidence on the rationing rule. The cross-reference to a long-horizon measurement the issue also asks for is left undone, no such run being reproducible by a documented command.~~ — **Merged PR #324.**
...
COMPLETE (merged to main):
  ...
  #231 README reference list repaired — the misattributed duplicate removed,
       the second duplicate merged, both open-access violations re-sourced
  ...
  #252 The suite narrows on a documentation-only PR: a classification job
       ...
       value moves (PR #253)
  #227 The twelve wip/* checkpoint refs are deleted from the remote, closed
       as completed under #237 rather than worked separately (PR #259)
  #237 The Further Development scan table is reconciled to its entries and
       check_markdown.R asserts the correspondence, so a row can no longer
       name a gap or an impact differently from the entry it points at.
       Entry L22 trimmed from four paragraphs to one, the replication-count
       derivation it duplicated having its single home in CLAUDE.md. The
       twelve wip/* refs verified byte-identical to data/sensitivity/ or
       superseded, and deleted, which closes #227 with it. No baseline value
       moves (PR #259)
  #236 The configuration globals are restored on the error path.
       capture_config_globals()/restore_config_globals() are registered with
       on.exit(add = TRUE) at the point of save in both capacity sweeps, both
       screens and run_scenario(), which had no restore at all, so a failed
       sweep can no longer leave the session on the swept configuration and
       report the next run as the baseline. Entry-point validation added to
       analyse_run(), analyse_replications(), the two sweeps and app.R's two
       configuration-loading boundaries, naming the missing column or field.
       check_config_restore.R (seven assertions, all seven fail without the
       fix) and check_input_validation.R (twenty) are added; the
       argument-threading alternative is Further Development L31. Seed-42
       reproduces byte for byte (PR #257)
  #233 The analysis pipeline is stream-neutral, so analysing one run twice
       gives one answer. with_preserved_rng() saves and restores
       .Random.seed around the Role 4 length-of-stay draw and the mass
       casualty timeline's jitter takes an explicit seed; no seed is set
       around the draw itself, so it stays at the stream position it held
       and no published figure moves. check_analysis_idempotence.R holds
       both entry points to it and fails on six of eight assertions against
       the unfixed pipeline. Post-hoc placement of the draw recorded as
       Further Development L30. Seed-42 reproduces byte for byte (PR #255)
  #241 The code standard applied. The Shiny console's two test suites added
       and wired into the gate; analyse_run() 1,418 lines to 398,
       analyse_replications() 810 to 229, server 2,293 to 195, run_sobol()
       240 to under 100, run_morris() 212 to under 100 and extract_kpis()
       205 to 126, behind roughly a hundred named functions; the literal
       1440 from 122 occurrences to one; functions over the length limit
       from eighteen to nine. Each decomposition verified by artifact or
       response comparison, and the screen drivers behind
       check_screen_order.R, written before either was touched and
       validated by fault injection. Three checks added that read binding
       structure rather than behaviour, covering fault classes no artifact
       comparison can see. Closed with its length criterion unmet; Issue
       270 carries it forward. Seed-42 reproduces byte for byte on every
       merge (PRs #261, #263, #264, #265, #266, #267, #269)
  #262 check_markdown.R's scan table check no longer aborts outside a UTF-8
       locale. Both regexec() calls match byte-wise and the check asserts
       its own entry-heading match is byte-wise, so a future non-ASCII
       pattern cannot reintroduce the fault silently (PR #268)
  #238 The two analysis papers are re-cut by method: five replicated
       experiments move to the multi-run paper, each gaining a design
       statement naming its replication count, seed and non-default
       parameters, and the single-run paper is retitled Model Verification
       and Baseline Behaviour to name its subject rather than its method.
       Documentation only; no published value moves (PR #280)
  #239 The single-run paper is brought to publication standard: the
       changelog voice removed, a methods and a limitations section added,
       the reference list taken from three entries to sixteen, and every
       numeric claim and figure description verified against a run in the
       pinned container. Eight figure descriptions disagreed with their
       plots and are corrected. Documentation only; the seed-42 evidence
       set and all thirty-one figures reproduce byte for byte (PR #282)
  #270 The five simulation-logic functions over the length limit are
       split, one PR each: run_once (147) into four assembly steps,
       build_casualty_trajectory (334) into phase builders, r2b_treat_wia
       (560) and r2e_treat_wia (766) into one builder per step and per
       phase with their locals lifted to file scope, and run_replications
       (116) into a captured generator state, the dispatch and the
       killed-worker filter. functions over the length limit from nine to
       four, which is the count D1 records as deliberate. Seed-42
       reproduces byte for byte on every merge, and each PR was also
       compared output for output against main (PRs #272, #273, #274,
       #275, #276)

  #240 The multi-run paper is brought to publication standard: an
       introduction, a methods section stating the interval method and the
       independence argument, and a limitations section added; the
       supersession narrative removed; both results tables given consistent
       uncertainty, the queue table gaining intervals from a re-measurement
       that reproduces every published figure. Retitled Replicated
       Experiments and Comparative Scenario Analysis. Closes Phase 7 (PR
       #284)
  #310 run.R's command line surface completed and validated at the boundary:
       --scenario, --mode, --images-dir and --max-cores added, --seed and the
       directory switches added to scripts/run_warmup.R, and every argument
       checked before any simulation runs. Rules extracted to R/cli.R so the
       regression check asserts them without a process per case (PR #317)
  #311 The single-run paper's two inferences about behaviour over time scoped
       to the 30-day window that supports them, the intensive care claim
       rewritten and cross-referenced to the replicated evidence and the
       evacuation policy table given the dimension it does not report. The
       second claim had already gone with PR #319 (PR #324)
  #316 The analysis pipeline no longer depends on what a run happened to
       produce: MODEL_ATTRIBUTE_KEYS lists all 77 attribute keys the model can
       set and build_attributes_wide() adds each as an all-NA column where the
       run set none, so a one-day run analyses end to end. A second, redundant
       column list in R/sensitivity.R is deleted with its 36 responses
       unchanged. Raised #327 (PR #326)
  #315 Every R2E bed type's queue is plotted, the holding pool included, both
       echelons now selecting bed resources by pattern and deriving the type
       from the name. The panels share one vertical scale, a free one drawing
       a pool queueing one deep as prominently as one queueing fourteen
       (PR #329)
  #327 Each R2B holding episode is bounded by the attribute its own exit route
       sets, rather than all three by return_day, so an episode still running
       at the window's close is counted and one ended by onward evacuation is
       charged the bed time it served. Cross-checked against the resource
       monitor. Raised #331, #333 and #335 (PR #332)
  #333 The two R2B holding thresholds are separated: r2b_evac_threshold() is
       named for the field it reads, and evac_threshold gains the
       env_data.json field and Shiny lever it never had, shipping disabled.
       Unblocks #335 (PR #334)
  #331 The two dwell KPIs and the transit between them are estimated by
       Kaplan-Meier over everyone who entered the echelon, reported as a
       restricted mean with the censored share beside it, rather than as a
       mean of the stays that closed. Raised #339 (PR #340)
  #342 The two tables claiming to index the codebase are asserted to do so:
       every R/ module, check script and scripts/ entry point has a row, and
       neither table names a path that no longer exists. Found R/cli.R
       missing from the README table on the day it was written (PR #343)
  #296 The post-operative intensive care gate is re-measured at 50 paired
       replications per arm: the load relief is resolved at 6.80 pp
       (95% CI [5.13, 8.47]) and the mortality effect is not, spanning zero
       and needing ~2,283 replications. Adds the icu_gating.enabled switch
       that makes the pre-gate model reachable from configuration (PR #350)
  #337 Every CI system-library install retries a failed fetch, bounds a
       silent connection at 30 seconds and is cut off at 15 minutes rather
       than hanging until the job timeout. Applied inline in all four jobs,
       timeout-minutes not being a composite action step property, with a
       check asserting the copies stay identical (PR #345)
  #295 Strategic evacuation and Role 4 demand are measured at 50 replications
       across thirteen configurations. Timing beats airframe-days: two
       sorties at a 10-day interval wait 2.30 days against 5.51 at 14. The
       evacuation wait holds a bed on both routes, corrected by PR #363, and
       demand peaks 1.36 days before the campaign ends rather than after
       (PR #356)
  #301 Queue length and the degraded post-operative care rate are reported as
       time series across 50 replications per intensity, settling whether the
       R2E theatre queue clears: at high intensity it stands empty for 5% of
       the campaign and is still growing on day 29, at moderate for 80% with
       a longest busy run of 3.0 days. Closes item 8 of the multi-run paper's
       Further Development table and raises a new one for the horizon (PR #352)
  #384 The comparative scenario analysis and the two capacity sweeps gain
       tracked evidence sets and protocol checks, the paper's centrepiece
       having existed nowhere in the repository. Three entry points gain
       --refresh-baseline, each fixing its own protocol rather than the
       caller's arguments. The predicted drift was found on re-measurement:
       the scenario queue table had mixed per-bed averages with pool totals,
       printing R2E intensive care at 0.078 against a measured 1.229, with
       the paper's own abstract already carrying the correct ratios. The rule
       deciding which figures need a set, the audit against it and the
       shared-parser decision are recorded in the supplement. Raised #387,
       #388 and #389 (PR #390)
  #387 The R2B pre-open hold window's eight-row table gains a runner
       (`R/hold_window.R`, `scripts/run_hold_window.R`), a tracked evidence
       set at 50 replications per arm (`data/hold_window/`) and a protocol
       check, its entry point having been `run.R` under a parameter override
       rather than a driver script. Regenerating the table on the fresh
       sample moved it: two of eight paired differences clear zero rather
       than three, the R2E theatre-entry-deferred effect no longer among
       them, and the headline claim and the supplement's
       replications-needed figures are restated from the tracked data.
       Closes the audit item #384 raised (PR #392)
  #388 The post-operative intensive care gate section gains a runner
       (`R/icu_gate.R`, `scripts/run_icu_gate.R`) on #387's arrangement
       rather than sharing its module, a tracked evidence set at 50
       replications per arm (`data/icu_gate/`) and a protocol check
       alongside the existing mechanism check. Regenerating the evidence
       set moved every figure: R2E intensive care utilisation falls 5.25
       points with the rule rather than 6.80, and the mortality point
       estimate changes direction, now costing a fraction of a life rather
       than saving one as the design predicts, with the interval still
       spanning zero. Closes the audit item #384 raised (PR #394)
  #389 The mass casualty stress test's four-row table gains a runner
       (`R/mass_casualty.R`, `scripts/run_mass_casualty.R`) on #387's
       arrangement but unpaired, a tracked evidence set at 62 replications
       per arm (`data/mass_casualty/`), the count derived rather than kept
       at 10, and a protocol check alongside the existing mechanism check.
       Regenerating at the derived count resolves the event-versus-ordinary
       died-of-wounds comparison for the first time (140 deaths against 91,
       non-overlapping intervals), upgrading it from direction only to
       measured, while the arm-to-arm comparison on ordinary casualties
       alone stays unresolved. Closes the last audit item #384 raised
       (PR #396)
  #382 The strategic evacuation experiment's published figures are asserted
       against `data/airlift/`, the four siblings each having had a protocol
       check and this one none, which is how its section drifted from its own
       evidence set undetected. The section's figures move into three marked
       tables so they can be checked at all, and the check is validated by
       fault injection rather than inspection. Raised #384 (PR #385)
  #362 A casualty's strategic evacuation staging stay is bounded by the
       release of its bed rather than by its departure, which only boarding
       sets, so a casualty who died awaiting a sortie is no longer charged
       the campaign's unserved remainder. The same defect is fixed in the
       published backlog series, along with a terminal-extension error found
       with it. Analysis-side and draw-free: seed 42 reproduces byte for
       byte and all 650 tracked replications now close exactly. Added
       `scripts/check_ame_backlog_exit.R`; raised #382 (PR #381)
  #376 Both national support base levers ship in force, the saturation
       threshold swept at the sustained horizon before it was set and
       anchored on the theatre queue an unrelieved campaign carries. Six
       replicated evidence sets re-measured once rather than twice, and five
       published findings changed in substance: a resolved mortality cost at
       the 30-day policy, the collapse cliff moving a step earlier, the
       withdrawn opening-window claim, the R2E pools scaling together, and a
       smaller R2B holding overload. Left #362 a larger residual (PR #379)
  #373 Role 4 staged reconstruction is represented as an interval rather
       than a count, the return-to-theatre protocol being the parameter and
       the procedure count its consequence, and the cohort is selected on the
       wound rather than on expected recovery, which would read the severity
       classification the evacuation decision already reads. Reconstruction
       becomes a third reason to evacuate. Ships disabled; raised #376
       (PR #377)
  #368 The operation a casualty released with the definitive repair
       outstanding carries rearward is reported rather than lost at boarding,
       in operations and theatre-minutes under the `ot` label, and conserved:
       the releasing theatre draws it and the analysis takes no draw. Assigned
       and never seized, so the echelon stays unsimulated. Fixed the intensive
       care such a casualty was owed and never given. Raised #373 (PR #374)
  #365 A damage control casualty can be released to strategic evacuation
       between the abbreviated operation and the definitive repair, the
       trigger being the number waiting to be operated on at R2E rather than
       a drawn share. Ships disabled and draws nothing, so seed 42 reproduces
       byte for byte; a released casualty holds an intensive care bed on the
       critical airlift route. The non-zero default is blocked on #368
       (PR #371)
  #314 Role 4 demand is configured, calibrated and reported in the bed types
       the deployed echelons field. The capacity half is withdrawn rather than
       delivered: the model sets the demand on the national support base and
       does not simulate it, so no establishment is stated and no shortfall
       reported, and the gate asserts the absence. Seed-42 intensive care is
       unchanged at 906 bed-days while the two ward labels collapse into 2,660
       holding bed-days. Raised #365 and #368 (PRs #366, #369)
  #313 The evacuation policy and the R2E holding establishment are both swept
       at 30 replications over 360 days per arm, and neither shipped default
       moves. 21 days is the only setting in the doctrinal 15 to 60 day range
       both admissible against the historical in-theatre envelope and stable
       at length; at that policy the holding pool stops binding 15 beds above
       its 30-bed establishment and no health outcome moves across a tripling
       of it. Closing the holding occupancy split on the way withdrew the
       published one-route claim. Mortality stays unresolved with the count a
       decision would need, and the national support base capacity question
       the issue scoped out sits with #314 (PRs #360, #363)
  #335 R2B holding capacity and the evacuation threshold are swept jointly
       at 10 replications over a fifteen-point grid. Ten beds per unit
       clears the forward queue (0.93 to 0.03 casualties) at no measured
       cost to R2E holding or intensive care; the threshold only matches
       that at an aggressive one-day setting, and does so by raising R2E
  ...
  #348 Closed on its cache-trap task alone (PR #398). Its two remaining
       ...
       re-screened; no issue currently tracks them. Raised #410.
  ...
  #339 The Morris screen re-run over 36 responses and 78 parameters,
       replacing the four rankings taken against superseded definitions;
       evac_threshold screened with its ranks annotated as its on/off
       transition, #348's coverage gaps not added. Raised #408 (PR #407)
  #405 Standardise every replicated experiment's horizon and replication
       ...
       Audited all thirteen experiments (PR #411), migrated the R2B
       pre-open hold window and the post-operative intensive care gate
       (PR #413), then migrated the transport fleet-size sweep and, for
       cross-table consistency, the forward ICU share frontier and the
       R2B holding threshold sweep (PR #415). All three costed migration
       candidates executed (PRs #411, #413, #415)

IN REVIEW (PRs open against main):
  (none)

UNBLOCKED (start now):
  #408 Re-run the Sobol decomposition on the parameters the #339 re-screen
       selects: two of its five (mass_casualty_min_cas, pri1_dcs_rate) have
...
*Prepared June 2026. Updated 23 August 2026 to reflect: the merge of Issue #155 (PR #226), the terminal canonical re-run, which rebuilds every published figure from commit `ed3c426` in the pinned Dev Container, retires the twenty-one accumulated provenance caveats as correct, rebuilds the Morris ranking at sixty-five parameters, withdraws three published claims that did not survive re-measurement, and tracks the sensitivity evidence set at `data/sensitivity/`; and the two issues it raised, #227 (checkpoint ref cleanup) and #228 (higher-resolution decomposition). Further updated 23 August 2026 to scaffold two new phases, Phase 6 (code quality and verification, Issues #230 to #237 and #241) and Phase 7 (publication, Issues #238 to #240), and to move #227 and #228 from BLOCKED to UNBLOCKED, the merge of #226 that gated them having landed; and again on the same date to record the merge of Issue #230 (PR #243), the verification baseline, which executes the fifteen regression checks as a suite for the first time and confirms the seed-42 reproduction by execution. No issue becomes ready on that merge, #230 having been one of two or three gates on each of #235, #241, #239 and #240 rather than the only one. Updated again on the same date to record the merge of Issue #232 (PR #245), which repairs the ten broken README image paths, widens `scripts/check_markdown.R` from heading anchors to every link and image target and to placeholder alt text, and, through the caption pass the issue asked for, audits all thirty-one tracked figures: three were rendering correctly while describing a model that no longer existed, and two new renderers now derive each from the data it illustrates. No published value moves and the seed-42 baseline reproduces byte for byte, in R 4.3.3 as well as in the pinned 4.4.2. No issue becomes ready on that merge, nothing in Phase 6 or Phase 7 having been gated on #232. Updated again on the same date to record the merge of Issue #231 (PR #247), which repairs the README reference list: the misattributed duplicate deleted, the second duplicate merged, both open-access violations re-sourced to substitutes already in the list, the uncited entry cited, and the list renumbered from sixty-seven entries to sixty-three in order of first appearance, with `scripts/check_references.R` added to hold all three academic documents to those properties. The URL verification found three further paywalled entries the issue had not named; two are swapped for PubMed Central copies of the same articles and the third, Hall et al. (2023), has no open-access copy and is left for its own issue. No published value moves and no issue becomes ready on that merge, nothing in Phase 6 or Phase 7 having been gated on #231. Updated again on the same date to record the merge of Issue #234 (PR #249), which replaces `docs/STYLE_GUIDE.md` with an enforceable R code standard of roughly sixty identified rules, each tagged machine-checkable, reviewer-applied or preference, audited against all 21,129 lines of R so that it records the conventions already in use and measures the gap where it does not. `CLAUDE.md`'s Code Standards section is rewritten to cite it by rule identifier and four files are added to its Repository Structure table. No published value moves and no R source is touched. Issue #235 becomes ready on this merge, #230 and #234 having been its two gates; #241 stays blocked on #235. Updated 24 August 2026 to record the merge of Issue #235 (PR #251), which turns the regression checks into a gate: a glob-discovered suite runner returning one exit status, a lint configuration encoding the `[lint]` rules of #234 and ratcheted against tracked per-rule counts rather than gated on zero, a seed-42 byte-for-byte reproduction check, and a GitHub Actions workflow running all three per pull request in the pinned container, with the calibration check weekly. The first run confirmed the seed-42 reproduction in the container and found that `check_references.R` had never passed since it was written, its URL pattern matching nothing under R's default regular expression engine; it is repaired in the same PR and the reference lists were sound throughout. Issue #241 becomes ready on this merge, #230, #234 and #235 having been its three gates. Updated again on the same date to record the merge of Issue #252 (PR #253), which narrows the suite on a documentation-only pull request through a classification job, the checking jobs narrowing what they run rather than being skipped so that a required status check always reports. No published value moves in either merge and no issue becomes ready on the second. Updated again on the same date to record the merge of Issue #233 (PR #255), which makes the analysis pipeline stream-neutral: `.Random.seed` is saved and restored around the Role 4 length-of-stay draw and the pooled mass casualty timeline's jitter takes an explicit seed, so analysing one run's output twice gives one answer. The stream is preserved rather than reseeded, which leaves the draw at the position it already held, so no published figure moves and the seed-42 evidence set reproduces byte for byte; `scripts/check_analysis_idempotence.R` is added and fails on six of its eight assertions against the unfixed pipeline. Moving the draw into the simulation is recorded as Further Development L30 rather than implemented. No issue becomes ready on this merge, #241 having never been gated on #233. Updated again on the same date to record the merge of Issue #236 (PR #257), which makes the configuration save and restore exception-safe: `capture_config_globals()` and `restore_config_globals()` are registered with `on.exit(..., add = TRUE)` at the point of save in both capacity sweeps, both sensitivity screens and `run_scenario()`, which had no restore at all, so a run that fails partway can no longer leave the session on the swept configuration and report the next run as the baseline. The same PR adds entry-point validation to the four analysis entry points and to `app.R`'s two configuration-loading boundaries, so a missing monitoring column or a malformed `env_data.json` field is named rather than surfacing as a subscript error inside a pipeline. `scripts/check_config_restore.R` and `scripts/check_input_validation.R` are added, the first failing on all seven of its assertions against the unfixed code; the argument-threading alternative is recorded as Further Development L31. No published value moves, the seed-42 evidence set reproducing byte for byte, and no issue becomes ready on this merge, nothing in Phase 6 or Phase 7 having been gated on #236. Updated 25 August 2026 to record the merge of Issue #237 (PR #259), the Phase 6 housekeeping pass: the README's Further Development scan table is reconciled to the three entries it had drifted from and `scripts/check_markdown.R` now asserts that correspondence, so a row can no longer name a gap or an impact differently from the entry it points at; entry L22 is trimmed from four paragraphs to the one-paragraph length of its neighbours, the replication-count derivation it duplicated having its single home in `CLAUDE.md`'s Key Parameters table. The remote held twelve `wip/*` checkpoint refs rather than the ten the issue enumerated, every one verified byte-identical to the tracked evidence set under `data/sensitivity/` or a superseded intermediate, and all twelve are deleted, which closes Issue #227 as completed alongside it. The `CLAUDE.md` Repository Structure omissions the issue named were already closed. No published value moves and the seed-42 evidence set reproduces byte for byte; no issue becomes ready on this merge, nothing in Phase 6 or Phase 7 having been gated on #237. Updated 26 August 2026 to record the six merges against Issue #241 (PRs #261, #263, #264, #265, #266 and #267), which add the Shiny console's two test suites and wire both into the gate, decompose `analyse_run()` from 1,418 lines to 398, `analyse_replications()` from 810 to 229 and `server` from 2,293 to 195 behind roughly a hundred named functions, reduce the literal `1440` from 122 occurrences to one, and take the count of functions over the length limit from eighteen to eleven. Each decomposition was verified by artifact comparison, 89 outputs byte-identical, and the seed-42 evidence set reproduces byte for byte on every merge; the lint ratchet ends below where it started. Three classes of fault surfaced that no artifact comparison could see, a stage returning a conditionally-bound name, a panel reading another's local, and an alias whose removal exposed a shadowed base function, and two new checks, `check_analysis_decomposition.R` and `check_console_bindings.R`, now assert the binding structure so all three are covered on every path at once. Issue #241 remains open on its seventh acceptance criterion, and Issue #262 was raised from the work. The post-merge chore convention asks for a `chore/` branch; this update was made on the development branch instead, the session having no authority to create one, and the deviation is recorded here rather than left implicit. Updated 27 August 2026 to record the merge of PR #269, the seventh and last against Issue #241, which splits `run_morris()` and `run_sobol()` behind `scripts/check_screen_order.R` and reduces `extract_kpis()` from 205 lines to 126, taking the count of functions over the length limit from eleven to nine. The check was written before either driver was touched and validated by fault injection rather than by passing, a screen being an eleven-hour run whose cache a fault in evaluation order would leave resuming into wrong responses with nothing reporting it. Issue #241 is closed with its seventh acceptance criterion unmet, the residual raised as Issue #270, which takes up the five simulation-logic functions on the reasoning that the deferral recorded through PR #267 no longer holds: `scripts/check_baseline_reproduction.R` settles stream-neutrality byte for byte in 44 seconds and gates every pull request, so what stops those splits is interface design rather than risk. The same update records the merge of Issue #262 (PR #268), which PR #268 itself did not record, and marks it complete in the summary table, the phase sequence and the dependency graph. No published value moves. The `chore/` branch deviation noted above applies to this update as well. Updated 27 August 2026 to record the merge of Issue #270 (PRs #272, #273, #274, #275 and #276), which splits the five simulation-logic functions the length limit had outlasted, one pull request per function: `run_once` into four named assembly steps, `build_casualty_trajectory` into phase builders over the three DNBI sub-type arms, `r2b_treat_wia` and `r2e_treat_wia` into one builder per step and per phase with their sub-trajectory locals lifted to file scope and the resources they use threaded through their signatures, and `run_replications` into a captured generator state, the dispatch and the killed-worker filter. The count of functions over the limit falls from nine to four, which is the count `docs/STYLE_GUIDE.md`'s D1 conformance entry records as deliberate, so that entry no longer carries a simulation-logic group. Every pull request reproduces the seed-42 evidence set byte for byte and, additionally, matches `main` output for output across the whole of `outputs/`; the one difference that comparison found, the row order of arrivals still in theatre in `outputs/mon_arrivals.csv`, reproduces between two runs of identical code and is therefore pre-existing nondeterminism in simmer's monitor output rather than an effect of any split. Three verbatim duplications were removed with the reading, and the judgement recorded in `r2e_treat_wia`'s own header, that `join()` copying a segment made a split unverifiable, is answered: an appended phase adds to the same trajectory object, and no `join()` was introduced that the undivided bodies did not already make. No published value moves and no issue becomes ready on this merge, nothing in Phase 6 or Phase 7 having been gated on #270. The `chore/` branch deviation noted above applies to this update as well, and for the same reason. Updated 27 August 2026 to record the merge of PR #278, which runs the fast check suite several checks at a time and takes the pull request gate from 16 min 51 s to 7 min 27 s on the four-core runner, all twenty-four checks passing and the working tree left clean in each of the three arrangements measured. The work was taken up directly rather than from an issue, so it carries a Recently Merged entry under its pull request number and no row in the summary table, no phase sequence item and no dependency graph node. No published value moves and no issue becomes ready on this merge. The `chore/` branch deviation applies to this update as well. Updated 27 August 2026 to record the merge of Issue #238 (PR #280), which re-cuts the two analysis papers by method: five replicated experiments move to `docs/Multi_Run_Analysis.md`, each gaining a design statement naming its replication count, its seed and any non-default parameter, and `docs/Single_Run_Analysis.md` is retitled Model Verification and Baseline Behaviour and left reporting one seed-42 run and nothing else. The issue enumerated four experiments; the force regeneration trend table was the fifth, its acceptance criterion reaching further than its table. The evacuation-policy comparison stayed, being three single runs rather than a replicated experiment, and the multi-run paper keeps a title now narrower than its contents, that retitle left to #240. The merge touches five markdown files and no R source, configuration or tracked artifact, so no published value moves. Issues #239 and #240 both become ready, #238 having been the gate that settled what each document contains. Updated 27 August 2026 to record the merge of Issue #239 (PR #282), which brings `docs/Single_Run_Analysis.md` to publication standard: twenty-two issue-number references and five blockquoted maintainer notes removed with their live content promoted into a new Methods section, a Limitations section cross-referencing the README's `L` entries by identifier, and a reference list taken from three entries to sixteen. The figure-verification pass was executed in the pinned Dev Container rather than reasoned about, and found more than the issue anticipated: the seed-42 evidence set and all thirty-one tracked figures reproduce byte for byte, so no artifact needed regenerating, but reading each figure against the text describing it found eight descriptions disagreeing with their own plots, three of them substantive, and three prose claims the current baseline contradicts. No published value moves and no issue becomes ready on this merge, #240 having already been ready and nothing else in Phase 7 having been gated on #239. One correction is left for its own change: `CLAUDE.md`'s Key Parameters table describes Role 4 peak occupancy as still rising at the run's end, which the figure contradicts, and is corrected in this same update. The `chore/` branch deviation noted above applies to this update as well, the session being directed to a single named development branch. Updated 27 August 2026 to record the merge of Issue #240 (PR #284), which brings `docs/Multi_Run_Analysis.md` to publication standard and completes Phase 7: the document is retitled Replicated Experiments and Comparative Scenario Analysis, gains an introduction, a methods section stating the interval method and the replication-independence argument in its own text, and a limitations section, and loses the supersession narrative that ran through its interpretation, conclusion and experiment sections, the withdrawn forward-surgery claim becoming a limitation carrying its own power calculation. Both results tables now report uncertainty consistently, the queue table gaining intervals from a fresh 50-replication measurement of both profiles that reproduces every published casualty, mortality and queue figure exactly; two queue cells move under the aggregation convention the table now states, a resource idle throughout a replication contributing a zero rather than dropping out, which takes `moderate_intensity` R2E Hold from 0.622 to 0.598 and Transport from 0.00386 to 0.0038. The replication-count power analysis moves out of `CLAUDE.md` into the paper's methods, leaving a cross-reference, and the stale n≥30 design claim is corrected in the three other places it was repeated. No seed-42 baseline value moves and all twenty-four fast checks pass. No issue becomes ready on this merge, #240 having been the last open item in Phase 7. The `chore/` branch deviation applies to this update as well, and for the same reason. Updated 28 August 2026 to record the merge of PR #288, which rewrites the narrative prose of `docs/Single_Run_Analysis.md` for a readership of operational planners and military staff, splitting the clause-chained sentences, replacing the framing openers with the claims they framed, and dropping the restatements of the run configuration the Methods table already fixes. The work was taken up directly rather than from an issue, so it carries a Recently Merged entry under its pull request number and no row in the summary table, no phase sequence item and no dependency graph node, on the precedent PR #278 set. Ninety prose lines are replaced one for one and nothing else: the numeric tokens differ only in repetition count, the heading and image lines diff empty, and the citation sequence still runs one to sixteen with every entry cited. No published value moves and no issue becomes ready on this merge. The check suite was not run in the session, R being absent from its container, and the assertions of `check_markdown.R` and `check_references.R` were checked by hand for the document while the Actions gate ran the suite itself. The `chore/` branch deviation applies to this update as well, the session being directed to a single named development branch. Updated 29 August 2026 to record the merge of PR #292, which retitles both analysis papers around the system they model, "Applying Discrete Event Simulation to the Land-Based Trauma System", each keeping the subtitle that states its unit of analysis, and repairs the README Conclusion's link text for the multi-run paper, which still named it by the title Issue #240 superseded. The work was taken up directly rather than from an issue, so it carries a Recently Merged entry under its pull request number and no row in the summary table, no phase sequence item and no dependency graph node. Three lines change across three files, no published value moves and no issue becomes ready on this merge. The `chore/` branch deviation applies to this update as well, the session being directed to a single named development branch. Updated 6 September 2026 to record the merge of PR #298, which reframes `docs/Multi_Run_Analysis.md` around planning decisions and cuts it to journal length, moving the experimental designs and statistical derivations to a new `docs/Multi_Run_Supplement.md` and the reinforcement experiment with them. The work was taken up directly from a three-round review rather than from an issue, so it carries a Recently Merged entry under its pull request number and no phase sequence item or dependency graph node; it does close Issue #297, which has a summary table row, and raised #295 and #296, which remain open and are also tabled. `scripts/render_paper_figures.R` and three figures are added, each parsed from the paper's own tables so it cannot disagree with them. No published value moves and the seed-42 evidence set reproduces byte for byte. The first CI run failed on the roxygen ratchet, the new script's file-scope constants being undocumented, and was fixed in the same pull request. That update was made on the development branch; the repository owner then authorised a `chore/` branch, and PR #299 recorded it on `chore/post-pr298-action-plan-update`, which resolves the deviation carried since PR #267. Updated 7 September 2026 to record the merge of PR #302, the third review round on the multi-run paper, which withdraws the paper's claim that the establishment copes at moderate intensity as contradicting its own theatre contention finding, corrects a modelled died-of-wounds mechanism that had been reported as a measured one, removes direct address to the reader and the "binding constraint" language throughout, and retitles the paper "Surgical Hours, Not Operating Theatres". Two review requests needing per-timestep simulation output were raised as Issues #300 and #301 rather than half-met. No published value moves and no issue becomes ready on this merge. This update is made on `chore/post-pr302-action-plan-update`, per the convention. Updated 7 September 2026 to record the merges of PR #304, which rewrites `docs/Multi_Run_Supplement.md` as a standalone academic paper carrying its own abstract, introduction, limitations, conclusion and thirteen-entry reference list, brings it under `scripts/check_markdown.R` and `scripts/check_references.R` alongside the other academic documents, and widens `CLAUDE.md`'s academic-output rules from three documents to four; and of PR #305, which points the README at that paper in the two places it introduces its companions and cross-references it from the Multi-run Replication Framework section. Neither was raised as an issue, so each carries a Recently Merged entry under its pull request number and no summary table row, phase sequence item or dependency graph node. No published value moves in either merge, no R source, configuration or tracked artifact is touched, and no issue becomes ready. Both updates are recorded here on `chore/post-pr305-action-plan-update`, per the convention. Updated 8 September 2026 to record the merge of PR #308, which adds `docs/Treatment_Flow_Configurability.md`, a design supplement evaluating how far the `env_data.json` parameter mechanism can be extended to let a user adapt the treatment flow itself rather than only its magnitudes, and sets out four options with their costs and a recommended sequence gating the largest of them behind Further Development L31 and Issue #4. Nothing is implemented and no issue is raised, the document existing to precede an issue set rather than to close one, so it carries a Recently Merged entry under its pull request number and no summary table row, phase sequence item or dependency graph node. No published value moves, no R source, configuration or tracked artifact is touched, and no issue becomes ready. This update is recorded on `chore/post-pr308-action-plan-update`, per the convention. Updated 9 September 2026 to record the merge of Issue #310 (PR #317), which completes `run.R`'s command line surface and validates it at the boundary: `--scenario`, `--mode`, `--images-dir` and `--max-cores` added there, and `--seed`, `--output-dir`, `--images-dir` and `--refresh-baseline` to `scripts/run_warmup.R`, whose Welch measurement had no way to be reproduced from the command line despite a figure derived from it being published in `CLAUDE.md`'s Key Parameters table. The validation rules live in a new `R/cli.R` rather than inside the CLI block, so `scripts/check_input_validation.R` asserts all twenty of them by calling them, one spawned invocation covering the wiring; testing them by spawning a process per case was measured at over two minutes against that check's tracked 16 seconds, which the per-PR gate would have paid on every pull request. The merge also closes an artifact-policy leak the issue did not name, `scripts/run_warmup.R` having overwritten the tracked `images/welch_plot_icu_queue.png` on every default invocation, the failure mode Issue #154 closed for `run.R` and left open at the other entry point. Two defects found in testing are recorded rather than fixed: the short-run crash raised as Issue #316, pre-existing on `main` and triggered by no casualty having returned to duty rather than by the run length itself, and the unknown-scenario message's differing format, left as it stands rather than duplicating the list of valid profiles into a second authority. No published value moves, the seed-42 evidence set reproducing byte for byte, and both ratchets hold exactly at their baselines. Issue #312's stated dependency on #310 is satisfied and it was already labelled ready, so no label changes; #313 stays blocked on #312. Six issues raised in the session that produced this work are tabled as open: #311 to #316. This update is recorded on `chore/post-pr317-action-plan-update`, per the convention. Updated 10 September 2026 to record the merge of PR #319, which ships three sustained-campaign defaults and refreshes every document that reports a figure measured under the old ones: the theatre evacuation policy from 30 days to 21, the reinforcement demand cycle from disabled to 7 days, and the strategic sortie cancellation probability from 0.15 to 0. The third is a scoping decision rather than a calibration one, this model's business with the strategic echelon being to state the demand the land-based trauma system generates rather than to simulate the reliability of the airlift meeting it, and it is the change that stops the strategic evacuation backlog forming: all 193 evacuation decisions now board and arrive inside the campaign against 99 of 135 with 36 left holding R2E beds. The Morris screen was re-run at the re-scoped bounds and its tracked evidence set, its seven scatter plots, the README ranking table and Further Development L18 republished from it, which moved the leading cluster: `mass_casualty_max_cas` rises from fourth to second and is now statistically tied with `pri1_surg_prob`, and only the leading parameter is separated from the field at r = 20. Two published separation figures did not reproduce under any method tried and were replaced with recomputed values and an explicit statement of the method, a correction that also reaches #228, which quotes one of them. A new strategic airlift reliability sweep in `docs/Multi_Run_Analysis.md` measures what the zero-cancellation default assumes away, its design recorded in `docs/Multi_Run_Supplement.md` as the seventh experiment and the one no tracked command can re-execute. The reference to #312 is deliberately non-closing: the pull request ships the defaults that issue's evidence motivated and closes one of its tasks, and #312 stays open with the long-horizon protocol outstanding. Two pieces of work followed the merge and are recorded on #312 rather than in this document: the trajectory rebuild cost was bisected to `build_casualty_trajectory()` at roughly 125 MB per call, and an attempted fix caching one trajectory across runs was reverted after `check_replication_independence.R` and `check_pre_open_window.R` both failed on it, a simmer trajectory carrying state and so not being reusable across environments. Issue #320 was raised from that work, a replication lost to memory pressure being warned about but dropped silently from the count that reaches published metadata. No issue becomes ready on this merge. This update is recorded on `chore/post-pr319-action-plan-update`, per the convention. Updated 10 September 2026 to record the merge of Issue #320 (PR #322), which stops a replication lost to memory pressure being dropped silently from the published count: the framework now returns the count that contributed alongside the count requested, the scenario runner returns both, and the Welch figure's subtitle names the realised one. A run losing any replication now stops, `MAX_REPLICATION_LOSS` shipping at zero. An earlier revision of the pull request set it at a tenth and the repository owner challenged that in review; the reasoning that a loss costs precision alone assumes the dead replications are a random subset of those dispatched, which is not established, the memory killer taking the largest process and heavier campaigns carrying more monitoring data, so the survivors are biased low on the responses the papers report. The threshold survives as a parameter a caller may raise deliberately for a run whose cost justifies it. The issue's task to record the realised count in the outputs needed no code, all three paths already deriving it from the data, and none was written. `scripts/check_replication_loss_reporting.R` asserts the accounting in thirteen places and was validated by fault injection rather than by passing. No simulation output changed and no baseline refresh was required; the lint baseline rose by four conventional `<<-` findings and was refreshed, and the roxygen ratchet held. No issue becomes ready on this merge, the measurement integrity it restores being a prerequisite for the replicated re-measurement queued in #295, #296, #300, #228 and #312 rather than a gate any of them declares. This update is recorded on `chore/post-pr322-action-plan-update`, per the convention. Updated 10 September 2026 to record the merge of Issue #311 (PR #324), which scopes the single-run paper's inference that intensive care saturation is a standing condition of the four-bed establishment to the 30-day window that supports it, a window being too short to separate a standing shortfall from the opening phase of an accumulation, and adds the general form of that caveat to the paper's Scope of a Single Run section and a note to the evacuation policy table naming the R2E holding pool it does not report. The issue's second claim and its "busy but not saturated" heading had already been removed by PR #319 when the sortie cancellation default changed to zero; both were verified absent rather than assumed absent and the finding is recorded as a comment on the issue. The cross-reference to long-horizon evidence the issue also asks for is deliberately left undone, no such measurement being reproducible by a documented command, citing one that is not being the failure the rescoping exists to prevent. One document changes, no R source, configuration or tracked artifact is touched, no published value moves and no issue becomes ready. This update is recorded on `chore/post-pr324-action-plan-update`, per the convention. Updated 11 September 2026 to record the merge of Issue #316 (PR #326), which stops the analysis pipeline depending on what a run happened to produce. The wide pivot of the attributes monitor carried a column only where some casualty set that attribute, so a one-day campaign ran to completion and then died on `object 'return_day' not found`; `MODEL_ATTRIBUTE_KEYS` now lists all 77 keys the model can set and `build_attributes_wide()` adds each as an all-NA column where the run set none. The hand-written list of fifteen it replaces had already proved incomplete, a second list of fourteen further keys standing in `R/sensitivity.R` for the same reason at a different call site, and that one is deleted with all 36 sensitivity responses unchanged. The new check was validated by fault injection, four of its ten assertions failing against the previous key list and one against the previous guard, and the merge also registers `scripts/check_replication_loss_reporting.R`, which PR #322 added to none of the three places a check is recorded. No simulation output moves, the seed-42 evidence set reproducing byte for byte and a 30-day run matching `main` output for output, so no baseline refresh was required. Issue #327 was raised from the work and remains open: the same function drops any hold episode still running when the run ends, 9 of 50 at seed 42, which reaches a tracked figure and the single-run paper's reading of it. No issue becomes ready on this merge. This update is recorded on `chore/post-pr326-action-plan-update`, per the convention. Updated 11 September 2026 to record the merge of Issue #315 (PR #329), which plots every R2E bed type's queue rather than the two the plotter named, the holding pool having been excluded by construction while the R2B plotter selected on a wildcard and covered all of its own. At seed 42 the omitted series measures a pool mean queue of 1.73 against 2.29 for the theatres and 0.464 for intensive care, so it was comparable to one plotted pool and roughly 3.7 times the other, shallow across many beds rather than deep on few. The panels share one vertical scale rather than the free scales the issue recommended: that recommendation rested on a shared axis flattening the smaller series, which measurement contradicts, while a free scale draws a pool never exceeding one casualty exactly as prominently as one reaching fourteen. The figure was rendered both ways and the repository owner settled it. A new check defends the selection rule against a bed type no configuration ships and found two further defects in the process, an unasserted fourth R2B bed type and a colour scale that silently stops drawing past its palette's eighth. Only the tracked image moves and no published value changes. No issue becomes ready on this merge. This update is recorded on `chore/post-pr329-action-plan-update`, per the convention. Updated 11 September 2026 to record the merge of Issue #327 (PR #332), which bounds each R2B holding episode by the attribute its own exit route sets. There are three routes out of a holding bed and only one sets `return_day`: the issue named the episodes still running at the window's close, which were dropped, and the work found the larger error, an episode ended by onward evacuation being charged the bed until a `return_day` set later at R2E. Under the 4320-minute threshold the analysis documents recommend as a lever, the old reconstruction reported 303.2 holding bed-days against a true 172.9. The corrected reconstruction agrees with the resource monitor at both configurations, which is the independent measurement the new check cross-references, and the figure's axis label and its capacity line were corrected with it, each R2B unit now carrying its own panel against its own establishment. The audit the issue asked for covered every start-and-end attribute pair the pipeline differences and raised #331: the R2B and R2E dwell KPIs drop 21.6% and 18.0% of stays, always the longest, and both are screened Morris responses. Reading the same code raised #333, the two R2B holding thresholds sharing one identifier while one of them had no configuration entry, and #335, the joint sweep of holding capacity against that threshold and the decision on whether it joins the Morris screen. Only the tracked figure moves and no published value changes. No issue becomes ready on this merge. This update is recorded on `chore/post-pr332-action-plan-update`, per the convention. Updated 11 September 2026 to record the merge of Issue #333 (PR #334), which separates the two R2B holding thresholds that had converged onto one identifier. `hold_threshold` is an occupancy fraction read at Role 1 deciding whether a casualty is sent to an R2B unit at all, and `evac_threshold` a duration read at R2B deciding how long they may stay once there; a function named `r2b_hold_threshold()` read the second while the Morris parameter of that name sets the first. Every read was checked and none was wired to the wrong field, so nothing was computed wrongly and no published value moves; what the collision cost was the next change. The function is renamed for the field it reads, and `evac_threshold`, which had no `env_data.json` entry at all and so could not be set through any tracked entry point despite the analysis documents naming it as the cheapest remedy for the R2B holding shortfall, gains one shipping at the disabled value together with a Shiny registry entry. Setting it alone moves 46 of 65 holding casualties onto the early-evacuation branch, against none at the shipped setting. Issue #335 becomes ready on this merge, the configuration entry being what it was gated on. This update is recorded on `chore/post-pr334-action-plan-update`, per the convention. Updated 11 September 2026 to record the merge of Issue #331 (PR #340), which estimates the two dwell KPIs and the transit between them over everyone who entered the echelon rather than over those who also left. A casualty still present when the run ends was dropped, and the dropped are the late arrivals and the long stayers: 21.6% of R2B stays and 18.0% of R2E stays at seed 42, the dropped R2B stays having already run twenty-two times longer than the completed mean. The three intervals with an identifiable cohort are now estimated by Kaplan-Meier and reported as a restricted mean with the censored share beside it; time to first surgery is excluded, a casualty with no surgery time being either one still waiting or one who never needed an operation. Two findings shaped the fix beyond what the issue anticipated: the censored share is configuration-dependent rather than a constant offset, reaching 55.5% of R2E stays under `high_intensity`, so the figure was least trustworthy in the congested regime it is consulted for; and the obvious restriction horizon is wrong, the full window putting 63% of the R2B figure beyond the last observed completion, where the curve can only be held flat. R2B dwell moves from 1,022 to 2,920 minutes and R2E from 3,505 to 3,788, the transit mean is unchanged to five decimal places, and the R2B p90 becomes unavailable because the curve never reaches it. No tracked artifact moves and the evidence set reproduces byte for byte. The tracked Morris rankings for both dwell responses rest on the superseded definition and cannot be re-derived from a cache holding scalar responses alone, so they are marked as superseded and the re-screen is raised as #339. No issue becomes ready on this merge, #339 having been raised rather than unblocked. This update is recorded on `chore/post-pr340-action-plan-update`, per the convention. Updated 11 September 2026 to record the merge of Issue #342 (PR #343), which asserts that the two tables claiming to index the codebase actually do. `CLAUDE.md`'s Repository Structure table says it covers the repository as a whole and the README's Codebase Structure table is what it points a reader at for each `R/` module; nothing checked either claim and both had drifted. `R/cli.R` had no README row from the day it was created despite being named in the README's own prose, and PR #340 had added two files without rows, corrected by hand in PR #341. A missing row breaks nothing that runs, so only a reviewer noticing ever caught one. The new check asserts that every `R/` module has a row in both tables, that every `scripts/check_*.R` and other `scripts/` entry point has one in `CLAUDE.md`'s, and that neither table names a path that no longer exists, taking its file set from `git ls-files` so an untracked scratch file cannot fail it. It failed on `R/cli.R` the day it was written and covers its own registration, and was validated by three fault injections, a rename being caught twice over on coverage and on staleness. No simulation output is in scope and no issue becomes ready on this merge. This update is recorded on `chore/post-pr343-action-plan-update`, per the convention. Updated 11 September 2026 to record the merge of Issue #337 (PR #345), which bounds and retries the CI system-library install. The step ran `apt-get` with no retry configuration, no transport timeout and no per-step timeout, so a stalled Ubuntu archive mirror hung it until the job timeout fired, and one run was observed wedged past 25 minutes and cancelled by hand against a 7m39s worst case that completed. Each of the four copies now writes an `apt.conf.d` fragment before `apt-get update`, five retries and 30 second transport timeouts, and carries a 15 minute step timeout; writing the fragment before the update is what makes the guards cover both commands, and nothing about what is installed changes. The guards are inline rather than in a composite action because `timeout-minutes` is not a step property a composite action supports, and a call-site timeout was not relied on, an unenforced timeout being worse than none. `scripts/check_ci_apt_guards.R` therefore asserts the four copies stay identical and each guards the update as well as the install, validated by three fault injections that each fail two assertions. The hardened step was confirmed running successfully in all three container jobs on the pull request itself. No simulation output is in scope and no issue becomes ready on this merge. This update is recorded on `chore/post-pr345-action-plan-update`, per the convention. Updated 12 September 2026 to record PR #347, which advances Issue #312 without closing it, discharging the task that issue names as gating every other. Building the casualty trajectory retained roughly 175 MB per call that neither releasing it nor garbage collection reclaimed, and the cause is a per-activity cost in simmer rather than in this project's code: 6.0 R cons cells per activity on a trajectory of nothing but timeouts, linear from 250 activities to 8,000, against 12,612 activities in the casualty trajectory. That corrects two conclusions recorded on the issue, which held that simmer was not implicated and that the global `env` binding was the likely retainer; dropping that binding releases about a tenth. `dispatch_replications()` now gives each replication its own forked process, so the retention is returned on exit and peak memory is flat in the replication count, 884 MB at four replications and 796 MB at eight against 1,340 MB and 2,084 MB batched. `docs/Multi_Run_Supplement.md` loses an argument the change falsifies, losses no longer arriving in batches. No published value moves and no issue becomes ready. Issue #348 was raised from the audit that followed, carrying the two Morris coverage gaps and the cache trap out of #312's comments so their sequencing against #335 and #339 is visible, and #295 is recorded as owning the airlift sweep entry point #312 had also listed. This update is recorded on `chore/post-pr347-action-plan-update`, per the convention. Updated again on 12 September 2026 to record the merge of Issue #296 (PR #350), which re-measures the post-operative intensive care gate at 50 paired replications per arm under common random numbers and splits the published finding in two: the gate's relief of the R2E intensive care pool is resolved at 6.80 percentage points (95% CI [5.13, 8.47]), and its effect on died-of-wounds counts is not, the paired difference of 0.060 spanning zero with the opposite sign to the one the design predicts and needing roughly 2,283 replications to resolve. The merge also adds the `icu_gating.enabled` switch that makes the pre-gate model reachable from configuration, and the check asserting it reproduces that model. The seed-42 baseline is unchanged and no issue becomes ready, #296 having gated nothing. This update is recorded on `chore/post-pr350-action-plan-update`, per the convention. Updated again on 12 September 2026 to record the merge of Issue #301 (PR #352), which reports queue length by resource pool and the degraded post-operative care rate as time series across 50 replications per casualty intensity, and so settles a question the paper's summary statistics could not reach: whether the R2E theatre queue recurs in peaks that clear, which surge capability answers, or never clears, which only establishment does. It never clears at high intensity, standing empty for 5% of the campaign and still growing on day 29, and clears readily at moderate, standing empty for 80% with a longest unbroken busy run of 3.0 days, so the paper's first planning option is now stated as an establishment measure at one intensity and a surge measure at the other. Item 8 of the multi-run paper's Further Development table is closed by the merge and a new item raised in its place for the longer horizon the growing queue calls for, which belongs to the open Issue #312. The seed-42 baseline is unchanged and no issue becomes ready. This update is recorded on `chore/post-pr352-action-plan-update`, per the convention. Updated again on 12 September 2026 to record the second partial advance of Issue #312 (PR #354), which establishes the sustained-operations horizon: a replicated runner that reduces each replication to a daily series inside the worker that produced it, a 30-replication 360-day measurement at both casualty intensities, the duration pilot, the warm-up re-derivation and the audit of horizon-dependent findings. The measurement corrects the issue's own premise, moderate intensity converging at the shipped defaults while four high-intensity responses grow without bound across a simulated year. #312 stays open, the airlift sweep entry point belonging to Issue #295, which is noted in the dependency graph as gaining the runner that entry point can be built on, and a re-screen at length being unscoped. The seed-42 baseline is unchanged. This update is recorded on `chore/post-pr354-action-plan-update`, per the convention. Updated again on 12 September 2026 to record the merge of Issue #295 (PR #356), which measures strategic evacuation and national support base demand at 50 replications across thirteen configurations and refines the single-campaign reading in three places: timing matters more than airframe-days, the evacuation wait consumes a holding bed on the standard route alone, and demand peaks shortly before the campaign ends rather than after it. Two measurement defects were found and fixed rather than shipped, an irreproducible Role 4 peak and a sortie censored by the horizon counted as cancelled. The seed-42 baseline is unchanged and no issue becomes ready; Issue #312's remaining airlift task is narrowed rather than discharged, the 360-day collapse experiment still having no entry point of its own. This update is recorded on `chore/post-pr356-action-plan-update`, per the convention. Updated again on 13 September 2026 to record the merge of Issue #358's advance on Issue #312 (PR #358), which gives the 360-day strategic airlift collapse experiment the entry point the previous merge left it without, and, in re-executing it, moves two of its six arms and withdraws the finding that the risk saturates above 20% sortie loss: the rate climbs from 36.7% to 70.0% on intervals that barely overlap. The classifier's separation is narrower than the previous text claimed and is now stated with the threshold range the counts survive, and the classified pool's daily series is tracked so that the early-campaign predictor claim is recomputable rather than resting on one run. The supplement's fifth design limitation is deleted, the seed-42 baseline is unchanged and no issue becomes ready; #312 stays open on the sensitivity re-screen scoping and the warm-up classification at length. This update is recorded on `chore/post-pr358-action-plan-update`, per the convention. Updated again on 13 September 2026 to record the merge of PR #360's advance on Issue #313, which sweeps the evacuation policy across the doctrinal 15 to 60 day range at the sustained horizon and supplies the justification the shipped 21-day default lacked, PR #319 having moved it on the single comparison #313 itself called insufficient. The historical in-theatre envelope excludes three of the five policies and only 21 of the two admissible ones is stable at length. Two of the issue's premises were corrected: returns to duty peak at 30 days rather than rising with the policy, and the mortality effect that gated the decision is undetectable across the range and is reported with the replication count a decision would need. The seed-42 baseline is unchanged and no issue becomes ready; #313 stays open on the holding establishment sweep, the missing hold-duration attribute and any further decision on the default. This update is recorded on `chore/post-pr360-action-plan-update`, per the convention. Updated 16 September 2026 to record the merge of Issue #313 (PR #363), which closes the issue: the R2E holding occupancy split is closed exactly rather than by residual, which withdrew the published claim that only the standard evacuation route stages in holding beds and re-measured every share resting on it; the holding establishment is swept at 30, 45, 60 and 90 beds and found flat above 45, the pool having stopped binding; and the recorded decision is that neither shipped default moves, each now resting on a sweep rather than on a single comparison. No seed-42 value moves and the tracked evidence set reproduces byte for byte. No issue becomes ready on that merge; the national support base capacity question the issue scoped out sits with #314, which was already open, and the split's residual over-count is raised as #362. Updated 16 September 2026 again to record the partial advance of Issue #314 (PR #366), which configures the Role 4 ward mapping and splits a stay between an intensive care phase and a step-down ward, calibrated to a Role 4 source so that the seed-42 intensive care share falls from 75.6% to 25.4% without any numeric published figure moving. The establishment block ships but its comparison is computed and never reported, which the entry records as outstanding rather than delivered; #314 stays open on that, the multi-run capacity line and a re-measurement. Issue #365 was raised from the same work and is tabled as open. Updated 16 September 2026 again to record the completion of Issue #314 (PR #369), which closes it by withdrawing four of its tasks rather than delivering them: the national support base is given no capacity, a stated establishment and any shortfall against it being statements about an echelon this model does not simulate, and the absence is now asserted by the regression gate rather than left as a convention. The census reports the bed types the deployed echelons field, which retires a ward label naming a specialty the model never represented. No numeric published figure moves and the seed-42 evidence set reproduces byte for byte. The one task left undone, re-measuring the issue's own evidence table, is recorded as moot, its premise having been the capacity work now withdrawn. Updated 17 September 2026 to record the merge of Issue #365 (PR #371), which lets a damage control casualty be released to strategic evacuation between the abbreviated operation and the definitive repair when the number waiting to be operated on at R2E reaches a configured threshold. The lever ships disabled and the decision draws nothing, so the seed-42 evidence set reproduces byte for byte and no published figure moves. Two defects in the first commit were found by the owner reading the design rather than by the check: the trigger counted only the half of the theatre queue already holding a room, and a released casualty waited for their flight in a holding bed rather than in intensive care. #368 was rewritten to carry the non-zero shipped default, without which a released casualty's outstanding operation leaves the model at boarding. Updated 17 September 2026 again to record the merge of Issue #368 (PR #374), which closes that half: the operation is reported as Role 4 theatre demand in operations and theatre-minutes, conserved from the draw the releasing theatre took, and assigned rather than seized so no capacity, queue or shortfall enters an echelon the model does not simulate. A defect from the preceding merge was fixed with it, such a casualty having been owed no Role 4 intensive care at all. The seed-42 evidence set is unchanged, `images/role4_census.png` included. Staged reconstruction is split to #373 and blocked on what the evacuation decision must carry. Updated 17 September 2026 a third time to record the merge of Issue #373 (PR #377), which closes that split: the staged sequence is represented as an interval rather than a count, on the 48 to 72 hour return-to-theatre protocol, so the procedure count follows from the debridement phase instead of being configured; the cohort is selected on the wound rather than on expected recovery, which would have read the same severity classification the evacuation decision already reads; and reconstruction becomes a third reason to evacuate, recorded in `evacuation_reason`. The blocker the earlier entries recorded was real but misnamed: what blocks a conditioned rate is that the model carries no anatomical description of a wound, so the share reproduces the right proportion of casualties rather than the right casualties, and that limit is now recorded under Further Development L33. Both national support base levers ship disabled and the seed-42 evidence set reproduces byte for byte; #376 enables them together and re-measures every replicated evidence set once. Updated 18 September 2026 to record the merge of Issue #376 (PR #379), which is that campaign: both levers now ship in force, the saturation threshold swept at the sustained horizon before it was set and anchored at 8 on the theatre queue an unrelieved campaign carries, and six replicated evidence sets re-measured once rather than twice. Five published findings changed in substance rather than in their digits, and each is recorded where it appears: the 30-day evacuation policy now carries a resolved mortality cost, the airlift collapse cliff moves from 15% to 10% sortie cancellation, the claim that a campaign's outcome cannot be predicted from its opening is withdrawn, the three R2E pools now scale together rather than the theatre queue standing apart, and the R2B holding overload falls from 60% to 37%. Three regression checks failed on code rather than on stale documentation; two were assertions that were never theorems, and the third is a real degradation of the holding occupancy split, whose bounds are raised with the measurement and the cause stated at each while the root cause stays with #362. Updated 18 September 2026 a second time to record the merge of Issue #362 (PR #381), which finds that root cause and closes it: a casualty's strategic evacuation staging stay was bounded by a departure time only boarding sets, so a casualty who died awaiting a sortie was charged the campaign's whole unserved remainder, which is why every residual was an over-count and none an under-count. The stay is now bounded by the release of its bed, recorded on both routes out of the wait, and the same defect is fixed in the published backlog series along with a terminal-extension error found while verifying the regenerated figure. Both fixes are analysis-side and draw-free, so the model is untouched and the seed-42 evidence set reproduces byte for byte; `data/airlift/` is regenerated and all 650 replications now close exactly, against 619, with the bounds in `scripts/check_holding_occupancy_split.R` tightened rather than relaxed. Two figure corrections came with it and neither is caused by the fix: this document's sibling table in `CLAUDE.md` claimed no seed-42 run had produced such a death, contradicting its own DOW count row, and the national support base section of the multi-run paper had not been refreshed since PR #379, printing 26.96 casualties still waiting at high intensity against a tracked 108.28. The verification gap that let the second happen, four replicated experiments having a protocol check and this one having none, is raised as #382; #339 gains two more superseded responses and is retitled, and #348 gains a fourth driver against its sequencing table. Updated 18 September 2026 a third time to record the merge of Issue #382 (PR #385), which closes that gap: `scripts/check_airlift_protocol.R` asserts the experiment's parameters against the supplement's marker comments, its tracked responses against the documented thirteen configurations, and every figure of the paper's three tables against the tracked summary, with a missing table, row or column failing rather than passing quietly. The section's figures move out of prose and into tables, which is the question the issue asked be settled rather than assumed and is what makes them checkable at all; no figure changes value. The check is validated by fault injection rather than by inspection, the historical drift among the five faults injected, and the supplement's account of the holding occupancy split is corrected with it, having still described the two-component estimator PR #363 replaced. The audit the issue's last task called for found the larger gap behind it and is raised as #384: of the twelve experiments the companion paper publishes, five have a tracked evidence set and a protocol check and seven have neither, the comparative scenario analysis among them, its runner writing only to the gitignored outputs/ so that the paper's centrepiece exists nowhere in the repository. Updated 19 September 2026 to record the merge of Issue #384 (PR #390), which gives the comparative scenario analysis and the two capacity sweeps tracked evidence sets and protocol checks, records in `docs/Multi_Run_Supplement.md` the rule deciding which published figures require one and the audit of every experiment against it, and corrects the scenario queue table, which had mixed per-bed averages with pool totals while the paper's own abstract already carried the correct ratios; and to move #387, #388 and #389, the three experiments that audit records as still unbacked, from BLOCKED to UNBLOCKED, the merge that gated them having landed. Updated 23 September 2026 to record the merge of Issue #387 (PR #392), which gives the R2B pre-open hold window's eight-row table a runner of its own, `R/hold_window.R` and `scripts/run_hold_window.R`, and a tracked evidence set at `data/hold_window/`; regenerating the table on the fresh sample moved it, two of the eight paired differences clearing zero rather than three, and the section's headline claim and the supplement's replications-needed figures are restated from the tracked data. No seed-42 value moves. This update is recorded on `chore/post-pr392-action-plan-update`, per the convention. Updated 23 September 2026 to record the merge of Issue #388 (PR #394), which gives the post-operative intensive care gate section a runner of its own, `R/icu_gate.R` and `scripts/run_icu_gate.R`, on #387's arrangement rather than sharing its module, and a tracked evidence set at `data/icu_gate/`; regenerating the evidence set moved every published figure, including the mortality point estimate's direction, which now costs a fraction of a life rather than saving one as the design predicts, though the interval still spans zero. No seed-42 value moves. This update is recorded on `chore/post-pr394-action-plan-update`, per the convention. Updated 24 September 2026 to record the merge of Issue #389 (PR #396), which gives the mass casualty stress test's four-row table a runner of its own, `R/mass_casualty.R` and `scripts/run_mass_casualty.R`, on #387's arrangement but unpaired, and a tracked evidence set at `data/mass_casualty/`, the replication count derived at 62 rather than kept at the untracked 10; regenerating at that count resolves the event-versus-ordinary died-of-wounds comparison for the first time, upgrading it from direction only to measured, while the arm-to-arm comparison on ordinary casualties alone stays unresolved. This was the last of the three experiments #384's audit recorded as unbacked, so the supplement's audit table now records every experiment as backed. No seed-42 value moves. This update is recorded on `chore/post-pr396-action-plan-update`, per the convention. Updated 24 September 2026 to record PR #398, which advances Issue #348 without closing it, closing the cache trap alone: a response added to `morris_kpis` or `SOBOL_RESPONSES` after a design point cache existed was invisible to `cache_append()`, either corrupting the file on the first re-evaluated row or, had the gap been backfilled with `NA`, reading every point as cached with the new response silently and permanently empty. `cache_check_schema()` now checks a cache's header against the response set the evaluation loop is about to look up before that loop starts, archiving a cache that does not carry all of it under a `.stale-<timestamp>` suffix rather than resuming it. The issue's two coverage gaps, the missing holding bed queue responses and the excluded R2E bed counts, remain open, gated on whichever of #335 or #339 next triggers a re-screen, since a Morris design is a function of its parameter count. No trajectory, arrival process or `env_data.json` value is touched and the tracked evidence set's caches already carry every response and parameter this change checks for, so no seed-42 value moves and no baseline refresh was required. No issue becomes ready on this merge. This update is recorded on `chore/post-pr398-action-plan-update`, per the convention. Updated 24 September 2026 to record the merge of Issue #335 (PR #400), which sweeps R2B holding capacity jointly against the evacuation threshold at ten replications over a fifteen-point grid: ten beds per unit clears the forward queue with no measured cost to R2E holding or intensive care, while the threshold only matches that relief at an aggressive one-day setting and does so by raising R2E holding utilisation instead. The sweep also settles the issue's Morris screening question without a re-screen: `evac_threshold` stays outside the screened set, the measured response confirming the discontinuity at its disabled default that a screening elementary effect across that step would otherwise measure as the switch rather than the magnitude. No seed-42 value moves. #348's two remaining coverage gaps stay gated on #339 alone, #335 having settled its own question without adding a parameter to that design. This update is recorded on `chore/post-pr400-action-plan-update`, per the convention. Updated 24 September 2026 to record the merge of Issue #300 (PR #402), which re-runs the transport fleet-size sweep under the `high_intensity` scenario profile on the existing moderate-intensity design: `plot_transport_capacity_margin_by_fleet_size()` and `scripts/run_transport_sweep.R` gain a `scenario` parameter, resolved against the parsed configuration before the fleet-size override, and a non-default scenario writes its own suffixed tracked files. The boundary the sweep locates does not move under surge; three ambulances and four trucks remain sufficient, so the qualification that Option 4's evidence rests on one casualty rate is removed and README Further Development entry L19 is narrowed to the mass-casualty-injection half of the gap that remains. No seed-42 value moves and no issue becomes ready on this merge. This update is recorded on `chore/post-pr402-action-plan-update`, per the convention. Updated 25 September 2026 to reflect: the merge of Issue #339 (PR #407), which re-runs the Morris screen over thirty-six responses and seventy-eight parameters, replacing the four rankings taken against superseded response definitions, adds the thirteen parameters an audit found neither screened nor excluded, screens `evac_threshold` with its ranks annotated as its on/off transition, and leaves #348's coverage gaps unaddressed. No seed-42 value moves. Issue #408, re-running the Sobol decomposition on the new selection, becomes ready. This update is recorded on `chore/post-pr407-action-plan-update`, per the convention. Updated 25 September 2026 to reflect: the merge of Issue #405's audit task (PR #411), which decides three of the thirteen replicated experiments (the R2B pre-open hold window, the post-operative intensive care gate, the transport fleet-size sweep) are migration candidates to the sustained-operations protocol and costs each, and records the retain reasoning for the remaining six; no experiment is re-run and #405 stays open. This update is recorded on `chore/post-pr411-action-plan-update`, per the convention. Updated 25 September 2026 to reflect: the merge of Issue #405's first execution step (PR #413), which migrates the R2B pre-open hold window and the post-operative intensive care gate to the sustained-operations protocol, re-runs both, and finds that most count-based responses in both become harder to resolve at the longer horizon rather than easier, contrary to the migration's own costed rationale; only a time-weighted share and one within-arm accumulating count improve. No seed-42 value moves and #405 stays open, the transport fleet-size sweep remaining the one unexecuted migration candidate. This update is recorded on `chore/post-pr413-action-plan-update`, per the convention. Updated 25 September 2026 to reflect: the merge of Issue #405 (PR #415), which executes the transport fleet-size sweep, the third migration candidate #411 costed, and widens the decision during review to migrate the forward ICU share frontier and the R2B holding threshold sweep alongside it, so all three capacity sweeps share one 30-replication, 360-day protocol rather than three different ones. Transport utilisation, which ran the wrong way at 10 replications, now falls monotonically with fleet size at both casualty intensities, and the R2B holding threshold sweep's returns-to-duty and died-of-wounds responses now separate at the one-day threshold, both genuine resolutions rather than restatements. No seed-42 value moves. All three costed migration candidates are now executed, closing #405. This update is recorded on `chore/post-pr415-action-plan-update`, per the convention. Updated 26 September 2026 to reflect: the merge of Issue #410 (PR #417), which closes the two coverage gaps Issue #348 raised alongside its cache-schema defect and left orphaned when PR #398 fixed the defect alone. `r2b_hold_q`/`r2e_hold_q` join the screened responses and `r2e_icu_beds`/`r2e_hold_beds` join the screened parameters, the full r = 20 Morris screen is re-run at the resulting 80-parameter, 1,620-point design, and `r2e_icu_beds` leads its own `r2e_icu_q` response, the only response in the tracked set where a fixed establishment count outranks every casualty-load or clinical-probability parameter. No seed-42 value moves. No issue becomes ready on this merge; #408 was already `status: ready` from #339's merge and its target ranking has moved again. This update is recorded on `chore/post-pr417-action-plan-update`, per the convention.*
