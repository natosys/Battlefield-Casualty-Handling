...
Findings demonstrate that the current system design is capable of managing moderate casualty volumes, historically represented by the Falklands conflict. Following correction of DNBI sub-categorisation and implementation of OT-bypass routing (surgical patients bypass R2B to R2E when the operating theatre is occupied or queued), the primary system constraint is identified as R2B holding bed capacity, which saturates progressively over a 30-day operation. The bypass logic routes the majority of R2B surgical candidates (130 of 160 in the baseline run) directly to the R2E Heavy, where OT utilisation reaches a mean of 34.0% — elevated but not saturated. R2B OT operates at only 5.5% utilisation, reflecting the high bypass rate under realistic capacity constraints. The study identifies areas of risk, recommends targeted system refinements, and proposes enhancements to the simulation architecture—including dynamic DOW modelling, pulsed strategic evacuation, and stochastic mass-casualty events.
...
The plot below outlines a summary of the casualty handling at the R2B. The data indicates a system that is managing its surgical throughput effectively in the early operational period but developing holding-bed saturation as the campaign progresses.

![Alt text](images/r2b_handling.png)

Daily casualty treatments at R2B 1 and R2B 2 show the impact of OT-bypass routing (Issue #35): patients flagged for surgery check OT availability at R2B and, if the theatre is occupied or any queue exists, bypass immediately to R2E Heavy rather than waiting. Under seed 42 (30 days), **130 of 160 surgical candidates at R2B were bypassed to R2E**, with only 30 surgeries performed at R2B. R2B OT utilisation was **5.5%**, and the OT queue remained flat at zero throughout the run — confirming the bypass logic is functioning as designed. The 12-hour OT availability window (single surgical team per R2B) means that any surgical patient arriving during the off-shift is also bypassed, consistent with the clinical intent.

**Holding bed queues at R2B remain the primary identified bottleneck.** R2B 1 hold bed queues build steadily from approximately Day 15 onward, reaching 8–9 patients waiting by Days 21–22 and remaining elevated through Day 30. R2B 2 shows a distinct pattern — a peak of 9–10 patients queuing around Days 10–12 before resolving — consistent with an early surge of disease DNBI evacuees entering the holding pathway. These queue lengths substantially exceed the five hold beds per R2B unit, indicating that holding capacity is structurally insufficient for the cumulative DNBI holding demand generated over a 30-day operation. This saturation is independent of the OT bypass fix: it is driven by disease DNBI evacuees occupying hold beds for extended durations (mode 5 days), not by surgical post-operative holding.

![Alt text](images/r2b_bed_queues.png)

### R2E Heavy Handling

Following DNBI sub-categorisation (Issue #7) and OT-bypass routing (Issue #35), the R2E Heavy receives the majority of battlefield surgical demand. With 130 bypass patients from R2B added to those routed directly from R1, the R2E performed **122 first surgeries** in the baseline run (seed 42, 30 days), compared with 30 at R2B. This represents the intended clinical model: R2B performs damage control surgery only when OT is immediately available; all other surgical candidates flow to the R2E for definitive care.

![Alt text](images/r2eheavy_bed_queue_3_teams.png)

**R2E OT utilisation has increased materially** following the bypass fix. Mean OT utilisation across the two R2E operating theatres was **34.0%** (seed 42, 30 days) — OT 1 at 50.0% and OT 2 at 17.9% — reflecting asymmetric routing as the first available theatre absorbs the bulk of bypass demand. OT queues at the R2E reach a maximum of four patients waiting on isolated occasions. At 34.0% mean utilisation, R2E OT capacity is not currently saturated under Falklands-equivalent casualty rates, but the R2E now operates as the primary surgical node for the force.

**ICU is the primary binding constraint at the R2E Heavy.** This finding holds in every one of 10 independent replications (seed 42, 30 days per run). Across all replications, mean ICU utilisation is **71.2%** (range 60.6–80.6%) versus mean OT utilisation of **33.4%** (range 29.7–36.9%). In the seed-42 single run, per-bed ICU utilisation is 78.0%, 73.6%, 63.7%, and 58.1% — consistent with the replication range. The ICU queue is non-zero for a mean of 43.8 cumulative bed-days across replications (seed-42 equivalent: 11.5 days at system level, 38% of the run), and sustains a queue of three or more for a mean of 6.5 cumulative bed-days. These are not isolated transient surges: the ICU operates under chronic demand pressure across all stochastic realisations tested. By contrast, OT queue-bed-days ≥3 average 0.7 across replications. With R2E now processing 122 surgical cases (seed 42), the post-surgical ICU requirement has risen commensurately; under the current four-bed establishment the ICU constitutes the single greatest throughput constraint in the R2E system.

![Alt text](images/r2eheavy_gantt.png)

The Gantt chart illustrates the utilisation asymmetry: ICU beds are occupied throughout most of the run, while OT activity is more episodic and holding beds see near-zero occupancy. The high (~90%) strategic evacuation rate applied to post-surgical and post-ICU patients prevents holding bed saturation, and resuscitation beds see very limited use. Taken together, the R2E's under-utilised resources are at the periphery (holding, resuscitation); its core post-surgical pathway (ICU) is under sustained pressure — a finding that contrasts with the R2B holding bed saturation described above and identifies the ICU as the system constraint warranting priority attention.
...
The single-run analysis, viewed in its entirety, demonstrates that the modelled deployed health system is capable of sustaining a steady operational tempo for a single brigade under baseline casualty assumptions derived from the Falklands conflict. Role 1 elements show sufficient responsiveness and throughput, and the dual-node R2B configuration absorbs the majority of cases without systemic bypass to higher care.

Following correction of DNBI sub-categorisation (Issue #7) and implementation of OT-bypass routing (Issue #35), the model now reflects a clinically coherent surgical cascade: R2B performs damage control surgery only when OT is immediately available (30 surgeries in the baseline run), and all other surgical candidates bypass to R2E, which performed 122 first surgeries. R2E OT utilisation is 34.0% mean — elevated relative to the pre-bypass finding but not saturated. **Two system constraints are identified in this run.** At R2B, holding bed queues build progressively, reaching 8–9 patients queued at R2B 1 by Day 21 and remaining elevated through the end of the run — driven by disease DNBI evacuees occupying hold beds for multi-day durations, not by post-surgical patients. At R2E, the ICU is the binding constraint, operating at 68.4% mean utilisation with a queue present for 38% of the run; the OT is not the limiting resource within R2E.

The R2E Heavy operates as the primary surgical node. OT capacity is not saturated at baseline (34.0% mean utilisation, seed 42); however, **ICU is the identified binding constraint at R2E**, a finding confirmed across all 10 replications (71.2% mean utilisation, range 60.6–80.6%). Resuscitation and holding beds see very limited demand. Two distinct system constraints therefore emerge at different echelons: R2B holding bed saturation (driven by multi-day DNBI disease occupancy) and R2E ICU throughput (driven by post-surgical recovery demand). The primary system levers indicated by this analysis are R2B holding bed expansion or a higher evacuation threshold from R2B holding, and increased R2E ICU capacity to match the elevated surgical throughput from the bypass fix.
