...
Both compositional splits that must always sum to 1 — the Triage Priority Split (Priority 1/2/3) and the DNBI Sub-Type Split (Battle Fatigue/Disease/NBI, Issue #7) — render as a two-handle range slider with a colour-coded breakdown bar rather than three independent numeric fields, so the constraint is structural rather than something a planner can violate and then be told about after the fact. `app.R` converts each slider's two breakpoints to its three underlying `env_data.json` values (and back) at the point Quick Run or Save Configuration reads the form, via a shared `inject_split()`/`render_split_bar()` pair generalised across both splits. The DNBI subgroup's one non-compositional field, Disease Surgical Candidacy (the proportion of disease DNBI casualties who nonetheless require surgery), renders separately below the slider since it is not part of the three-way split. Each slider's track is recoloured (`split_slider_recolor_script()`) to show all three segments in the same colours as the breakdown bar above it, rather than the default ion.rangeSlider styling, which paints only the region between the two handles — implying a single "selected range" — leaving the two outer segments in a neutral track colour that does not convey they are meaningful shares too.
