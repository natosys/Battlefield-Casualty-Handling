...
> **Note on configuration:** the figures in this section are the only ones in this document produced under a non-default configuration. Mass casualty injection ships disabled (`mass_casualty.event.rate_per_day = 0`), so both columns below, the single run that follows them and `images/mass_casualty_events.png` were all produced with that rate temporarily set to 0.2, and none of them can be reproduced by a shipped-configuration run. The figure is therefore the one tracked image that `run.R --refresh-baseline` cannot write, and is copied into place from the run's own output directory; that is what let it fall three model revisions behind the prose around it before Issue #232.
