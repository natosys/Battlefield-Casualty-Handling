...
> Issue #203 closes the shortfall the per-minute rate cap was imposing on every stream. The cap clamps a draw at three times the stream's own mean, which lowers the mean it realises, and the generators were parameterised from the configured mean directly: the WIA streams therefore realised 79.0% of the daily rate their configuration named, the KIA streams 78.7%, `dnbi_cbt` 95.2%, `dnbi_spt` 99.2% and any exponential stream 95.0%. The generators now solve for the location at which the clamped draw averages to the configured mean (`solve_ln_location()`/`solve_exp_mean()`, `R/environment.R`), once at construction. Like Issue #148, and unlike the RNG-stream shifts recorded above it, this moves casualty generation itself rather than only the draw order downstream of it, and it moves it in one direction: total casualties rise 382 to 443, WIA 151 to 190, KIA 57 to 74 and DNBI 174 to 179, each stream in proportion to what the cap had been taking from it. Everything downstream rises with them.

> Two things about the measurement. The correction is exact rather than approximate, the clamped mean of the solved parameterisation agreeing with the configured mean to nine significant figures on every shipped stream, and it removes the coupling by which editing `sd_daily` alone moved a stream's realised mean: across coefficients of variation from 0.28 to 4.52 at a fixed mean, the realised mean now spans 0.001 casualties per day against 0.707 before. `scripts/check_arrival_rate_fidelity.R` is new and asserts both properties by exercising the shipped closures over a 1,000-day horizon; it measures every stream a second time with the correction disabled and fails if any of those would have passed the band applied to the corrected ones, so it registers its own removal. And the per-resource utilisation and queue rows below are measured as busy time against the run's full observation window at the resource group's summed capacity, the basis on which the pre-change run reproduces every previously published per-resource figure exactly; the R2E surgical section rows are the one exception, re-based here onto each section's own rostered capacity trace because the previous basis could not be reconstructed, and the pre-change run measures 21.4/33.3/14.8 on the basis used here.

> `data/arrivals_*.txt`, `logs/logs.txt` and `images/*.png` **were** regenerated and committed as one set from a single `--refresh-baseline` invocation, following the Issue #154 precedent. The `Welch ICU-queue CMA` row, `images/welch_plot_icu_queue.png`, `images/r2b_icu_share_frontier.png` and the published Morris ranking were **not** re-run and are flagged as such below and in README; the Morris set is unchanged, no parameter being added or removed by this issue.

| Metric | Baseline value |
|---|---|
| Total casualties (30 days) | 443 (post-Issue-203; was 382 post-Issue-148 and post-Issue-146, 386 post-Issue-173, 385 post-Issue-159, 386 post-Issue-160, 387 post-Issue-161, 400 pre-Issue-18). The rise is the rate cap's shortfall removed, not a change in any configured rate |
| WIA (combat + support) | 190 (post-Issue-203; combat 127 / support 63; was 151 post-Issue-148 and post-Issue-146, 149 post-Issue-173, 148 post-Issue-159, 149 post-Issue-160, 150 post-Issue-161, 154 pre-Issue-18). The WIA streams had been realising 79.0% of their configured mean, the largest shortfall of any shipped stream alongside KIA |
| KIA (combat + support) | 74 (post-Issue-203; combat 50 / support 24; was 57 post-Issue-148 and post-Issue-146, 67 post-Issue-173, post-Issue-159, post-Issue-160 and post-Issue-161, 70 pre-Issue-18). The KIA streams had been realising 78.7% of their configured mean |
| DNBI (combat + support) | 179 (post-Issue-203; combat 146 / support 33; was 174 post-Issue-148 and post-Issue-146, 170 post-Issue-173, post-Issue-159, post-Issue-160 and post-Issue-161, 176 pre-Issue-18). Moves least of the three, the DNBI streams being the least variable and so the least clamped, at 95.2% and 99.2% of their configured means |
| Effective force size, end of run (seed 42, post-Issue-203) | combat: 2305 of 2500 initial (−7.8%); support: 1164 of 1250 initial (−6.9%); no reinforcement (`force_regeneration.reinforcement.demand_interval_days = 0`, the shipped default). Was combat 2330 / support 1184 post-Issue-146 |
...
| Priority 1 share (seed 42, post-Issue-203) | 239 of 443 classified (53.9%) — P1 239 / P2 63 / P3 67 / KIA 74; was P1 219 / P2 50 / P3 56 / KIA 57 post-Issue-146. Every generated casualty carries an assigned priority in this run |
| DOW count (seed 42, post-Issue-203) | 0 total; was 1 (r2b=1) post-Issue-146. At n≤3 a change of one carries no statistical weight; see the pooled multi-replication row below for the figure that does |
...
| Mean DOW/run (150 reps pooled, post-Issue-148) | Not re-pooled for Issue #146, which lowers the quantity: a controlled 50-replication pair at the same control seed puts the treated-cohort rate at 0.627% (95% CI [0.446%, 0.807%]) with the pre-open window off against 0.436% ([0.319%, 0.553%]) at the shipped 60 minutes, both spanning or sitting below the ~0.46% Ajax Bay bound the one-sided check applies, so neither `p1_p_max` nor `p2_p_max` is adjusted. The pooled figures below are as last measured for Issue #148 and predate that movement. 0.99; 95% CI [0.82, 1.15]. DOW/WIA rate 0.653% (95% CI [0.542%, 0.764%]); treated-cohort DOW rate (casualties reaching R2B or R2E) 0.436% (95% CI [0.361%, 0.511%]), the quantity the historical Ajax Bay target of ~0.46% applies to, and an interval that now spans it rather than sitting below. Since the target is an upper bound the check is one-sided, so spanning is agreement, and `p1_p_max`/`p2_p_max` are unchanged in both configurations. Pooled across three independent 50-replication measurements, at control seeds 42, 777 and 20260808, because one measurement does not resolve a response averaging about one death per replication: individually those three returned treated-cohort rates of 0.485%, 0.402% and 0.422%, a spread of 0.083 percentage points. The superseded row read 0.85 (95% CI [0.69, 1.01]) with a treated-cohort rate of 0.38% (95% CI [0.31%, 0.45%]); the rise is an effect of Issue #148 rather than sampling variation, the treated cohort having grown with the WIA count while the killed-in-action count fell. Measured by `scripts/check_dow_calibration.R`, which passes for both shipped configurations; `moderate_intensity` measures 0.271% (95% CI [0.211%, 0.330%]). Precision: the per-replication standard deviation is 0.0046, so 150 replications resolve the treated-cohort rate to about ±0.07 percentage points and no third decimal place is reported. See README Further Development L22 |
| Replication count for mortality figures (post-Issue-148) | Per-replication sd of the treated-cohort DOW rate is 0.0046 (base, 150 reps). A 95% half-width of 0.15 pp needs 39 replications, 0.10 pp needs 84, 0.05 pp needs 328. The 50-replication figures elsewhere in this table carry roughly ±0.13 pp on this response; the queue and occupancy rows are time-weighted over far more events per replication and are better resolved at the same count |
| DNBI sub-types (seed 42, post-Issue-203) | battle_fatigue=45, disease=109, nbi=25 (post-Issue-203; was battle_fatigue=38, disease=105, nbi=34 post-Issue-146) — the total rises with DNBI generation, and the split between sub-types moves with the RNG stream despite sub-type assignment itself being unrelated code |
| bf_rtd (seed 42, post-Issue-203) | 39 (post-Issue-203; was 32 post-Issue-146), tracking the rise in the battle fatigue sub-type count above |
| clinical_rtd (seed 42, post-Issue-203) | 122 (r1: 71, r2b: 39, r2e: 12) (post-Issue-203; was 114, split 65 / 39 / 10, post-Issue-146) |
| total_rtd (seed 42, post-Issue-203) | 161 (post-Issue-203; was 146 post-Issue-146). As a share of arrivals this falls, 38.2% to 36.3%, the larger casualty count including more casualties whose recovery extends past the 30-day window |
| Realised in-theatre share (seed 42, post-Issue-203) | 25.3% of 162 R2E dispositions retained in theatre under the shipped 30-day `evacuation_policy_days`. An output of the policy, not an input. Was 24.5% of 151 dispositions post-Issue-146; the share is close to unmoved while the count rises with generation, which is what a policy applied to an unchanged severity mix should do. The 50-replication figure was not recomputed for Issue #203 and remains 26.8% (95% CI [25.5%, 28.0%]), inside the 7.6%-42.1% historical range cited in README Return to Duty |
| Evacuation is severity-conditioned (seed 42, post-Issue-203) | Sorting the 162 dispositions into quartiles by drawn `recovery_to_duty_days` gives evacuation rates of 0% / 100% / 100% / 100% from shortest to longest recovery; was 2.6% / 100% / 100% / 100% post-Issue-146. An unconditioned draw would give the same rate in every quartile. The 50-replication figure was not recomputed and remains 0.0% / 93.5% / 100% / 100% |
| Pre-flight critical ICU hold (seed 42, post-Issue-203) | 12 critical-route evacuees drew the ventilated pre-flight path (`critical_hold.ventilated_share = 0.15`); 8 completed within the run at a mean of 29.1 h, median 26.6 h, p90 38 h. Was 15 with 8 completed at a mean of 25.0 h post-Issue-146. The 50-replication figures were not recomputed and remain 31.9 h mean, 24.3 h median, 32 h p90 as last measured post-Issue-160 |
...
| Surgical pathway split (seed 42, post-Issue-203) | 153 casualties operated on: 74 damage control (48.4%), 79 single-stage. By priority, Priority 1 125 operated / 70 damage control (56.0%, configured rate 55%), Priority 2 28 operated / 4 damage control (14.3%, configured 20%); no Priority 3 casualty reached theatre in this run. Was 120 operated, 66 damage control (55.0%) post-Issue-146. The 50-replication share was not recomputed and remains 48.3% (95% CI [47.0%, 49.5%]) |
...
| R2B surgical decision point (seed 42, post-Issue-203) | 147 casualties reached it (65 operated at R2B, 82 bypassed to R2E); was 118 (49 operated, 69 bypassed) post-Issue-146 |
| R2B surgeries (seed 42, post-Issue-203) | 65 (post-Issue-203; was 49 post-Issue-146) |
| R2E surgeries — first op (seed 42, post-Issue-203) | 116 (post-Issue-203; was 96 post-Issue-146) |
| R2E surgeries — second op (seed 42, post-Issue-203) | 38 (unchanged from post-Issue-146 at this seed). Only a damage control casualty whose abbreviated operation was performed at R2E returns for a second procedure, so this stays at roughly the damage control share of casualties not operated on forward |
| R2B pre-transport bypass (seed 42, post-Issue-203) | upstream R1-threshold bypass: 131 (was 118 post-Issue-146); at-R2B hold-full bypass: 0 (unchanged); R2B hold queue (both full): 0 (unchanged) — these are three independent, non-summing counts |
| R2B OT bypass reason (seed 42, post-Issue-203) | at-R2B subset, 82 total (was 69 post-Issue-146): team off-shift 65 (was 53), OT busy/queued 17 (was 16). The off-shift share is essentially unmoved at 79% against 77%, which is what a larger caseload against an unchanged roster should give |
| R2B pre-open hold (seed 42, post-Issue-203) | 7 casualties held forward for a section about to reopen, all 7 operated on within the run, mean hold 26.7 minutes, longest 48.1, against the shipped 60-minute `r2b.surgery.pre_open_window_min`. Was 7 held at a mean of 40.9 minutes post-Issue-146. The 50-replication figure was not recomputed and remains 5.6 held per run (95% CI [5.0, 6.3]) |
| OT utilisation — echelon aggregate (seed 42, post-Issue-203) | R2B: 6.5%, R2E: 14.5% (was R2B: 4.9%, R2E: 15.6% post-Issue-146) |
| R2B OT utilisation — 24h room (seed 42, post-Issue-203) | T1: 9.9%, T2: 8.4% (was T1: 7.6%, T2: 6.2% post-Issue-146). The 50-replication figures were not recomputed for Issue #203 |
| R2B OT utilisation — shift time (seed 42, post-Issue-203) | T1: 19.8%, T2: 16.8% — theatre occupancy measured against the section's own rostered capacity trace. Was T1: 15.1%, T2: 12.3% pre-Issue-203 on the same basis; the previously published 13.9%/11.9% used a basis that could not be reconstructed and ran about 8% lower. This does not run at exactly twice the 24-hour room figure above: the pre-open hold occupies the theatre while the section is off shift, so some of the room's occupancy falls outside the roster this row divides by |
| R2E OT utilisation — 24h room (seed 42, post-Issue-203) | OT1: 38.6%, OT2: 27.8% (was OT1: 41.6%, OT2: 22.6% post-Issue-146). A theatre is seized before its surgical section, so a room reads as occupied while its casualty is still queued for staff; much of this is that wait, not operating time. The load evens out between the two rooms as it rises |
| R2E OT queue ≥1 (seed 42, post-Issue-203) | OT1: 13.7% of run, OT2: 12.5% (was OT1: 15.9%, OT2: 10.4% post-Issue-146) |
| R2E surgical section utilisation (seed 42, post-Issue-203) | Section 1: 19.0%, Section 2: 36.7%, Section 3: 26.6% of the time their own rosters had them open, queued for 0.14%, 1.40% and 0.01% of it; the pre-change run measures 21.4/33.3/14.8 on the same basis. These are re-based onto each section's own rostered capacity trace, the previously published 19.7/31.3/13.7 having used a basis that could not be reconstructed. Section 2 remains the busiest, being the section rostered to the second shift and so carrying the whole night-time surgical load alone |
| R2E ICU utilisation — mean (seed 42, post-Issue-203) | ICU1: 92.8%, ICU2: 93.2%, ICU3: 81.9%, ICU4: 77.5% (was ICU1: 90.0%, ICU2: 86.0%, ICU3: 82.1%, ICU4: 78.9% post-Issue-146). The 50-replication figure was not recomputed for Issue #203 and remains a four-bed mean of 79.7% (95% CI [78.4%, 81.0%]) |
| R2E ICU queue ≥1 (seed 42, post-Issue-203) | ICU1: 14.2% of run, ICU2: 13.4%, ICU3: 2.9%, ICU4: 0% (was ICU1: 35.6%, ICU2: 16.6%, ICU3: 10.8%, ICU4: 1.2% post-Issue-146). The queue falls while occupancy rises, the two ICU episodes being spread across a larger operated cohort rather than concentrated behind the first bed |
| Transport utilisation — platform aggregate (seed 42, post-Issue-203) | HX240M: 5.0%, PMVAmb: 11.8% (was HX240M: 2.7%, PMVAmb: 10.2% post-Issue-146) |
| PMV Ambulance utilisation (seed 42, post-Issue-203) | 11.8%; max queue 0 throughout run; per-vehicle 26.8% / 7.7% / 0.9% (was 24.1% / 6.3% / 0.3% post-Issue-146) |
| HX240M utilisation (seed 42, post-Issue-203) | 5.0%; max queue 0 throughout run; per-vehicle 8.3% / 1.7% / 0.0% (was 7.1% / 0.7% / 0.1% post-Issue-146) — carries R2B→R2E mortuary road-move traffic in addition to R1→mortuary KIA |
...
| R2E post-op pathway — stabilisation (seed 42, post-Issue-203) | icu=46, hold=50 (post-Issue-203; was icu=44, hold=43 post-Issue-146). The degraded share rises from 49% to 52% as the larger operated cohort presses on four ICU beds. `surgery_deferred` = 13 (was 7 post-Issue-146); the 50-replication figure was not recomputed |
| R2E post-definitive care pathway (seed 42, post-Issue-203) | icu=48, hold=94 — 34% of operated casualties receive post-definitive care in an ICU bed, down from 44% post-Issue-146, the remainder taking the degraded holding-bed fallback. The 50-replication ICU share was not recomputed and remains 48.6% (95% CI [47.0%, 50.1%]). Four ICU beds cover a smaller share of both episodes as generation rises to its configured rate |
| R2E post-op DOW rate — icu vs hold (seed 42, post-Issue-203) | 0/46 vs 0/50 (single-run; was 0/44 vs 0/43 post-Issue-146). The saturated-ICU 90-day stress test from Issue #43, which confirmed the mechanism fires with the hold pathway riskier than the icu pathway, has not been re-run since |
| Role 4 demand (seed 42, post-Issue-203) | 121 strategic evacuation decisions (85 critical route, 36 standard); 106 boarded and reached Role 4 by day 30, 15 still queued at R2E; Role 4 peak occupancy 68.0 concurrent patients (day 18); unconstrained-baseline demand would need 28 sorties. Was 114 decisions, 105 boarded, 9 queued, peak 67.0 on day 26 post-Issue-146 |
| Strategic AME actual performance (seed 42, post-Issue-203) | C-17A Globemaster III at 36 critical / 54 standard places; 106 evacuated at a mean wait of 1.2 days (p10-p90 0.0-4.0). Four sorties flew, boarding 36/21/11/6 on the critical route and 15/11/5/1 on the standard route; the first fills the critical cabin exactly. Was 105 evacuated at a mean wait of 0.7 days post-Issue-146 |
| AME wait-time DOW poll (seed 42, post-Issue-203) | `dow_echelon=5`, daily poll interval (`role4.ame.dow_check_interval = 1440` min); 0 deaths observed, as in every run since the poll was added; see README [AME Wait Checkpoint](README.md#ame-wait-checkpoint) for why no single-run count should be read as evidence about the mechanism's magnitude |
