...
| Transport Fleet-Size Sweep | 10 reps x 30d x 9 points x 2 configs | No, and the design record already flags the estimate as barely pinned down | No | Migrate |
| Forward ICU Share Decision Frontier | 20 reps x 30d x 5 points | No derivation on record | Yes, by analogy | Retain |
| R2B Holding Capacity vs. Evacuation Threshold Sweep | 10 reps x 30d x 15 points | No derivation on record | Yes, by analogy | Retain |
...
**Two experiments at default-load, sub-boundary configurations are retained on the analogy [Whether to Re-Screen at Length](#whether-to-re-screen-at-length) already makes.** The forward ICU share frontier and the R2B holding capacity/evacuation threshold sweep both run under the shipped default configuration, whose casualty load sits below moderate intensity, the only intensity at which every response still converges. Neither sweep touches a response found to drift, and both already read a monotonic trend at their current counts (10 and 20), so the case for migrating them rests on the same coverage-gap and cost reasoning that argument states for the screens rather than on any finding specific to either sweep; no further work is recorded for either here.
...
- **Transport Fleet-Size Sweep.** The design record already states the utilisation estimate is barely pinned down at 10 replications and 30 days. Migrating to the sustained-operations count and horizon, at all nine fleet-size points and both configurations, costs about $30/10 \times 3.8 \approx 11.4$ times the current run, the largest multiple of the three because this sweep already carries nine points and two configurations that the paired two-arm designs above do not; it remains costed rather than executed, and the two migrations above are reason to expect it would need re-costing against a measured rather than an assumed variance before it is attempted.

None of these three migrations has been executed. Costing a migration and deciding it is warranted, which this section does, is a distinct step from running it under `--refresh-baseline` and updating the sections of `docs/Multi_Run_Analysis.md` and this document that report it, which is what #405's remaining implementation tasks require and what a follow-up pull request carries out. Running three multi-hour-to-multi-day experiments and rewriting the sections that depend on them is a larger and more failure-prone change than the audit and decision recorded here, which is also why #405 anticipates the work spanning more than one pull request.
