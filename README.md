...
| Fill Distribution — Maximum (fraction of demand) | b | 1 |
...
The dominating rate holds the population term at $P_{\max}$, the pool's establishment strength:

$$
\bar{\lambda}_d = X_d \times \frac{P_{\max}}{1000 \times 1440}
$$

$P_{\max}$ bounds $F(t)$ for the whole run: every casualty debits its pool, and the reinforcement credit is clamped so it can never carry a pool above the strength it started at (Step 6). Candidate gaps are then drawn from an Exponential($\bar{\lambda}_d$) distribution by inverse-CDF, and a candidate at time $t$ is accepted with probability $F(t) / P_{\max}$, so an accepted arrival occurs at intensity $\bar{\lambda}_d \times F(t)/P_{\max} = \lambda_d(t)$ as intended. A candidate falling past the end of its day is discarded and sampling restarts at the day boundary under the next day's rate, which is exact for a piecewise-constant rate because the exponential distribution is memoryless.
...
The reinforcement demand/fulfillment cycle's shipped defaults (`fill_min_frac = 0.2`, `fill_mode_frac = 0.85`, `fill_max_frac = 1.0`, `fulfillment_lag_days = 7`) give a fill-fraction distribution with a long left tail toward severe under-fill (5th percentile ≈ 0.36 of demand) and a short right tail approaching but never reaching full delivery (99th percentile ≈ 0.97). Full fulfillment is the *less* likely outcome even though the mode sits close to it, because the long lower tail pulls the mean well below the mode (≈0.68 vs. 0.85 at the shipped defaults). Demand and the fill fraction are both resolved at submission time, not at fulfillment.

The maximum is bounded at 1 because establishment strength is a hard ceiling: a package larger than the shortfall it was requested against would have nowhere to go, so a value above 1 would name an over-delivery the model could never apply. Such a configuration is rejected rather than silently clipped (`validate_fill_distribution()`, `R/trajectories.R`), which is what keeps the parameter's realised effect equal to its configured one. Measured over 498 delivered cycles in each pool, the realised mean fill fraction is 0.680 (standard error 0.008) for the combat pool and 0.673 (0.008) for the support pool, against the distribution's own mean of 0.6833, and nothing delivered failed to reach the population. One case remains in which it could, recorded as Further Development L29. `scripts/check_lever_realisation.R` asserts all of this.
...
| Reinforcement fill distribution mode | `fr_fill_mode_frac`          | 0.85         | 0.5   | 1.0   | B    |
...
| L29 | Reinforcement into a refilled pool has nowhere to go | Low |
...
**L29 — Reinforcement into a refilled pool has nowhere to go.** A reinforcement package joins the population the moment it arrives, there being no formation-level reserve in the model to hold personnel anywhere else, and a pool cannot be carried above its own establishment strength. Those two facts together leave one case with no representation. Demand and the fill fraction are both resolved when a cycle is submitted, so a pool refilled by return to duty during the fulfillment lag can have less room at delivery than it had shortfall at submission, and the difference is lost. What the model would need in order to keep it is a way to represent a formation above establishment, which would in turn need the casualty generator's dominating rate rebased off establishment strength (see [Casualty Generation](#casualty-generation)); re-assessing demand at delivery rather than at submission is the lighter alternative, at the cost of the submission-time resolution the mechanism is built on. The loss is counted rather than dropped silently, in the `reinf_combat_unabsorbed` and `reinf_support_unabsorbed` globals, so any run reports its own exposure: across five 90-day runs at a 7-day demand cycle and 7-day fulfillment lag, both counters read zero, sustained casualty production depleting each pool faster than return to duty refills it. The case is reachable only where return to duty outpaces attrition across a whole fulfillment lag, which is a configuration a planner would have to seek out.
