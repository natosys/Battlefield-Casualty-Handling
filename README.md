...
| `R/replication.R`                  | Multi-run replication framework: `run_once` (single replication with `wrap()`, assembled from the named steps `init_mass_casualty_stream`, `add_casualty_generators`, `add_reinforcement_cycle` and `add_strategic_evac`), `run_replications` (parallel `mclapply` over *n* replications), and `summarise_replications` (time-weighted KPI summary with 95% CI) |
