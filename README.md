...
This document is the project's **system reference**: it describes the codebase structure, the literature and doctrinal basis for every modelled algorithm and parameter, the resource and trajectory model (including the R1/R2B/R2E Heavy trajectory flowcharts), the model's known limitations, and the development environment. It does not present simulation results — those are published as three companion analysis documents:
...
- **[docs/Multi_Run_Supplement.md](docs/Multi_Run_Supplement.md)** — the standalone companion paper to the multi-run analysis, holding the full experimental design of every replicated experiment, the replication framework and interval derivations they rest on, and the force regeneration comparison.
...
- [Model Parameters and Model Configuration](#model-parameters-and-model-configuration)
  ...
  - [Casualty Generation](#casualty-generation)
    ...
    - [2. Daily Rate Sampling and Scaling](#2-daily-rate-sampling-and-scaling)
    - [3. Arrival Time Sampling by Thinning](#3-arrival-time-sampling-by-thinning)
    - [4. Realised Daily Mean and Variance](#4-realised-daily-mean-and-variance)
...
- [Simulation Design](#simulation-design)
  - [Codebase Structure](#codebase-structure)
    ...
    - [Shiny Console Structure](#shiny-console-structure)
  ...
  - [Role 4 (National Support Base) Demand Modelling](#role-4-national-support-base-demand-modelling)
    - [Operating Theatre Demand](#operating-theatre-demand)
      - [Staged Reconstruction](#staged-reconstruction)
...
Recent doctrinal analyses [[9]](#references), [[2]](#references) and [[5]](#references) emphasized the shift toward prolonged casualty care, contested evacuation, and the need for distributed medical assets. A related analysis [[9]](#references) frames return-to-duty as a direct force-sustainment lever rather than a purely clinical outcome and is implemented in the model directly ([Force Regeneration and the Endogenous Feedback Loop](#6-force-regeneration-and-the-endogenous-feedback-loop)), coupling casualty production to a live, time-varying effective force size. These insights were incorporated into the simulation design to ensure alignment with contemporary operational realities.

#### Damage Control Surgery and Post-Operative Critical Care

A systematic review [[21]](#references) and an account of damage control technique in abdominal surgery [[22]](#references) establish the three-phase structure that this project's R2B and R2E Heavy trajectories implement. Operative-time data reported for a damage control cohort within [[21]](#references) directly sourced the DAMCON surgery duration distribution, corroborated by the rapid-closure operative-window principle central to damage control technique [[23]](#references) and by outcomes literature from an austere-setting DCS series [[24]](#references) and abdominal-trauma DCS practice [[25]](#references). Post-operative critical care requirements are established by [[26]](#references), [[23]](#references), [[25]](#references) and [[27]](#references); descriptions of post-DCS stabilisation timeframes [[21]](#references), [[28]](#references), [[25]](#references) informed the ICU length-of-stay parameters at both echelons. Task-time estimation for the R2B/R2E resuscitation phase, where no single published source tabulates an end-to-end duration, drew on the treatment-process optimisation methodology of [[29]](#references), constructing an estimate from collated task-duration components rather than a single reported figure.

#### Preventable Death and Time-Dependent Mortality

An analysis of 4,596 battlefield deaths during Operations Enduring Freedom and Iraqi Freedom [[30]](#references) found that 87.3% resulted from haemorrhage, predominantly within 30–90 minutes of injury; a study of a "golden hour" surgical-capability policy [[31]](#references) found it reduced preventable prehospital death from 32% to 3.5% in a Special Operations context. Together these establish the direct empirical link between time-to-care and survivability that motivates modelling died-of-wounds probability as a function of elapsed time since injury; the logistic curve's shape parameters are anchored to the mortality time-windows these two studies report. The treatment-efficacy multipliers that further modify the died-of-wounds ceiling by care received at each phase are similarly evidence-based rather than estimated outright: a 41% relative mortality reduction from balanced-component damage control resuscitation is reported in [[32]](#references), and the PROMMTT study [[33]](#references) provides the haemorrhage-specific mortality basis used to derive the surgical efficacy factor.

#### Strategic Aeromedical Evacuation (AME) and Role 4 Doctrine

The strategic AME and Role 4 demand model draws on [[34]](#references), which establishes the triage framework, echelon functions, and the Casualty Staging Unit/CCATT-CCAST acuity concepts underpinning the model's critical/standard AME pool split. Role 4 ward assignment and length-of-stay parameters follow the general injury-severity length-of-stay gradient described in [[35]](#references), applied by informed extension since no open-access source tabulates Role 4 ward assignment by this project's exact category scheme.

#### Mass Casualty Event Simulation

The compound Poisson mass casualty event mechanism follows the general approach to LSCO casualty-surge simulation described in [[36]](#references), and complements the discrete-event mass-casualty-incident stress-testing precedent of SIMEDIS [[37]](#references), which injects a fixed, deterministic victim count rather than a stochastically varying one. An account of aligning field hospital training with LSCO reality [[38]](#references) informed the blast-dominant triage priority distribution applied to mass-casualty-derived casualties.

#### Statistical Methods for Simulation Verification, Replication, and Sensitivity Analysis

Moving from a single illustrative run to a defensible multi-run analytical framework required its own literature base. Verification and validation followed the framework in [[39]](#references) and the general DES methodology in [[40]](#references). Warm-up classification used Welch's graphical procedure and the treatment of initialisation bias in [[41]](#references), which places warm-up deletion within infinite-horizon steady-state analysis, together with the terminating-versus-steady-state classification in [[42]](#references); a campaign with a defined finite horizon falls on the terminating side of that division. Multi-run replication drew on a demonstration of L'Ecuyer-CMRG parallel RNG streams in R [[43]](#references) and the corresponding parallel-package documentation [[44]](#references), with the conditions a variance reduction scheme must meet taken from a treatment of antithetic variates [[40]](#references) and the choice of unit of analysis and replication count from a review of simulation output analysis [[42]](#references). Global sensitivity screening used Morris Elementary Effects [[45]](#references), implemented via R's `sensitivity` package [[46]](#references) and the OpenMOLE documentation [[47]](#references), and Sobol variance-based decomposition [[48]](#references). A critical-care DES study [[49]](#references) informed the Shiny application's default replication count and 95% CI reporting.

#### Disease and Non-Battle Injury Evidence

Disease DNBI sub-categorisation and its emergency surgical candidacy rate draw on documented evidence of acute surgical disease conditions occurring in deployed populations. Acute appendicitis incidence among deployed Japanese soldiers in Burma is reported in [[50]](#references), and humanitarian surgical care delivered at US military treatment facilities in Afghanistan is documented in [[51]](#references), together supporting the inclusion of conditions such as appendicitis, cholecystitis, and perforated peptic ulcer within the disease DNBI pathway's surgical candidacy branch, distinct from the battle fatigue and NBI sub-types that do not carry the same surgical risk profile.
...
Arrival times for the casualty streams are driven by a stateful generator closure (`make_ln_arrival_generator()`/`make_exp_arrival_generator()`, `R/environment.R`) passed directly to simmer's `add_generator()`, called once per arrival during the run itself (this excludes mass casualty event timing, which remains pre-computed and exogenous — see [Mass Casualty Event Injection](#5-mass-casualty-event-injection)). Each stream is a doubly stochastic Poisson process, also called a Cox process: a rate is drawn afresh for each simulated day from the stream's configured distribution, and arrivals within that day are Poisson at that rate, scaled by the live effective force size so arrival timing reacts to the replication's own in-run events (see [Force Regeneration and the Endogenous Feedback Loop](#6-force-regeneration-and-the-endogenous-feedback-loop)). The two timescales carry different evidence and are kept separate for that reason: the configured distribution describes how one day differs from the next, which is what FORECAS [[8]](#references) fitted, while the Poisson placement within a day is the standard model of individually unrelated events arriving at a known rate. The general process is outlined below.

[[8]](#references) fits casualty incidence to one of two distribution families, selected by battle intensity and troop type rather than a single distribution applying universally: a lognormal model for moderate/light-intensity combat troops and for support troops at all intensities, and an exponential model for combat troops in high-intensity battles. `generate_casualty_arrivals()` (`R/environment.R`) dispatches each casualty stream to `make_ln_arrival_generator()` or `make_exp_arrival_generator()` based on an explicit `distribution` field read from `env_data$vars$generators`. Both families share the same daily rate scaling, thinning sampler, and pair of realised properties (steps 2–4 below).
...
- $\lambda$ = exponential rate parameter passed to the daily rate draw (no $\sigma$ term — a reported standard deviation for an exponential-fitted stream is retained in `env_data.json` for citation only and plays no role in generation)

Nothing trims the drawn value, so each stream realises exactly the mean and the coefficient of variation its configuration names, and the two configured fields are independent of one another: editing a stream's standard deviation alone leaves its realised mean where it was. Because the draw is now made once per day rather than once per minute, both parameters describe the quantity they were fitted as, a daily rate and its day-to-day variation, and both reach the realised arrival stream (Step 4). `scripts/check_arrival_rate_fidelity.R` holds both families to both properties, exercising the shipped generator closures over a 20,000-day horizon.

#### 2. Daily Rate Sampling and Scaling

Draws one sample from the stream's selected distribution for each simulated day, representing that day's casualty rate, then scales it by population size to give an intensity per minute per 1000 personnel.

For each simulation day $d \in \{1, 2, \dots, n_{\text{days}}\}$, the arrival intensity in effect during that day is:

$$
\lambda_d(t) = X_d \times \frac{F(t)}{1000 \times 1440}
$$

Where:

- $X_d \sim \text{LogNormal}(\mu_{\log}, \sigma_{\log}^2)$ (lognormal streams) or $X_d \sim \text{Exponential}(\lambda)$ (exponential streams) is the rate drawn for day $d$
- $F(t)$ = population size (support or combat), not a fixed constant but a live, time-varying effective force size; see [Force Regeneration and the Endogenous Feedback Loop](#6-force-regeneration-and-the-endogenous-feedback-loop) below
- $\lambda_d(t)$ = arrival intensity, in casualties per simulated minute

The timescale of the draw is the substantive modelling choice here, not a computational detail. A rate redrawn every minute makes a day's casualty count an average of 1,440 independent draws, and the central limit theorem flattens the configured distribution long before those draws can accumulate into a daily total: the combat WIA stream realised a daily standard deviation of 0.50 against a mean of 4.42, where a plain Poisson process at the same rate gives 2.10, and in 5,000 simulated days it never produced a day worse than six casualties. Drawing once per day instead lets the configured distribution act at the timescale it was fitted at, and $\sigma$ becomes evidence about how one day differs from the next rather than a parameter averaged out of the result.

The drawn value is passed through untrimmed, which is what makes the realised daily rate the configured one: any threshold applied to a draw lowers the distribution's mean, and lowers it by an amount set by the stream's coefficient of variation rather than by anything a planner has chosen. Nothing stands between the configured parameters and the realised ones.

#### 3. Arrival Time Sampling by Thinning

Arrival times within a day are sampled directly, rather than detected on a grid of simulated minutes. The intensity $\lambda_d(t)$ is not constant within the day, because $F(t)$ moves as the run generates casualties and returns them to duty, so the process is a non-homogeneous Poisson process and is simulated by thinning [[52]](#references): candidate arrival times are generated under a dominating rate that is easy to sample from, and a controlled fraction of them is rejected so that what survives has the intended intensity.

The dominating rate holds the population term at $P_{\max}$, an upper bound on the pool's force size:

$$
\bar{\lambda}_d = X_d \times \frac{P_{\max}}{1000 \times 1440}
$$

$P_{\max}$ has to bound $F(t)$ for the whole run, or the acceptance probability below saturates at 1 and the stream generates at the dominating rate rather than the intended one. With reinforcement disabled, the shipped default, establishment strength is that bound: every casualty debits its pool and every return to duty credits back a casualty already debited. Reinforcement can carry a pool above establishment (Step 6), so an enabled configuration widens the bound to $(1 + \texttt{fill\_max\_frac})$ times establishment strength, which the demands in flight at any moment cannot exceed (`reinforcement_force_bound()`, `R/environment.R`). Widening it costs proposal draws in proportion, since the extra candidates are rejected, and that cost falls only on runs that enable reinforcement. Candidate gaps are then drawn from an Exponential($\bar{\lambda}_d$) distribution by inverse-CDF, and a candidate at time $t$ is accepted with probability $F(t) / P_{\max}$, so an accepted arrival occurs at intensity $\bar{\lambda}_d \times F(t)/P_{\max} = \lambda_d(t)$ as intended. A candidate falling past the end of its day is discarded and sampling restarts at the day boundary under the next day's rate, which is exact for a piecewise-constant rate because the exponential distribution is memoryless.

Two properties follow that the retired minute grid did not have. Arrival times are continuous, so no separate jitter step is needed to keep the six independent streams from registering tied event times for simmer's event queue to order arbitrarily: two continuous arrival times coincide with probability zero. And the cost of generation is linear in the rate actually drawn, a day proposing $X_d \times P_{\max}/1000$ candidates in expectation, where the minute walk performed 1,440 iterations per day whatever it drew. A lognormal stream at 500 casualties per day with a standard deviation of 5,000, two orders of magnitude above anything the model ships, generates its 200-day arrival stream in about a second. `scripts/check_arrival_rate_fidelity.R` exercises streams of that scale.

Each call to the generator closure returns the gap, in simulation minutes, between this arrival and the previous one (the value simmer's `add_generator()` `distribution` argument itself expects) rather than a pre-computed, already-sorted vector of arrival times; because each stream's own arrivals are produced in strictly increasing time order by construction, no separate sorting step is needed.

#### 4. Realised Daily Mean and Variance

A day's casualty count $N_d$ is Poisson conditional on the rate that day drew, and that rate is itself random, so the law of total variance gives the two quantities the construction delivers at force size $P$:

$$
\mathbb{E}[N_d] = \mu \times \frac{P}{1000}
\qquad
\mathrm{Var}[N_d] = \underbrace{\mu \times \frac{P}{1000}}_{\text{within-day}} + \underbrace{\left(\sigma \times \frac{P}{1000}\right)^2}_{\text{between-day}}
$$

The mean is the configured one, and the variance is the Poisson term a process of unrelated arrivals carries in any case, plus the between-day term the configuration's $\sigma$ names. Both configured fields therefore reach the output, which is the property the minute-by-minute accumulation lost: it delivered the mean and suppressed the standard deviation to roughly a quarter of the Poisson floor.

At the shipped combat WIA parameterisation against a full combat establishment, measured over 30,000 simulated days, the stream's daily count has a mean of 4.41 and a standard deviation of 9.5 against the 9.1 the formula above predicts. Its median day carries two casualties, its 99th-percentile day 38 and its busiest day in 30,000 carries 564; under the retired generator the busiest day in 5,000 carried six. Peak-day volume is what drives contention for operating theatres, intensive care beds and strategic airlift, so this is the property on which the model's queue and occupancy findings depend most and the one it previously represented least well.

`scripts/check_arrival_rate_fidelity.R` asserts both quantities against the formula above for every stream the project ships across all three profiles, and asserts separately that editing $\sigma$ alone moves the realised variance without moving the realised mean. The variance is judged against a multiplicative band rather than a sampling interval, because a sample variance of a lognormal-mixed count is itself a heavy-tailed estimator; the band is wide relative to that estimator's error and narrow relative to the defect it guards against, which was low by a factor of 58.

#### 5. Mass Casualty Event Injection

The background streams above model a sustained casualty rate; a mass casualty event models a single discrete incident, attributable to itself, whose casualties arrive together and are analysed as a group. The division of labour between the two is narrower than it was, and worth stating precisely, because the background streams now carry the day-to-day variation their configuration names (Step 4) and can therefore reach into the event sizes this feature injects. Measured over 30,000 simulated days at the shipped combat WIA parameterisation, the background stream delivers ten or more casualties within a two-hour window about once every six 30-day runs, twenty or more about once in fifty, and its heaviest such window in the whole horizon carries 67, which spans the 20 to 60 casualties a configured event injects. What it cannot supply is the attribution: a background surge is a heavy draw of the same process, tagged as nothing in particular, while an event's casualties carry a `mass_casualty_event_id` and can be followed as a cohort through the system. The event parameters therefore stand as they are, being a description of an incident rather than a compensation for a background that could not vary, and no recalibration against the restored variation is called for. `generate_mass_casualty_events()` (`R/environment.R`) overlays mass casualty events on the background `wia_cbt` combat-WIA stream via one of two selectable event-timing modes (`mass_casualty.event.mode`), sharing an identical per-event casualty-count/injection-window mechanism (`mass_casualty_event_casualties()`) regardless of which mode generated the event's start time. This complements the discrete-event mass-casualty-incident stress-testing precedent of SIMEDIS [[37]](#references), which, unlike either mode here, injects a fixed, deterministic victim count per scenario rather than a stochastically varying one.

The feature ships disabled by default (`mode = "poisson"`, `mass_casualty.event.rate_per_day = 0` in `env_data.json`).

**Mode 1 — `"poisson"` (default).** A compound Poisson process, informed by the compound Poisson parameterisation of Fischer et al. [[36]](#references) and the blast-dominant injury context reported for contemporary LSCO [[38]](#references). Event start times are drawn from a Poisson process with rate `mass_casualty.event.rate_per_day` via the standard exponential inter-arrival construction (`mass_casualty_event_starts_poisson()`):
...
**Event size** (both modes). Each fired event injects a number of casualties drawn from $\text{Uniform}(\text{min\_cas}, \text{max\_cas})$ (default 20–60), rounded to the nearest integer. That figure is the incident's total, which is how an incident's size is normally reported and planned against, not a count of survivors.

**Wounded and killed** (both modes). The total is split between casualties who reach the treatment system wounded and those killed at or near the point of injury, by a single $\text{Binomial}(n, \text{kia\_fraction})$ draw per event. The killed are overlaid on the background `kia_cbt` stream and the wounded on `wia_cbt`, so each takes the pathway its own background stream already takes: the killed are handled by the R1 mortuary section, moved by the KIA transport fleet, and never triaged. No open-access source was identified that tabulates event-level killed-to-wounded ratios for a comparable large scale combat campaign, the same evidentiary gap already noted for this feature's rate, size and window defaults, so the shipped `kia_fraction` of 0.28 is an informed estimate: the killed share implied by the model's own combat streams, whose FORECAS-derived means are 0.68 killed against 1.77 wounded per 1,000 per day [[8]](#references). That assumes an event's mortality mix matches sustained-tempo combat rather than asserting an unsourced departure from it. Were the true share higher, as the blast-dominant injury pattern behind the event triage split (below) might suggest, the model would understate mortuary and KIA transport contention during a surge and overstate the wounded load on the surgical echelons; the two errors move in opposite directions, so an event's total load on the system is less sensitive to this parameter than either component is. `scripts/check_mass_casualty_kia_split.R` asserts that an event's casualties are conserved across the split and that the parameter reaches nothing while injection is disabled.
...
**Triage priority** (both modes). Mass-casualty-derived casualties draw triage priority from an independently set distribution. The default distribution is intended to reflect the higher proportion of immediately life-threatening injuries in blast/fragmentation trauma relative to the mixed injury pattern of the background stream, consistent with the blast/fragmentation injury share reported in [[38]](#references).

**Stream merge and tagging.** An event's arrival times, from whichever mode is active, are interleaved into the matching background stream by `wrap_with_mass_casualty()` (`R/environment.R`), which wraps that stream's generator closure and emits whichever of the next background candidate and the next pending event arrival is earlier. Event and background casualties therefore reach the same trajectory through one chronologically ordered stream, and the background stream stays force-size-reactive while event timing stays exogenous. Each casualty is tagged with a `mass_casualty_event` attribute (1 = event-derived, 0 = background) and the identifier of the event it came from at the point of triage in `build_casualty_trajectory()`, on both pathways, enabling the post-hoc stress-test analysis in [Mass Casualty Event Stress Test](docs/Multi_Run_Analysis.md#mass-casualty-event-stress-test).

Disease and non-battle injury are deliberately outside this mechanism, on different grounds from the killed. A mass casualty event is a discrete tactical incident, and disease and non-battle injury are by definition not caused by one, so there is no causal link along which an event could generate them; the background DNBI streams carry that load at their own tempo throughout. Omitting the killed was a modelling gap, since an incident does produce them; omitting disease and non-battle injury is a modelling decision, and the two should not be read as one unmodelled thing.

#### 6. Force Regeneration and the Endogenous Feedback Loop

The population term $F(t)$ in the intensity formula (Step 2 above) is a live, time-varying effective force size, read from a simmer global (`effective_force_combat`/`effective_force_support`) that the running simulation updates as casualties occur and return to duty or where reinforcements are introduced, and resolved afresh at each candidate arrival the thinning sampler proposes (Step 3). Casualty production is thereby a function of the effective force, consistent with [[8]](#references) and [[9]](#references).
...
3. **Reinforcement** — every `force_regeneration.reinforcement.demand_interval_days` (`env_data.json`), each pool independently submits a demand equal to its current shortfall against establishment strength, net of any shortfall already claimed by an earlier, still-pending cycle (`initial − current − pending`, floored at 0), then, after a configurable `fulfillment_lag_days`, delivers a fraction of that demand drawn from a Triangular(`fill_min_frac`, `fill_mode_frac`, `fill_max_frac`) distribution and credits the whole of it (`build_reinforcement_trajectory()`, `R/trajectories.R`). Reinforcement joins the population on arrival, there being no formation-level reserve in the model to hold it in, so a package larger than the shortfall it was requested against carries the pool over establishment strength until casualties bring it back down. This is the one genuinely periodic mechanism in the model. It ships enabled on a 7-day demand cycle, and setting `demand_interval_days` to zero disables it, adding no generator and consuming no random draws.

**Why reinforcement ships enabled.** A disabled cycle models a force that is never replaced. Over a run of a few weeks that is immaterial, because attrition has not yet moved the population term far from establishment strength. Over a campaign it is not: the pool decays without a floor, and because casualty production is proportional to the effective force, arrivals decay with it. A long run made that way measures a force being consumed rather than a health system under sustained load, and the two questions have different answers. Reinforcement holds each pool near establishment, which is what makes a sustained-campaign question answerable at all. The cadence is set to 7 days to match the strategic aeromedical evacuation sortie interval (`role4.ame.schedule_interval_days`), so replacements arrive on the airbridge cadence that carries casualties out, and the 7-day `fulfillment_lag_days` already shipped gives a continuous pipeline with one cycle in flight at any moment rather than a monthly step. A planner should note that the mechanism raises health system load rather than relieving it: a pool held closer to establishment is a larger population at risk, so it generates proportionally more casualties through the same beds, and the force generation gained is paid for in casualty volume. The measured size of that trade is reported in [Multi-Run Analysis](docs/Multi_Run_Analysis.md).
...
A fraction above 1 is a package larger than the shortfall it was requested against, and the whole of it is credited, so the fraction the model applies is the fraction the distribution drew. Measured over 490 delivered cycles in each pool, the realised mean fill fraction is 0.715 (standard error 0.009) for the combat pool and 0.709 (0.009) for the support pool, against the distribution's own mean of 0.7167. About 4.4% of draws exceed the demand they were drawn against, and in that measurement 2.9% and 2.5% of cycles did so, the gap being the cycles whose shortfall grew between submission and delivery. What the surplus produces is a pool briefly over establishment strength, peaking 0.28% and 0.48% above it there. It is a transient rather than a new equilibrium, because demand is the shortfall floored at zero: an over-strength pool requests nothing, so no later cycle can build on the surplus and casualties bring it back down. `scripts/check_lever_realisation.R` asserts that every person delivered joins the population and that the over-strength state stays inside the bound the arrival sampler assumes.

Over-strength is representable but not easily reached at the model's own casualty rates. Across five 90-day runs with a 7-day demand cycle and a 7-day fulfillment lag, neither pool ever rose above establishment strength: casualty production kept both in shortfall throughout, so every delivery of at most 1.1 times a growing shortfall still fell short of it. Reaching over-strength needs a lightly attrited pool, a long demand cycle relative to the losses, or a fill maximum well above 1.

The reinforcement demand cycle, fulfillment lag, and fill distribution are the key modelling levers a planner controls: a baseline run with no reinforcement isolates the pure depletion effect of sustained casualty production against RTD-only regeneration; a run with a short demand cycle, short lag, and a fill distribution weighted toward full delivery approximates well-sustained LSCO reinforcement and keeps daily casualty volume closer to constant. The depletion case is shown for one campaign in [Force Regeneration Feedback Loop](docs/Single_Run_Analysis.md#force-regeneration-feedback-loop), and both are compared across replications in [Force Regeneration Under Reinforcement](docs/Multi_Run_Supplement.md#force-regeneration-under-reinforcement).
...
The 17% NBI proportion is drawn from FORECAS empirical data ([[8]](#references), pp 22–23). The remaining split between battle fatigue and disease is derived from historical LSCO data: approximately 25–30% of total DNBI evacuations across conflict periods are documented as psychiatric and battle fatigue cases [[9]](#references); 25% is used as a central estimate, with an over-estimate artificially reducing R2B/R2E load and an under-estimate over-loading the surgical pathway with non-surgical cases. With NBI fixed at 17% from [[8]](#references), disease is the residual category, representing approximately 53–58% of total DNBI — rounded to 58% as the central estimate for the model, since no open-access source directly measures the deployed disease DNBI proportion; because this proportion determines the fraction of DNBI routed to R2B holding rather than to surgery, a higher disease proportion would increase holding bed demand without affecting OT throughput.

The disease sub-type's 6% emergency surgical candidacy rate is an informed estimate derived from population-level surgical incidence in military-age males: appendicitis alone occurs at approximately 35–50 per 10,000 per year in this demographic [[50]](#references), with acute cholecystitis, perforated peptic ulcer, and complicated soft tissue infections adding further surgical demand, and against approximately 100 disease DNBI presentations per month in the modelled force these conditions yield approximately 3–6 surgical cases — consistent with emergency surgical care for disease conditions being documented as a significant component of deployed hospital workload [[51]](#references).

Only the NBI sub-type (17% of DNBI) enters the routine surgical pathway on the same terms as WIA; disease contributes a small additional load through its 6% emergency surgical rate, and battle fatigue contributes none. OT demand is therefore driven primarily by WIA and NBI casualties. Across 100 replications (30 days, control seed 42), the mean number of casualties requiring surgery per replication was 183.5 (SD 36.0; range 112 to 274), against a mean of 434.5 casualties generated per replication. Of the DNBI sub-types, NBI casualties required surgery at a rate of 81.1% (consistent with the WIA-equivalent trajectory), disease casualties at 6.0% (matching the 6% emergency surgical rate assumption), and battle fatigue casualties at 0.0% (by design). The dispersion is as informative as the mean: the per-replication standard deviation is roughly a fifth of the mean, and the heaviest replication carries almost two and a half times the surgical caseload of the lightest, because each stream draws its rate once per simulated day (see [Casualty Generation](#casualty-generation)) and so realises the between-day variation the configuration names.
...
This allows the incorporation of the well-documented relationship between time from injury and preventable death. An analysis of 4,596 battlefield deaths during Operations Enduring Freedom and Iraqi Freedom [[30]](#references) found that 87.3% resulted from haemorrhage, with the majority occurring within 30–90 minutes of injury. A study of a "golden hour" policy mandating surgical capability within 60 minutes of injury [[31]](#references) found it reduced preventable prehospital death rates from 32% to 3.5% in a Special Operations context, establishing a direct empirical link between time-to-care and survivability.
...
$p_{max}$ and the treatment efficacy factors ([Treatment Efficacy Modifiers](#treatment-efficacy-modifiers), below) were calibrated together: $p_{max} = 0.020$ reproduces the treated-cohort target specifically in combination with the OIF/OEF-era multipliers detailed there, so the two are entangled. Both shipped scenario profiles ([Scenario Profiles](#scenario-profiles)) resolve that entanglement rather than inheriting it, each pairing treatment efficacy factors appropriate to the era of care it represents with a ceiling re-calibrated against a mortality target drawn from the same campaign as its casualty rates: `moderate_intensity` against this same Ajax Bay bound, and `high_intensity` against the rate reported for the US Army on Okinawa. The `default` scenario runs this base configuration, retaining the OIF/OEF-era factors against the Falklands ceiling, which is the one pairing in the model that draws its two halves from different conflicts.
...
| -------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1 TCCC                    | 0.83   | Non-compressible haemorrhage (truncal, junctional) is identified as the mechanism in 90% of potentially preventable battlefield deaths [[30]](#references) — injuries beyond the scope of TCCC intervention. TCCC skills (tourniquet, wound packing, airway management) address the remaining 10%, yielding a modest 17% ceiling reduction. |
| R2B DCR (resus)            | 0.56   | Damage control resuscitation with balanced haemostatic products reduces laparotomy mortality from 22% to 13% [[32]](#references) — a 41% relative reduction — reflecting the haemostatic benefit of early plasma and platelet administration.                                                                              |
| R2B DCS (surgery)          | 0.32   | The PROMMTT study [[33]](#references) reported a 40% overall mortality rate in massively transfused surgical patients, with exsanguination accounting for 33.3% of deaths — approximately 13% haemorrhage-specific post-DCS mortality. This implies a 68% relative reduction from the pre-DCS ceiling, applied as a factor of 0.32.                |
| R2E DCR (resus)            | 0.56   | Same factor as R2B DCR [[32]](#references); applied only when full resuscitation occurs at R2E (i.e., the casualty bypassed R2B). Casualties pre-resuscitated at R2B receive a short resus at R2E; this factor is not re-applied, avoiding double-counting the DCR effect.                                                                                     |
| R2E DCS 1st op             | 0.25   | Post-operative mortality in optimally resuscitated DCS patients is approximately 3–5% at 30 days — a 75% relative reduction from the pre-first-DCS ceiling [[32]](#references).                                                                                                                                                                                |
...
Once a casualty is queued awaiting strategic AME (see [Role 4 (National Support Base) Demand Modelling](#role-4-national-support-base-demand-modelling)), the wait for evacuation capacity can be unbounded (see [Strategic Evacuation and Role 4 Demand](docs/Single_Run_Analysis.md#strategic-evacuation-and-role-4-demand)). `r2e_ame_dow_poll()` (`R/trajectories.R`) checks DOW risk periodically while the casualty waits, rather than once as at the other checkpoints, using the same conditional-increment logistic mechanism (`dow_prob_conditional()`, the same priority-based parameters and `dow_ceiling`):

1. **Capacity already available** — the casualty boards immediately, with no poll and no artificial minimum wait.
2. **Capacity unavailable** — the casualty enters a poll loop: wait `role4.ame.dow_check_interval` minutes, roll DOW (conditional on elapsed time since injury, same as every other checkpoint), then re-check AME capacity and repeat if still unavailable. A casualty who dies during this poll releases the R2E bed they were holding, records that release in `ame_hold_end`, and is routed to KIA processing exactly as at every other DOW checkpoint. Recording the release is what lets the holding pool's occupancy split charge that casualty for the bed time it served rather than for the whole of the wait it never completed, a casualty who dies here having no departure time to be bounded by. `dow_echelon = 5` distinguishes this checkpoint in `outputs/dow_by_echelon.csv` (`"ame_wait"`).
...
| Parameter group                                                     | `moderate_intensity` profile                                                                     | `high_intensity` profile                                                              |
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------- |
| Casualty generation rates and distribution family (`generators.*`)  | Inherited from base (already Falklands-sourced: FORECAS Table A.8 [[8]](#references), lognormal) | **Overridden**, FORECAS Tables A.7 and A.9 [[8]](#references), exponential (see below) |
| DOW ceiling and shape (`dow.params`)                                | **Overridden**, re-calibrated (see below)                                                        | **Overridden**, re-calibrated (see below)                                              |
| DOW treatment efficacy (`dow.treatment_efficacy`)                   | **Overridden**, era-appropriate factors (see below)                                              | **Overridden**, era-appropriate factors (see below)                                    |
| Priority distribution (`r1.priority`)                               | Inherited from base (no Falklands-specific triage data identified)                               | Inherited from base (no Okinawa-specific triage data identified)                       |
| DNBI composition, surgery/evacuation probabilities (`r1.other`)     | Inherited from base (already Falklands/FORECAS-sourced where cited)                              | Inherited from base (no Okinawa-specific source identified)                            |
| Transport time distributions (`*.wia_transport`, `*.kia_transport`) | Inherited from base (no Falklands-specific transport-time source identified)                     | Inherited from base (no Okinawa-specific transport-time source identified)             |
...
| ------------------------ | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1 TCCC                  | 1.0                  | TCCC is a post-1990s doctrine [[30]](#references); no equivalent tourniquet-forward or haemostatic-dressing prehospital doctrine is documented for 1982 British forces in the available sources, so no ceiling reduction is attributed to this checkpoint.                                                                                                        |
| R2B / R2E resuscitation  | 0.90                 | The base factor from [[32]](#references) is specific to balanced-component damage control resuscitation. A modest benefit from whole-blood/crystalloid resuscitation (available in 1982) is retained; the specific balanced-ratio benefit is not.                                                                                                |
...
The profile also overrides `dow.params` and `dow.treatment_efficacy`, on the same basis as `moderate_intensity`: the base ceiling was calibrated jointly with OIF/OEF-era efficacy factors against a Falklands mortality target (see [Parameter Calibration](#parameter-calibration)), and carrying that pairing into a 1945 casualty stream would credit a campaign fought without modern treatment technique with the benefit of it. The base value each factor modifies is given in [Treatment Efficacy Modifiers](#treatment-efficacy-modifiers).

| Factor                                            | `high_intensity` | Rationale                                                                                                                                                                                                                                                                                        |
| ------------------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| R1 TCCC                                           | 1.0              | TCCC is a post-1990s doctrine [[30]](#references). The 1945 battalion aid man carried morphine, sulfanilamide and dried plasma, none of which reaches the non-compressible haemorrhage that dominates potentially preventable death, so no ceiling reduction is attributed to this checkpoint.  |
| R2B / R2E resuscitation                           | 0.95             | Resuscitation was plasma-led, and plasma restores circulating volume without oxygen-carrying capacity. Whole blood reached the Pacific in quantity only late in the war, the Okinawa plan forecasting one pint per casualty against one pint per three casualties in 1943 [[54]](#references). |
| R2B DCS / R2E DCS 1st op                          | 0.70             | Forward surgery with anaesthesia, transfusion and penicillin was materially protective, and is why the war's died-of-wounds rate fell below 5% in its second half [[11]](#references). Damage control as a staged technique did not exist; a 1945 operation was a single definitive procedure.  |
| R2E DCS 2nd op                                    | 0.90             | Informed estimate, weakened beyond `moderate_intensity`'s 0.80 on the same reasoning: a second visit to the table in 1945 was re-exploration or delayed primary closure, not the planned definitive repair the base factor describes.                                                            |
| R2E post-op hold penalty, R2B forward ICU penalty | Unchanged        | Within-era relative degradations (recovery outside intensive care; intensive care without an intensivist) rather than period-specific treatment technology, and therefore not scenario-eligible.                                                                                                  |

These factors are informed estimates, disclosed as such because no open-access source quantifies 1945 Pacific-theatre casualty care in this model's multiplicative-ceiling terms. As under `moderate_intensity`, the paired ceiling re-calibration absorbs the magnitude of the estimate, so the aggregate mortality rate reaches its target whatever the precise factor values; what the values change is how mortality risk is distributed across care phases, not the total.

The calibration target is Okinawa's own died-of-wounds rate: the US Army on Okinawa reported that 3.4% of casualties who reached a hospital alive subsequently died there, against a World War II average of 4.5% [[54]](#references), and an independent recalculation of the war's aggregate figure on a consistent denominator gives 3.5% [[11]](#references). That denominator is the population reaching a treatment facility, which is the cohort the Ajax Bay target measures and the cohort the model reports, so each profile is compared against its own campaign on a like basis. Unlike the Ajax Bay figure, this one is a reported rate against a stated denominator rather than a bound derived from an inexact one, so the model is calibrated to reach it rather than only to stay beneath it.

With the factors above, `dow.params` was re-calibrated by the same iterative Monte Carlo procedure used for the base configuration and for `moderate_intensity`, reaching the target at `p1_p_max` = 0.052 and `p2_p_max` = 0.042, up from the base 0.020 and 0.016. The floors, the shape parameters and the P3 flat rate are unchanged from base, as under `moderate_intensity`; only the two ceilings are re-calibrated. The ceilings rise here where `moderate_intensity`'s fall, and the two movements have separate causes: `moderate_intensity` holds the same mortality target while weakening the factors that reduce the ceiling, so its ceiling must come down to compensate, whereas `high_intensity` weakens those factors against a target roughly seven times higher, which weaker factors alone do not reach. Pooling three independent 50-replication measurements (30 days, 150 replications in total) produced:

| Metric                  | `high_intensity` (150-rep)         | Historical comparator                                                     |
| ----------------------- | ---------------------------------- | ------------------------------------------------------------------------- |
| Treated-cohort DOW rate | 3.471% (95% CI [3.360%, 3.583%])   | 3.4% (US Army, Okinawa [[54]](#references))                               |
| DOW/WIA rate            | 3.164% (95% CI [3.066%, 3.262%])   | Not historically constrained (see [Further Development](#further-development), L22) |

The treated-cohort interval spans the historical rate. The three measurements behind it returned 3.592%, 3.463% and 3.359%, a spread wider than any one of them reports on its own, which is why the figure is pooled rather than measured once (see [Parameter Calibration](#parameter-calibration)). `scripts/check_dow_calibration.R` asserts the agreement, at the tolerance of two percentage points the profile was calibrated to.

A 30-replication run (30 days, seed 42) of each profile produced:

| Metric                                 | `moderate_intensity` (30-rep) | `high_intensity` (30-rep) |
| -------------------------------------- | ----------------------------- | ------------------------- |
| Mean WIA/run                           | 184.8                         | 688.3                     |
| Mean KIA/run                           | 68.7                          | 167.5                     |
| WIA+KIA ratio vs. `moderate_intensity` | 1.00×                         | 3.38×                     |

Neither profile trims its draws, so every stream under either profile realises the daily mean and the coefficient of variation its configuration names (see [Casualty Generation](#casualty-generation)). DNBI is not overridden by either profile and stays lognormal under both.

Mean DNBI per run falls under `high_intensity`, from 180.1 to 172.0, even though the profile leaves DNBI generation untouched. Casualty rates are set per 1,000 troops and scaled by the live force size (see [Force Regeneration and the Endogenous Feedback Loop](#6-force-regeneration-and-the-endogenous-feedback-loop)), so heavier battle attrition leaves fewer troops in theatre to fall sick. Battle attrition is heavier still now that the profile carries its own mortality model, since a casualty who dies of wounds never returns to duty.
...
The simulation is built as a Discrete Event Simulation (DES), it is written in R  using the simmer package [[17]](#references). DES has been used as a proven way to simulate healthcare systems and support healthcare decision-making (as shown in [[18]](#references)).
...
| `R/constants.R`                    | Values shared across modules: `DAY_MIN` (minutes per simulated day) and `MODEL_ATTRIBUTE_KEYS`, every per-casualty attribute key the model can set. Each module that needs one sources this file, the modules under `R/` being otherwise independent of each other |
| `R/queue_series.R`                 | A resource pool's total queue as a step function of time, its time-weighted mean over a series of bins, and whether it ever cleared. The resource monitor records each bed separately, so the pool total appears in none of its rows and is recovered by differencing each bed's series into changes and accumulating them in time order. Base R and independent of every other module, so `R/analysis.R` and `R/long_horizon.R` both source it |
| `R/long_horizon.R`                 | Sustained-operations replicated runner. Reduces each replication to a daily series inside the forked worker that produced it, so the parent process holds the series rather than the monitoring data; `block_means()` collapses that series to the per-block means the horizon's stability statement rests on |
| `R/airlift.R`                      | Strategic evacuation and Role 4 demand across replications: the per-replication response set, the sweeps over sortie reliability and sortie interval, and the split of R2E holding bed occupancy into in-theatre recovery and evacuation wait, the two demands that pool carries |
| `R/policy_sweep.R`                 | The evacuation policy and R2E holding establishment trade across replications at the sustained horizon: `reduce_policy_replication()` reduces one replication to the response set inside the forked worker that produced it, `run_policy_measurement()` sweeps a policy at a paired control seed, `set_hold_establishment()` varies the bed count the way the transport fleet size is varied, and `policy_paired_difference()`/`policy_replications_for()` report a difference within replication and the count an unresolved one would need. Forward stability is measured over the campaign's closing days via `policy_pool_state()` |
| `R/hold_window.R`                  | The R2B pre-open hold window across replications, comparing the shipped 60-minute window against a window of zero: `reduce_hold_window_replication()` reduces one replication to a row of casualty counts read off arrival attributes, `run_hold_window_measurement()` runs one arm at a paired control seed, and `hold_window_paired_difference()`/`hold_window_replications_for()` report the within-replication difference and the count an unresolved one would need, on `R/policy_sweep.R`'s arrangement |
| `R/icu_gate.R`                     | The post-operative intensive care gate across replications, comparing the shipped rationing rule against a configuration that reconstructs the model as it stood before the gate existed: `reduce_icu_gate_replication()` reduces one replication to a row carrying the R2E intensive care pool's occupancy and the post-operative pathway death counts, `run_icu_gate_measurement()` runs one arm at a paired control seed, and `icu_gate_paired_difference()`/`icu_gate_replications_for()` report the within-replication difference and the count an unresolved one would need, on `R/hold_window.R`'s arrangement for a paired two-arm comparison |
| `R/mass_casualty.R`                | The mass casualty event stress test across replications, comparing an arm with events injected against a background-only arm run under its own control seed rather than a paired one: `reduce_mass_casualty_replication()` reduces one replication to a row of casualty and death counts split by the `mass_casualty_event` attribute every casualty carries, `run_mass_casualty_measurement()` runs one arm, and `mass_casualty_dow_rate()` pools the died-of-wounds counts across an arm's replications into an exact binomial rate and interval rather than averaging a per-replication rate too rare to summarise that way |
| `R/censoring.R`                    | Right-censored interval estimation: the Kaplan-Meier survival curve, its restricted mean and quantile, and `censored_interval_stats()`, which summarises one interval over every casualty who entered it. Base R and independent of every other module, so `R/analysis.R` and `R/sensitivity.R` both source it and a KPI cannot drift from the screened response of the same name. Holds `INTERVAL_RESTRICTION_MIN`, the horizon the means are restricted to |
...
| `R/trajectories.R`                 | All simmer `trajectory()` definitions: R1, R2B, R2E, and core casualty flow. `build_casualty_trajectory()` is assembled from named builders, one per phase of the R1 pathway: `assign_injury_attributes()` and `assign_clinical_attributes()`, then `r1_wia_dnbi_path()` over `r1_battle_fatigue_hold()`, `r1_disease_evac_decision()` and `r1_nbi_wia_standard_path()`, with `r1_mortuary_handling()` and `r1_monitor_recovery()` shared by the arms that reach them. `r2b_treat_wia()` is assembled the same way, one builder per step of the R2B pathway: `r2b_dow_check()`, `r2b_resuscitation()`, `r2b_surgery_gate()` over `r2b_ot_check_path()` and `r2b_surgery_path()`, `r2b_no_surgery_path()` over its three holding-bed arms, and `r2b_evac_decision()`, with `r2b_evacuate_to_r2e()` shared by the four routes that leave for R2E. `r2e_treat_wia()` is assembled from one builder per phase, `r2e_arrival_dow_check()`, `r2e_resuscitation()`, `r2e_surgical_branch()` over `r2e_ot_surgery()` and the stabilisation and recovery trajectories, `r2e_second_surgery()`, `r2e_post_definitive_care()` and `r2e_disposition()` over `r2e_strategic_evac()` |
| `R/replication.R`                  | Multi-run replication framework: `run_once` (single replication with `wrap()`, assembled from the named steps `init_mass_casualty_stream`, `add_casualty_generators`, `add_reinforcement_cycle` and `add_strategic_evac`), `run_replications` (parallel `mclapply` over *n* replications, over `capture_rng_state`/`restore_rng_state`, `dispatch_replications` and `drop_failed_replications`), and `summarise_replications` (time-weighted KPI summary with 95% CI) |
| `R/analysis.R`                     | Analysis and visualisation pipeline; accepts monitoring data objects rather than reading from hardcoded CSV paths. `analyse_run()` and `analyse_replications()` are orchestrators over named single-purpose functions, one per stage: data preparation, per-domain summary, plotting and writing (see [Analysis Pipeline Structure](#analysis-pipeline-structure)). A bed queue figure covers every bed type its echelon fields, both echelons' plotters selecting bed resources by pattern and deriving the type from the resource name, so a bed type added to the establishment appears without a change here |
...
| `scripts/render_dow_survival.R`    | Renders the DOW survival function figure from the live `env_data.json` curve parameters, for the base configuration or a named scenario profile, so a re-fitted ceiling cannot leave the figure behind the calibration table |
| `scripts/render_morris_plots.R`    | Re-renders a completed screen's scatter plots from its saved design and responses, checking each against the ranking it accompanies before writing, so a plot cannot drift away from the table it illustrates |
...
| `scripts/test_sobol_separation.R`  | Tests whether a completed decomposition can tell one parameter from the next, bootstrapping the design rather than the indices so the correlation between two indices estimated from the same evaluations is retained, and reports the sample size each unestablished separation would need |
...
| `scripts/run_transport_sweep.R`    | CLI entry point for the transport fleet-size sweep (see [Transport Fleet-Size Sweep](docs/Multi_Run_Analysis.md#transport-fleet-size-sweep)) |
...
| `scripts/check_references.R`     | Checks the reference list of this README and the two analysis documents: that every `[[n]]` in the text resolves to an entry and every entry is cited at least once, that the list is numbered from one in order of first appearance, that no two entries name the same URL, and that each entry carries a URL and a retrieval date; exits non-zero on failure |
...
| `scripts/check_dow_calibration.R`  | Checks each shipped configuration's treated-cohort died-of-wounds rate against the historical anchor of the campaign it models, the Ajax Bay bound for the two Falklands-calibrated configurations and the reported Okinawa rate for `high_intensity`, pooling independent measurements because one does not resolve a response this sparse; exits non-zero on failure |
| `scripts/check_analysis_idempotence.R` | Checks that analysing one run's monitoring data twice gives one answer: that two `analyse_run()` calls return the same Role 4 census and write byte-identical outputs, that `analyse_replications()` does the same including the jittered mass casualty timeline, and that neither advances the caller's random number stream; exits non-zero on failure |
...
| `scripts/check_measurement_reproducibility.R` | Checks that a multi-replication measurement is a function of its control seed alone: that it repeats at that seed, that it does not depend on what preceded it in the session, that `run_replications()` leaves the caller's generator kind and stream position alone, and that a replication reproduces from its seed whichever path dispatched it; exits non-zero on failure |
...
| `scripts/check_console_bindings.R` | Checks that no console panel function reads a name another panel binds locally. Such a name parses and loads, and fails only when a user opens the panel that reads it, which no R-level test sees; also checks that no panel function is left uncalled. Exits non-zero on failure |
...
| `scripts/check_bed_queue_coverage.R` | Checks that each echelon's bed queue figure covers every bed type that echelon fields, that each panel label counts the beds in the pool it names, and that the panels share one vertical scale rather than each filling its own. The coverage assertion is made against a bed type no configuration ships, injected into the monitor, so it holds for a type added later rather than for today's establishment; exits non-zero on failure |
| `scripts/check_hold_episode_reconstruction.R` | Checks that an R2B holding episode is bounded by the attribute its own exit route sets, there being three routes out of a holding bed and only one of them setting `return_day`, and cross-checks the reconstructed bed-days against the resource monitor, which counts the beds directly and owes nothing to the attributes; exits non-zero on failure |
...
| `tests/playwright/`                | The console's browser test suite, driving a running app in headless Chromium (see [docs/Continuous_Integration.md](docs/Continuous_Integration.md)) |
| `package.json`, `package-lock.json`, `playwright.config.js` | The Node toolchain the browser suite needs, kept separate from `renv.lock` so that no browser automation package enters the R dependency set |
...
| `.github/`                         | Pull request template and the GitHub Actions workflow that runs the fast suite, the lint ratchet, the seed-42 reproduction and the console's browser suite on every pull request against `main` (see [docs/Continuous_Integration.md](docs/Continuous_Integration.md)) |
...
#### Shiny Console Structure

`server()` in `app.R` is an orchestrator rather than a worker. It creates the reactive values its panels share, then hands each panel the values that panel uses: one function per Configure group, one per asynchronous run (Quick Run, Full Analysis, Morris screening, Sobol decomposition, the transport sweep), and one per Analyse tab. A reader tracing what a control does goes to the function named for the panel it sits on.

The panels are plain functions rather than Shiny modules, and the distinction matters. A module namespaces its inputs, so converting to one would rename every control the app owns and break the browser suite's selectors along with any bookmark a user has. A plain function called from `server()` runs in the same reactive domain, so observers and outputs register exactly as before, and every input identifier is unchanged. Each panel returns every name it binds, and `server()` rebinds them, so its environment holds what it held before the split; that is what keeps `shiny::testServer()` able to reach the reactive graph.

What a panel reads is therefore visible in its signature, which is the property `scripts/check_console_bindings.R` defends: a panel reading a name another panel binds locally would parse, load, and fail only when a user opened it.
...
The console carries verification of its own, split by what each half can see. `shiny::testServer()` advances the reactive graph in process, with no browser and no HTTP server, and so covers the reactive state machine (the run, screening and sweep states; the configuration reactives derived from the loaded file; the validation that stops a run before it starts) at the cost of a few seconds; that suite lives in `tests/testthat` and runs inside the fast suite through `scripts/check_testthat.R`. What the console renders can only be seen in a browser, so the second half drives a running app in headless Chromium from `tests/playwright`, asserting that a parameter edit round-trips, that a Quick Run completes and populates the Analyse panel, and that each result tab renders. Playwright is used rather than a snapshot-based R harness for two reasons: it keeps browser automation out of `renv.lock` entirely, its dependencies living in `package.json` alongside rather than inside the R dependency set, and it drives the app over HTTP, so it is indifferent to how the server function is structured internally. The cost is a Node toolchain in an otherwise pure-R repository, accepted deliberately. Neither suite asserts appearance: a plot is checked for having rendered and for having real dimensions, never compared pixel by pixel, so a layout regression that breaks no assertion passes both.

A GitHub Actions workflow runs the fast suite, the lint ratchet, the seed-42 reproduction and the console's browser suite on every pull request against `main`, in the same pinned container the baseline figures are produced in, so that a difference the gate reports is a difference in the code rather than in the environment. A pull request that touches no code runs only the three checks that read the tracked documents, decided from the diff rather than from a path filter so that every job still reports. The slow suite runs weekly and on demand, and [docs/Continuous_Integration.md](docs/Continuous_Integration.md) is the operating guide: what runs when, how to dispatch the slow suite, and what each way the gate can fail calls for. The reproduction check is the one that protects the published figures: a change that shifts the random number stream invalidates the seed-42 tables in this document, `docs/Single_Run_Analysis.md` and `docs/Multi_Run_Analysis.md` at once, and no other check in the suite would notice.
...
Replications are dispatched in parallel via `mclapply` whenever the platform is not Windows and more than one replication is requested; any other case runs them sequentially through `lapply`. Both paths run under `RNGkind("L'Ecuyer-CMRG")`, and the parallel path passes `mc.set.seed = TRUE`, which assigns each worker a distinct substream of the underlying MRG32k3a generator. That generator has period $\rho \approx 2^{191}$ with substream spacing $2^{76}$, so stream overlap is impossible within any realistic simulation workload; the practical application of this mechanism in R via the `parallel` and `rstream` packages is demonstrated in [[43]](#references). The worker count is taken from the `mc.cores` option where set, falling back to `parallel::detectCores(logical = FALSE)`, the physical rather than logical core count.

Each replication is dispatched as its own forked process rather than as part of a pre-divided batch. That is a memory constraint rather than a scheduling preference. Building the casualty trajectory retains roughly 175 MB that neither releasing it nor requesting garbage collection reclaims, and the retention is a per-activity cost in the simulation library rather than in this project's code: a minimal trajectory of nothing but timeouts retains a constant 6.0 R cons cells per activity, linear from 250 activities to 8,000, and the casualty trajectory carries 12,612 of them. A worker that runs several replications before exiting therefore accumulates that cost once per replication, and peak memory grows with the replication count; a worker that runs one and exits returns it to the operating system. Measured at 30 days on two cores, peak memory across the process tree ran 1,340 MB at four replications and 2,084 MB at eight when batched, against 884 MB and 796 MB when each replication had its own process. `scripts/check_replication_memory.R` asserts that the peak stays flat as the count rises, so the arrangement cannot be reverted silently.
...
**A measurement is a function of its control seed alone**, and of nothing else about the session it was taken in. `run_replications()` snapshots the caller's generator kind and stream position on entry and restores both on exit, so it mutates no global random number state and leaves the caller's stream where it found it. The per-replication seeds are drawn before the generator kind is set, under whichever kind the caller supplied, because `RNGkind()` re-initialises `.Random.seed` from the system clock every time it is called, including when called with the kind already in effect; drawing the seeds after it would make them a function of the wall clock rather than of the caller's `set.seed()` value. Two properties follow, and `scripts/check_measurement_reproducibility.R` asserts both. A figure stated at a control seed reproduces at that seed whatever else the session has already measured, so a scenario measured on its own agrees with the same scenario measured third in a comparison, and a before-and-after comparison across a code change is a comparison of the change rather than of how the two arms were invoked. And because both dispatch paths run under one generator, a replication's output depends on its seed rather than on the path that dispatched it, so a single-replication measurement, a Windows measurement and an `mclapply` batch agree with one another.

**Analysing a run twice gives one answer.** The pipeline is a report over monitoring data the simulation has already produced, so it must return the same figures however many times it is run over the same data. One derivation inside it draws random numbers, the Role 4 length of stay (see [Role 4 (National Support Base) Demand Modelling](#role-4-national-support-base-demand-modelling)), and the pooled mass casualty timeline jitters its points when it renders. Both are made under a saved and restored random number stream, so the pipeline consumes no net randomness and a second analysis resumes from the position the first started at rather than continuing past it. `scripts/check_analysis_idempotence.R` asserts the idempotence of both entry points and the stream neutrality it rests on.
...
How each replicated experiment is configured on top of this framework, and how many replications a given response needs before its interval resolves anything, is set out in [Experimental Designs](docs/Multi_Run_Supplement.md#experimental-designs) and [Replication Count and Resolution](docs/Multi_Run_Supplement.md#replication-count-and-resolution).

#### Warm-up Period Analysis

Discrete event simulations are classified as either **terminating** or **steady-state** based on the nature of the system being modelled [[42]](#references). A terminating simulation has a natural, finite end state (for example, an operational campaign concluding after a defined horizon); the run begins under well-defined initial conditions, and behaviour across the entire horizon, including the start-up period, is of direct interest. A steady-state simulation models a perpetual system in which the long-run equilibrium is the quantity of interest; here, the initialisation transient is an artefact that must be discarded before meaningful statistics can be collected. The choice of classification governs whether warm-up exclusion is appropriate.

Welch's graphical method [[41]](#references) was applied to characterise the simulation's time-varying behaviour and determine which classification applies. The method involves: (1) running at least 10 independent replications of an extended simulation (90 days); (2) computing the cross-replication cumulative moving average (CMA) of a sensitive KPI at each time point; and (3) determining whether the CMA converges to a stable level. The R2E ICU queue was selected as the KPI, being the most congestion-sensitive resource in the model.
...
| Theatre evacuation policy       | `evacuation_policy_days` | 21 days | 15 | 60 | A    |
...
| R2E holding bed count            | `r2e_hold_beds`     | 30 beds   | 20    | 40    | B    |
| R2E intensive care bed count     | `r2e_icu_beds`      | 4 beds    | 2     | 6     | B    |

**Died of Wounds — logistic curve and treatment efficacy**

| Parameter                           | Variable                                | Baseline | Lower  | Upper | Rule |
| ----------------------------------- | --------------------------------------- | -------- | ------ | ----- | ---- |
| P1 DOW max probability              | `p1_p_max`                              | 2.0%     | 1.0%   | 4.0%  | A    |
...
| P2 DOW max probability              | `p2_p_max`                              | 1.6%     | 0.8%   | 3.2%  | A    |
...
| R2B dwell time | C1, C3, C4 | `r2b_dwell_mean` | Restricted mean of the distribution over every casualty who entered the echelon, censored stays included |
| R2B→R2E transit time | C1, C3 | `r2b_r2e_transit_mean` | Restricted mean of the distribution; no transit is censored at any configuration measured, so this is the arithmetic mean |
| R2E dwell time | C1, C3, C4 | `r2e_dwell_mean` | Restricted mean of the distribution over every casualty who entered the echelon, censored stays included |
...
| Resource queue length over time | C3, C4 | `r2b_ot_q`, `r2e_ot_q`, `r2e_icu_q`, `r2b_hold_q`, `r2e_hold_q`, `transport_q` | Time-weighted mean queue length |
...
| `r2e_hold_beds`              | R2E Holding Bed Count                     | Health System Design - Capacity |
| `r2e_hold_mode`              | R2E Base Recovery-to-Duty Duration (Mode) | Health System Design - Capacity |
| `r2e_icu_beds`               | R2E Intensive Care Bed Count              | Health System Design - Capacity |
...
Eight parameters take their bounds from neither rule. `r2b_icu_share` is swept across the whole of its domain, 0 to 1, because it is a policy lever with no baseline to vary around: the shipped value of zero is one end of the range a planner may choose from rather than an estimate of anything. `r2b_forward_hold_max` is swept from zero, which disables forward holding outright, to 2,880 minutes, which exceeds the longest stabilisation requirement the model can draw and so leaves the share acting alone. `r2b_pre_open_window` is swept on the same basis, from zero, which restores instant diversion, to 360 minutes, half a 12-hour off-shift period and long enough that the hold is plainly no longer a bridge to an imminent shift; its 60-minute baseline is an informed estimate rather than a measured quantity (see Further Development L28), which is the second reason a proportional margin around it would claim more than the value supports. `r2b_icu_penalty` takes the 95% confidence interval its own source reports, 1.09 to 1.59, in preference to a fixed proportional margin around the point estimate, which is both better grounded and narrower than Rule A would give. `ame_failure_probability` is swept from zero, the shipped value, to 0.30, on the same basis as the forward ICU share: a sortie cancellation rate is a property of the theatre a planner states rather than a quantity this model estimates, so the shipped zero is one end of the range rather than a baseline to vary around, and the screen measures what the trauma system absorbs across the range a campaign might present. `r2b_evac_threshold` and `saturation_queue_threshold` take the ranges their own planning sweeps measure, 0 to 10,080 minutes and 0 to 24 queued casualties, each from its shipped value to a point past which the lever stops changing the outcome; the evacuation threshold ships at zero, which makes its rank a measurement of switching it on, as the discussion of the current ranking below sets out. `role4_return_interval_mode` spans its whole distribution envelope, 2 to 3 days, which is itself the 48-to-72-hour return interval its source states, so no narrower range is defensible and no wider one fits inside the envelope.
...
```bash
# Full Morris screening: r=20 trajectories × (80 + 1) = 1,620 design points, 5 reps each
...
Outputs are written to `outputs/morris_ranking_<response>.csv`, one ranking per response in the [Screening Response Set](#screening-response-set), each carrying the response's criteria mapping and its degeneracy diagnostics alongside the per-parameter µ\* and σ. `outputs/morris_ranking.csv` repeats the primary system OT queue ranking under its historical filename. Scatter plots are written per response to `outputs/images/morris_<response>.png`, which is gitignored; `--images-dir images` redirects them to the tracked baseline location, which is a deliberate act rather than a default, since a screen writes one plot per response and would otherwise scatter untracked files through a tracked directory. When `--sobol` is specified, first-order (S1) and total-order (ST) indices for the top-ranked parameters are written to `outputs/sobol_<kpi>.csv`. Each row carries a `flag` column marking an index outside the theoretical range, and a sidecar `sobol_run_metadata.csv` records the design that produced it: the sample size, the estimator, the bootstrap resample count, the selected parameters and the commit. Three further scripts interrogate a completed decomposition without running the model again. `scripts/compare_sobol_estimators.R` recomputes the cached responses under two alternative pick-freeze estimators [[56]](#references), `scripts/test_sobol_separation.R` tests which orderings the sample actually establishes, and `scripts/measure_noise_floor.R` measures how much of the variance is replication noise. What the current decomposition supports, and what it does not, is set out in Further Development L29.

**Current ranking.** The table below is `outputs/morris_ranking.csv` for the current seventy-eight-parameter set, run at r = 20 trajectories with 5 replications over 30 days at seed 42: 1,580 design points, about fourteen hours of computation. Every screened parameter carries a measured rank, the six composition balance coordinates included, interleaved by µ\* rather than reported separately. The screen was made in an unpinned R 4.3.3 environment rather than the pinned Dev Container, and the host restarted once during the run, so the last 140 points were evaluated in a second attempt resuming from the design point cache; `scripts/check_screen_order.R` asserts that a resumed screen re-evaluates nothing and evaluates its design in index order, which is the property that makes the assembled result identical to one produced in a single pass.

`mass_casualty_rate` leads at µ\* = 10.98, nearly double the next parameter, but at this trajectory count no parameter is separated from the one below it: the leader would need 45 trajectories to separate from `mass_casualty_max_cas` at 5.61, and the pairs below it far more. `pri1_evac_prob` at 5.33 and `pri1_surg_prob` at 4.20 complete a leading group whose internal order the screen does not establish.

The comparison the balance coordinates were added to make now resolves differently. `triage_p1_balance`, which governs the Priority 1 share of the casualty mix, ranks **ninth of seventy-eight** at µ\* = 3.27, below `pri1_evac_prob` at rank 3 and `pri1_surg_prob` at rank 4, the two Priority 1 conditional rates defined on that share. Neither ordering is established at r = 20: separating the coordinate from `pri1_evac_prob` would need 51 trajectories and from `pri1_surg_prob` some 146, so the three sit in one band. Two other coordinates rank above it, `mc_p1_balance` at 6 and `dnbi_bf_nbi_balance` at 8, and the remaining three sit lower, `dnbi_disease_balance` at 24, `triage_p2_p3_balance` at 36 and `mc_p2_p3_balance` at 47. Each of the three composition groups therefore places a coordinate in the top ten, which is the condition under which a group is carried into a confirmatory decomposition and treated as a single object through the whole-composition Dirichlet sampling `run_sobol()` provides; that decomposition has not been run against this ranking (see [`data/sensitivity/`](data/sensitivity/README.md)).

An earlier screen at r = 5 put `triage_p1_balance` first at µ\* = 8.63, and that result did not survive the quadrupled trajectory count. It carried the largest σ in that table, 10.69, giving a standard error of ±4.78 on a µ\* of 8.63: its rank was never separable from noise. This is the clearest illustration in the project of why the trajectory count matters, and why a rank should not be read as an ordering when the gaps between neighbours are smaller than their standard errors. Separation requirements quoted in this section are the trajectory count at which two parameters' µ\* would differ at 95%, computed from the elementary effects themselves: µ\* is a mean of absolute effects, so its standard error is the standard deviation of those absolute effects divided by $\sqrt{r}$. On that basis ranks 1 and 2 would need 45 trajectories, ranks 2 and 3 some 7,000, ranks 3 and 4 some 184 and ranks 4 and 5 some 43,000. The leading cluster is a group rather than a sequence.

Three cautions on reading the table. **It is screened at a 30-day horizon**, so it ranks parameters by their influence on a month of campaign, and a parameter whose effect accumulates over a longer operation is ranked below what a sustained campaign would give it; the strategic airlift cancellation probability is the known instance, its influence on the holding pool building over many months rather than weeks. It is also a ranking on one response, the system OT queue; a screening run writes a ranking for each of the thirty-six responses in the set, and the time-to-care, return-to-duty and strategic evacuation responses are read from their own files. And `r2b_ot_q` carried no usable variation across the design and is flagged degenerate in its own ranking file, µ\* and σ written as NA rather than zero, which is the R2B theatre queue reading zero at every design point rather than a failure of the screen.

Eight parameters sit at µ\* exactly zero, and for two different reasons. `p3_flat`, like `r2b_icu_penalty` just above it, is a mortality parameter that acts on who dies rather than on how long a queue grows, so a queue-ranked screen is the wrong instrument for it. The other seven are Role 4 parameters, and they are zero by construction on every in-theatre response: the national support base census is computed after the simulation has finished ([Length of Stay](#length-of-stay)), so nothing in theatre can read them. The four length-of-stay modes and `reconstruction_share` do register on the two Role 4 occupancy responses, though well down both tables, at 51st or below. Three of the seven, `role4_return_interval_mode`, `role4_post_reconstruction_return_rate` and `role4_icu_continuation_mode`, are zero on all thirty-six responses, because none of the set measures what they change: the number of operations owed at the national support base, the return of a casualty after reconstruction, and the split of a stay between intensive care and a step-down ward, which conserves the stay's length and therefore the census. They stay in the design so that a response added later can rank them without a new parameter set, and until then their rank carries no information.

`r2b_evac_threshold` needs reading differently from every other row. It ships disabled at zero, and a Morris elementary effect taken across its first grid step measures switching the threshold on rather than the effect of its magnitude; the planning sweep behind [R2B Holding Capacity vs. Evacuation Threshold Sweep](docs/Multi_Run_Analysis.md#r2b-holding-capacity-vs-evacuation-threshold-sweep) measured that the single step from disabled to one day produces most of its whole effect on the R2B holding queue. It ranks 43rd on the system queue but first on R2B dwell, both return-to-duty rates at R2B and R2E, and the p90 time to first surgery, and every one of those ranks is a measurement of the on/off transition. It is screened so that the other parameters are evaluated across configurations in which the threshold is in force as well as disabled; its own rank should not be read as a planning influence, and the sweep is where its magnitude is measured.

| Rank | Parameter | µ\* | σ | Rank | Parameter | µ\* | σ |
| ---- | --------- | ---- | -- | ---- | --------- | ---- | -- |
| 1 | `mass_casualty_rate` | 10.9779 | 15.2183 | 40 | `r1_wia_treat_mode` | 1.4737 | 2.2557 |
| 2 | `mass_casualty_max_cas` | 5.6103 | 10.0608 | 41 | `r2e_vent_share` | 1.4342 | 1.9696 |
| 3 | `pri1_evac_prob` | 5.3332 | 6.6284 | 42 | `post_op_hold_mode` | 1.3949 | 2.1602 |
| 4 | `pri1_surg_prob` | 4.1984 | 4.4708 | 43 | `r2b_evac_threshold` | 1.3256 | 1.8175 |
| 5 | `mass_casualty_kia_fraction` | 4.1255 | 6.4210 | 44 | `reconstruction_share` | 1.2896 | 2.3289 |
| 6 | `mc_p1_balance` | 4.1170 | 6.9559 | 45 | `r2e_critical_hold_mode` | 1.2711 | 1.7462 |
| 7 | `surg_mode` | 3.8395 | 5.7254 | 46 | `dnbi_cbt_mean` | 1.1433 | 1.5485 |
| 8 | `dnbi_bf_nbi_balance` | 3.2838 | 5.2158 | 47 | `mc_p2_p3_balance` | 1.1329 | 1.8577 |
| 9 | `triage_p1_balance` | 3.2749 | 4.2231 | 48 | `r2b_hold_threshold` | 1.1174 | 1.7055 |
| 10 | `kia_spt_mean` | 3.0104 | 5.2953 | 49 | `r2b_icu_share` | 0.9265 | 1.4232 |
| 11 | `r1_transport` | 2.9546 | 6.5976 | 50 | `p1_t_mid` | 0.9073 | 1.5056 |
| 12 | `mass_casualty_min_cas` | 2.7583 | 3.9966 | 51 | `r1_tccc_factor` | 0.8295 | 1.3264 |
| 13 | `r2b_pre_open_window` | 2.7092 | 4.9832 | 52 | `r2b_forward_hold_max` | 0.7162 | 1.7247 |
| 14 | `post_definitive_icu_mode` | 2.6012 | 3.9449 | 53 | `pri3_dnbi_surg_prob` | 0.7140 | 1.0969 |
| 15 | `saturation_queue_threshold` | 2.5520 | 4.0203 | 54 | `pri3_dcs_rate` | 0.6735 | 1.5695 |
| 16 | `long_resus_mode` | 2.4820 | 3.7501 | 55 | `ame_failure_probability` | 0.4640 | 0.7526 |
| 17 | `disease_surgery_pct` | 2.2831 | 4.0692 | 56 | `p2_p_max` | 0.4360 | 0.7443 |
| 18 | `wia_spt_mean` | 2.2528 | 3.5975 | 57 | `r2b_dcs_factor` | 0.4139 | 1.6251 |
| 19 | `pri2_dcs_rate` | 2.1502 | 3.1938 | 58 | `p1_k` | 0.4076 | 0.8671 |
| 20 | `dnbi_spt_mean` | 2.1097 | 3.0731 | 59 | `fr_fulfillment_lag_days` | 0.3047 | 0.6025 |
| 21 | `evacuation_policy_days` | 2.0802 | 4.1310 | 60 | `r2b_resus_factor` | 0.2916 | 0.5789 |
| 22 | `r1_recovery_mode` | 2.0602 | 3.0533 | 61 | `p2_t_mid` | 0.2032 | 0.4763 |
| 23 | `stabilisation_icu_mode` | 2.0531 | 2.4836 | 62 | `p1_p_base` | 0.2001 | 0.4535 |
| 24 | `dnbi_disease_balance` | 2.0036 | 2.9767 | 63 | `fr_fill_mode_frac` | 0.1642 | 0.3350 |
| 25 | `pri1_dcs_rate` | 1.9904 | 3.8450 | 64 | `p2_k` | 0.1449 | 0.4741 |
| 26 | `ame_schedule_interval_days` | 1.9708 | 3.0879 | 65 | `r2e_resus_factor` | 0.1042 | 0.2636 |
| 27 | `short_resus_mode` | 1.9676 | 2.7877 | 66 | `r2e_dcs1_factor` | 0.0881 | 0.3093 |
| 28 | `p1_p_max` | 1.9448 | 3.6594 | 67 | `p2_p_base` | 0.0576 | 0.1591 |
| 29 | `kia_cbt_mean` | 1.9274 | 2.7029 | 68 | `r2e_postop_hold_penalty` | 0.0405 | 0.1794 |
| 30 | `pri3_other_surg_prob` | 1.9179 | 3.9331 | 69 | `r2e_dcs2_factor` | 0.0233 | 0.1041 |
| 31 | `pri2_surg_prob` | 1.8690 | 2.5569 | 70 | `r2b_icu_penalty` | 0.0002 | 0.0010 |
| 32 | `ot_hours` | 1.8525 | 2.8946 | 71 | `p3_flat` | 0.0000 | 0.0000 |
| 33 | `r2b_hold_mode` | 1.7847 | 4.1118 | 72 | `role4_return_interval_mode` | 0.0000 | 0.0000 |
| 34 | `pri2_evac_prob` | 1.6848 | 2.4739 | 73 | `role4_post_reconstruction_return_rate` | 0.0000 | 0.0000 |
| 35 | `r2b_transport` | 1.6824 | 2.4134 | 74 | `role4_los_p1_surgical_mode` | 0.0000 | 0.0000 |
| 36 | `triage_p2_p3_balance` | 1.6782 | 2.3349 | 75 | `role4_los_p1_nonsurgical_mode` | 0.0000 | 0.0000 |
| 37 | `fr_demand_interval_days` | 1.6605 | 2.5179 | 76 | `role4_los_p2_mode` | 0.0000 | 0.0000 |
| 38 | `wia_cbt_mean` | 1.6177 | 2.4809 | 77 | `role4_los_p3_dnbi_mode` | 0.0000 | 0.0000 |
| 39 | `r2e_hold_mode` | 1.6084 | 2.6234 | 78 | `role4_icu_continuation_mode` | 0.0000 | 0.0000 |

**What the ranking tells you.** The casualty load dominates. Mass Casualty Event Rate (`mass_casualty_rate`) leads at µ\* 10.98, followed by Mass Casualty Event Size Maximum (`mass_casualty_max_cas`) at 5.61, Priority 1 Strategic Evacuation Rate (`pri1_evac_prob`) at 5.33 and Priority 1 Surgical Candidacy (`pri1_surg_prob`) at 4.20, against a median of 1.54 across the set. Before relying on a run, check that the mass casualty configuration matches the scenario being modelled: an error there shifts surgical queue depth more than an error anywhere else.

Nine of the top ten are Scenario / Casualty Context parameters, so results depend most on assumptions about the casualty load rather than on decisions the health system can make. The tenth is Capacity, Surgery Duration (`surg_mode`) at rank 7, and no Policy parameter reaches the top ten: the highest are the R2B Pre-Open Hold Window (`r2b_pre_open_window`) at 13 and the Forward Theatre Saturation Release Threshold (`saturation_queue_threshold`) at 15, with the Theatre Evacuation Policy (`evacuation_policy_days`) at 21. A parameter's category tells you what a high rank means in practice and matters more than its exact position; see "Reading the plots: colour coding" below.

**Effects are not independent.** σ exceeds µ\* for 70 of the 78 parameters, meaning most parameters' influence depends on where the others are set. A parameter that looks inert at the baseline may matter once other values move, so conclusions drawn by changing one input at a time will understate what the model can do. Vary related parameters together, or compare whole scenario profiles, rather than tuning singly.

**Two results to read carefully.** The Reinforcement Demand Cycle ranks 37th, mid-table, and that ranking surrounds the shipped 7-day cycle rather than the disabled baseline an earlier screen was taken from; a parameter's elementary effects are local to the baseline they are screened from, so this is the figure to read for the configuration as it ships. Separately, ranks toward the bottom sit very close together: the ten lowest span 0.023 down to zero, and eight are exactly zero for the reasons given above. Treat the lower half of the table as a band rather than an order.

**Influence varies widely inside a single mechanism.** Reinforcement is the clearest case: the demand cycle ranks 37th while Reinforcement Fulfillment Lag (`fr_fulfillment_lag_days`) ranks 59th and Reinforcement Fill Distribution (`fr_fill_mode_frac`) 63rd of 78, so cycle length is the value to get right when configuring it. Mass casualty events run the other way, every part of them ranking high: Event Rate (`mass_casualty_rate`) 1st, Event Size Maximum (`mass_casualty_max_cas`) 2nd, the Wounded/Killed Split (`mass_casualty_kia_fraction`) 5th, the triage balance of an event's casualties (`mc_p1_balance`) 6th and Event Size Minimum (`mass_casualty_min_cas`) 12th, so the whole mechanism needs to match the scenario rather than one value within it.

**The two dwell-time responses.** R2B and R2E dwell were re-screened under their current definition, the Kaplan-Meier restricted mean over every casualty who entered the echelon, and their rankings replace those taken under the earlier definition, which dropped any stay still running when the window closed. Across the sixty-five parameters both screens share, the order held broadly: the rank correlation with the superseded ranking is 0.89 for R2B dwell and 0.88 for R2E dwell. At R2B the leaders below `r2b_evac_threshold`, whose rank is the on/off transition described above, are the parameters that led before: Priority 1 Surgical Candidacy, the Hold-Bed Reroute Threshold, the R2B Holding Bed Duration and Priority 2 Surgical Candidacy. At R2E the Theatre Evacuation Policy leads at more than 2.5 times the next parameter. Three parameters moved sharply: the AME Sortie Interval rises from 11th to 3rd, the Priority 2 against Priority 3 triage balance from 31st to 7th and Surgery Duration from 27th to 9th. The sortie interval's rise is the one the correction was expected to produce, since a casualty waiting for a flight holds an R2E bed past day 30 more often the longer the interval; the screen does not by itself explain the other two.
...
- **Orange, Scenario / Casualty Context (55 parameters).** Facts about the operating environment or the casualty population: casualty generation rates, the DOW curve, clinical-need composition, treatment efficacy, transport times between echelons, and the course of care at the national support base. Nobody chooses these. A high rank means the conclusion depends on how severe the scenario turns out to be.
- **Green, Health System Design - Capacity (13 parameters).** How long a treatment or holding step takes at current resourcing, such as Surgery Duration (`surg_mode`) and R1 WIA Treatment Time (`r1_wia_treat_mode`), plus the R2E intensive care and holding bed establishment counts (`r2e_icu_beds`, `r2e_hold_beds`). Shortening these needs investment in staff, equipment or training; the two bed counts need investment in the establishment itself. A high rank shows where capacity investment would have the largest effect.
- **Blue, Health System Design - Policy (12 parameters).** Thresholds, cadences and scheduling rules set by standing order: R2B Hold-Bed Reroute Threshold (`r2b_hold_threshold`), R2B Holding Evacuation Threshold (`r2b_evac_threshold`), R2B Pre-Open Hold Window (`r2b_pre_open_window`), Forward Theatre Saturation Release Threshold (`saturation_queue_threshold`), OT Shift Length (`ot_hours`), AME Sortie Interval (`ame_schedule_interval_days`), the reinforcement demand cycle, and Theatre Evacuation Policy (`evacuation_policy_days`). These can be changed by decision, without new resources, so a high rank here is the most immediately actionable result the screen produces.

Four assignments are judgement calls, and each affects how a result should be read. Transport times (`r1_transport`, `r2b_transport`) are Context rather than Capacity, because terrain and distance dominate them rather than vehicle numbers. AME Sortie Cancellation Probability (`ame_failure_probability`) is Context despite sitting among Policy AME settings, since weather, tasking and airframe availability drive it. The Forward ICU DOW Penalty (`r2b_icu_penalty`) is Context while the Forward ICU Share it prices (`r2b_icu_share`) is Policy: the mortality cost of an intensive care section without an intensivist is a fact about the establishment, while how much care to deliver forward is a disposition a commander orders. Reinforcement Fulfillment Lag and Fill Distribution are Policy on the view that commanders influence both through how they prioritise requests, which is the least clear-cut call in the set. The Role 4 parameters are all Context, the course of care at the national support base being outside the deployed health system's control, and the Pre-Flight Critical Hold Ventilated Share (`r2e_vent_share`) is Context while its duration (`r2e_critical_hold_mode`) is Capacity, on the same reasoning as the forward ICU pair. The `category` field in `R/sensitivity.R` records the rule applied.

A scatter plot is written for every response in the set. The seven reproduced below are the ones the tracked baseline carries, all of them rendered from the r = 20 screen whose rankings the table above reports; a screening run writes all thirty-six to its own output directory, and refreshes these seven in place only when explicitly pointed at the tracked location. `scripts/render_morris_plots.R` re-renders them from the screen's saved design and responses without running the model again, and refuses to write a plot whose recomputed µ\* and σ do not match the ranking it accompanies, so the plots below and the table above cannot describe different screens.

![Morris screening scatter plot of the mean system operating theatre queue across R2B and R2E, plotting each of the 78 screened parameters at its mean absolute elementary effect on the horizontal axis against the standard deviation of its elementary effects on the vertical axis, with points coloured orange for scenario context, green for capacity and blue for policy](images/morris_system_ot_q.png)

The mass casualty event rate sits alone at the right of the plot at nearly twice the importance of the next parameter, and the six highest-importance points are orange without exception, all six being properties of the casualty load or its evacuation. The leading capacity point, surgery duration, sits at about a third of the leader, and the leading policy points, the R2B pre-open hold window and the forward theatre saturation release threshold, at about a quarter. The cloud fans upward and to the right, so a parameter that matters more also interacts more; the seven Role 4 parameters and the Priority 3 mortality rate sit together at the origin.
...
No scatter is drawn because the response is degenerate: the forward theatre queue stayed at zero at all 1,620 design points, so no parameter produced an elementary effect and none can be ranked. That is a result about the forward theatre rather than a gap in the screen, and it is why the system queue panel above and the R2E panel below are the same picture.

![Morris screening scatter plot of the mean R2E operating theatre queue, plotting each of the 78 screened parameters at its mean absolute elementary effect on the horizontal axis against the standard deviation of its elementary effects on the vertical axis, with points coloured orange for scenario context, green for capacity and blue for policy](images/morris_r2e_ot_q.png)

The panel is identical to the system queue above, point for point, the forward theatre contributing nothing to the system total, so every conclusion drawn about system theatre queueing is a conclusion about R2E alone.

![Morris screening scatter plot of the mean R2E intensive care queue, plotting each of the 78 screened parameters at its mean absolute elementary effect on the horizontal axis against the standard deviation of its elementary effects on the vertical axis, with points coloured orange for scenario context, green for capacity and blue for policy](images/morris_r2e_icu_q.png)

The mass casualty event rate leads on importance again, but narrowly, at 1.11 against 0.90 for the event size minimum, and the strongest interactions belong to the event size minimum and the event triage balance. The first policy parameter is the forward theatre saturation release threshold, at rank 6: a casualty released with the definitive repair outstanding waits for the critical airlift route in an intensive care bed, so the threshold acts on the intensive care queue directly rather than through the theatre.

![Morris screening scatter plot of the total died-of-wounds count, plotting each of the 78 screened parameters at its mean absolute elementary effect on the horizontal axis against the standard deviation of its elementary effects on the vertical axis, with points coloured orange for scenario context, green for capacity and blue for policy](images/morris_dow_count.png)

The mass casualty event rate and the Priority 1 mortality ceiling stand clear of every other parameter on importance, at 5.19 and 4.74 against 2.12 for the next, so how often the model is asked to absorb a surge, and how lethal a Priority 1 wound is, matter more for mortality than any capacity or policy parameter in the screen. The remaining seventy-six cluster at or below an importance of 2.12, most of them with interaction terms larger than their own importance.

![Morris screening scatter plot of the mean transport queue across the PMV Ambulance and HX240M fleets, plotting each of the 78 screened parameters at its mean absolute elementary effect on the horizontal axis against the standard deviation of its elementary effects on the vertical axis, with points coloured orange for scenario context, green for capacity and blue for policy](images/morris_transport_q.png)

The whole spread of importance sits below 0.24 of a vehicle, which is consistent with the fleet queueing rarely at the shipped establishment. The mass casualty event rate and the event size maximum take the two leading positions, and the only non-context parameter among the leading eight is the R1 WIA treatment time, which carries the plot's third-largest interaction term. Nothing in the plot is a vehicle count, fleet size being swept directly rather than screened.
...
Utilisation is the flattest response in the set. The whole spread of importance falls below 0.07, the mass casualty event rate leads at 0.066, and the next seven lie within a fifth of each other at about half its importance, led by the pre-flight critical hold duration, so no point below the leader stands clear of the rest. Vehicles spend most of the run idle whatever the parameters do, so this response separates parameters weakly and is best read alongside the queue panel above rather than on its own.
...
Each profile in a comparison runs under the same set of per-replication seeds, because `run_replications()` leaves the caller's random number stream where it found it (see [Multi-run Replication Framework](#multi-run-replication-framework)) and the entry point seeds that stream once. Two things follow. A profile measured on its own reproduces the row it takes in a comparison, at the same control seed, whichever position it occupies in the scenario list. And the profiles are compared on common random numbers, a standard variance reduction for comparing system configurations under a common source of randomness [[40]](#references): the profiles differ only in the parameters the overlay changes, so the difference between them is estimated more precisely than two independently seeded sets of replications would estimate it. Replications within a profile remain independent of one another, each taking its own seed, so each profile's own interval is unaffected by the pairing across profiles.
...
A *Run Sobol Decomposition* button becomes available once Morris finishes, pre-selecting the top five parameters by µ\* in a checkbox group that can still be adjusted. It reuses the replications-per-point and duration values already set, and reports progress the same way. Results are drawn as a grouped bar chart with 95% bootstrap confidence intervals, following [[48]](#references): S1 is the share of output variance a parameter explains acting alone, and ST the share it explains including every interaction it takes part in. A parameter with high ST but low S1 matters only in combination with others, and a one-parameter-at-a-time check would miss it.

A **Transport Fleet Capacity Margin Sweep** panel sits below Sobol Decomposition and runs independently of either, so neither needs to have run first. It re-runs the model across a range of fleet sizes for a chosen vehicle type and plots the resulting queue and utilisation against a dashed line marking the current establishment, showing how much margin the fleet holds before queues form (see [Transport Fleet-Size Sweep](docs/Multi_Run_Analysis.md#transport-fleet-size-sweep)).

Run time scales with r × (p + 1) × reps, where p is the number of screened parameters. At 78 parameters, the shipped default of r = 20 is a long job: the published ranking took about fourteen hours of computation at five replications, roughly 32 seconds per design point on four cores, and `--cache-dir` with `scripts/screen_cache.sh` exists so a run that long survives an interrupted host. Actual times are hardware-dependent. A deliberately small design runs quickly but produces high-uncertainty estimates, with some parameters showing near-zero effect purely through sampling noise, and is useful for checking the pipeline runs rather than for interpreting results.
...
Damage control is a staged sequence, and a casualty who goes through it needs intensive care at two separate points for two different reasons [[21]](#references), [[22]](#references). The first is stabilisation: after the abbreviated operation that controls haemorrhage and contamination, intensive care corrects the hypothermia, coagulopathy and acidosis that would make definitive repair unsurvivable. Only then is the casualty returned to theatre. The second comes after that definitive repair, and covers weaning from ventilation, organ support and watching for complications. The model draws the two separately, because they answer to different things. Stabilisation can be delivered at either echelon, and is what forward holding moves; post-definitive care necessarily follows the definitive operation, and the model serves it at R2E, which is the only echelon the model gives a definitive repair to. A theatre under evacuation pressure may instead move a casualty rearward between the abbreviated operation and the definitive repair, so that the repair and the intensive care after it both fall to the national support base. The model represents that release under a configurable threshold on forward surgical load, and ships with it in force; Further Development L33 records what it does not yet cover. See [R2E Heavy Trajectory](#r2e-heavy-trajectory) for both durations.
...
For everyone else, surgical candidacy is assessed next, behind an ICU availability gate that is checked before theatre entry rather than at the point of post-operative admission. If an ICU bed is free, surgery proceeds and ICU recovery follows. If ICU is full and the casualty is Priority 1, surgery still proceeds, because withholding it would leave an unoperated Priority 1 casualty at near-certain risk of dying of wounds, but recovery is in a holding bed instead, at elevated risk. If ICU is full and the casualty is Priority 2 or lower, theatre entry is deferred until a bed frees. Surgery takes 41 to 210 minutes, most often 95, drawn from the same operative-time data as R2B [[21]](#references).
...
Whether that second procedure is performed in theatre at all depends on how heavily the surgical sections are loaded when the casualty is due back. `second_surgery.saturation_queue_threshold` sets the number of casualties waiting to be operated on at or above which a casualty awaiting their definitive repair is released to strategic evacuation instead, carrying that repair and the intensive care following it to the national support base. Theatre entry seizes a room and then a surgical section, so the count is of casualties queued for either, which is the backlog a surgical team would recognise rather than the part of it already inside the theatre suite. Moving a casualty rearward between the two operations is a recognised option where onward evacuation is timely: forward surgical facilities became "adept at performing the initial abbreviated surgery, contingent on expeditious evacuation timeframes to transfer patients to role 3 or 4" facilities, against a doctrinal two-hour post-operative window that recent experience of large scale combat has seen stretched to "8 to 24 hours, or even days" [[59]](#references). No open-access source reports the share of casualties handled this way, or the surgical load at which a theatre begins to do it, so the threshold is an informed estimate exposed as a planning lever rather than a calibrated quantity. It ships at eight queued casualties, set from the mean theatre queue an otherwise identical campaign carries over its closing 90 days with the release disabled, so the release fires when the backlog reaches what a theatre without it sustains. Every figure this project reports is measured at that setting, at which the release fires under sustained operations and at high casualty intensity and does not fire at all in a 30-day campaign at the Falklands-modified rates, whose theatre queue averages about one casualty. A released casualty serves no post-definitive episode here, there being no repair for one to follow, and is not retained in theatre whatever their drawn recovery, the operation they are waiting for being the one this echelon has no capacity to give them. This is one of three reasons a casualty leaves theatre, alongside a recovery beyond the evacuation policy and a requirement for reconstruction at the national support base; `evacuation_reason` records which decided each disposition. They wait for their flight in an intensive care bed on the critical airlift route, whatever their triage priority: a casualty whose definitive repair is outstanding is not one a holding bed is for, so the release moves work rearward at the cost of forward intensive care rather than at no forward cost at all.

After the definitive repair, on either route, the casualty receives post-definitive intensive care of 360 to 2,880 minutes, most often 1,440. This is an informed estimate rather than a sourced quantity. The mode is anchored on the deployed norm that coalition casualties admitted to a forward intensive care unit are usually evacuated out of theatre within 24 hours [[60]](#references), but that is a figure for the whole intensive care cohort rather than for the post-definitive phase specifically, and no open-access source reports a post-definitive-repair intensive care duration for a deployed facility. The spread around the mode is not sourced at all. Uncertainty is correspondingly high, and because this episode applies to every casualty who has an operation, on either surgical pathway, it is a direct multiplier on R2E intensive care demand. The episode is bounded in this way because it is the theatre-level portion only: a casualty evacuated strategically continues critical care at Role 4, which this model represents as unconstrained demand rather than as a resource (see [Role 4 (National Support Base) Demand Modelling](#role-4-national-support-base-demand-modelling)). When every intensive care bed is occupied the episode is served in a holding bed instead, at the same elevated risk the saturated Priority 1 route carries, since a casualty who has already been operated on cannot be made to wait indefinitely for a bed.

After post-operative recovery a casualty either stays in theatre or is evacuated, and the model decides which by representing the theatre evacuation policy rather than by drawing a fixed share. Doctrine defines the policy as a duration threshold: "a theater that evacuates out of the theater all patients requiring 30 or more days of hospitalization is said to have a '30-day evacuation policy'", and the threshold itself is a command decision, so that "a theater may have an evacuation policy of 15 days whereas another theater may have one of 60 days" [[61]](#references). `draw_recovery_to_duty()` (`R/trajectories.R`) therefore draws each casualty an expected recovery duration at the close of clinical care, and the disposition follows from comparing it against `evacuation_policy_days`, shipped at 21 days and exposed as a planning lever in the Configure panel. The source names 30 days only as its worked example and is explicit that the threshold is a command decision spanning at least 15 to 60 days, so a 21-day policy sits inside doctrine rather than departing from it. The value ships at 21 because of what the threshold does to the holding pool: a retained casualty occupies an R2E holding bed for the whole of the recovery that retained it, so the policy sets the bed-days each retention costs, and a longer threshold retains more casualties and holds each of them for longer. Under a 30-day policy the convalescent load is large enough that the holding pool no longer settles over a sustained campaign, and the intensive care beds behind it degrade with it, because the same pool carries both in-theatre recovery and the wait for a strategic evacuation sortie. At 21 days the pool serves that clinical function with capacity to spare. The threshold is a planning lever rather than a property of the model, and both regimes are measured in [Multi-Run Analysis](docs/Multi_Run_Analysis.md). The source states the threshold in days of hospitalisation; the model treats that as the casualty's expected time to being fit for duty, which is the same quantity only where a casualty is held until fit, so the two diverge for anyone who would convalesce outside a hospital bed and the model retains such casualties slightly too readily. A casualty retained in theatre then occupies a holding bed for exactly the duration that retained it, so its bed-days and its prognosis cannot disagree.

The recovery duration is a base convalescence distribution of 3 to 63 days, most often 27, scaled by a severity factor keyed to the same four categories that set a casualty's Role 4 ward and length of stay (see [Role 4 (National Support Base) Demand Modelling](#role-4-national-support-base-demand-modelling)): 2.33 for an operated Priority 1 casualty, 1.67 for an unoperated one, 1.33 for Priority 2, and 1.0 for Priority 3 and DNBI. Severity therefore drives the prognosis, the ward and the evacuation route from one classification instead of from independent draws, and evacuees emerge as the long-recovery tail of the distribution rather than as an arbitrary sample. The factors themselves are informed estimates: no open-access source tabulates recovery-to-duty durations by triage priority for this population, so they were anchored to the severity gradient already present in the Role 4 length-of-stay values [[35]](#references) and then chosen so that the realised in-theatre share sits inside the historical range discussed in [Return to Duty](#return-to-duty). Uncertainty is correspondingly high, and because the factors set both the retention share and the holding-bed load, an error in them moves R2E bed demand and strategic airlift demand together and in opposite directions.

The share retained in theatre is an output rather than an input. At the shipped configuration it runs at 11.6% across 50 replications, with a 95% confidence interval of 10.9% to 12.2% over some 179 dispositions per run, inside the 7.6% to 42.1% range of historical in-theatre return-to-duty rates recorded in [[9]](#references).
...
flowchart TD
    ...
    PDC -- No --> T0
    T0 --> T{"Repair Complete and<br>Recovery Within<br>Evacuation Policy?"}
    T -- Yes --> U["Seize Hold Bed"]
    U --> V["Recover at R2E<br>(for the drawn duration)"]
    V --> W["Release Hold Bed"]
    W --> X["Return to Duty"]
    X --> Z
    T -- No --> Y{"Priority 1 & Surgical,<br>or Repair Outstanding?"}
    Y -- Yes --> YV{"Ventilated, or<br>Repair Outstanding?"}
...
| LoS category    | Assignment criteria                                         | Role 4 bed | R2E bed while awaiting AME | AME pool       | `env_data.json` key  |
| --------------- | ----------------------------------------------------------- | ---------- | -------------------------- | -------------- | -------------------- |
| P1 Surgical     | Priority 1, `treatment_received = 1`                        | `icu`, stepping down to `hold` | Hold bed, after a bounded pre-flight ICU period for the ventilated share | `ame_critical` | `los_p1_surgical`    |
| P1 Non-Surgical | Priority 1, `treatment_received = 0`                        | `hold`     | Hold bed                   | `ame`          | `los_p1_nonsurgical` |
| P2              | Priority 2 (any `treatment_received`)                       | `hold`     | Hold bed                   | `ame`          | `los_p2`             |
| P3 / DNBI       | Priority 3 WIA, or any DNBI casualty regardless of priority | `hold`     | Hold bed                   | `ame`          | `los_p3_dnbi`        |

Both routes stage in a holding bed, the Casualty Staging Unit equivalent, because every casualty reaching this point has by construction already completed post-operative recovery; the critical and standard split is a distinction in airlift seat type, not in bed type. The exception is the ventilated minority of the critical pool, shipped at 15% and configurable as `critical_hold.ventilated_share`, who genuinely need intensive care up to the point of flight. They hold an ICU bed for 12 to 36 hours, most often 24, and then step down to a holding bed. The bound comes from a deployed intensive care study at Camp Bastion which records that coalition soldiers admitted there "are usually evacuated within 24 h of admission" [[60]](#references); the share itself is an informed estimate, since no open-access source reports what fraction of strategic evacuees require continuing critical care, so uncertainty is high. Were it much larger, R2E ICU would again be consumed by evacuation rather than by treatment. The step-down seizes the holding bed before releasing the ICU bed, so a ventilated casualty is never moved out of intensive care before somewhere exists to move it to; the cost of that ordering is that a saturated holding pool blocks the ICU bed, which is what `ame_icu_hold_minutes` measures.


The bed each category is admitted to is configured rather than fixed in code, as `role4.wards` in `env_data.json`, alongside the set of bed types the census reports. Those are the bed types the deployed echelons field, `icu` and `hold`, rather than a taxonomy of its own: a Role 4 bed sized by this model is the same kind of bed a forward one is, and naming it otherwise would invite sizing a service rather than an establishment. Two of the four types the echelons field carry no Role 4 demand here and are therefore absent rather than reported as zero. A casualty arrives by scheduled sortie having completed post-definitive care, so the model gives Role 4 no reception episode and no `resus` demand. It does perform surgery there: `ot` demand arises from the casualty released with the definitive repair still outstanding, who carries that operation rearward, and from the casualty whose wounds require staged soft-tissue coverage (see [Operating Theatre Demand](#operating-theatre-demand) below). The `resus` absence is a property of the model rather than of a national support base, which in practice operates on repatriated casualties repeatedly; Further Development L33 records the boundary it follows from. A stay can also be split between two bed types rather than held in one, which is what `role4.icu_continuation` governs.

The shipped requirement is 1 day at minimum, 4 most often and 21 at maximum, a mean of 8.7 days, and it is calibrated rather than assumed. Johnston, Henning and Harrison report a mean Level 3 intensive care stay of 7.6 days across 570 admissions of 527 UK service personnel repatriated to a Role 4 facility between 2007 and 2013 [[62]](#references), which is the Role 4 portion of the episode directly. Theatre serves a mean of 1.08 days of it before the casualty travels, that being the mean of the post-definitive distribution above, so the total is the sum of the two and the model reproduces the source figure after the deduction. Three limitations attach. The source is a conference abstract rather than a full paper, so it reports the means without the case mix behind them. The triangular form cannot reproduce the reported spread, an SD of 11.6 days against a mean of 7.6, so the long tail of very protracted stays is truncated at 21 days and the model will understate the small number of casualties who occupy an intensive care bed for months. And the source's Level 2 figure of 2.0 days has no counterpart here, the model giving the surgical ward cohort no critical care phase at all, so their requirement is carried as ward time instead. A casualty admitted to intensive care does not hold that bed for the whole of its length of stay. The model serves the intensive care the casualty is still owed and then steps it down to a surgical ward for the remainder, so the reported intensive care demand is an estimate of the critical care requirement rather than a count of everyone who ever needed it. That matters because intensive care is the scarcest and most expensive part of a national support base establishment, and a ward split that held every Priority 1 surgical casualty in an intensive care bed for a stay of up to 45 days would overstate it several times over: at seed 42 the same casualties account for 25.4% of Role 4 bed-days under the step-down against 75.6% without it. The step-down applies to the intensive care cohort alone, a casualty admitted to a surgical or general ward having no critical care phase to divide, and it is a separate mechanism from the pre-flight step-down described below, which moves a ventilated casualty from an intensive care bed to a holding bed at R2E before the flight rather than after it. Further Development L33 records the related boundary.

The mapping rests on one judgement rather than a source: that a casualty who was not operated on in theatre needs no intensive care at the national support base, whatever their in-theatre priority. Priority 1 non-surgical, Priority 2 and Priority 3 and DNBI casualties therefore all take a holding bed, and only the operated Priority 1 cohort reaches intensive care. No open-access source tabulates bed assignment by this scheme, so uncertainty is high. The earlier mapping distinguished a surgical from a general ward within that holding group, which moved occupancy between two labels on an uncited basis and without changing total Role 4 bed-days; collapsing them onto the holding bed the echelons already field removes a distinction the model had no evidence for. Intensive care occupancy is unaffected by the change, since only Priority 1 surgical casualties ever reached it.

#### Operating Theatre Demand

A casualty released to strategic evacuation with the definitive repair outstanding (see [R2E Heavy Trajectory](#r2e-heavy-trajectory)) arrives owing an operation, and the model reports that requirement rather than losing it. `role4.surgery` governs the report, which states operations and theatre-minutes owed per day under the `ot` label the deployed echelons use. Theatre demand is reported separately from the bed census rather than inside it, because a theatre is a throughput resource and a bed is an occupancy one: an operation of ninety-five minutes occupies no bed for a day, and adding the two would report a quantity in two units at once.
...
A casualty whose wounds require staged soft-tissue coverage does not receive one operation at the national support base but a sequence of them. A series at one such facility reports 264 operations across 50 combat casualties, an average of 5.2 each, over the three weeks following the Afghanistan withdrawal [[63]](#references), of which nearly half were irrigation and debridement with vacuum-assisted closure. The model represents the interval those procedures fall on rather than their count, because the interval is what the treating facilities describe. The Royal Centre for Defence Medicine states the protocol directly: repeated debridement is "a return to theatre every 48-72 h until no further sign of necrosis or infection has occurred" [[64]](#references). The model represents that interval rather than a count, so the number of procedures a casualty receives follows from how long their debridement phase runs instead of from a configured number. The phase ends at the reconstruction, which is what steps the casualty down from intensive care, and `role4.surgery.return_interval_min`/`_mode`/`_max` set the interval at 2, 2.5 and 3 days.

After the reconstruction the model reports nothing further, and the literature supports that. A review of war-related lower extremity reconstruction reports "flap failure leading to amputation in only four out of 359 cases", and complete failure rates of 0% and 1.4% for free and pedicled flaps in a series performed after serial debridement [[65]](#references). `post_reconstruction_return_rate` exists so that small residual is representable and ships at zero, consuming no draw.

Which casualties enter the sequence is the harder question, and it is answered on the wound rather than on the casualty's expected recovery. A latent class analysis of 5,227 combat casualties with an Injury Severity Score of 9 or more who survived through all levels of care identifies seven mutually exclusive injury profiles, of which the open wounds profile, "extensive open wounds, including amputations and pelvic/urogenital injuries", accounts for 18.8% [[66]](#references). `role4.surgery.reconstruction_share` is set from that figure and applies to Priority 1 casualties operated on in theatre, the population the source's inclusion criteria most closely match. Two limits attach. The share is a single number applied to a cohort the model identifies by triage priority and whether an operation was performed, the model carrying no anatomical description of a wound, so it reproduces the right proportion rather than the right casualties. And the source reports profiles rather than treatment, so reading the open wounds profile as the reconstruction cohort is a clinical interpretation rather than a figure the paper states.
...
The four length-of-stay distributions, in days, are 10/21/45, 7/14/30, 5/10/21 and 2/5/14 for minimum, most likely and maximum. They are informed estimates reflecting a severity gradient, longest for Priority 1 surgical casualties and shortest for DNBI, directionally consistent with the length-of-stay patterns in [[35]](#references) but not extracted from its tables, so uncertainty is high. Peak Role 4 occupancy and total bed-days scale roughly linearly with these values, so an error biases the figure reported to national planners, but nothing in theatre is affected, since the census is computed downstream of all in-theatre logic.

Each casualty's length of stay is drawn after the simulation has finished, at the point the census is computed, rather than at the moment of evacuation. This rests on the assumption that the duration is of no consequence inside the theatre, which follows from Role 4 being modelled as unconstrained demand: a casualty who boards a sortie leaves the simulated system, releases every theatre resource held, and never returns, so nothing in theatre can read the value. The draw is made with the random number stream saved and restored around it, so the analysis consumes none of it and censusing one run's output twice gives one answer. Were the assumption wrong, which it would become the moment Role 4 were given finite capacity and the ability to refuse or defer an admission, the duration would have to be drawn inside the simulation as a casualty attribute set at evacuation, and Further Development records this.
...
Three aircraft ship, all from the RAAF's account of its aeromedical evacuation capability [[67]](#references), which reports that an AME-configured C-17A "can transport 54 ambulatory and 36 high dependency stretcher patients" and that "the C-130J and C-27J can carry 97 and 21 stretcher patients respectively":
...
The simulation produces a defined set of Key Performance Indicators (KPIs) organised by planner decision domain. Each KPI is selected against five criteria derived from military medical doctrine and discrete event simulation methodology [[39]](#references):

- **C1 — Doctrinal Standard Compliance:** Variable measures compliance with a named standard in AJP-4.10 [[34]](#references).
...
- **Criteria:** C1, C3, C4
- **Computation:** `r2b_departure_time - r2b_treatment_start_time` over every casualty who reached `r2b_treatment_start_time`, by the censored-interval estimator described below; report the restricted mean, the p90, and the censored count and share.
...
- **Criteria:** C1, C3
- **Computation:** `r2e_arrival_time - r2b_departure_time` over every casualty who departed R2B, by the same estimator; report the restricted mean, the p90, and the censored count and share.
...
- **Criteria:** C1, C3, C4
- **Computation:** `r2e_departure_time - r2e_arrival_time` over every casualty who reached `r2e_arrival_time`, by the same estimator; report the restricted mean, the p90, and the censored count and share.

**Intervals that are still open when the run ends.** A campaign of fixed length cuts every interval measured inside it, and the intervals it cuts are not a random sample: they belong to the casualties still present, which is both the late arrivals and the long stayers. Differencing the two timestamps and keeping the casualties who carry both therefore reports a mean of the short stays, and reports it as though it were a mean of all of them. At seed 42 over thirty days this dropped 21.6% of R2B stays and 18.0% of R2E stays, and the share moves with the configuration rather than being a property of the model: under the Okinawa-intensity profile it reaches 55.5% of R2E stays, so the figure was least trustworthy in exactly the congested regime it is consulted for.

The three intervals whose cohort at risk is known are therefore summarised over everyone who entered them, by the Kaplan-Meier product-limit estimator [[69]](#references), which treats a stay still running as the lower bound it is rather than discarding it. The reported figure is the restricted mean [[70]](#references): the area under the survival curve out to a horizon of seven days, which is the planning cycle the reinforcement demand interval and the strategic aeromedical sortie interval already run on. Restricting it is not a convenience. Beyond the last observed completion the curve can only be held flat, and the area under that flat tail is assumption rather than measurement, so a horizon inside the observed support is what keeps the figure a measurement; at seed 42 no part of either dwell mean lies beyond it. It makes the figure a mean stay within seven days rather than a mean stay, and a casualty holding a bed for twenty days contributes seven.

The p90 carries no such horizon, being a point on the curve rather than an area under it, and is reported as unavailable where the curve does not reach it. That is the honest reading at seed 42 for R2B, whose curve falls only to 0.202: nine in ten stays cannot be bounded from a thirty-day run, and the p90 of the stays that happened to close is a p90 of a different and shorter-staying population. Time to first surgery is excluded from this treatment because its cohort is not identifiable, a casualty with no surgery time being either one still waiting or one who never needed an operation, and the attributes do not separate them; it reports the completed-case mean alongside the count of casualties carrying no surgery time, which bounds the censoring from above rather than measuring it.
...
**Role 4 Daily Bed Occupancy by Bed Type.** Mean concurrent Role 4 (national support base) patients by bed type (`icu`, `hold`) per simulation day, derived from strategically evacuated casualties' assigned length-of-stay, admitted from the day of *actual* AME departure (`evacuation_day`), not the day the evacuation decision was made (see [Role 4 (National Support Base) Demand Modelling](#role-4-national-support-base-demand-modelling)).

- **Doctrinal basis:** AJP-4.10 [[34]](#references) mandates that Role 4 requirements be derived from theatre casualty estimates.
- **Criteria:** C2, C3, C5
...
- **Doctrinal basis:** AJP-4.10 [[34]](#references) strategic evacuation planning function.
- **Criteria:** C2, C4, C5
...
- **Doctrinal basis:** AJP-4.10 [[34]](#references) strategic evacuation timeliness planning; the CSU/CCATT-CCAST distinction (see [Role 4 (National Support Base) Demand Modelling](#role-4-national-support-base-demand-modelling)) is exactly what the route decomposition is designed to make visible.
- **Criteria:** C2, C4, C5
...
- **Doctrinal basis:** AJP-4.10 [[34]](#references). Backlog size is the direct visible consequence of a schedule/capacity combination inadequate to theatre demand; reporting the two pools separately is necessary because, as the seed-42 results show, one pool can be saturated while the other clears completely.
- **Criteria:** C3, C4, C5
- **Computation:** `compute_ame_backlog()`/`plot_ame_queue()` (`R/analysis.R`) reconstruct the backlog from per-casualty event timestamps: `r2e_departure_time` (a `+1` event, when the Strategic Evac disposition is decided and the AME wait begins; `ame_route` selects the pool) and `ame_hold_end` (a `-1` event, NA while still waiting), cumulatively summed in event-time order per (replication, pool), rather than from the `"ame"`/`"ame_critical"` resource monitor's own `queue` column. The wait ends at `ame_hold_end`, which is set on both routes out of it, rather than at the boarding time alone: a casualty who dies awaiting a sortie leaves the queue without ever departing, and counting only departures left it in the backlog for the remainder of the campaign. This is a correction, not a stylistic choice: `r2e_ame_wait_and_board()` (R/trajectories.R) uses a manual `timeout()`/`rollback()` polling loop (`r2e_ame_dow_poll()`) rather than `select()`/`seize_selected()` or a blocking `seize()`, calling `seize(resource_name, 1)` only once capacity is already confirmed available, so a waiting casualty never registers in simmer's own queue tracking for these two resources, and the `queue` column is structurally always 0 regardless of the true backlog. An initial implementation of this plot read that column directly and, verified against a real seed-42 run with 93 casualties genuinely still awaiting AME at run end, rendered a flat zero line for the entire run on both pools; the event-based reconstruction instead reproduces the peak backlog figures reported in [Strategic Evacuation and Role 4 Demand](docs/Single_Run_Analysis.md#strategic-evacuation-and-role-4-demand) below. Faceted by pool (and by replication when more than one is present), with independent y-axis scales given the pools' very different capacity magnitudes.
...
| L17 | R2E holding beds carry recovery and evacuation on one pool | High |
...
| L18 | Screening resolution and response coverage | Medium |
| L19 | Transport capacity margin untested under mass casualty injection | Medium |
| L21 | R2B surgical throughput options cannot be tested | Medium |
| L22 | The died-of-wounds calibration target is a bounded treated-cohort rate | Medium |
...
**L17 — R2E holding beds carry recovery and evacuation on one pool.** A single pool of 30 beds serves four unrelated demands, and the split between them is measured rather than assumed. Both airlift routes stage in a holding bed: a casualty awaiting the standard pool seizes one at the evacuation decision, and one on the critical pool that is ventilated holds an intensive care bed for its pre-flight period and then steps down into a holding bed for the remainder of its wait, so the route does not decide whether a casualty appears here. Across 50 replications the evacuation wait holds 20.5% of the pool's occupancy (95% CI [18.0%, 22.9%]) at moderate intensity and 40.4% ([38.3%, 42.4%]) at high, the remainder being in-theatre recovery, the post-definitive holding fallback and the post-operative damage control hold. Every lever that delays a sortie raises that share, reaching 44.6% ([38.8%, 50.5%]) at a 40% cancellation rate and 63.0% ([61.3%, 64.7%]) at a 14-day sortie interval, where the evacuation wait becomes the pool's largest single consumer. What the coupling does is push back into intensive care: a ventilated casualty cannot step down from its bounded pre-flight period while the holding pool is full, and that hold runs 23.7 hours at a 3-day interval against 88.1 at 14. Any R2E intensive care or theatre-gating capacity finding must therefore be read alongside the holding pool's occupancy rather than in isolation. Separating the staging pool from the clinical holding beds, or expanding the pool, would each reduce the coupling; the defaults were deliberately not tuned to hide it. The four components account for the pool exactly in each of the 650 tracked replications. They did not until each stay was bounded by the instant its bed was released rather than by the instant its casualty left theatre: a casualty who died while awaiting a sortie releases its staging bed but never departs, so it was charged with the whole of a wait it did not serve, which is why the residual was always an over-count and never the reverse.
...
**L12 — Scenario calibration incomplete.** The moderate intensity profile reproduces a killed-to-wounded ratio of 0.452 against a published campaign record of 0.328 (255 killed against 777 injured [[15]](#references), [[16]](#references); the parliamentary total counts injuries to Service personnel and civilians, including 109 cold injuries, so it is broader than a strict wounded-in-action count and the ratio derived from it is correspondingly approximate). This follows from the underlying casualty generation rates rather than from the profile itself: each stream realises the daily mean its configuration names, so the realised ratio is the configured ratio of the two means. The high intensity profile is sourced to Okinawa in its generation rates, its distribution family and its died-of-wounds model, but its priority split, sub-type composition and transport times are still inherited from the Falklands-calibrated base, so a casualty stream drawn from one campaign is still triaged and moved on another campaign's assumptions. A third, Vietnam-intensity profile cannot be added from the current source, whose appendix carries no standalone Vietnam combat-troop distribution table. Each part needs different work: recalibrating the generator, finding Okinawa-specific triage, sub-type and transport data, and finding a source that tabulates Vietnam rates.
...
**L18 — Screening resolution and response coverage.** The Morris screen covers seventy-eight parameters at twenty trajectories over a 30-day horizon, which resolves the influential group from the negligible tail but does not order parameters within the leading cluster: the standard error of a µ\* is the standard deviation of its absolute elementary effects divided by $\sqrt{r}$, so neighbours separated by less than that are not distinguishable. No parameter is separated from the one below it at this trajectory count; separating the leader from the second would need 45 trajectories and the second from the third some 7,000, which is not a practical target, so the leading parameters should be reported as a group. The published ranking is also on one response, the system OT queue. A screening run writes a ranking file for each of the thirty-six responses at no extra simulation cost, so those files exist, and the strategic airlift finding shows the cost of leaving them uninterpreted: `ame_failure_probability` ranks 55th of 78 on the queue while ranking 5th on mean Role 4 occupancy and 7th on the standard-route evacuation backlog. Whether a parameter ranks differently on a time-to-care, return-to-duty or strategic evacuation response is otherwise unresolved.

**L19 — Transport capacity margin untested under mass casualty injection.** The fleet-size sweep now runs at both the Falklands-derived and Okinawa-derived rates (`docs/Multi_Run_Analysis.md`'s Option 4), so the margin it reports is established across both steady casualty intensities. It remains untested against a mass casualty event, the burst-arrival condition most likely to saturate transport briefly even where a steady rate does not. Re-running the sweep with injection enabled would establish whether the finding holds under that condition too.
...
**L29 — The variance decomposition resolves group membership, not order.** The Sobol decomposition that follows the Morris screen is limited by replication noise, and the limit has been measured rather than inferred. At N = 200 over the five leading parameters, at four replications per design point, `scripts/measure_noise_floor.R` puts replication noise at 32.9% (95% CI [28.2%, 37.5%]) of the total variance of the system OT queue, 62.9% of transport utilisation and 89.1% of the transport queue, the last with an interval reaching 1.0. Two of the seven screened responses carry nothing usable at all: the transport queue is almost entirely noise, and the R2B theatre queue is constant across the design. Noise enters the total variance the indices are shares of, so every reported index is understated, and by how much depends on how far common random numbers cancel the noise in the estimator's numerator, which for parameters acting on casualty arrival they largely do not. The two bounding corrections put the leading parameter's total-order index between 0.36 and 0.85 and the second between 0.16 and 0.65, so the reported values are published uncorrected with the bracket stated. What survives either bound is the structure rather than the numbers: the same two parameters lead, in the same order, with the same three beneath them, and the separation of the second from the third holds under the correction because a uniform rescaling moves a difference and its interval together. `scripts/test_sobol_separation.R` establishes which orderings the sample supports, bootstrapping the design rather than the indices so that two indices computed from the same evaluations keep their correlation: the second parameter separates from the third at P = 0.992, and the leading pair does not separate from each other. The decomposition therefore establishes a leading pair and a negligible tail, not a ranking, which is independently what the Morris screen concluded about the same parameters. Closing the remaining gap is a sample-size question with a measured answer, and the answer is that a larger design alone does not reach it: noise is a bias set by the replication count per point, not by the number of points, so the leading pair separates only at roughly N = 800 with 8 to 12 replications per point, some three to four days of computation against the thirty-two hours a wider design at the present replication count would take while converging on indices a third too low.

**L32 — The establishment and the evacuation policy are never swept jointly, so their interaction is unknown.** Shortening the evacuation policy and adding R2E holding beds are substitutes: both free forward capacity, the first by retaining fewer casualties and the second by holding more of them, and a planner choosing between them needs their relative cost. Each has now been swept alone, and the establishment sweep reports that at the shipped 21-day policy the pool stops binding 15 beds above its establishment and buys no health outcome beyond that. That answers the substitution only where it is least interesting, because the establishment has no effect at the shipped policy to trade away. The question a force structure review would ask is whether a large enough pool would make a longer policy stable, and that is a property of the joint grid rather than of either axis: the policy sweep's saturation begins at 30 days, which the establishment sweep never visits. The two costs are not commensurable either: a shorter policy is paid for in returns to duty and in demand on the national support base, an extra bed in establishment and lift. Closing this requires running the existing sweep over the cross product of the two, which is twenty cells of 30 replications over 360 days and a matter of compute rather than of method.

**L33 — The reconstruction cohort is identified by proportion rather than by the wound, and the saturation threshold rests on no source.** Both levers that move work from the deployed trauma system to the national support base are now in force, and each carries a limit that is a property of the model rather than of its setting. The reconstruction cohort is drawn as a share of Priority 1 casualties operated on in theatre, because the model carries no anatomical description of a wound: it knows triage priority, whether an operation was performed and the damage control pathway, none of which distinguishes a casualty whose wounds need staged soft-tissue coverage from one whose do not. The share therefore reproduces the right proportion of casualties needing reconstruction rather than the right casualties, and closing that means giving a casualty an injury pattern at the point of injury, which conditions far more than this echelon. The saturation threshold has a different weakness: no open-access source reports the surgical backlog at which a theatre begins evacuating between the two operations, so the shipped eight is an informed estimate anchored on a measured property of the model, the mean theatre queue an unrelieved campaign carries, rather than on evidence about how theatres behave. A sweep across the range establishes what the setting costs and buys (see `docs/Multi_Run_Analysis.md`), which bounds the consequence of the estimate being wrong without removing it. The reconstruction sequence is also counted but not timed, no open-access source reporting how long a debridement or a flap takes at this echelon, so the reported theatre-minutes cover only the conserved definitive repairs while the operation count covers every procedure.
...
The modelled system covers triage, evacuation and definitive care, including a phased model of damage control surgery drawn from the clinical literature. A statistical framework of multi-run replication, warm-up analysis, and Morris and Sobol sensitivity screening allows each reported finding to carry its uncertainty rather than rest on a single run. Results are published separately, in [Baseline Performance and System Constraints in a Single Campaign](docs/Single_Run_Analysis.md) and [Surgical Hours, Not Operating Theatres](docs/Multi_Run_Analysis.md), with the designs and statistical method behind the replicated experiments in [Experimental Design and Statistical Method for a Replicated Simulation of the Land-Based Trauma System](docs/Multi_Run_Supplement.md).
...
[15] Blaker, P. (1982, October 18). Falkland Islands (Casualties). *Parliamentary Debates, Commons*, written answers. Retrieved 02 Aug 26, from https://api.parliament.uk/historic-hansard/written-answers/1982/oct/18/falkland-islands-casualties

[16] Silkin, J. (1982, December 21). Falklands Campaign. *Parliamentary Debates, Commons*. Retrieved 02 Aug 26, from https://api.parliament.uk/historic-hansard/commons/1982/dec/21/falklands-campaign

[17] Ucar I, Smeets B, Azcorra A (2019). “simmer: Discrete-Event Simulation for R.” *Journal of Statistical Software*, 90(2), 1–30. Retrieved 25 Jun 26, from https://doi.org/10.18637/jss.v090.i02

[18] Maddeh, M., Ayouni, S., Al-Otaibi, S., Alazzam, M. B., Alturki, N. M., & Hajjej, F. (2023). Discrete-Event Simulation Model for Monitoring Elderly and Patient’s Smart Beds. *Journal of Disability Research*, *2*(3), 1-9. DOI: 10.57197/JDR-2023-0026. Retrieved 26 Jul 25, from https://www.scienceopen.com/hosted-document?doi=10.57197/JDR-2023-0026

[19] Kemple, W. G., & Lacy, L. W. (1995). *Modeling command and control: The design and implementation of the C2 model*. Defense Technical Information Center. Retrieved 20 Jul 25, from https://apps.dtic.mil/sti/html/tr/ADA304910/

[20] Wang, Y., & Pinsky, E. (2023). Geometry of deviation measures for triangular distributions. *Frontiers in Applied Mathematics and Statistics*, *9*, 1274787. Retrieved 26 Jul 25, from https://doi.org/10.3389/fams.2023.1274787

[21] Zizzo, M., Ruiz, C. C., Zanelli, M., Bassi, M. C., Sanguedolce, F., Ascani, S., & Annessi, V. (2020). Damage control surgery for the treatment of perforated acute colonic diverticulitis: a systematic review. *Medicine*, *99*(48), e23323. Retrieved 26 Jul 25, from https://pmc.ncbi.nlm.nih.gov/articles/PMC7710165/

[22] Karamarković, A. Damage Control in Abdominal Surgery. *Clin Surg. 2016; 1*, *1118*. Retrieved 02 Aug 25, from https://www.clinicsinsurgery.com/open-access/damage-control-in-abdominal-surgery-2563.pdf

[23] Beldowicz, B.C. (2018). The evolution of damage control in concept and practice. *Clinics in Colon and Rectal Surgery*, *31*(1), 30–35. Retrieved 25 Jun 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC5787400/

[24] Abri, M. A., Snani, S. A., Almayahi, J., Sharqi, A. A., & Qadhi, H. A. The Outcome of Damage Control Surgery at Sultan Qaboos University Hospital. *World J Surg Surgical Res. 2022; 5*, *1428*. Retrieved 26 Jul 25, from https://www.surgeryresearchjournal.com/open-access/the-outcome-of-damage-control-surgery-at-sultan-qaboos-university-9532.pdf
...
[26] Turner, J., & Wilson, A. (2024). Backed into a corner: damage control surgery in the rural or austere setting. *Trauma Surgery & Acute Care Open*, *9*(Suppl 2), e001391. Retrieved 02 Jul 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC11029234/

[27] Hardcastle, T. C., Gaarder, C., Balogh, Z., et al. (2025). Guidelines for Enhanced Recovery After Trauma and Intensive Care (ERATIC): ERAS Society and IATSIC Recommendations: Paper 1: Initial Care — Pre and Intraoperative Care Until ICU, Including Non-Operative Management. *World Journal of Surgery*, *49*(8), 1997–2028. Retrieved 02 Jul 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC12338446/

[28] Nickson, C. (2020, November 3). *Damage Control Resuscitation*. Life in the Fastlane. Retrieved 27 July, 2025, from https://litfl.com/damage-control-resuscitation/

[29] Hodický, J., Procházka, D., Jersák, R., Stodola, P., & Drozd, J. (2020). Optimization of the casualties' treatment process: Blended military experiment. *Entropy*, *22*(6), 706. Retrieved 25 Jun 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC7517244/

[30] Eastridge, B. J., Mabry, R. L., Seguin, P., Cantrell, J., Tops, T., Uribe, P., ... & Blackbourne, L. H. (2012). Death on the battlefield (2001–2011): implications for the future of combat casualty care. *Journal of Trauma and Acute Care Surgery*, *73*(6 Suppl 5), S431–S437. Retrieved 29 Jun 26, from https://apps.dtic.mil/sti/pdfs/ADA609611.pdf

[31] Kotwal, R. S., Montgomery, H. R., Kotwal, B. M., Champion, H. R., Butler Jr, F. K., Mabry, R. L., ... & Holcomb, J. B. (2011). Eliminating preventable death on the battlefield. *Archives of Surgery*, *146*(12), 1350–1358. Retrieved 29 Jun 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC5832013/

[32] Braverman, M. A., Smith, A., Arshad, M. I., Cannon, J. W., Borgman, M. A., Holcomb, J. B., Etchill, E. W., DuBose, J. J., Rasmussen, T. E., Edwards, J., Epley, E., Glaser, J. J., Redfield, C. S., Schreiber, M. A., & Morrison, J. J. (2021). Damage control resuscitation in patients undergoing emergency laparotomy: outcomes and implications. *Journal of Trauma and Acute Care Surgery*, *92*(2), 321–328. Retrieved 01 Jul 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC8600903/

[33] Holcomb, J. B., Del Junco, D. J., Fox, E. E., Wade, C. E., Cohen, M. J., Schreiber, M. A., Alarcon, L. H., Bai, Y., Brasel, K. J., Bulger, E. M., Cotton, B. A., Matijevic, N., Muskat, P., Myers, J. G., Phelan, H. A., White, C. E., Zhang, J., Rahbar, M. H., & PROMMTT Study Group. (2013). The prospective, observational, multicenter, major trauma transfusion (PROMMTT) study: comparative effectiveness of a time-varying treatment with competing risks. *JAMA Surgery*, *148*(2), 127–136. Retrieved 01 Jul 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC3773975/

[34] NATO Standardization Office. (2019). *AJP-4.10 allied joint doctrine for medical support* (Edition C, Version 1). NATO Standardization Office. Retrieved 25 Jun 26, from https://www.coemed.org/files/stanags/01_AJP/AJP-4.10_EDC_V1_E_2228.pdf

[35] Nessen, S. C., Lounsbury, D. E., & Hetz, S. P. (Eds.). (2008). *War Surgery in Afghanistan and Iraq: A Series of Cases, 2003–2007*. Borden Institute, Office of The Surgeon General, US Army. Retrieved 13 Jul 26, from https://medcoe.army.mil/borden-tb-war-surgery-afg-iraq/

[36] Fischer, J., Al-Husseini, M., Krishnamoorthy, R., Kumar, V., & Kochenderfer, M. J. (2025). Digital simulations to enhance military medical evacuation decision-making. Open-access preprint retrieved 02 Jul 26, from https://arxiv.org/abs/2507.06373

[37] Debacker, M., Van Utterbeeck, F., Ullrich, C., Dhondt, E., & Hubloue, I. (2016). SIMEDIS: a discrete-event simulation model for testing responses to mass casualty incidents. *Journal of Medical Systems*, *40*(12), 273. Retrieved 10 Jul 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC5069323/

[38] Dilday, T. (2026, April 20). *From MASCAL to Campaign Medicine: Aligning Field Hospital Training with LSCO Reality*. U.S. Army. Retrieved 10 Jul 26, from https://www.army.mil/article/290575

[39] Sargent, R. G. (2010). Verification and validation of simulation models. In *Proceedings of the 2010 Winter Simulation Conference* (pp. 166–183). IEEE. Retrieved 25 Jun 26, from https://www.informs-sim.org/wsc10papers/016.pdf

[40] Rossetti, M. D. (2023). *Simulation Modeling using the Kotlin Simulation Library (KSL)* (open-access, CC BY-NC-ND 4.0), including §9.2 Variance Reduction Techniques. Retrieved 26 Jun 26, from https://rossetti.github.io/KSLBook/

[41] Rossetti, M. D. *Simulation Modeling and Arena*, Chapter 5: Statistical Analysis for Infinite Horizon Simulation Models, §5.2–5.3: Replication-Deletion Method and Welch's Graphical Procedure. Retrieved 25 Jun 26, from https://rossetti.github.io/RossettiArenaBook/05-Chapter5.html

[42] Law, A.M. (2020). Statistical analysis of simulation output data: the practical state of the art. In *Proceedings of the 2020 Winter Simulation Conference* (pp. 1117–1127). INFORMS Simulation Society. Retrieved 25 Jun 26, from https://informs-sim.org/wsc20papers/134.pdf
...
[45] Morris, M. D. (1991). Factorial sampling plans for preliminary computational experiments. *Technometrics*, *33*(2), 161–174. Retrieved 11 Jul 26, from https://www.stat.cmu.edu/technometrics/90-00/vol-33-02/v3302161.pdf

[46] Pujol, G., Iooss, B., Janon, A., Gilquin, L., Le Gratiet, L., Lemaitre, P., Marrel, A., Meynaoui, A., Nelson, B. L., Monod, H., Fruth, J., Ratto, M., Touati, T., & Weber, F. (2024). *sensitivity: Global Sensitivity Analysis of Model Outputs and Related Quantities*. R package version 1.30.1. Retrieved 25 Jun 26, from https://cran.r-project.org/package=sensitivity

[47] OpenMOLE Community. (2024). *Sensitivity Analysis: Morris Screening Method*. OpenMOLE Documentation. Retrieved 25 Jun 26, from https://openmole.org/Sensitivity.html

[48] Saltelli, A., Annoni, P., Azzini, I., Campolongo, F., Ratto, M., & Tarantola, S. (2010). Variance based sensitivity analysis of model output. Design and estimator for the total sensitivity index. *Computer Physics Communications*, *181*(2), 259–270. Retrieved 11 Jul 26, from https://www.andreasaltelli.eu/file/repository/PUBLISHED_PAPER.pdf

[49] Williams, E., Szakmany, T., Spernaes, I., Muthuswamy, B., & Holborn, P. (2020). Discrete-event simulation modeling of critical care flow: New hospital, old challenges. *Critical Care Explorations*, *2*(9), e0174. Retrieved 11 Jul 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC7491890/

[50] Black, J. (2002). Acute appendicitis in Japanese soldiers in Burma: support for the "fibre" theory. *Gut*, *51*(2), 297. Retrieved 26 Jun 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC1773321/

[51] Weeks, S. R., Oh, J. S., Elster, E. A., & Learn, P. A. (2017). Humanitarian surgical care in the US military treatment facilities in Afghanistan from 2002 to 2013. *JAMA Surgery*, *153*(1), 84–86. Retrieved 26 Jun 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC5833623/

[52] Lewis, P. A. W., & Shedler, G. S. (1979). Simulation of nonhomogeneous Poisson processes by thinning. *Naval Research Logistics Quarterly*, *26*(3), 403-413. Naval Postgraduate School Calhoun repository. Retrieved 13 Aug 26, from https://calhoun.nps.edu/handle/10945/63159

[53] Yang, Q., Du, J. L., & Shao, F. (2019). Mortality rate and other clinical features observed in open vs closed format intensive care units: A systematic review and meta-analysis. *Medicine*, *98*(27), e16261. Retrieved 03 Aug 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC6635169/

[54] Marble, S. (2025). Both joint and not: Medical support at Okinawa, 1945. *Joint Force Quarterly*, *117*(2), article 11. National Defense University Press. Retrieved 17 Aug 26, from https://digitalcommons.ndu.edu/joint-force-quarterly/vol117/iss2/11/

[55] Pawlowsky-Glahn, V., Egozcue, J. J., & Tolosana-Delgado, R. (2007). *Lecture notes on compositional data analysis*. Universitat de Girona. Retrieved 07 Aug 26, from https://dugi-doc.udg.edu/bitstream/handle/10256/297/CoDa-book.pdf

[56] Puy, A., Becker, W., Lo Piano, S., & Saltelli, A. (2022). A comprehensive comparison of total-order estimators for global sensitivity analysis. *International Journal for Uncertainty Quantification*, *12*(2), 1-18. arXiv preprint. Retrieved 22 Aug 26, from https://arxiv.org/abs/2009.01147

[57] Godat, L., Kobayashi, L., Costantini, T., & Coimbra, R. (2013). Abdominal damage control surgery and reconstruction: world society of emergency surgery position paper. *World Journal of Emergency Surgery*, *8*, 53. Retrieved 03 Aug 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC3878509/
...
[59] Remondelli, M. H., Nye, K., Collin, T., Wang, J., Wang, A., Atwood, R., Bozzay, J., Do, W., Hardin, R., Elster, E., & Walker, P. F. (2026). Damage control surgery in large-scale combat operations: still a life-saving strategy or a bridge to nowhere? *Trauma Surgery & Acute Care Open*, *11*(3), e001995. Retrieved 16 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC13384188/

[60] Inwald, D. P., Arul, G. S., Montgomery, M., Henning, J., McNicholas, J., & Bree, S. (2013). Management of children in the deployed intensive care unit at Camp Bastion, Afghanistan. *Journal of the Royal Army Medical Corps*, *160*(3), 236–240. Retrieved 02 Aug 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC4154587/

[61] U.S. Army Medical Department Center and School. *Health Service Support in a Theater of Operations*, Subcourse MD0002, Evacuation Policy. Retrieved 02 Aug 26, from http://armymedical.tpub.com/MD0002/Evacuation-Policy-Health-Service-Support-in-a-Theater-of-Operations-88.htm

[62] Johnston, A. M., Henning, J., & Harrison, D. (2014). Outcomes of military patients treated at the UK Royal Centre for Defence Medicine 2007 to 2013. *Critical Care*, *18*(Suppl 1), P57. Retrieved 16 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC4068646/

[63] Remondelli, M. H., Barzanji, N., Funk, V., Nye, K., Wang, J., Bozzay, J., Walker, P., Runikera, R., Logeman, J., Jessie, E., Humphries, A., Elster, E., & Bradley, M. (2026). Flagship response: a descriptive analysis of definitive surgical care at the Walter Reed National Military Medical Center following the Afghanistan withdrawal. *Trauma Surgery & Acute Care Open*, *11*(3), e001976. Retrieved 17 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC13384166/

[64] Evriviades, D., Jeffery, S., Cubison, T., Lawton, G., Gill, M., & Mortiboy, D. (2011). Shaping the military wound: issues surrounding the reconstruction of injured servicemen at the Royal Centre for Defence Medicine. *Philosophical Transactions of the Royal Society B: Biological Sciences*, *366*(1562), 219-230. Retrieved 17 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC3013433/

[65] Connolly, M., Ibrahim, Z. R., & Johnson, O. N. (2016). Changing paradigms in lower extremity reconstruction in war-related injuries. *Military Medical Research*, *3*, 9. Retrieved 17 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC4818384/

[66] D'Souza, E. W., MacGregor, A. J., Dougherty, A. L., Olson, A. S., Champion, H. R., & Galarneau, M. R. (2022). Combat injury profiles among U.S. military personnel who survived serious wounds in Iraq and Afghanistan: A latent class analysis. *PLoS One*, *17*(4), e0266588. Retrieved 17 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC8985965/

[67] Royal Australian Air Force. *Aeromedical evacuation*. Australian Government, Department of Defence. Retrieved 02 Aug 26, from https://www.airforce.gov.au/our-work/humanitarian-support/aeromedical-evacuation

[68] U.S. Air Force. *C-130 Hercules*. Air Force Fact Sheet. Retrieved 02 Aug 26, from https://www.af.mil/About-Us/Fact-Sheets/Display/Article/1555054/c-130-hercules

[69] Goel, M. K., Khanna, P., & Kishore, J. (2010). Understanding survival analysis: Kaplan-Meier estimate. *International Journal of Ayurveda Research*, *1*(4), 274-278. Retrieved 11 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC3059453/

[70] Royston, P., & Parmar, M. K. B. (2013). Restricted mean survival time: an alternative to the hazard ratio for the design and analysis of randomized trials with a time-to-event outcome. *BMC Medical Research Methodology*, *13*, 152. Retrieved 11 Sep 26, from https://pmc.ncbi.nlm.nih.gov/articles/PMC3922847/

[71] Hall, A., Graham, B., Hanson, M., & Stern, C. (2023). Surgical capability utilization time for military casualties at role 2 and role 3 facilities. *Military Medicine*, *188*(11-12), e3368-e3370. Retrieved 10 Aug 26, from https://academic.oup.com/milmed/article/188/11-12/e3368/6961509
