...
| 189 | Replication confidence intervals treat antithetically paired runs as independent | High | Medium | Open |
...
UNBLOCKED (start now):
  #189 Replication CIs treat antithetically paired runs as independent
       (raised from #178: the antithetic negation reaches only the
       arrival generators, so partners share an unnegated trajectory
       stream and correlate +0.38 on DOW count, while every interval
       divides by the replication count. Affects summarise_replications(),
       summarise_scenario_totals() and the ci_mean()/ci_by_group() path.
