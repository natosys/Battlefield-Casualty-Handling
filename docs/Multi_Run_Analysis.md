...
Average R2E intensive care utilisation falls from 89.1% [87.5%, 90.6%] without the rule to 83.8% [82.2%, 85.5%] with it, a paired difference of 5.25 percentage points [3.20, 7.30]. The reduction in load is established.

The mortality cost is not. Average deaths of wounds per campaign read 1.14 [0.76, 1.52] without the rule and 1.36 [1.02, 1.70] with it, a paired difference of 0.220 [-0.207, +0.647]. The interval spans zero, so neither the size nor the **sign** of the effect is established: the point estimate here has the rule costing a fraction of a life rather than saving one, which is what the design predicts, and is well inside what 50 replications can resolve. Deaths of wounds vary enough between replications (paired standard deviation 1.50) that an interval whose half-width matched an effect this small would need roughly **180 replications**. That is the count the effect would require, not a count worth spending: an effect of two tenths of a death per campaign is not a planning quantity.
...
Within the rule, casualties recovering in a holding bed died at 0.14% against 0.00% for those recovering in intensive care. None of the 2,534 casualty-replications recovering in intensive care under the rule died of wounds, against 3 of the 2,120 recovering in a holding bed. That difference is built into the model rather than discovered by it: receiving reduced care changes the died-of-wounds curve applied to a casualty, so the figures measure how many casualties the rule sends down the higher-risk curve, not whether that curve is correct. The counts behind it are small in any case, so it establishes a direction rather than a size.
