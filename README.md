...
The ceiling values (`p1_p_max` = 0.0123, `p2_p_max` = 0.0100) were iteratively calibrated by Monte Carlo against that target. Two independent validation runs, at 50 replications (30 days, `seed = 42`) and at 100 replications (30 days, `seed = 7`), produced DOW rates of 0.484% (95% CI [0.340%, 0.629%]) and 0.403% (95% CI [0.296%, 0.510%]) of the wounded-in-action cohort; both confidence intervals span the 0.46% target, and both point estimates sit below it, which is the side of an upper bound the source permits. Pooled across the 150 replications the estimate is 0.430%. The 100-replication run measured 0.339% (95% CI [0.248%, 0.429%]) against the wider denominator of every casualty actually exposed to a DOW check, which adds the non-battle-injury sub-type of DNBI to the wounded in action; the narrower wounded-in-action denominator is reported as the headline because the source figure counts combat casualties.

Died of wounds is a rare event at these settings, at well under one death per 30-day run, so a replication estimate of the rate is coarse relative to the spacing of candidate ceilings. Three runs of a single unchanged configuration at 30, 50 and 100 replications spread across roughly a quarter of a percentage point during this calibration, which is larger than the difference between adjacent candidate ceilings. The calibration should therefore be read as locating each ceiling to within roughly a tenth of a percentage point of DOW rate, and a single run of fewer than about 100 replications should not be treated as confirming or refuting a ceiling on its own. The shape parameters are anchored to the aggregate mortality time-window analysis in [[29]](#References) and [[30]](#References); the logistic form itself is a standard S-shaped approximation for time-dependent failure processes [[41]](#References).
...
With these weaker factors, `dow.params` was re-calibrated by the same iterative Monte Carlo procedure used for the base configuration, reproducing the same historical target at `p1_p_max` = 0.0069 and `p2_p_max` = 0.0057, below the base 0.0123 and 0.0100. A lower ceiling is required to compensate for the weaker factors' smaller ceiling reduction. The floors (`p1_p_base`, `p2_p_base`), the shape parameters ($k$, $t_{mid}$ for both priorities), and the P3 flat rate (`p3_flat`) are unchanged from base under this profile; only the two ceilings are re-calibrated. Each of these terms is defined by the shifted logistic curve given in [Survival Function](#survival-function). A 100-replication run (30 days, `seed = 7`) of `moderate_intensity` produced:

| Metric       | `moderate_intensity` (100-rep)   | Historical target                                       |
| ------------ | -------------------------------- | ------------------------------------------------------- |
| Mean DOW/run | 0.65 (95% CI [0.49, 0.81])       | at most 0.68 (= 0.46% × 148.7 mean WIA)                 |
| DOW rate     | 0.437% (95% CI [0.329%, 0.544%]) | at most 0.46% (3 of more than 650) [[14]](#References)  |

The rate matches the historical target: the 95% confidence interval, [0.329%, 0.544%], contains the 0.46% upper bound derived in [Parameter Calibration](#parameter-calibration), and the point estimate sits below it, which is the side of the bound the source permits. Both ceilings were lowered when the target was corrected, from `p1_p_max` = 0.0089 and `p2_p_max` = 0.0074; at those values a 100-replication run of this profile gave 0.664% (95% CI [0.494%, 0.833%]), which does not span the corrected target. No killed-to-wounded comparator is reported against this profile, because no open-access campaign record gives killed and wounded counts covering the same population (see [Further Development](#further-development), L12).
...
| L22 | DOW calibration target is an upper bound on a treated cohort, imprecisely fitted | Medium |
...
**L22 — The DOW calibration target is an upper bound measured on a treated cohort, and the calibration against it is imprecise.** The historical target the mortality ceilings are tuned to is three deaths among the more than 650 casualties who reached the Ajax Bay medical chain [[14]](#References). Two things follow from the target itself. The denominator is reported as a lower bound, so the rate derived from it, 0.46%, bounds the historical outcome from above rather than estimating it, and the ceilings are therefore tuned to the most pessimistic reading the source allows. The denominator is also a treated cohort, counting only casualties who survived long enough to reach forward surgical care; a whole-of-campaign wounded-in-action denominator would include casualties who died before reaching care and would give a different, and for planning purposes more useful, rate. No open-access source reporting a British Falklands DOW count against such a denominator could be identified, and the parliamentary return that gives a campaign injury total counts Service personnel and civilians together while mixing wounded in action with cold injury and other non-battle injury [[53]](#References), so it cannot supply one either. A third limitation is procedural rather than evidential: died of wounds is rare enough at the calibrated ceilings that a replication estimate of the modelled rate carries a confidence interval wider than the gap between adjacent candidate ceilings, so the ceilings are located to roughly a tenth of a percentage point of DOW rate and no better. Closing the first two needs a source pairing a DOW count with a whole-of-WIA denominator for a conflict of comparable intensity, which would also let the ceilings be tuned to an estimate rather than to a bound; closing the third needs replication counts in the several hundreds per candidate ceiling, or a variance reduction technique aimed at rare-event estimation rather than the antithetic pairing currently applied to arrivals.
