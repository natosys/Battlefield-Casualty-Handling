...
**L17 — R2E holding beds carry recovery and evacuation on one pool.** A casualty awaiting a sortie holds a real R2E holding bed for the whole of its wait, and so does a casualty recovering to duty in theatre, so a single pool of 30 beds serves two unrelated demands. At the shipped configuration the evacuation wait is short, a mean of 0.5 days and a median of zero across a 90-day run, so most casualties board without occupying a bed for it at all and in-theatre recovery accounts for nearly all of the pool's occupancy. The pool is moderately loaded, averaging 66% occupancy with at least one casualty queued for 26% of that run, so the binding constraint is the bed count itself rather than airlift. What that does is push back into intensive care. A ventilated casualty on the critical route cannot step down from its bounded pre-flight ICU period while the holding pool is full, so its ICU occupancy stretches whenever that happens. It does not happen often at the shipped configuration: across the same run the median hold is 24.1 hours and the mean 24.7, the two sitting together because few holds are stretched at all, and the 90th percentile is 30.7 hours. The coupling is nonetheless real, and what suppresses it is that every scheduled sortie flies; `docs/Multi_Run_Analysis.md` measures the same pool where sorties are cancelled, and there the holding queue reaches 161 casualties in one campaign of six. Any R2E ICU or theatre-gating capacity finding must therefore be read alongside the holding pool's occupancy rather than in isolation. Separating the staging pool from the clinical holding beds, or expanding the pool, would each reduce the coupling; the defaults were deliberately not tuned to hide it.
