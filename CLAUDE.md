...
| Mean DOW/run (150 reps pooled, post-Issue-203) | Treated-cohort DOW rate (casualties reaching R2B or R2E) 0.443% (95% CI [0.385%, 0.500%]) at the re-fitted ceilings, pooled across three independent 50-replication measurements; the three returned 0.524%, 0.359% and 0.444% individually. This is the quantity the historical Ajax Bay bound of ~0.46% applies to, and the interval sits at it. `moderate_intensity` measures 0.290% (95% CI [0.240%, 0.340%]), unchanged by the re-fit since it overrides both ceilings, which makes it the control on this change. `scripts/check_dow_calibration.R` passes for both. The pre-re-fit configuration overshot: 0.528% (95% CI [0.460%, 0.595%]) at 150 replications and 0.513% ([0.462%, 0.564%]) at 250, an overshoot that survived the larger pool rather than dissolving into it, which is what warranted the re-fit. DOW/WIA rate is not pooled at 150 replications, the calibration check not reporting it; the 50-replication comparative measurement puts `moderate_intensity` at 0.55% (95% CI [0.40%, 0.69%]) and `high_intensity` at 0.75% ([0.68%, 0.83%]). See README Further Development L22 |
| Replication count for mortality figures (post-Issue-203) | Per-replication sd of the treated-cohort DOW rate is 0.0036 (base, 150 reps at the re-fitted ceilings). A 95% half-width of 0.15 pp needs 23 replications, 0.10 pp needs 50, 0.05 pp needs 199. The 50-replication figures elsewhere in this table carry roughly ±0.10 pp on this response; the queue and occupancy rows are time-weighted over far more events per replication and are better resolved at the same count. Single 50-replication measurements of the same configuration spanned 0.165 pp across control seeds during this issue's work, which is why the calibration pools three |
