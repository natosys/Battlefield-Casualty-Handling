...
Nothing trims the drawn value, so each stream realises exactly the mean and the coefficient of variation its configuration names, and the two configured fields are independent of one another: editing a stream's standard deviation alone leaves its realised mean where it was. `scripts/check_arrival_rate_fidelity.R` holds both families to both properties, exercising the shipped generator closures over a 1,000-day horizon and comparing each stream's realised daily mean against its configured `mean_daily`.

#### 2. Per-Minute Rate Sampling and Scaling

Draws samples from the stream's selected distribution representing per-minute casualty rates, then scales them according to population size and temporal resolution (per minute per 1000 personnel).

For each simulation minute $i \in \{1, 2, \dots, n_{\text{minutes}}\}$, the per-minute casualty rate is computed as:

$$
r_i = x_i \times \frac{P}{1000 \times 1440}
$$
...
- $r_i$ = scaled casualty rate for minute i
...
Both distributions have unbounded right tails, so an implausibly large draw is possible and its cost is worth stating. The walk itself is bounded: the closure performs exactly $n_{\text{minutes}}$ iterations across all its calls whatever any individual draw returns, so its execution time is set by the campaign horizon. The number of casualties that walk emits is bounded only in expectation, at the accumulated rate, since a minute that accrues several whole casualties emits all of them (Step 3). In practice neither is a constraint at any plausible parameterisation: a lognormal stream at 500 casualties per day with a standard deviation of 5,000, two orders of magnitude above anything the model ships, completes its 30-day walk in a fraction of a second and emits close to the 15,000 casualties that rate calls for. `scripts/check_arrival_rate_fidelity.R` exercises both properties on streams of that scale.
...
The number of casualties arriving in minute $i$ is the number of whole-casualty thresholds the cumulative total crossed during it:

$$
k_i = \lfloor C_i \rfloor - \lfloor C_{i-1} \rfloor
$$

so a minute in which the total crossed no threshold produces none, and a minute in which it crossed several produces all of them. The closure emits one casualty per call and holds the remainder as a debt, advancing to the next minute only once that debt is discharged, which is what makes the total number of casualties generated equal $\lfloor C_N \rfloor$ rather than the number of minutes in which a crossing happened. The distinction is invisible at the rates the model ships, where $k_i$ never exceeds one: a combat stream would need a single minute's draw above 576 casualties per day per 1,000 personnel for $k_i$ to reach two, which the shipped WIA parameterisation produces with probability $1.1 \times 10^{-7}$ per minute. It matters at rates several times higher, where a minute routinely accrues more than one casualty and treating $k_i$ as at most one would discard the majority of a stream.

#### 4. Temporal Randomisation

Introduces sub-minute jitter — a Uniform(0, 1) offset added to each detected arrival's whole-minute timestamp (`arrival_time <- minute_ptr + jitter`), so arrivals are not tied to the exact minute boundary the discrete per-minute computation grid produces them on. A minute producing $k_i$ casualties draws $k_i$ such offsets and sorts them, so each casualty is placed independently within the minute and the order between them is well defined; at $k_i = 1$, which is every minute at the rates the model ships, this is the single offset the minute has already drawn. Without this offset, every arrival from every one of the six independent casualty-generation streams would fall on a whole-minute timestamp, and two streams (e.g. combat WIA and combat KIA) could easily register an arrival in the same simulated minute, producing simultaneous, tied event times with no principled ordering between them. The jitter spreads each stream's arrivals continuously within their detected minute, giving simmer's event queue a well-ordered sequence of distinct timestamps to schedule against rather than a discretised, tick-aligned one. Each call to the generator closure returns the gap, in simulation minutes, between this arrival and the previous one — the value simmer's `add_generator()` `distribution` argument itself expects — rather than a pre-computed, already-sorted vector of arrival times; because each stream's own arrivals are produced in strictly increasing time order by construction, no separate sorting step is needed.
...
$$
r_i = x_i \times \frac{F(t_i)}{1000 \times 1440}
$$
...
| L27 | Casualty arrivals are far less variable day to day than a real arrival process | Medium |
...
**L27 — Casualty arrivals are far less variable day to day than a real arrival process.** Each stream realises the daily mean its configuration names (see [Casualty Generation](#casualty-generation)), but not the day-to-day spread around it. A day's casualty count is built by accumulating 1,440 independent per-minute rate draws and emitting an arrival at each whole-casualty crossing, so the count is close to an average of those draws and the central limit theorem flattens the distribution's own tail before it can reach a daily total. Measured over 5,000 simulated days, the combat WIA stream's daily count has a standard deviation of 0.50 against a mean of 4.42, where a Poisson process at the same rate would have 2.10; the stream is roughly four times less variable than the simplest defensible model of arrivals, and its busiest day in 5,000 carries six casualties. What follows is that peak-day casualty volume is understated, and with it the contention for theatres, intensive care beds and airlift that peaks drive, so every queue and utilisation figure in the analysis documents should be read as well resolved on its mean and as a lower bound on its peak. The flattening is a property of the accumulation itself rather than of any bound on the draws: measured against a generator that clamped every draw at three times the stream's mean, removing that clamp moves the daily standard deviation only from 0.49 to 0.50 and the busiest day in 5,000 from five casualties to six. Closing the gap therefore needs the minute-by-minute accumulation replaced. Simulating the arrival process by thinning [[65]](#references) is the established way to do that, generating candidate arrival times under a dominating rate and rejecting a controlled fraction, and is compatible with the live force-size dependence because the intensity would be read at the candidate time rather than at every minute.
