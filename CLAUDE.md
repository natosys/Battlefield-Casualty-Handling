...
| Mean DOW/run — base configuration (50-rep, seed=42, post-Issue-152) | 0.720; DOW/WIA 0.484%, 95% CI [0.340%, 0.629%] — spans the corrected 0.46% target, which is the calibration acceptance criterion. Against the wider denominator of every casualty exposed to a DOW check (WIA plus the NBI sub-type of DNBI): 0.406%, 95% CI [0.285%, 0.527%]. Measured at the pre-Issue-152 ceilings the same run gave DOW/WIA 0.955%, 95% CI [0.733%, 1.177%], which is what motivated the re-derivation |
| Mean DOW/run — `moderate_intensity` (50-rep, seed=42, post-Issue-152, new row) | 0.68, 95% CI [0.46, 0.90]; DOW/WIA 0.458%, 95% CI [0.311%, 0.605%] — spans the same 0.46% target at the profile's existing ceilings (`p1_p_max` 0.0089, `p2_p_max` 0.0074), which were therefore left unchanged |
