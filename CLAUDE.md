...
| `scripts/run_transport_sweep.R` | CLI entry point for the transport fleet-size sweep. `--refresh-baseline` is the only way to write this sweep's copy of the tracked `data/sweeps/`, and it fixes the swept range, replication count, horizon and seed rather than accepting the caller's |
...
| `data/airlift/` | Tracked strategic evacuation evidence set — the per-replication responses and their summary behind the national support base section of `docs/Multi_Run_Analysis.md`, at 50 replications per arm across two baselines and two sweeps, plus the collapse classification behind the strategic airlift reliability sweep at 30 replications over 360 days per arm and the classified pool's daily series, which is retained so that a statement about how a campaign's opening relates to its outcome stays recomputable. Written by `scripts/run_airlift_sweep.R --refresh-baseline` and `scripts/run_airlift_collapse.R --refresh-baseline` alone, each owning its own files |
| `data/scenarios/` | Tracked comparative scenario evidence set — the casualty totals, the per-resource queue summaries, and the per-pool queue comparison both per replication and reduced, behind the centrepiece tables of `docs/Multi_Run_Analysis.md`, at 50 replications over 30 days for each of the two casualty intensities. The per-replication pool series is kept because a pool's interval cannot be recovered from per-bed summaries, the pool total being in none of the monitor's rows. Written by `scripts/run_scenarios.R --refresh-baseline` alone |
| `data/sweeps/` | Tracked capacity sweep evidence set — the per-point means and intervals behind the transport fleet-size table and the forward ICU share frontier table of `docs/Multi_Run_Analysis.md`, at 10 replications per fleet size and 20 per share over 30 days. Written by `scripts/run_transport_sweep.R --refresh-baseline` and `scripts/run_icu_share_sweep.R --refresh-baseline` alone, each owning its own file |
| `data/long_horizon/` | Tracked sustained-operations evidence set — the reduced daily series and the per-block means behind every long-horizon statement in `docs/Multi_Run_Supplement.md`, at 30 replications over 360 days per casualty intensity. Written by `scripts/run_long_horizon.R --refresh-baseline` alone |
| `data/policy/` | Tracked evacuation policy and holding establishment evidence set — the per-replication responses, their summary and the paired differences against the shipped policy behind the policy sweep of `docs/Multi_Run_Analysis.md`, at 30 replications over 360 days for each of five policies, plus the per-arm checkpoints each was resumed from and the `establishment_sweep*` files of the holding establishment measurement. Written by `scripts/run_policy_sweep.R --refresh-baseline` alone |
| `data/hold_window/` | Tracked R2B pre-open hold window evidence set — both arms' per-replication responses, their summary and the paired differences behind Option 3's eight-row table in `docs/Multi_Run_Analysis.md`, at 50 replications over 30 days per arm. Written by `scripts/run_hold_window.R --refresh-baseline` alone |
| `data/icu_gate/` | Tracked post-operative intensive care gate evidence set — both arms' per-replication responses, their summary and the paired differences behind [The Post-Operative Intensive Care Gate](docs/Multi_Run_Analysis.md#the-post-operative-intensive-care-gate), at 50 replications over 30 days per arm. Written by `scripts/run_icu_gate.R --refresh-baseline` alone |
| `data/mass_casualty/` | Tracked mass casualty event stress test evidence set — both arms' per-replication responses, the total-casualty and event-count summary and the pooled died-of-wounds rate by casualty origin behind [Mass Casualty Events Degrade Care Without Revealing New Constraints](docs/Multi_Run_Analysis.md#mass-casualty-events-degrade-care-without-revealing-new-constraints), at 62 replications over 30 days per arm, plus the illustrative single run behind `images/mass_casualty_events.png`. Written by `scripts/run_mass_casualty.R --refresh-baseline` alone |
| `data/time_series/` | Tracked campaign time series evidence set — the per-replication queue series, queue clearance statistics and degraded post-operative care rates behind the two time series figures of `docs/Multi_Run_Analysis.md`, at 50 replications per casualty intensity. Kept rather than regenerated per render so that a figure and the percentages the paper quotes from it derive from one measurement; written by `scripts/render_time_series_figures.R --run --refresh-baseline` alone |
...
> **Provenance (both national support base levers enabled, Issue #376).** Every seed-42 figure below was re-measured after the two levers that move work from the deployed trauma system to the national support base were enabled together: `role4.surgery.reconstruction_share` from 0 to 0.2, and `r2eheavy.second_surgery.saturation_queue_threshold` from 0 to 8. They were flipped in one campaign rather than separately because each consumes a random draw and so moves every published figure, and enabling them one at a time would have re-run every replicated evidence set twice.
>
> The seed-42 campaign moves substantially, and almost all of that movement is stream shift rather than behaviour. The reconstruction cohort draws once per Priority 1 operated casualty at disposition, so seed 42 walks a different stream and produces 378 arrivals against 495; every count beneath that follows. The saturation threshold contributes nothing at this horizon, a 30-day campaign at the Falklands-modified rates never reaching a theatre queue of eight, which is the lever behaving as the saturation response it is rather than failing to fire. What does change behaviourally is that reconstruction now evacuates a casualty whatever their drawn recovery, and that the national support base carries reported operating theatre demand where it previously carried none.
>
> Six replicated evidence sets were re-measured with it, all tracked: the sustained-operations horizon, the evacuation policy and holding establishment sweeps, the strategic evacuation and airlift collapse experiments, the campaign time series and the comparative scenario tables, together with the new forward saturation release sweep behind the shipped threshold. Three published findings changed in substance rather than in their digits and are recorded where they appear in `docs/Multi_Run_Analysis.md`: the 30-day evacuation policy now carries a resolved mortality cost, the airlift collapse cliff moves from 15% to 10% sortie cancellation, and the claim that a campaign's outcome cannot be predicted from its opening is withdrawn.
>
> Two rows below are not re-measured at the new defaults and say so where they appear: the pooled died-of-wounds calibrations and the Welch warm-up figures. `scripts/check_dow_calibration.R` passes at the new defaults, so no ceiling was re-fitted, but the pooled three-measurement figures themselves predate this change. The measurements were made in an unpinned R 4.3.3 sandbox rather than the pinned Dev Container; `scripts/check_baseline_reproduction.R` reproduces the tracked evidence set byte for byte in this environment, and a maintainer re-run in `rocker/rstudio:4.4.2` remains the standard before these figures are treated as canonical.
...
>
> The comparative scenario tables stand at 50 replications per profile, and are now tracked in `data/scenarios/` rather than quoted from an untracked run (Issue #384): `moderate_intensity` 435.7 total casualties, 180.3 WIA, 1.00 DOW, 0.54% DOW/WIA, and `high_intensity` 1,046.7, 704.5, 24.02 and 3.40%. `scripts/check_scenario_protocol.R` asserts each of these against the tracked set, so this row and the paper's table cannot diverge. The figures quoted here before that measurement, 432.2, 184.5, 1.06 and 0.55%, and 1,050.4, 707.2, 23.80 and 3.35%, came from a run nobody could repeat; the movement between them and the tracked ones is the ordinary spread between two 50-replication measurements rather than a change in the model, no trajectory, arrival process or `env_data.json` value having moved. At the defaults in force for the Issue #155 refresh these read 437.8, 188.7, 0.78 and 0.42%, and 1,021.0, 684.3, 23.58 and 3.43%, and reproduced exactly in the pinned container, which is the reproducibility result this note was written to record. The measurements that do move are those whose generators had not been re-run since the arrival process was rebuilt, and they move because of that model change rather than because of the environment; each is identified where it appears.
>
...
|---|---|
| Total casualties (30 days) | 378 |
| WIA (combat + support) | 136 (combat 91 / support 45) |
| KIA (combat + support) | 78 (combat 64 / support 14) |
| DNBI (combat + support) | 164 (combat 132 / support 32) |
| Effective force size, end of run (seed 42) | combat: 2438 of 2500 initial (−2.5%); support: 1249 of 1250 initial (−0.1%), under the shipped 7-day reinforcement cycle. The pool is held far nearer establishment than an unreinforced run leaves it, which is the mechanism's purpose |
...
| Priority 1 share (seed 42) | 186 of 300 classified (62.0%) — P1 186 / P2 74 / P3 40, with 78 killed in action carrying no priority. Every casualty who reaches triage carries an assigned priority in this run |
| DOW count (seed 42) | 1 total (ame_wait=1). A single-run count at n≤4 carries no statistical weight; see the pooled multi-replication row below for the figure that does |
...
| Mean DOW/run (150 reps pooled, NOT re-measured at the Issue #376 defaults) | Treated-cohort DOW rate (casualties reaching R2B or R2E) 0.464% (95% CI [0.397%, 0.531%]) for the shipped `default` configuration, pooled across three independent 50-replication measurements returning 0.403%, 0.483% and 0.507%. This is the quantity the historical Ajax Bay bound of ~0.46% applies to, and the interval spans it. `moderate_intensity` measures 0.353% ([0.294%, 0.411%]) across 0.377%, 0.315% and 0.366%, and `high_intensity` 3.498% ([3.378%, 3.618%]) against the Okinawa target of 3.40%. `scripts/check_dow_calibration.R` passes for all three, so no ceiling was re-fitted for the defaults change. See README Further Development L22 |
...
| DNBI sub-types (seed 42) | battle_fatigue=41, disease=106, nbi=17 |
| bf_rtd (seed 42) | 39 |
| clinical_rtd (seed 42) | 95 (r1: 46, r2b: 37, r2e: 12) |
| total_rtd (seed 42) | 134, which is 35.4% of arrivals |
| Realised in-theatre share (seed 42) | 12.5% of 152 R2E dispositions retained in theatre under the shipped 21-day `evacuation_policy_days`. An output of the policy, not an input. The 30-replication sustained-horizon figure is 11.1% (95% CI [10.8%, 11.3%]), inside the 7.6%-42.1% historical range cited in README Return to Duty |
| Evacuation reason (seed 42) | 152 dispositions: 19 retained in theatre, 123 evacuated on a recovery beyond the policy and 10 evacuated because they require reconstruction at the national support base. `evacuation_reason` records which decided each disposition, and reconstruction evacuates a casualty whatever their drawn recovery |
| Pre-flight critical ICU hold (seed 42) | 10 critical-route evacuees drew the ventilated pre-flight path (`critical_hold.ventilated_share = 0.15`), holding intensive care a mean of 38.7 h, median 26.6 h, p90 81.0 h. The mean sits above the median because a hold stretches only when the holding pool is full (README Further Development L17) |
| Welch ICU-queue CMA (10 reps × 90 days, NOT re-measured at the Issue #376 defaults) | Final cross-replication CMA 0.342 with 14.2% of 2,159 increments decreasing, instantaneous mean queue peaking at 1.0. The queue roughly halves against the previous defaults, the 21-day evacuation policy freeing the holding beds that were blocking intensive care step-down. `WARM_UP_DAYS` remains 0; the basis for that classification at a sustained-campaign horizon is open work under Issue #312 |
| Surgical pathway split (seed 42) | 118 casualties operated on: 57 damage control (48.3%), 61 single-stage. By priority, Priority 1 87 operated / 48 damage control (55.2%, configured rate 55%), Priority 2 31 operated / 9 damage control (29.0%, configured 20%); no Priority 3 casualty reached theatre in this run |
...
| R2B surgical decision point (seed 42) | 116 casualties were recorded at it, all bypassed to R2E; 44 were operated at R2B |
| R2B surgeries (seed 42) | 44 |
| R2E surgeries — first op (seed 42) | 96 |
| R2E surgeries — second op (seed 42) | 32. Only a damage control casualty whose abbreviated operation was performed at R2E returns for a second procedure, and only where theatre was not saturated at the moment they were due back |
| R2B pre-transport bypass (seed 42) | at-R2B hold-full bypass: 1 — an independent count from the R2B surgical decision point above |
| R2B OT bypass reason (seed 42) | at-R2B subset: team off-shift 65, OT busy/queued 7 |
| R2B pre-open hold (seed 42) | 10 casualties held forward for a section about to reopen, mean hold 24.1 minutes, longest 44.8, against the shipped 60-minute `r2b.surgery.pre_open_window_min`. The tracked 50-replication figure (`data/hold_window/`) is 6.76 held per run (95% CI [6.10, 7.42]) |
| OT utilisation — echelon aggregate (seed 42) | R2B: 4.2%, R2E: 15.4% |
...
| R2E OT utilisation — 24h room (seed 42) | OT1: 38.7%, OT2: 26.6%. A theatre is seized before its surgical section, so a room reads as occupied while its casualty is still queued for staff; much of this is that wait, not operating time |
| R2E OT queue ≥1 (seed 42) | OT1: 20.0% of run, OT2: 13.6% |
...
| R2E ICU utilisation — mean (seed 42) | ICU1: 89.1%, ICU2: 85.6%, ICU3: 73.5%, ICU4: 66.2%, the four-bed pool holding a mean of 3.14 of its 4 beds |
| R2E ICU queue ≥1 (seed 42) | ICU1: 29.1% of run, ICU2: 3.7%, ICU3: 3.8%, ICU4: 0% |
| Transport utilisation — platform aggregate (seed 42) | HX240M: 2.8%, PMVAmb: 9.7% |
| PMV Ambulance utilisation (seed 42) | 9.7%; per-vehicle 19.8% / 7.3% / 1.9% |
| HX240M utilisation (seed 42) | 2.8%; per-vehicle 9.3% / 1.3% / 0.3% / 0.1% — carries R2B→R2E mortuary road-move traffic in addition to R1→mortuary KIA |
...
| R2E post-op pathway — stabilisation (seed 42) | icu=60, hold=27, so 69% of the stabilisation cohort reaches an intensive care bed. `surgery_deferred` = 21 |
| R2E post-definitive care pathway (seed 42) | The 30-replication sustained-horizon figure is 32.4% (95% CI [30.6%, 34.1%]) of operated casualties receiving post-definitive care in an ICU bed, the remainder taking the degraded holding-bed fallback (`data/policy/`) |
| R2E post-op DOW rate — icu vs hold (seed 42) | 0/60 vs 0/27 (single-run). The saturated-ICU 90-day stress test from Issue #43, which confirmed the mechanism fires with the hold pathway riskier than the icu pathway, has not been re-run since |
| Role 4 demand (seed 42) | 133 strategic evacuation decisions (74 critical route, 59 standard); 132 boarded and reached Role 4 by day 30 and one was still queued at R2E; Role 4 peak occupancy 76.0 concurrent patients on day 30, the campaign's last day |
| Role 4 bed composition (seed 42) | 529 intensive care bed-days (22.9%) and 1,776 holding bed-days (77.1%), reported in the bed types the deployed echelons field. A casualty admitted to intensive care is served the requirement theatre did not, then steps down to a holding bed. The peak and the total bed-days are unaffected by the split, both being properties of arrivals and stay length rather than of bed type. No `resus` demand is reported, the model giving Role 4 no reception episode. `ot` demand does arise: 35 operations are owed across 27 days of the campaign, all of them the reconstruction cohort's, carrying no theatre-minutes because no open-access source reports how long a debridement or a flap takes at this echelon. The released cohort, which does carry conserved theatre-minutes, is empty at this horizon, a 30-day campaign at these rates never reaching a theatre queue of eight (README Further Development L33) |
| Strategic AME actual performance (seed 42) | C-17A Globemaster III at 36 critical / 54 standard places; all four scheduled sorties flew, `failure_probability` shipping at 0, and 132 evacuated at a mean wait of 1.2 days (p10-p90 0.0-5.0), one casualty still queued when the window closes |
| AME wait-time DOW poll (seed 42) | `dow_echelon=5`, daily poll interval (`role4.ame.dow_check_interval = 1440` min); 1 death observed, which is the DOW count row above. This row previously read 0, which had stopped being true and was not re-measured; the casualty it missed is the one whose unbounded staging stay Issue #362 was raised for. See README [AME Wait Checkpoint](README.md#ame-wait-checkpoint) for why no single-run count should be read as evidence about the mechanism's magnitude |
