...
| Parameter                            | Variable             | Baseline | Lower | Upper | Rule |
| ------------------------------------ | -------------------- | -------- | ----- | ----- | ---- |
| Surgery duration (shared R2B/R2E)    | `surg_mode`          | 95 min   | 90    | 150   | A    |
...
One bound is also off centre. Surgery duration is screened over 90 to 150 minutes against a shipped mode of 95, so the range sits almost entirely above the baseline: the screen can shorten the procedure by at most five minutes and lengthen it by up to 55. The elementary effects remain valid differences taken across a clinically plausible range, but the parameter's influence is measured over a span the baseline sits at the edge of rather than in the middle of, which is not the comparison the other sixty-three rows make. Re-deriving the bounds under Rule A would give 57 to 133 minutes, which changes the design and so the ranking, placing it with the wider re-run rather than ahead of it.
