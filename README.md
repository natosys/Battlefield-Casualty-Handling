...
**ICU queues at the R2E are episodic**, reaching a peak of five patients waiting in isolated surges. These peaks are transient rather than sustained, but indicate that ICU capacity is tighter than the OT metric alone suggests. ICU capacity remains adequate for the modelled scenario but is the more constrained resource at R2E. With R2E now processing 122 surgical cases compared to 30 at R2B, the ICU load is correspondingly higher than before the bypass fix; the peak queue of five represents a meaningful constraint under surge conditions.
...
Following correction of DNBI sub-categorisation (Issue #7) and implementation of OT-bypass routing (Issue #35), the model now reflects a clinically coherent surgical cascade: R2B performs damage control surgery only when OT is immediately available (30 surgeries in the baseline run), and all other surgical candidates bypass to R2E, which performed 122 first surgeries. R2E OT utilisation is 34.0% mean — elevated relative to the pre-bypass finding but not saturated. **The dominant constraint identified in this run is R2B holding bed capacity.** Hold bed queues at R2B build progressively over the 30-day period, reaching 8–9 patients queued at R2B 1 by Day 21 and remaining elevated through the end of the run. This reflects the cumulative load of disease DNBI evacuees entering the R2B holding pathway and occupying hold beds for multi-day durations; it is not driven by post-surgical patients, as most surgical candidates now bypass directly to R2E.
