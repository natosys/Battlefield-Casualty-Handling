...
**L15 — R2B → R2E WIA Dead-Heading Return Leg Is Configured but Never Applied (Low Impact on Transport Fidelity)**
`r2b_transport_wia()` (`R/trajectories.R`) is the only function that reads `r2b.wia_transport.return_leg_multiplier` and seizes the shared PMVAmb fleet for the R2B → R2E leg — but it is never called from any trajectory. The R2B → R2E WIA evacuation actually used (every sub-path of `r2b_treat_wia()`'s Step 5 — immediate hold-then-evac, hold-full bypass, hold-queue-then-evac, and the ICU-wait fallback — all funnel through the same inline step) seizes each R2B team's own `evac` resource instead, applies the `r2b.wia_transport` mode/min/max duration, but never multiplies by `return_leg_multiplier` or models a return trip at all. The "R2B WIA Transport Return-Leg Multiplier" Configure field (Medevac) is therefore fully functional as a UI element — it reads and writes a real `env_data.json` path — but has no effect on any simulation output, unlike the equivalent R1 → R2B fields (`r1.wia_transport.return_leg_multiplier`, `r1.kia_transport.return_leg_multiplier`), which are both read inside the trajectory functions that are actually called and do affect PMVAmb/HX240M fleet queueing. This was found while building the Medevac panel's Medevac Chain diagram (Issue #14), whose caption states the discrepancy directly rather than drawing a return leg the model doesn't have. **Impact: Low** — the R2B → R2E leg's own outbound duration is unaffected and correctly modelled; only the dead-heading return-trip component (and, as a consequence, PMVAmb fleet queueing attributable to R2B → R2E dead-heading specifically) is not represented. Resolving this — either wiring `return_leg_multiplier` into the active Step 5 evacuation code, or removing the unused `r2b_transport_wia()` function and the now-misleading Configure field — is out of scope for this change; tracked as Issue #73.
