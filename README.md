...
**Current ranking.** The table below is `outputs/morris_ranking.csv` for the current eighty-parameter set, run at r = 20 trajectories with 5 replications over 30 days at seed 42: 1,620 design points. Every screened parameter carries a measured rank, the six composition balance coordinates included, interleaved by µ\* rather than reported separately.

`mass_casualty_rate` leads at µ\* = 13.04, well clear of the field: separating it from `pri1_evac_prob` at 4.72 needs only 11 trajectories, so the leader is established at r = 20. The next six parameters are not: `pri1_evac_prob` at 4.72, `pri1_surg_prob` at 4.35, `pri1_dcs_rate` at 4.21, `mc_p1_balance` at 4.20, `mass_casualty_kia_fraction` at 3.49 and `mass_casualty_max_cas` at 3.42 sit in one band, the closest pair (`pri1_dcs_rate` against `mc_p1_balance`) needing several million trajectories to separate and the widest gap in the band (`mass_casualty_kia_fraction` against `mass_casualty_max_cas`) needing 672. Every parameter in the band is either a mass casualty event property or a Priority 1 conditional rate, so the finding that matters is the band itself: casualty load and Priority 1 clinical probability dominate the queue together, and the screen does not resolve which of the two matters most.

The comparison the balance coordinates were added to make now resolves. `triage_p1_balance`, which governs the Priority 1 share of the casualty mix, ranks **22nd of eighty** at µ\* = 1.66, well below `pri1_evac_prob` at rank 2 and `pri1_surg_prob` at rank 3, the two Priority 1 conditional rates defined on that share; separating the coordinate from either needs only 16 and 13 trajectories respectively, so unlike the earlier screen this ordering is established at r = 20. The clinical rates conditioned on the Priority 1 share matter more for the queue than the share itself. Two other coordinates rank above `triage_p1_balance`, `mc_p1_balance` at 5 and `dnbi_disease_balance` at 9, and the remaining two sit lower, `triage_p2_p3_balance` at 19 and, well down the table, `dnbi_bf_nbi_balance` at 44 and `mc_p2_p3_balance` at 46. Two of the three composition groups therefore place a coordinate in the top ten (`mc_p1_balance` and `dnbi_disease_balance`), which is the condition under which a group is carried into a confirmatory decomposition and treated as a single object through the whole-composition Dirichlet sampling `run_sobol()` provides; that decomposition has not been run against this ranking (see [`data/sensitivity/`](data/sensitivity/README.md)).

Separation requirements quoted in this section are the trajectory count at which two parameters' µ\* would differ at 95%, computed from the elementary effects themselves: µ\* is a mean of absolute effects, so its standard error is the standard deviation of those absolute effects divided by $\sqrt{r}$. A rank should not be read as an ordering when the gap between neighbours is smaller than their combined standard error, which is most of the table below the leader.
...
Ten parameters sit at µ\* exactly zero, and for two different reasons. `p3_flat`, `r2b_dcs_factor` and `r2e_dcs2_factor` are mortality and treatment-efficacy parameters that act on who dies or how completely an operation succeeds rather than on how long a queue grows, so a queue-ranked screen is the wrong instrument for them. The other seven are Role 4 parameters, and they are zero by construction on every in-theatre response: the national support base census is computed after the simulation has finished ([Length of Stay](#length-of-stay)), so nothing in theatre can read them. The four length-of-stay modes and `reconstruction_share` do register on the two Role 4 occupancy responses, though well down both tables, at 51st or below. Three of the seven, `role4_return_interval_mode`, `role4_post_reconstruction_return_rate` and `role4_icu_continuation_mode`, are zero on all thirty-six responses, because none of the set measures what they change: the number of operations owed at the national support base, the return of a casualty after reconstruction, and the split of a stay between intensive care and a step-down ward, which conserves the stay's length and therefore the census. They stay in the design so that a response added later can rank them without a new parameter set, and until then their rank carries no information.

`r2b_evac_threshold` needs reading differently from every other row. It ships disabled at zero, and a Morris elementary effect taken across its first grid step measures switching the threshold on rather than the effect of its magnitude; the planning sweep behind [R2B Holding Capacity vs. Evacuation Threshold Sweep](docs/Multi_Run_Analysis.md#r2b-holding-capacity-vs-evacuation-threshold-sweep) measured that the single step from disabled to one day produces most of its whole effect on the R2B holding queue. It ranks 24th on the system queue but first on R2B dwell and both return-to-duty rates at R2B and R2E, and second on the p90 time to first surgery behind the mass casualty event rate, and every one of those ranks is a measurement of the on/off transition. It is screened so that the other parameters are evaluated across configurations in which the threshold is in force as well as disabled; its own rank should not be read as a planning influence, and the sweep is where its magnitude is measured.

| Rank | Parameter | µ\* | σ | Rank | Parameter | µ\* | σ |
| ---- | --------- | ---- | -- | ---- | --------- | ---- | -- |
| 1 | `mass_casualty_rate` | 13.0395 | 12.8106 | 41 | `r2e_vent_share` | 1.2301 | 1.7409 |
| 2 | `pri1_evac_prob` | 4.7177 | 5.9790 | 42 | `post_definitive_icu_mode` | 1.2146 | 2.2101 |
| 3 | `pri1_surg_prob` | 4.3510 | 4.6804 | 43 | `pri2_dcs_rate` | 1.1612 | 1.9840 |
| 4 | `pri1_dcs_rate` | 4.2089 | 6.0603 | 44 | `dnbi_bf_nbi_balance` | 1.1503 | 1.9067 |
| 5 | `mc_p1_balance` | 4.2003 | 7.1392 | 45 | `pri3_other_surg_prob` | 1.1087 | 1.9491 |
| 6 | `mass_casualty_kia_fraction` | 3.4886 | 6.1376 | 46 | `mc_p2_p3_balance` | 1.0505 | 1.8384 |
| 7 | `mass_casualty_max_cas` | 3.4247 | 5.9655 | 47 | `fr_demand_interval_days` | 0.9856 | 1.5188 |
| 8 | `wia_cbt_mean` | 2.6403 | 3.6336 | 48 | `disease_surgery_pct` | 0.9681 | 1.3674 |
| 9 | `dnbi_disease_balance` | 2.4464 | 3.0751 | 49 | `pri3_dnbi_surg_prob` | 0.9238 | 1.8086 |
| 10 | `surg_mode` | 2.3295 | 3.3315 | 50 | `r2e_critical_hold_mode` | 0.8981 | 1.2115 |
| 11 | `saturation_queue_threshold` | 2.0979 | 4.0166 | 51 | `p1_k` | 0.8433 | 2.0281 |
| 12 | `long_resus_mode` | 2.0822 | 3.4900 | 52 | `r2b_icu_share` | 0.7635 | 1.2206 |
| 13 | `p1_p_max` | 2.0347 | 3.5582 | 53 | `stabilisation_icu_mode` | 0.7269 | 1.1454 |
| 14 | `pri2_surg_prob` | 2.0124 | 2.8557 | 54 | `r1_tccc_factor` | 0.6999 | 1.1128 |
| 15 | `r2b_pre_open_window` | 2.0042 | 3.7265 | 55 | `reconstruction_share` | 0.6073 | 0.9532 |
| 16 | `mass_casualty_min_cas` | 1.9839 | 3.7352 | 56 | `pri3_dcs_rate` | 0.4624 | 1.1455 |
| 17 | `r2e_hold_mode` | 1.9394 | 3.3525 | 57 | `fr_fulfillment_lag_days` | 0.4465 | 0.6939 |
| 18 | `pri2_evac_prob` | 1.9165 | 3.0781 | 58 | `r2b_forward_hold_max` | 0.3753 | 0.6584 |
| 19 | `triage_p2_p3_balance` | 1.8886 | 3.0376 | 59 | `p2_p_max` | 0.3206 | 0.4634 |
| 20 | `kia_cbt_mean` | 1.8376 | 3.2072 | 60 | `ame_failure_probability` | 0.2546 | 0.4882 |
| 21 | `r2e_hold_beds` | 1.7897 | 2.6426 | 61 | `p1_p_base` | 0.2367 | 0.5228 |
| 22 | `triage_p1_balance` | 1.6593 | 1.7765 | 62 | `r2b_resus_factor` | 0.2192 | 0.3752 |
| 23 | `short_resus_mode` | 1.6344 | 3.1447 | 63 | `p2_t_mid` | 0.2074 | 0.5679 |
| 24 | `r2b_evac_threshold` | 1.5757 | 2.4116 | 64 | `fr_fill_mode_frac` | 0.1338 | 0.2487 |
| 25 | `r2b_hold_threshold` | 1.5500 | 1.8787 | 65 | `r2e_resus_factor` | 0.0814 | 0.2665 |
| 26 | `wia_spt_mean` | 1.5491 | 2.9472 | 66 | `p2_k` | 0.0494 | 0.1655 |
| 27 | `evacuation_policy_days` | 1.5485 | 2.1801 | 67 | `r2e_dcs1_factor` | 0.0420 | 0.1880 |
| 28 | `dnbi_cbt_mean` | 1.5394 | 1.9949 | 68 | `r2b_icu_penalty` | 0.0241 | 0.1080 |
| 29 | `ame_schedule_interval_days` | 1.5287 | 3.3085 | 69 | `p2_p_base` | 0.0230 | 0.0910 |
| 30 | `dnbi_spt_mean` | 1.4991 | 2.2114 | 70 | `r2e_postop_hold_penalty` | 0.0195 | 0.0683 |
| 31 | `r2e_icu_beds` | 1.4958 | 2.0839 | 71 | `p3_flat` | 0.0000 | 0.0000 |
| 32 | `kia_spt_mean` | 1.4530 | 2.0125 | 72 | `r2b_dcs_factor` | 0.0000 | 0.0000 |
| 33 | `ot_hours` | 1.4307 | 2.1521 | 73 | `r2e_dcs2_factor` | 0.0000 | 0.0000 |
| 34 | `p1_t_mid` | 1.4063 | 2.5840 | 74 | `role4_return_interval_mode` | 0.0000 | 0.0000 |
| 35 | `r1_wia_treat_mode` | 1.3342 | 1.5674 | 75 | `role4_post_reconstruction_return_rate` | 0.0000 | 0.0000 |
| 36 | `r1_transport` | 1.3111 | 1.7232 | 76 | `role4_los_p1_surgical_mode` | 0.0000 | 0.0000 |
| 37 | `r1_recovery_mode` | 1.3046 | 1.8244 | 77 | `role4_los_p1_nonsurgical_mode` | 0.0000 | 0.0000 |
| 38 | `post_op_hold_mode` | 1.2909 | 1.6546 | 78 | `role4_los_p2_mode` | 0.0000 | 0.0000 |
| 39 | `r2b_hold_mode` | 1.2871 | 1.8178 | 79 | `role4_los_p3_dnbi_mode` | 0.0000 | 0.0000 |
| 40 | `r2b_transport` | 1.2412 | 1.7630 | 80 | `role4_icu_continuation_mode` | 0.0000 | 0.0000 |

**What the ranking tells you.** The casualty load dominates. Mass Casualty Event Rate (`mass_casualty_rate`) leads at µ\* 13.04, followed by Priority 1 Strategic Evacuation Rate (`pri1_evac_prob`) at 4.72, Priority 1 Surgical Candidacy (`pri1_surg_prob`) at 4.35 and Priority 1 Damage Control Rate (`pri1_dcs_rate`) at 4.21, against a median of 1.24 across the set. Before relying on a run, check that the mass casualty configuration matches the scenario being modelled: an error there shifts surgical queue depth more than an error anywhere else.

Nine of the top ten are Scenario / Casualty Context parameters, so results depend most on assumptions about the casualty load rather than on decisions the health system can make. The tenth is Capacity, Surgery Duration (`surg_mode`) at rank 10, and no Policy parameter reaches the top ten: the highest are the Forward Theatre Saturation Release Threshold (`saturation_queue_threshold`) at 11 and the R2B Pre-Open Hold Window (`r2b_pre_open_window`) at 15, with the Theatre Evacuation Policy (`evacuation_policy_days`) at 27. A parameter's category tells you what a high rank means in practice and matters more than its exact position; see "Reading the plots: colour coding" below.

**Effects are not independent.** σ exceeds µ\* for 69 of the 80 parameters, meaning most parameters' influence depends on where the others are set. A parameter that looks inert at the baseline may matter once other values move, so conclusions drawn by changing one input at a time will understate what the model can do. Vary related parameters together, or compare whole scenario profiles, rather than tuning singly.

**Two results to read carefully.** The Reinforcement Demand Cycle ranks 47th, mid-table, and that ranking surrounds the shipped 7-day cycle rather than the disabled baseline an earlier screen was taken from; a parameter's elementary effects are local to the baseline they are screened from, so this is the figure to read for the configuration as it ships. Separately, ranks toward the bottom sit very close together: the ten lowest are all exactly zero, for the reasons given above. Treat the lower half of the table as a band rather than an order.

**Influence varies widely inside a single mechanism.** Reinforcement is the clearest case: the demand cycle ranks 47th while Reinforcement Fulfillment Lag (`fr_fulfillment_lag_days`) ranks 57th and Reinforcement Fill Distribution (`fr_fill_mode_frac`) 64th of 80, so cycle length is the value to get right when configuring it. Mass casualty events run the other way, every part of them ranking high: Event Rate (`mass_casualty_rate`) 1st, the triage balance of an event's casualties (`mc_p1_balance`) 5th, the Wounded/Killed Split (`mass_casualty_kia_fraction`) 6th, Event Size Maximum (`mass_casualty_max_cas`) 7th and Event Size Minimum (`mass_casualty_min_cas`) 16th, so the whole mechanism needs to match the scenario rather than one value within it.

**The two dwell-time responses.** R2B and R2E dwell are screened under the Kaplan-Meier restricted mean over every casualty who entered the echelon, rather than only over those who also left it (see `R/censoring.R` in [Codebase Structure](#codebase-structure)). At R2B the leaders below `r2b_evac_threshold`, whose rank is the on/off transition described above, are Priority 1 Surgical Candidacy, the R2B Hold-Bed Reroute Threshold, Priority 2 Surgical Candidacy and the DNBI Balance between disease and battle fatigue or non-battle injury (`dnbi_disease_balance`). At R2E the Theatre Evacuation Policy leads at more than three times the next parameter, the R2E Base Recovery-to-Duty Duration (`r2e_hold_mode`).
...
![Morris screening scatter plot of the mean system operating theatre queue across R2B and R2E, plotting each of the 80 screened parameters at its mean absolute elementary effect on the horizontal axis against the standard deviation of its elementary effects on the vertical axis, with points coloured orange for scenario context, green for capacity and blue for policy](images/morris_system_ot_q.png)

The mass casualty event rate sits alone at the right of the plot at nearly three times the importance of the next parameter, and the six highest-importance points are orange without exception, all six being properties of the casualty load or Priority 1 clinical probability. The leading capacity point, surgery duration, sits at about a sixth of the leader, and the leading policy points, the forward theatre saturation release threshold and the R2B pre-open hold window, at about the same fraction. The cloud fans upward and to the right, so a parameter that matters more also interacts more; the seven Role 4 parameters, the Priority 3 mortality rate and the two damage-control efficacy multipliers that reach zero sit together at the origin.
...
![Morris screening scatter plot of the mean R2E intensive care queue, plotting each of the 80 screened parameters at its mean absolute elementary effect on the horizontal axis against the standard deviation of its elementary effects on the vertical axis, with points coloured orange for scenario context, green for capacity and blue for policy](images/morris_r2e_icu_q.png)

The R2E Intensive Care Bed Count (`r2e_icu_beds`) leads on this response, at 1.51 against 1.25 for the Priority 1 Damage Control Rate, the only response in the tracked set where the establishment itself outranks every casualty-load or clinical-probability parameter: the pool this bed count screens is the one this queue measures directly. The Theatre Evacuation Policy is the first policy parameter, at rank 3, ahead of the R2B Holding Evacuation Threshold at rank 7: a casualty retained past the policy window occupies the intensive care bed a released casualty would otherwise have vacated, and the R2B threshold's own on/off transition (described above) shifts load onto the same pool. The Forward Theatre Saturation Release Threshold, which acts on this queue through the critical airlift wait of a casualty released with the definitive repair outstanding, ranks 26th, well down the table.

![Morris screening scatter plot of the total died-of-wounds count, plotting each of the 80 screened parameters at its mean absolute elementary effect on the horizontal axis against the standard deviation of its elementary effects on the vertical axis, with points coloured orange for scenario context, green for capacity and blue for policy](images/morris_dow_count.png)

The Priority 1 mortality ceiling and the mass casualty event rate stand clear of every other parameter on importance, at 3.92 and 3.44 against 1.74 for the next, so how lethal a Priority 1 wound is and how often the model is asked to absorb a surge matter more for mortality than any capacity or policy parameter in the screen. The remaining seventy-eight cluster at or below an importance of 1.74, most of them with interaction terms larger than their own importance.

![Morris screening scatter plot of the mean transport queue across the PMV Ambulance and HX240M fleets, plotting each of the 80 screened parameters at its mean absolute elementary effect on the horizontal axis against the standard deviation of its elementary effects on the vertical axis, with points coloured orange for scenario context, green for capacity and blue for policy](images/morris_transport_q.png)

The whole spread of importance sits below 0.26 of a vehicle, which is consistent with the fleet queueing rarely at the shipped establishment. The mass casualty event rate and the event size maximum take the two leading positions, and the two non-context parameters among the leading eight are Capacity: the R2E Post-Op Holding-Bed Duration at rank 7 and the R1 Battle Fatigue Hold Duration at rank 8. Nothing in the plot is a vehicle count, fleet size being swept directly rather than screened.
...
**L18 — Screening resolution and response coverage.** The Morris screen covers eighty parameters at twenty trajectories over a 30-day horizon, which resolves the leader from the negligible tail but does not order most of the leading cluster: the standard error of a µ\* is the standard deviation of its absolute elementary effects divided by $\sqrt{r}$, so neighbours separated by less than that are not distinguishable. The leader is separated from the second at this trajectory count, needing only 11 trajectories, but the second and third parameters would need some 1,650 to separate and the third and fourth some 11,200, which is not a practical target, so the leading cluster below the leader should be reported as a group. The published ranking is also on one response, the system OT queue. A screening run writes a ranking file for each of the thirty-six responses at no extra simulation cost, so those files exist, and the strategic airlift finding shows the cost of leaving them uninterpreted: `ame_failure_probability` ranks 60th of 80 on the queue while ranking 26th on mean Role 4 occupancy and 38th on the standard-route evacuation backlog. Whether a parameter ranks differently on a time-to-care, return-to-duty or strategic evacuation response is otherwise unresolved.
