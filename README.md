...
OT rooms are modelled as physical spaces available 24 hours per day. The surgical team operates on a 12-hour shift schedule and is the operative constraint on surgical access. Under seed 42 (30 days), **114 of 170 surgical candidates at R2B were bypassed to R2E** — either because the OT bed was occupied, a queue existed, or the surgical team was off-shift. **41 surgeries** were performed at R2B when both OT bed and team were simultaneously available. R2B OT utilisation was **5.4% (OT1) and 8.5% (OT2) against 24-hour room time**, equivalent to approximately **10.8% and 17.0% against available team shift time**. The OT queue remained flat at zero throughout the run, confirming the bypass logic is functioning as designed.
...
**Intervention Scenario D — Capacity-aware hold routing (Issue #39, implemented).** The R2B No Surgery trajectory now implements a three-stage routing policy at hold bed entry:

1. **R2B capacity available** — patient seizes a hold bed immediately (existing behaviour).
2. **R2B full, R2E hold has capacity** — patient bypasses to R2E via evacuation-team transport (`r2b_hold_bypass = 1`). This is also the fallback when the R2B queue cap is exceeded.
3. **R2B full, R2E hold full, queue within cap** — patient joins the R2B hold queue (`r2b_hold_queued = 1`). Queuing is only permitted when both echelons are saturated. The queue cap is computed as floor(R2B\_beds / (R2B\_beds + R2E\_beds) × R2B\_beds): with 10 R2B and 30 R2E hold beds this equals floor(10/40 × 10) = **2 patients**. Above that cap, further arrivals fall through to stage 2 (bypass to R2E).

Patients in stages 1 and 3 follow the same clinical pathway once a hold bed is seized: drawn hold duration, optional evac-threshold early evacuation, or RTD. Stage 2 bypass patients enter `r2e_treat_wia()` directly without a hold duration at R2B. The analysis pipeline reports both `r2b_hold_bypass_count` and `r2b_hold_queued_count`. This policy eliminates unbounded hold queuing while permitting a small, bounded R2B queue when R2E is simultaneously saturated — the clinically realistic scenario where both echelons are overwhelmed and no immediate downstream capacity exists.
...
Stream decomposition analysis (Issue #39) confirms that the five hold beds per R2B unit are structurally insufficient: expected concurrent occupancy is approximately 15.5 beds against a 10-bed total capacity across both R2B units (see R2B Hold Bed Saturation section). Four interventions have been analysed: hold duration reduction (insufficient alone), hold bed expansion to 8–10 per unit (structurally resolves the overload), an evacuation threshold policy routing long-duration holders to R2E (implemented; activation requires setting `vars.r2b.holding.evac_threshold` in `env_data.json`), and a capacity-aware three-stage routing policy (Issue #39, implemented) that routes overflow patients to R2E when R2B hold is full and R2E has capacity, or permits a small bounded queue (cap = 2 patients, floor(10/40 × 10)) when both echelons are simultaneously saturated. This eliminates unbounded hold queuing while retaining a clinically realistic queue in extreme saturation scenarios. **Impact: Medium** — patients are always dispositioned in finite time; overflow increases R2E medical hold load, which becomes consequential once time-dependent DOW (Issue #5) is merged. **Impact rises to High once Issue #5 is merged**, at which point hold routing will directly affect modelled mortality pathways.
