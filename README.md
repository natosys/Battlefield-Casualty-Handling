...
Dead-heading is implemented for the two legs served by the pooled, brigade-level PMV Ambulance/HX2 40M fleets — R1→R2B (PMV Ambulance, WIA) and R1→mortuary (HX2 40M, KIA/DOW) — using simmer's `clone()`/`synchronize()` activities. After the outbound timeout, the entity is cloned into two parallel branches: a vehicle branch that runs an unladen return-leg timeout before releasing the asset, and a casualty branch with no further activity. `synchronize(wait = FALSE)` then lets the casualty branch continue immediately into the rest of the trajectory (it is always the first of the two clones to reach that point), while the vehicle branch is discarded once it later arrives there having completed the return leg and released the resource. The resource therefore remains occupied — and unavailable to other casualties — for the full round trip, while the casualty's own care pathway is unaffected by the vehicle's return travel time. R2B→R2E WIA transport is deliberately excluded: it seizes each R2B team's own organic `evac` resource rather than the shared PMVAmb fleet, and is not modelled with a dead-heading return leg (Issue #73). Mortuary transfers at R2B and R2E (`r2b_transport_kia()`, `r2e_transport_kia()`) similarly use collocated evacuation team personnel rather than a pooled vehicle asset and are unaffected.

Return leg duration for the two dead-heading legs is modelled as a fresh triangular draw from the same outbound distribution, scaled by a configurable `return_leg_multiplier` (`vars.r1.wia_transport.return_leg_multiplier`, `vars.r1.kia_transport.return_leg_multiplier` in `env_data.json`). The default is 1.0 (a symmetric round trip): tactical vehicle rate-of-march planning for these vehicle classes over these distances is governed by terrain, threat, and route conditions rather than payload, so laden and unladen legs are not doctrinally differentiated by travel time. The multiplier is retained as a configurable parameter — and included in the Morris sensitivity screen — so that a faster or slower return (e.g. an unobstructed empty-vehicle route, or a return leg exposed to different threat conditions) can be tested as a scenario variation rather than assumed by default.
...
> **MODEL ASSUMPTION — Dead-Head Return Leg Multiplier:** The return leg duration for each of the two dead-heading transport legs (R1→R2B PMV Ambulance, R1→mortuary HX2 40M) is modelled by default as 1.0× the outbound triangular distribution (a fresh draw from the same min/max/mode, i.e. a symmetric round trip), configurable via `return_leg_multiplier`. R2B→R2E WIA transport is not a dead-heading leg (Issue #73) and has no `return_leg_multiplier`.
...
`r2b_transport_wia()` (`R/trajectories.R`) previously sat alongside the R2B → R2E WIA evacuation code actually in use as dead code: it read `r2b.wia_transport.return_leg_multiplier` and seized the shared PMVAmb fleet, but was never called from any trajectory. The evacuation code actually used (every sub-path of `r2b_treat_wia()`'s Step 5 — immediate hold-then-evac, hold-full bypass, hold-queue-then-evac, and the ICU-wait fallback — all funnel through the same inline step) seizes each R2B team's own `evac` resource instead, and does not model a return trip. Resolved by formalising the local-asset design as intentional rather than wiring in the shared fleet: `r2b_transport_wia()` and the now-misleading "R2B WIA Transport Return-Leg Multiplier" Configure field (and its `r2b.wia_transport.return_leg_multiplier` `env_data.json` entry and Morris `apply_params()` write) were removed. R2B → R2E WIA transport represents an organic R2B unit asset, distinct from the brigade-pooled PMVAmb fleet used for R1 → R2B, and is not modelled with a dead-heading return leg — the Medevac Chain diagram (Issue #14) and its caption now state this directly rather than flagging it as an open discrepancy. This is a documentation- and dead-code-only change: the R2B → R2E leg's own outbound duration, PMVAmb/HX240M fleet queueing, and all seed-42 baseline figures are unaffected, since the removed code path was never invoked and the removed `env_data.json` field was never read by any live trajectory.
