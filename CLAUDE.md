...
> **Provenance caveat (Issue #159 refresh):** every seed-42 single-run row below marked "post-Issue-159" was regenerated in an **unpinned R 4.3.3 sandbox** (no Docker access in this session's development environment to build the project's pinned `rocker/rstudio:4.4.2` Dev Container), carrying the same accuracy caveat as the Issue #18, #23, #161, #154, #156 and #160 refreshes — a maintainer re-run in the pinned container is needed before these figures are fully authoritative. Issue #159 makes a casualty's post-operative ICU requirement a single quantity, drawn once and divided between R2B and R2E by the new `r2b.post_op_icu.share`, where previously R2B delivered none of it and R2E separately shortened its own episode for R2B-operated casualties: an R2B-operated casualty received roughly 28% of the ICU time an otherwise identical R2E-operated one did. This is **not** RNG-stream-neutral even at the shipped share of zero, which is the configuration these rows describe: retiring the `post_surgery` draw removes a `sample()` call from `r2e_icu_recovery` for every R2B-operated casualty, and per the single-global-RNG-stream mechanism documented for Issue #18/#23/#161 that shifts the draw position for every subsequent draw by any entity, with Issue #18's force-regeneration feedback loop carrying the change back into arrival timing (total casualties 386 to 384). The substantive effect is that R2E ICU load rises sharply, because the 75% of R2B-operated casualties who previously drew a 60-minute stay now receive the full requirement: per-bed ICU utilisation moves from 82.8/87.2/61.1/54.2% to 96.2/86.7/80.5/80.4%, deferred surgeries from 6 to 14, and the binding constraint moves off the surgical roster, whose sections no longer queue at all. `r2b_icu_penalty` (1.31) is new in `dow.treatment_efficacy`; `r2eheavy.recovery.post_surgery` and `r2eheavy.short_icu` are removed, and the Morris set swaps `post_surgery_prob` and `short_icu_mode` for `r2b_icu_share` and `r2b_icu_penalty`, holding at fifty-three parameters. The published Morris ranking has **not** been re-run and is flagged as such in README; the 50-replication multi-run rows were **not** recomputed and remain flagged pending. `data/arrivals_*.txt`, `logs/logs.txt` and `images/*.png` **were** regenerated and committed as one set from a single `--refresh-baseline` invocation, following the Issue #154 precedent.

| Metric | Baseline value |
|---|---|
| Total casualties (30 days) | 384 (post-Issue-159; was 386 post-Issue-160, 385 post-Issue-156, 387 post-Issue-161, 400 pre-Issue-18) |
| WIA (combat + support) | 147 (post-Issue-159; was 149 post-Issue-160, 148 post-Issue-156, 150 post-Issue-161, 154 pre-Issue-18) |
| KIA (combat + support) | 67 (post-Issue-159, unchanged from post-Issue-160, post-Issue-156 and post-Issue-161; was 70 pre-Issue-18) |
| DNBI (combat + support) | 170 (post-Issue-159, unchanged from post-Issue-160, post-Issue-156 and post-Issue-161; was 176 pre-Issue-18) |
| Effective force size, end of run (seed 42, post-Issue-159) | combat: 2326 of 2500 initial (−7.0%); support: 1175 of 1250 initial (−6.0%); no reinforcement (`force_regeneration.reinforcement.demand_interval_days = 0`, the shipped default). Was combat 2330 / support 1176 when last measured, post-Issue-18 |
...
| Priority 1 share (seed 42, post-Issue-159) | 218 of 384 classified (56.8%) — P1 218 / P2 57 / P3 42 / KIA 67; was P1 208 / P2 66 / P3 45 / KIA 67 post-Issue-160. Every generated casualty carries an assigned priority in this run |
| DOW count (seed 42, post-Issue-159) | 3 total: r2b=3, ame_wait=0; was 1 (r2b=1) post-Issue-160. At n≤3 a change of two carries no statistical weight; see the 50-replication row below for the figure that does |
...
| DNBI sub-types (seed 42, post-Issue-159) | battle_fatigue=46, disease=91, nbi=33 (post-Issue-159; was battle_fatigue=31, disease=109, nbi=30 post-Issue-160) — RNG-stream-shifted despite DNBI sub-type assignment itself being unrelated code, per the single-global-RNG-stream mechanism (see provenance caveat above) |
| bf_rtd (seed 42, post-Issue-159) | 44 (post-Issue-159; was 28 post-Issue-160) |
| clinical_rtd (seed 42, post-Issue-159) | 91 (r1: 47, r2b: 36, r2e: 8) (post-Issue-159; was 104 — r1: 52, r2b: 41, r2e: 11 — post-Issue-160) |
| total_rtd (seed 42, post-Issue-159) | 135 (post-Issue-159; was 132 post-Issue-160) |
| Realised in-theatre share (seed 42, post-Issue-159) | 24.8% of 153 R2E dispositions retained in theatre under the shipped 30-day `evacuation_policy_days`. An output of the policy, not an input. Across 50 replications 26.9% (95% CI [25.8%, 27.9%]), inside the 7.6%-42.1% historical range cited in README Return to Duty. Was 30.3% of 119 dispositions (50-rep 28.9%) post-Issue-156; the disposition count rises because casualties clear R2E instead of queueing for a sortie |
...
| R2B surgical decision point (seed 42, post-Issue-159) | 119 casualties reached it (50 operated at R2B, 69 bypassed to R2E). Supersedes the "R2B surgical candidates = 141" row carried since Issue #76, which counted a different quantity and had not been recomputed since |
| R2B surgeries (seed 42, post-Issue-159) | 50 (post-Issue-159; was 46 post-Issue-160) |
| R2E surgeries — first op (seed 42, post-Issue-159) | 122 (post-Issue-159; was 123 post-Issue-160) |
| R2E surgeries — second op (seed 42, post-Issue-159) | 66 (post-Issue-159; was 73 post-Issue-160) |
| R2B pre-transport bypass (seed 42, post-Issue-159) | upstream R1-threshold bypass: 115 (was 134 post-Issue-160); at-R2B hold-full bypass: 0 (unchanged); R2B hold queue (both full): 0 (unchanged) — these are three independent, non-summing counts |
| R2B OT bypass reason (seed 42, post-Issue-159) | at-R2B subset, 69 total (was 80 post-Issue-160): team off-shift 59 (was 73), OT busy/queued 10 (was 7) |
| OT utilisation — echelon aggregate (seed 42, post-Issue-159) | R2B: 3.8%, R2E: 21.1% (was R2B: 3.7%, R2E: 23.4% post-Issue-160) |
| R2B OT utilisation — 24h room (seed 42, post-Issue-159) | T1: 7.7%, T2: 6.4% (was T1: 5.1%, T2: 7.3% post-Issue-160) |
...
| R2E OT utilisation — 24h room (seed 42, post-Issue-159) | OT1: 54.5%, OT2: 40.0% (was OT1: 59.6%, OT2: 44.0% post-Issue-160). A theatre is seized before its surgical section, so a room reads as occupied while its casualty is still queued for staff; much of this is that wait, not operating time |
| R2E OT queue ≥1 (seed 42, post-Issue-159) | OT1: 22.4% of run, OT2: 16.7% (was OT1: 31.1%, OT2: 22.5% post-Issue-160) |
...
| R2E ICU utilisation — mean (seed 42, post-Issue-159) | ICU1: 96.2%, ICU2: 86.7%, ICU3: 80.5%, ICU4: 80.4% (was ICU1: 82.8%, ICU2: 87.2%, ICU3: 61.1%, ICU4: 54.2% post-Issue-160). The rise is the point of the change: three in four R2B-operated casualties previously drew a 60-minute R2E stay in place of the full requirement. The 50-replication four-bed mean was not recomputed and remains 68.9% as last measured post-Issue-160 |
| R2E ICU queue ≥1 (seed 42, post-Issue-159) | ICU1: 50.6% of run, ICU2: 42.1%, ICU3: 7.5%, ICU4: 0% (was ICU1: 23.3%, ICU2: 24.7%, ICU3: 0%, ICU4: 0% post-Issue-160) |
...
| PMV Ambulance utilisation (seed 42, post-Issue-159) | 10.2%; max queue 0 throughout run; per-vehicle 23.4% / 6.5% / 1.8% (was 25.1% / 6.0% / 1.0% post-Issue-160) |
| HX240M utilisation (seed 42, post-Issue-159) | 4.8%; max queue 0 throughout run; per-vehicle 9.1% / 3.6% (was 8.8% / 0.7% post-Issue-160) — carries R2B→R2E mortuary road-move traffic in addition to R1→mortuary KIA |
...
| R2E post-op pathway (seed 42, post-Issue-159) | icu=57, hold=65 (post-Issue-159; was icu=85, hold=38 post-Issue-160) — the degraded-care share rises back above half, because a conserved ICU requirement keeps beds occupied far longer than the short stay it replaced; see README Limitations L17 and L24. `surgery_deferred` = 14 (was 6 post-Issue-160) |
| R2E post-op DOW rate — icu vs hold (seed 42, post-Issue-159) | 0/57 vs 0/65 (single-run; was 0/85 vs 0/38 post-Issue-160). The saturated-ICU 90-day stress test from Issue #43, which confirmed the mechanism fires with the hold pathway riskier than the icu pathway, has not been re-run since |
