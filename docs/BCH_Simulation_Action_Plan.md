...
### Issue 113 — Split README into System Reference and Two Analysis Documents ✓

**Merged:** PR #145, branch `claude/issue-113-d3m60w`

Splits the project's academic output into three documents. `README.md` becomes the system reference, carrying code structure, trajectory logic, the resource model, diagrams, inline assumptions and Further Development, with no simulation results. `docs/Single_Run_Analysis.md` carries the illustrative seed-42 30-day narrative, and `docs/Multi_Run_Analysis.md` the multi-run comparative analysis at n≥30 with 95% confidence intervals, generated by running the comparative scenario runner across both shipped scenario profiles. `scripts/check_markdown.R` was extended to maintain the table of contents and return-to-top links across all three, and `scripts/check_env_data_summary.R` updated for the new layout.

The bulk of the work was a section-by-section review of the resulting README against the project's own standards. Every claim was checked against source rather than read: this corrected a Priority 1 DOW probability quoted as 6% where the shipped logistic caps at 2.3% and evaluates to 0.14% at the stated elapsed time, an incremental R2B figure of 1 to 3% against an actual 0.14%, KIA processing durations that quoted the transport distribution at three echelons, a disease sub-type described as having zero surgical candidacy against a shipped 6%, five Configure panels where six render, nineteen triangular curve groups where the app detects twenty-one, and a fabricated citation claiming a 30-replication minimum that its cited source does not state. Two trajectory diagrams were corrected: the R2B holding path showed one of three branches, and the R2E diagram routed non-surgical casualties into an operation while omitting the checkpoint at which casualties can die awaiting strategic evacuation.

Formatting was brought into line throughout: all blockquote sections removed, including seventeen model-output cards and six model-assumption blocks converted to the prose form the standards require; issue-number references removed from every section not exempt; named-author citations replaced by reference numbers; em dashes removed from prose; and two dead reference URLs repaired. The Limitations and Further Development sections were merged into a single forward-looking section, since five of seven development items restated a limitation and both had accumulated completed work. `CLAUDE.md` is updated to match, and now specifies that a closed gap is deleted rather than marked resolved, that entry identifiers are never reused, and that a grouped list must be re-checked against its headings after reordering.

**Seed-42 baseline (30 days, single run):** Unaffected. This PR touches no `R/` module, `env_data.json`, or `app.R`; changes are confined to documentation, the two maintenance scripts, regenerated plot images, and `renv.lock`.

**Unblocked by this merge:** No new issues unblocked. No open issue lists Issue #113 as a dependency.
