...
**Known limitations (documented in README L12):** the `moderate_intensity` KIA:WIA ratio (0.452) differs from the 255:777 (0.328) record cited here — a pre-existing characteristic of the Issue #1 casualty generator calibration, not introduced or corrected here. Issue #152 subsequently found that neither figure in that 255:777 pairing is supported by the reference it was attributed to, and that the two counts do not cover the same population in any case; README L12 now records the properly sourced figures and withdraws the ratio as a validation target. The same issue superseded the 0.52% DOW/WIA target this profile was validated against, for the reasons recorded under Issue 5 above, and re-derived this profile's ceilings against the corrected target (`p1_p_max` 0.0089 to 0.0069, `p2_p_max` 0.0074 to 0.0057); the re-validation is recorded under Issue 152. `high_intensity` is an explicitly unvalidated skeleton (only casualty generation rate and distribution family are sourced). No Vietnam-calibrated profile exists — none was fabricated in place of a genuine source.
...
**Calibration:** P1 p_max = 0.023, k = 0.04, t_mid = 120 min; P2 p_max = 0.019, k = 0.025, t_mid = 180 min. Calibrated against the Falklands 1982 historical rate of 3 DOW / 580 WIA ≈ 0.52% (Payne 1983). A 50-replication validation run (seed = NULL) produced mean 0.70 DOW/run (0.45% of WIA), 95% CI [0.41, 0.95] — encompassing the 0.52% target.

> **Superseded by Issue #152.** The 0.52% target recorded above, and its attribution to Payne (1983), are both incorrect. Payne (1983) reports 233 casualties operated on with three deaths and does not report the denominator 580 or any DOW/WIA rate; neither does Westphalen (2018), the reference README.md attributed the same figure to. Issue #152 replaced the target with the treated-cohort rate that Westphalen (2018) does report — three died of wounds among the more than 650 casualties who reached the Ajax Bay medical chain — and re-validated the ceilings against it. See README [Parameter Calibration](../README.md#parameter-calibration) for the current basis. Seed-42 single run: 4 DOW. The values `p_max` and the OIF/OEF-era efficacy factors are entangled; substituting Falklands-era efficacy values (Issue #54) requires re-calibrating `p_max` upward.
