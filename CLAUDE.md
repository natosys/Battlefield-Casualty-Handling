...
| Mean DOW/run (50-rep, seed=NULL, post-Issue-173) | 1.37; 95% CI [1.10, 1.63]. DOW/WIA rate 0.920% (95% CI [0.741%, 1.100%]); treated-cohort DOW rate (casualties reaching R2B or R2E) 0.617% (95% CI [0.503%, 0.732%]), the quantity the historical Ajax Bay target of ~0.46% applies to. The same 50-replication measurement made on the pre-Issue-173 code gives 1.90 (95% CI [1.53, 2.27]) and a treated-cohort rate of 0.876% (95% CI [0.702%, 1.050%]), so the pathway split moves the model substantially back toward the target, having been carried above it by Issue #159's addition of a post-definitive intensive care episode. The interval no longer spans the target from above, and `p1_p_max` and `p2_p_max` were nonetheless left unchanged: recalibrating them is a separate decision from this issue's, and the previous 50-replication figure of 0.530% predates Issue #159 and so is not a like-for-like comparison. See README Further Development L22 |
