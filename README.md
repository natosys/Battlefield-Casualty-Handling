...
Daily casualty treatments at R2B 1 and R2B 2 show the impact of OT-bypass routing (Issue #35): patients flagged for surgery check OT availability at R2B and, if the theatre is occupied or any queue exists, bypass immediately to R2E Heavy rather than waiting. Under seed 42 (30 days), **74 of 103 surgical candidates at R2B were bypassed to R2E**, with only 29 surgeries performed at R2B. R2B OT utilisation was **5.4%**, and the OT queue remained flat at zero throughout the run — confirming the bypass logic is functioning as designed. The 12-hour OT availability window (single surgical team per R2B) means that any surgical patient arriving during the off-shift is also bypassed, consistent with the clinical intent.
...
Following DNBI sub-categorisation (Issue #7) and OT-bypass routing (Issue #35), the R2E Heavy receives the majority of battlefield surgical demand. With 74 bypass patients from R2B added to those routed directly from R1, the R2E performed **102 first surgeries** in the baseline run (seed 42, 30 days), compared with 29 at R2B. This represents the intended clinical model: R2B performs damage control surgery only when OT is immediately available; all other surgical candidates flow to the R2E for definitive care.

![Alt text](images/r2eheavy_bed_queue_3_teams.png)

**R2E OT utilisation has increased materially** following the bypass fix. Mean OT utilisation across the two R2E operating theatres was **30.8%** (seed 42, 30 days) — OT 1 at 46.7% and OT 2 at 14.8% — reflecting asymmetric routing as the first available theatre absorbs the bulk of bypass demand. OT queues at the R2E remain sporadic and brief, reaching a maximum of two patients waiting on isolated occasions and returning to zero throughout the run. At 30.8% mean utilisation, R2E OT capacity is not currently saturated under Falklands-equivalent casualty rates, but the R2E now operates as the primary surgical node for the force.

**ICU queues at the R2E are modest**, with transient peaks of 1–2 patients appearing around Days 22–23 and briefly on Day 30. These are short-lived and represent isolated surges rather than sustained saturation. ICU capacity is therefore adequate for the modelled scenario. With R2E now processing 102 surgical cases compared to 29 at R2B, the ICU load is correspondingly higher than before the bypass fix; however, it remains within establishment capacity under current casualty rates.

![Alt text](images/r2eheavy_gantt.png)

The Gantt chart confirms that the R2E's OT and ICU beds see intermittent use throughout the run, with substantial idle capacity between surgical episodes. The 30 holding beds show near-zero occupancy, consistent with the high (~90%) strategic evacuation rate applied to post-surgical and post-ICU patients. Resuscitation beds likewise see very limited use. Taken together, the R2E is substantially under-utilised relative to its establishment under this scenario — a finding that contrasts sharply with the R2B holding bed saturation described above.
...
Following correction of DNBI sub-categorisation (Issue #7) and implementation of OT-bypass routing (Issue #35), the model now reflects a clinically coherent surgical cascade: R2B performs damage control surgery only when OT is immediately available (29 surgeries in the baseline run), and all other surgical candidates bypass to R2E, which performed 102 first surgeries. R2E OT utilisation is 30.8% mean — elevated relative to the pre-bypass finding of 10.8% but not saturated. **The dominant constraint identified in this run is R2B holding bed capacity.** Hold bed queues at R2B build progressively over the 30-day period, reaching 8–9 patients queued at R2B 1 by Day 21 and remaining elevated through the end of the run. This reflects the cumulative load of disease DNBI evacuees entering the R2B holding pathway and occupying hold beds for multi-day durations; it is not driven by post-surgical patients, as most surgical candidates now bypass directly to R2E.

The R2E Heavy operates as the primary surgical node and remains within capacity at baseline. ICU queues are transient; resuscitation and holding beds see very limited demand. The mismatch — R2B holding saturation with R2E surgical capacity adequate — suggests that the primary system lever is R2B holding bed expansion or a higher evacuation threshold from R2B holding to strategic evacuation, rather than surgical capacity adjustment.
