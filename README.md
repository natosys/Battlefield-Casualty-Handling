...
The ward each category is admitted to is configured rather than fixed in code, as `role4.wards` in `env_data.json`, alongside the set of wards the census reports. A stay can also be split between two wards rather than held in one, which is what `role4.icu_continuation` governs. Where it is enabled, a casualty's Role 4 intensive care is the whole post-operative requirement less whatever R2E already served, recorded per casualty as `post_definitive_min`, and the step-down ward carries the remainder of the stay. That is the same contract the model already applies forward, where one stabilisation requirement is drawn once and divided between R2B and R2E so that the total cannot depend on the route (see [R2E Heavy Trajectory](#r2e-heavy-trajectory)); Role 4 extends it one echelon further. Because the remainder is taken as a difference rather than drawn, the two phases sum to the drawn length of stay exactly, and a casualty theatre never operated on has no episode to continue and so keeps the whole stay on its admission ward.
