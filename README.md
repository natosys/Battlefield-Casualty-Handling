...
> **Consequence if wrong:** A multiplier below 1.0 (faster return) understates queue formation; a multiplier above 1.0 (slower return) overstates it. Return route conditions are assumed symmetric with the outbound route; asymmetric terrain, threat, or traffic conditions on the return leg are not separately modelled.
...
The table below is the current `outputs/morris_ranking.csv` (r=20, 5 reps, 30 days, seed 42; Issue #73 follow-up re-run), ranked by µ\* on the system OT queue:
...
| 10 | `long_icu_mode` | 0.0116 | 0.0156 |

No parameter dominates on this KPI: µ\* values span a narrow 2.2× range (0.0116–0.0259) rather than showing the order-of-magnitude separation that would identify one or two clearly rate-limiting inputs. Every parameter's σ is comparable to or exceeds its µ\* (σ/µ\* ranging 0.84–1.34), indicating nonlinear or interaction-dominated effects throughout rather than the simple linear effects a low-σ, high-µ\* parameter would show — consistent with the branching, resource-contingent routing logic in `R/trajectories.R`, where a parameter's effect depends on which branch a casualty happens to take. `return_leg_multiplier` still ranks near the bottom (9th of 10) on this specific KPI even with its scope doubled from two legs to four, consistent with transport timing having limited leverage over surgical bottleneck queues specifically — **but see the Issue #73 follow-up note above: the same parameter is now the most influential of all ten on two of the screen's other tracked KPIs (transport utilisation, DOW count), a materially different picture that this single system-OT-queue-focused table does not show.** The reshuffling elsewhere in this table relative to the Issue #75 version (e.g. `pri1_surg_prob` falling from 1st to 8th, `r1_transport` rising from 5th to 1st) reflects the same RNG-stream-shift effect documented throughout this project for any change that adds or removes random draws (Issues #5, #6, #24, and now #73's follow-up) — each design point's µ\*/σ is computed under a different realised stochastic environment, so some reordering among closely-clustered parameters is expected noise, not necessarily a change in true influence; the transport-utilisation and DOW-count findings are treated as a real signal rather than noise because the effect size is large and mechanistically direct (doubling the number of legs a multiplier scales doubles its reach), not a same-order-of-magnitude reshuffle.
...
The Medevac Chain diagram (Issue #14) now draws all four legs — R1→R2B WIA (PMVAmb), R1→mortuary KIA (HX2 40M), R2B→R2E WIA (organic evac resource), and R2B→R2E KIA/mortuary (HX2 40M) — each with a working return-leg multiplier label, and the mortuary marker now renders only at R2E.
