...
- **Avoid duplicating content** already documented elsewhere in the same document, or, per the cross-reference rule above, in one of the other two documents; cross-reference the existing location instead of restating it.
...
> **Provenance caveat (Issue #152 refresh):** every seed-42 single-run row below marked "post-Issue-152" was regenerated in an **unpinned R 4.3.3 sandbox** (no Docker access in this session's development environment to build the project's pinned `rocker/rstudio:4.4.2` Dev Container), carrying the same accuracy caveat as the Issue #18, #23 and #161 refreshes — a maintainer re-run in the pinned container is needed before these figures are fully authoritative. Issue #152 replaced the DOW calibration target, which could not be traced to either source cited for it, with the treated-cohort rate reference [14] does report (three deaths among more than 650 casualties reaching the Ajax Bay medical chain, so at most 0.46%), and re-derived the base configuration's ceilings against it: `p1_p_max` 0.023 → 0.0123 and `p2_p_max` 0.019 → 0.0100. The `moderate_intensity` profile's ceilings were re-derived against the same target: `p1_p_max` 0.0089 → 0.0069 and `p2_p_max` 0.0074 → 0.0057. An initial 50-replication check had suggested that profile already met the target (0.458%), but a 100-replication run at an independent seed gave 0.664% (95% CI [0.494%, 0.833%]), which does not span it; the 50-replication result was a low draw. At the re-derived ceilings the same 100-replication configuration gives 0.437% (95% CI [0.329%, 0.544%]). The lesson is recorded in README L22: at these rates a validation run of fewer than about 100 replications cannot confirm or refute a ceiling on its own. Changing the base ceilings is RNG-stream-shifting by the same single-global-RNG-stream mechanism documented for Issues #43/#73/#76/#18/#23/#161: the number of `runif()` draws is unchanged, but each draw's *outcome* at a DOW checkpoint decides whether a casualty continues through the trajectory, so downstream draw timing moves for every entity, and Issue #18's force-regeneration feedback loop propagates that back into casualty generation (total casualties 387 → 385). The Morris screening bounds for the two ceilings were re-anchored to the new calibrated values in the same change; the published screening ranking predates those bounds and is flagged as such in README L18 rather than re-run (a full re-run is roughly two hours and would regenerate every ranking table and plot, beyond this issue's scope). The `R2B OT utilisation — shift time` row and the 50-replication multi-run rows carried forward from earlier refreshes were **not** recomputed and remain flagged pending. `data/arrivals_*.txt`, `logs/logs.txt` and `images/*.png` were **not** regenerated or committed, consistent with the unpinned-sandbox precedent set by the Issue #18, #23 and #161 refreshes.

| Metric | Baseline value |
|---|---|
| Total casualties (30 days) | 385 (post-Issue-152; was 387 post-Issue-161, 386 post-Issue-23 and post-Issue-18, 400 pre-Issue-18) |
| WIA (combat + support) | 148 (post-Issue-152; was 150 post-Issue-161, 149 post-Issue-23, 154 pre-Issue-18) |
| KIA (combat + support) | 67 (post-Issue-152, unchanged from post-Issue-161 and post-Issue-23; was 70 pre-Issue-18) |
| DNBI (combat + support) | 170 (post-Issue-152, unchanged from post-Issue-161 and post-Issue-23; was 176 pre-Issue-18) |
| Effective force size, end of run (seed 42, post-Issue-152) | combat: 2328 of 2500 initial (−6.9%); support: 1179 of 1250 initial (−5.7%); no reinforcement (`force_regeneration.reinforcement.demand_interval_days = 0`, the shipped default). Was combat 2330 (−6.8%), support 1176 (−5.9%) post-Issue-18; the intervening Issue #23 and #161 shifts were never measured on this row |
...
| Priority 1 share (seed 42, post-Issue-152) | 205 of 385 classified (53.2%) — P1 205 / P2 67 / P3 46 / KIA 67; was P1 202 / P2 64 / P3 53 / KIA 67 of 386 classified post-Issue-161. Every generated casualty carries an assigned priority in this run. Not directly comparable to the pre-Issue-73-follow-up "~53.3% of 400" figure below, which predates multiple RNG-stream shifts |
...
| DOW count (seed 42, post-Issue-152) | 0 total (`outputs/dow_by_echelon.csv` empty); was 2 (r2b=2) post-Issue-161. Expected at the re-derived, roughly halved ceilings: the 50-replication mean at these settings is 0.72 DOW/run, so a single 30-day run draws zero deaths a substantial fraction of the time. A zero here is not evidence the DOW mechanism is inactive, and no single-run DOW count should be read as evidence about the mechanism's magnitude — see the 50-replication row below |
| DOW rate — P1 p_max (logistic) | 1.23% ceiling (post-Issue-152; was 2.3%) — re-derived against the corrected Falklands 1982 treated-cohort target (at most 0.46%) |
| DOW rate — P2 p_max (logistic) | 1.00% ceiling (post-Issue-152; was 1.9%) — re-derived against the same corrected target |
| DOW rate — P3 flat | 0.1% (structural placeholder; P3 never evacuated) |
| Mean DOW/run — base configuration (post-Issue-152) | Two independent validation runs, both spanning the corrected 0.46% target: 50-rep seed=42 gives DOW/run 0.720, DOW/WIA 0.484%, 95% CI [0.340%, 0.629%]; 100-rep seed=7 gives DOW/run 0.60, DOW/WIA 0.403%, 95% CI [0.296%, 0.510%]. Pooled over the 150 replications: 0.430%. Against the wider denominator of every casualty exposed to a DOW check (WIA plus the NBI sub-type of DNBI), the 100-rep run gives 0.339%, 95% CI [0.248%, 0.429%]. At the pre-Issue-152 ceilings the 50-rep run gave DOW/WIA 0.955%, 95% CI [0.733%, 1.177%], which is what motivated the re-derivation |
| Mean DOW/run — `moderate_intensity` (100-rep, seed=7, post-Issue-152, new row) | 0.65, 95% CI [0.49, 0.81]; DOW/WIA 0.437%, 95% CI [0.329%, 0.544%] — spans the 0.46% target at the re-derived ceilings (`p1_p_max` 0.0069, `p2_p_max` 0.0057). At the previous ceilings (0.0089 / 0.0074) the same configuration gave 0.664%, 95% CI [0.494%, 0.833%], which does not span it |
| DNBI sub-types (seed 42, post-Issue-152) | battle_fatigue=38, disease=95, nbi=37 (post-Issue-152; was battle_fatigue=40, disease=94, nbi=36 post-Issue-161) — RNG-stream-shifted despite DNBI sub-type assignment itself being unrelated code, per the single-global-RNG-stream mechanism (see provenance caveat above) |
| bf_rtd (seed 42, post-Issue-152) | 32 (post-Issue-152; was 34 post-Issue-161) |
| clinical_rtd (seed 42, post-Issue-152) | 110 (r1: 53, r2b: 47, r2e: 10) (post-Issue-152; was 114 — r1: 60, r2b: 44, r2e: 10 — post-Issue-161) |
| total_rtd (seed 42, post-Issue-152) | 142 (post-Issue-152; was 148 post-Issue-161) |
| R2B surgical candidates (seed 42, post-Issue-76) | 141 (was 132 pre-Issue-76); not recomputed post-Issue-18, post-Issue-23, post-Issue-161 or post-Issue-152 |
| R2B surgeries (seed 42, post-Issue-152) | 52 (post-Issue-152, unchanged from post-Issue-161; was 49 post-Issue-23) |
| R2E surgeries — first op (seed 42, post-Issue-152) | 102 (post-Issue-152; was 103 post-Issue-161) |
| R2E surgeries — second op (seed 42, post-Issue-152) | 55 (post-Issue-152, unchanged from post-Issue-161) |
| R2B pre-transport bypass (seed 42, post-Issue-152) | upstream R1-threshold bypass: 116 (was 109 post-Issue-161); at-R2B hold-full bypass: 0 (unchanged); R2B hold queue (both full): 0 (unchanged) — these are three independent, non-summing counts |
| R2B OT bypass reason (seed 42, post-Issue-152) | at-R2B subset, 74 total (was 73 post-Issue-161): team off-shift 68 (was 67), OT busy/queued 6 (unchanged) |
| OT utilisation — echelon aggregate (seed 42, post-Issue-152, coarser grain than the per-room rows below) | R2B: 5.2%, R2E: 13.6% (was R2B: 4.2%, R2E: 13.6% post-Issue-161) — the per-room breakdown in the rows immediately below was independently re-derived in this refresh from `outputs/mon_resources.csv` |
| R2B OT utilisation — 24h room (seed 42, post-Issue-152) | T1: 6.6%, T2: 7.2% (was T1: 7.8%, T2: 5.9% post-Issue-161) |
...
| R2E OT utilisation — 24h room (seed 42, post-Issue-152) | OT1: 34.4%, OT2: 14.3% (was OT1: 35.7%, OT2: 15.0% post-Issue-161) |
| R2E OT queue ≥1 (seed 42, post-Issue-152) | OT1: 3.06% of run, OT2: 0.08% of run (was OT1: 3.10%, OT2: 0.08% post-Issue-161) |
| R2E surgical section utilisation (seed 42, post-Issue-152) | Section 1: 11.2%, Section 2: 19.4%, Section 3: 11.2% **of the full 24-hour run window**; queued ≥1 for 3.31%, 3.63% and 0.00% of the same window. Note the denominator differs from the post-Issue-161 row this replaces, which reported 19.9% / 35.4% / 19.1% of *rostered* time — the two are not directly comparable. Section 2 remains the highest because it is the section rostered to the second shift and so carries the whole night-time surgical load alone |
| R2E ICU utilisation — mean (seed 42, post-Issue-152) | ICU1: 97.5%, ICU2: 97.2%, ICU3: 95.4%, ICU4: 95.0% (was ICU1: 100.0%, ICU2: 100.0%, ICU3: 98.8%, ICU4: 99.8% post-Issue-161). Still effectively saturated: the critical-route AME wait holds an ICU bed for a mean of roughly twelve days, competing directly with post-operative recovery for the same four beds (README Limitation L17) |
| R2E ICU queue ≥1 (seed 42, post-Issue-152) | ICU1: 93.5% of run, ICU2: 92.4%, ICU3: 91.1%, ICU4: 90.9% (was ICU1: 95.5%, ICU2: 95.0%, ICU3: 94.3%, ICU4: 95.5% post-Issue-161) — the Issue #23 AME-wait effect, essentially unchanged by this refresh |
| Transport utilisation — platform aggregate (seed 42, post-Issue-152, coarser grain than the per-vehicle rows below) | HX240M: 4.6%, PMVAmb: 10.4% (was HX240M: 4.8%, PMVAmb: 10.0% post-Issue-161) |
| PMV Ambulance utilisation (seed 42, post-Issue-152) | 10.4%; max queue 0 throughout run; per-vehicle 24.2% / 6.4% / 0.7% (was 23.8% / 5.8% / 1.4% post-Issue-161) |
| HX240M utilisation (seed 42, post-Issue-152) | 4.6%; max queue 0 throughout run; per-vehicle 9.0% / 0.3% (was 9.7% / 2.3% post-Issue-161) — carries R2B→R2E mortuary road-move traffic in addition to R1→mortuary KIA |
...
| R2E post-op pathway (seed 42, post-Issue-152) | icu=9, hold=93 (post-Issue-152; was icu=9, hold=94 post-Issue-161) — the critical-pool AME wait holds an R2E ICU bed for a mean of roughly twelve days (7-day sortie interval baseline), competing directly with post-operative ICU recovery for the same finite bed pool; see README Limitation L17. `surgery_deferred` = 27 (was 24 post-Issue-161) |
| R2E post-op DOW rate — icu vs hold (seed 42, post-Issue-152) | 0/9 vs 0/93 (single-run; was 0/9 vs 0/94 post-Issue-161; saturated-ICU 90-day stress test from Issue #43 confirms mechanism fires, hold pathway > icu pathway — stress test predates this refresh and has not been re-run) |
...
| Role 4 demand (seed 42, post-Issue-152) | 123 strategic evacuation decisions (84 critical-route, 39 standard-route); 40 boarded and reached Role 4 by day 30 (8 critical, 32 standard), 83 still queued at R2E (76 critical, 7 standard); Role 4 peak occupancy 19.0 concurrent patients (day 29); unconstrained-baseline demand would need 29 sorties at same-day/uncapped/best-case (20/sortie) capacity. Was 116 decisions (81/35), 76 queued (73/3), peak 20.0 (day 22), 29 sorties post-Issue-161 |
| Strategic AME actual performance (seed 42, post-Issue-152) | Configuration A (2 critical/8 standard) selected at all 4 of 4 successful sorties (7-day interval, 15% failure probability, 0 cancellations drawn); critical-pool mean wait 12.0 days (p10–p90 4.3–18.9, unchanged from post-Issue-161); standard-pool mean wait 3.8 days (p10–p90 1.0–6.0; was 4.0 days post-Issue-161); see [Strategic Evacuation and Role 4 Demand](docs/Single_Run_Analysis.md#strategic-evacuation-and-role-4-demand) |
| AME wait-time DOW poll (seed 42, post-Issue-152) | `dow_echelon=5`, daily poll interval (`role4.ame.dow_check_interval = 1440` min); 0 deaths observed in this seed-42 run, unchanged from post-Issue-161; see README [AME Wait Checkpoint](README.md#ame-wait-checkpoint) for why neither single-run count should be read as evidence about the mechanism's magnitude |
