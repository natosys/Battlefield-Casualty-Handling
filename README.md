...
```bash
# Full Morris screening: r=20 trajectories × (65 + 1) = 1,320 design points, 5 reps each
...
**Current ranking.** The table below is `outputs/morris_ranking.csv` for the current sixty-five-parameter set, run at r = 20 trajectories with 5 replications over 30 days at seed 42: 1,320 design points, about ten hours of computation. Every screened parameter carries a measured rank, the six composition balance coordinates included, interleaved by µ\* rather than reported separately. The screen was made in an unpinned R 4.3.3 environment rather than the pinned Dev Container, and the host reclaimed its filesystem several times during the run, so the 1,320 points were evaluated across several attempts resuming from the design point cache; `scripts/check_screen_order.R` asserts that a resumed screen re-evaluates nothing and evaluates its design in index order, which is the property that makes the assembled result identical to one produced in a single pass.

`mass_casualty_rate` leads by a wide margin at µ\* = 17.87, more than double the next parameter and the only one separated from the field at this trajectory count. `mass_casualty_max_cas` follows at 7.47 and `pri1_surg_prob` at 7.28, and those two are not distinguishable from each other: their µ\* values differ by less than a fifth of a standard error.

The comparison the balance coordinates were added to make resolves in the same direction as before, though less sharply. `triage_p1_balance`, which governs the Priority 1 share of the casualty mix, ranks **thirteenth of sixty-five** at µ\* = 3.79, below `pri1_surg_prob` at rank 3 and `pri1_evac_prob` at rank 7, the two Priority 1 conditional rates defined on that share. Neither of those orderings is established at r = 20: separating the coordinate from `pri1_surg_prob` would need 22 trajectories and from `pri1_evac_prob` some 270, so what the screen supports is that the three sit in one band, not that the share matters less than the conditional rates. The other coordinates sit nearby, `mc_p2_p3_balance` at 16, `dnbi_bf_nbi_balance` at 21, `triage_p2_p3_balance` at 24, `mc_p1_balance` at 33 and `dnbi_disease_balance` at 37, so the three composition groups are mid-table influences rather than dominant ones. No coordinate reaches the top ten, which is the condition under which a group would be carried into a confirmatory decomposition and treated as a single object; the whole-composition Dirichlet sampling `run_sobol()` provides is therefore available and unexercised, none of the three groups having earned it on this ranking.

An earlier screen at r = 5 put `triage_p1_balance` first at µ\* = 8.63, and that result did not survive the quadrupled trajectory count. It carried the largest σ in that table, 10.69, giving a standard error of ±4.78 on a µ\* of 8.63: its rank was never separable from noise. This is the clearest illustration in the project of why the trajectory count matters, and why a rank should not be read as an ordering when the gaps between neighbours are smaller than their standard errors. Separation requirements quoted in this section are the trajectory count at which two parameters' µ\* intervals would stop overlapping at 95%, computed from the elementary effects themselves: µ\* is a mean of absolute effects, so its standard error is the standard deviation of those absolute effects divided by $\sqrt{r}$. On that basis the leader is separated from the field at r = 20, needing only 8 trajectories, while ranks 2 and 3 would need some 13,500 and ranks 3 and 4 some 308. The leading cluster below the top is a group rather than a sequence.

Three cautions on reading the table. **It is screened at a 30-day horizon**, so it ranks parameters by their influence on a month of campaign, and a parameter whose effect accumulates over a longer operation is ranked below what a sustained campaign would give it; the strategic airlift cancellation probability is the known instance, its influence on the holding pool building over many months rather than weeks. It is also a ranking on one response, the system OT queue; a screening run writes a ranking for each of the thirty-six responses in the set, and no time-to-care, return-to-duty or strategic evacuation response is ranked here. And `r2b_ot_q` carried no usable variation across the design and is flagged degenerate in its own ranking file, µ\* and σ written as NA rather than zero, which is the R2B theatre queue reading zero at every design point rather than a failure of the screen. The two parameters at µ\* exactly zero, `p3_flat` and `r2b_icu_penalty`, together with the small group just above them, are mortality parameters and treatment efficacy multipliers that act on who dies rather than on how long a queue grows, so a queue-ranked screen is the wrong instrument for them and their own responses in the wider set are where they should be read.

| Rank | Parameter | µ\* | σ | Rank | Parameter | µ\* | σ |
| ---- | --------- | ---- | -- | ---- | --------- | ---- | -- |
| 1 | `mass_casualty_rate` | 17.8713 | 12.3260 | 34 | `r2e_hold_mode` | 2.3309 | 4.1297 |
| 2 | `mass_casualty_max_cas` | 7.4662 | 7.9728 | 35 | `post_definitive_icu_mode` | 2.1998 | 2.9859 |
| 3 | `pri1_surg_prob` | 7.2803 | 7.9769 | 36 | `pri3_other_surg_prob` | 2.1667 | 3.2615 |
| 4 | `pri1_dcs_rate` | 6.1999 | 6.3489 | 37 | `dnbi_disease_balance` | 2.1563 | 3.3711 |
| 5 | `mass_casualty_min_cas` | 5.2243 | 6.4441 | 38 | `disease_surgery_pct` | 2.0528 | 2.8501 |
| 6 | `evacuation_policy_days` | 4.4533 | 5.2958 | 39 | `pri2_surg_prob` | 1.9594 | 2.9469 |
| 7 | `pri1_evac_prob` | 4.4518 | 5.1514 | 40 | `r2b_icu_share` | 1.9308 | 3.3620 |
| 8 | `pri2_evac_prob` | 4.2457 | 5.8013 | 41 | `dnbi_spt_mean` | 1.8168 | 2.5435 |
| 9 | `post_op_hold_mode` | 4.1219 | 5.4569 | 42 | `r2b_forward_hold_max` | 1.7511 | 3.3532 |
| 10 | `ot_hours` | 3.9745 | 4.5412 | 43 | `r2b_hold_threshold` | 1.7359 | 2.4644 |
| 11 | `wia_spt_mean` | 3.9723 | 6.7703 | 44 | `ame_schedule_interval_days` | 1.7310 | 2.3391 |
| 12 | `kia_spt_mean` | 3.8562 | 6.1609 | 45 | `r1_tccc_factor` | 1.7230 | 3.2033 |
| 13 | `triage_p1_balance` | 3.7857 | 4.0117 | 46 | `p1_p_max` | 1.7191 | 2.6261 |
| 14 | `r1_recovery_mode` | 3.5381 | 5.4130 | 47 | `pri3_dnbi_surg_prob` | 1.1559 | 1.8009 |
| 15 | `kia_cbt_mean` | 3.2210 | 4.9010 | 48 | `p2_p_max` | 0.9749 | 2.3463 |
| 16 | `mc_p2_p3_balance` | 3.1803 | 5.9146 | 49 | `p1_k` | 0.8767 | 2.1378 |
| 17 | `surg_mode` | 3.1118 | 4.0519 | 50 | `ame_failure_probability` | 0.8163 | 1.3106 |
| 18 | `wia_cbt_mean` | 3.0851 | 3.3117 | 51 | `fr_fulfillment_lag_days` | 0.7741 | 1.1977 |
| 19 | `long_resus_mode` | 3.0220 | 5.4540 | 52 | `pri3_dcs_rate` | 0.6752 | 1.4844 |
| 20 | `stabilisation_icu_mode` | 3.0121 | 4.0290 | 53 | `p2_k` | 0.6563 | 2.8179 |
| 21 | `dnbi_bf_nbi_balance` | 2.9741 | 4.6342 | 54 | `p1_p_base` | 0.4911 | 0.8160 |
| 22 | `short_resus_mode` | 2.9098 | 4.2620 | 55 | `p2_t_mid` | 0.4304 | 1.0018 |
| 23 | `dnbi_cbt_mean` | 2.8519 | 4.5371 | 56 | `fr_fill_mode_frac` | 0.2858 | 0.6160 |
| 24 | `triage_p2_p3_balance` | 2.8401 | 3.9190 | 57 | `r2e_resus_factor` | 0.1981 | 0.6932 |
| 25 | `fr_demand_interval_days` | 2.8255 | 3.8925 | 58 | `r2b_dcs_factor` | 0.1635 | 0.5474 |
| 26 | `pri2_dcs_rate` | 2.8167 | 3.4532 | 59 | `r2b_resus_factor` | 0.1579 | 0.5103 |
| 27 | `r2b_transport` | 2.7432 | 4.0024 | 60 | `r2e_postop_hold_penalty` | 0.1560 | 0.6976 |
| 28 | `r2b_pre_open_window` | 2.7294 | 4.0447 | 61 | `r2e_dcs1_factor` | 0.1511 | 0.6755 |
| 29 | `r1_wia_treat_mode` | 2.6937 | 4.9471 | 62 | `p2_p_base` | 0.0396 | 0.1738 |
| 30 | `r2b_hold_mode` | 2.6527 | 5.1142 | 63 | `r2e_dcs2_factor` | 0.0150 | 0.0502 |
| 31 | `r1_transport` | 2.4355 | 3.8762 | 64 | `p3_flat` | 0.0000 | 0.0000 |
| 32 | `p1_t_mid` | 2.3995 | 3.6683 | 65 | `r2b_icu_penalty` | 0.0000 | 0.0000 |
| 33 | `mc_p1_balance` | 2.3525 | 5.5129 |  |  |  |  |

**What the ranking tells you.** The casualty load dominates. Mass Casualty Event Rate (`mass_casualty_rate`) leads at µ\* 17.87, followed by Mass Casualty Event Size Maximum (`mass_casualty_max_cas`) at 7.47, Priority 1 Surgical Candidacy (`pri1_surg_prob`) at 7.28 and Priority 1 Damage Control Rate (`pri1_dcs_rate`) at 6.20, against a median of 2.35 across the set. Before relying on a run, check that the mass casualty configuration matches the scenario being modelled: an error there shifts surgical queue depth more than an error anywhere else.

Seven of the top ten are Scenario / Casualty Context parameters, so results depend most on assumptions about the casualty load rather than on decisions the health system can make. Two are Policy, the Theatre Evacuation Policy (`evacuation_policy_days`) at rank 6 and OT Shift Length (`ot_hours`) at rank 10, and one is Capacity, the R2E Post-Operative Holding Duration (`post_op_hold_mode`) at rank 9. A parameter's category tells you what a high rank means in practice and matters more than its exact position; see "Reading the plots: colour coding" below.

**Effects are not independent.** σ exceeds µ\* for 62 of the 65 parameters, meaning most parameters' influence depends on where the others are set. A parameter that looks inert at the baseline may matter once other values move, so conclusions drawn by changing one input at a time will understate what the model can do. Vary related parameters together, or compare whole scenario profiles, rather than tuning singly.

**Two results to read carefully.** The Reinforcement Demand Cycle ranks 25th, mid-table, and that ranking now surrounds the shipped 7-day cycle rather than the disabled baseline an earlier screen was taken from; a parameter's elementary effects are local to the baseline they are screened from, so this is the figure to read for the configuration as it ships. Separately, ranks toward the bottom sit very close together: the ten lowest span 0.29 down to zero, and the two lowest are exactly zero. Treat the lower half of the table as a band rather than an order.

**Influence varies widely inside a single mechanism.** Reinforcement is the clearest case: the demand cycle ranks 25th while Reinforcement Fulfillment Lag (`fr_fulfillment_lag_days`) ranks 51st and Reinforcement Fill Distribution (`fr_fill_mode_frac`) 56th of 65, so cycle length is the value to get right when configuring it. Mass casualty sizing runs the other way, every part of it ranking high: Event Rate (`mass_casualty_rate`) 1st, Event Size Maximum (`mass_casualty_max_cas`) 2nd and Event Size Minimum (`mass_casualty_min_cas`) 5th, so the whole mechanism needs to match the scenario rather than one value within it.
...
The mass casualty event rate sits alone at the right of the plot at more than twice the importance of the next parameter, and the five highest-importance points are orange without exception, all five being properties of the casualty load. The leading policy point, the theatre evacuation policy, and the leading capacity points, the resuscitation and treatment durations, sit at roughly a quarter of the leader. The cloud fans upward and to the right, so a parameter that matters more also interacts more.
...
No scatter is drawn because the response is degenerate: the forward theatre queue stayed at zero at all 1,320 design points, so no parameter produced an elementary effect and none can be ranked. That is a result about the forward theatre rather than a gap in the screen, and it is why the system queue panel above and the R2E panel below are the same picture.
...
The mass casualty event rate leads on importance again, at roughly twice the next parameter, but the intensive care queue is the one response in this set where policy parameters carry the strongest interactions: the theatre evacuation policy and the R2B hold-bed reroute threshold hold the two highest positions on the vertical axis at middling importance. Both govern how long a casualty occupies a bed rather than how many casualties arrive, which is the quantity a four-bed intensive care section is most exposed to.
...
The mass casualty event rate stands well clear of every other parameter on importance, and the Priority 1 mortality ceiling is the only other point to separate itself from the cloud, so how often the model is asked to absorb a surge matters more for mortality than any capacity or policy parameter in the screen. The remaining sixty-three cluster below an importance of 2.2, most of them with interaction terms larger than their own importance.
...
The whole spread of importance sits below 0.28 of a vehicle, which is consistent with the fleet queueing rarely at the shipped establishment. The mass casualty event rate and the event size maximum take the two leading positions, and the only non-context parameter near them is the long resuscitation duration, which carries the plot's largest interaction term at half the leader's importance. Nothing in the plot is a vehicle count, fleet size being swept directly rather than screened.
...
Utilisation is the flattest response in the set and the only one whose leading parameter is not the mass casualty event rate. The whole spread of importance falls below 0.05, the top seven parameters lie within 12% of each other, and the cloud sits close to a single diagonal, so no point stands clear of the rest. Vehicles spend most of the run idle whatever the parameters do, so this response separates parameters weakly and is best read alongside the queue panel above rather than on its own.
...
**L18 — Screening resolution and response coverage.** The Morris screen covers sixty-five parameters at twenty trajectories over a 30-day horizon, which resolves the influential group from the negligible tail but does not order parameters within the leading cluster: the standard error of a µ\* is the standard deviation of its absolute elementary effects divided by $\sqrt{r}$, so neighbours separated by less than that are not distinguishable. Only the leading parameter is separated from the field at this trajectory count; separating the second-ranked from the third would need some 13,500 trajectories, which is not a practical target, so the parameters below the leader should be reported as a group. The published ranking is also on one response, the system OT queue. A screening run writes a ranking file for each of the thirty-six responses at no extra simulation cost, so those files exist, and the strategic airlift finding shows the cost of leaving them uninterpreted: `ame_failure_probability` ranks 50th of 65 on the queue while ranking 6th on mean Role 4 occupancy and 12th on the standard-route evacuation backlog. Whether a parameter ranks differently on a time-to-care, return-to-duty or strategic evacuation response is otherwise unresolved.
