...
> The multi-replication work is the died-of-wounds calibration at 150 replications per shipped configuration, which passes for both, and the 50-replication comparative scenario tables in `docs/Multi_Run_Analysis.md`, both re-measured. Every ratio in the comparative tables compresses, because `moderate_intensity` draws all six streams from lognormals whose WIA and KIA arms had been realising about 79% of their means, while `high_intensity` overrides its WIA and KIA streams to exponential and those had been realising 95%: the baseline arm rises by a sixth while the surge arm rises by a twentieth. `images/scenario_comparison.png` was regenerated with them.
...
| Mean DOW/run (150 reps pooled, post-Issue-203) | Treated-cohort DOW rate (casualties reaching R2B or R2E) 0.457% (95% CI [0.391%, 0.524%]), pooled across three independent 50-replication measurements; the three returned 0.513%, 0.435% and 0.425% individually. This is the quantity the historical Ajax Bay bound of ~0.46% applies to, and the interval spans it. Since the target is an upper bound the check is one-sided, so spanning is agreement, and `p1_p_max`/`p2_p_max` are unchanged in both configurations. Was 0.436% (95% CI [0.361%, 0.511%]) post-Issue-148; the rise is the larger treated cohort the corrected generation rates produce, which Issue #203 predicted and which does not reach overshoot. Measured by `scripts/check_dow_calibration.R`, which passes for both shipped configurations; `moderate_intensity` measures 0.279% (95% CI [0.227%, 0.331%]), was 0.271%. DOW/WIA rate was not re-pooled at 150 replications, the calibration check not reporting it; the 50-replication comparative measurement puts `moderate_intensity` at 0.44% (95% CI [0.32%, 0.56%]). See README Further Development L22 |
| Replication count for mortality figures (post-Issue-203) | Per-replication sd of the treated-cohort DOW rate is 0.0041 (base, 150 reps; was 0.0046 post-Issue-148). A 95% half-width of 0.15 pp needs 29 replications, 0.10 pp needs 65, 0.05 pp needs 258. The 50-replication figures elsewhere in this table carry roughly ±0.12 pp on this response; the queue and occupancy rows are time-weighted over far more events per replication and are better resolved at the same count |
