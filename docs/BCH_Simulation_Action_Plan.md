...
### Phase 1 — Statistical Foundation (Issues 1 ✓, 22 ✓, 2 ✓, 3 ✓, 24 ✓, 75 ✓, 157 ✓)
...
UNBLOCKED (start now):
  #195 surg_mode screened over 90-150 against a shipped 95-minute mode, so
       the range sits almost entirely above the baseline it should surround.
       Bounds were set against the old 120-minute mode and not re-derived
       when Issue #76 re-sourced the duration. Should land before the
       canonical re-run, since changing a bound changes the design
