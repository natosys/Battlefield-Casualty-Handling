...
Seven response variables are computed at each design point. Five are time-weighted mean queue depths: R2B OT, R2E OT, their sum as the system OT queue, R2E ICU, and transport pooled across the PMV Ambulance and HX240M fleets. The sixth is mean transport utilisation over the same fleets. These six are averaged across the design point's replications; the seventh, total DOW count, is summed across them instead, so its µ\* values sit on a different scale and are not comparable with the other six. Transport utilisation is needed alongside the transport queue because, at the baseline casualty rate, transport assets rarely queue: availability tightens well before a queue forms, so a queue-only response would under-detect transport parameters. The system OT queue is the ranking response, from which `run_morris()` builds `outputs/morris_ranking.csv` and on which the table below is sorted; the Shiny Sensitivity panel instead displays the R2E ICU queue scatter ([Shiny Application](#shiny-application)). Ranking by µ\* on the system OT queue identifies the inputs most responsible for surgical bottleneck severity.
