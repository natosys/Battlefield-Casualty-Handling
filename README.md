...
**Fixed establishment/capacity counts.** Population sizes, team/bed/vehicle counts (`pop_combat`, `r1_team_count`, `r2b_bed_ot`, `transport_PMVAmb_qty`, etc.) and the two named AME aircraft configurations' fixed critical/standard capacity pairs (`ame_config_a`/`ame_config_b`, four fields) represent discrete establishment or hardware decisions a planner sets explicitly, not continuous parameters carrying epistemic uncertainty about a true underlying value. Varying them under Morris would answer "what if the establishment were different" rather than "how uncertain is the estimate of this quantity", a legitimate but different question, already addressed by this project's scenario-comparison and capacity-margin-sweep tooling ([Comparative Scenario Runner](#comparative-scenario-runner), [Transport Fleet Capacity Margin](docs/Single_Run_Analysis.md#transport-fleet-capacity-margin)).
...
**Polling-loop intervals.** `icu_defer_check_interval` (R2B/R2E OT-entry defer poll, a shared field) and `ame_dow_check_interval` (strategic-AME-wait DOW poll) are not screened. Both drive a `timeout()`+`rollback()` polling loop in `R/trajectories.R` that approximates continuous real-time monitoring (ICU-bed availability while OT entry is deferred; died-of-wounds risk while queued for strategic evacuation) at a fixed simulation-clock interval, a numerical discretisation choice, not a real-world decision a health system commander makes. This is a different kind of parameter from a genuine scheduling policy such as `ame_schedule_interval_days` (a real sortie-cadence decision, still screened): in reality, clinical staff and evacuation coordinators monitor these conditions continuously, not on a fixed poll interval, so there is no standing order a planner could actually issue to change either value. Screening them would answer "how sensitive are the tracked KPIs to this simulation's time-discretisation granularity", a legitimate numerical-convergence question, but a different one from the epistemic-uncertainty question this screen is designed to answer for the fifty-three parameters it covers, and risked misleading a planner into treating a model-resolution artefact as an actionable lever.

#### Parameter Name Reference

The grouped tables above and the ranking table below use each parameter's raw `morris_params$name` (the identifier that also appears in `outputs/morris_ranking.csv`, in `R/sensitivity.R`'s `apply_params()`, and on the axis labels of every saved `images/morris_*.png` plot). The table below maps all fifty-three variable names to their plain-English title and category, sorted alphabetically by variable for quick lookup when cross-referencing a code seen in a CSV or plot back to what it means. The two columns have separate sources in code: the title comes from `MORRIS_LABELS` (`app.R`), from which the Shiny app's Sensitivity Calibration and ranking tables are both rendered, so those two stay in sync by construction; the category comes from `morris_params$category` (`R/sensitivity.R`), which also drives the colour coding of every saved plot. The table below reproduces both rather than deriving from either, so it must be updated whenever a parameter is added, removed, retitled, or recategorised.
...
Seven response variables are monitored across all replications at each design point. Four are queue depths: mean R2B OT queue, mean R2E OT queue, their sum as the system OT queue, and mean R2E ICU queue. Total DOW count is tracked as the mortality response. Two cover the transport layer: mean transport queue and mean transport utilisation, pooled across the PMV Ambulance and HX240M resources. The utilisation KPI is necessary alongside the queue KPI because, under the current baseline casualty rate, transport assets rarely queue, since their availability is affected well before a queue forms, so a queue-only KPI would under-detect the influence of transport-related parameters. The system OT queue is the ranking response, from which `run_morris()` builds `outputs/morris_ranking.csv` and on which the table below is sorted; the Shiny Sensitivity panel instead displays the R2E ICU queue scatter ([Shiny Application](#shiny-application)). Parameters ranked by µ\* on the system OT queue identify the inputs most responsible for surgical bottleneck severity.

Screening bounds cover clinically plausible variation around the current baseline values, using one of two rules per parameter. **Rule A** applies to parameters whose baseline traces to a specific open-access source already cited in this document, and spans approximately baseline ±40%; examples are the DOW logistic shape parameters [[29]](#References), [[30]](#References), the treatment efficacy factors [[31]](#References), [[32]](#References), and the casualty generation rates [[8]](#References). **Rule B** applies to parameters whose entry in `R/app_params.R` discloses an informed estimate with no literature anchor, and spans baseline ×0.5 to ×2.0 for durations and rates, or approximately baseline ±0.15 to 0.25 for probabilities, clipped to a clinically sensible range. These bounds are informed clinical and logistics judgement rather than empirically derived uncertainty intervals, so uncertainty on them is medium overall and high for Rule B parameters individually. Narrow bounds understate the influence of a parameter whose true range is wider, and wide bounds conflate plausible with implausible regions; parameter ranking is sensitive to bound specification in a nonlinear model, though wider bounds would raise µ\* values without changing the ranking if the model is monotone. Two bounds must additionally respect a distribution's own fixed envelope: a screened triangular mode has to remain within that distribution's unscreened minimum and maximum, since `rtriangle()` requires $a \leq c \leq b$ and returns `NA` outside it. `fr_fill_mode_frac` and `post_op_hold_mode` are bounded accordingly, and this constraint is documented inline in `R/sensitivity.R`.
The screen is currently run at r = 5 Morris trajectories rather than the `--r` default of 20, giving 5 × 54 = 270 design points at 5 replications each. At r = 20 the fifty-three-parameter design would require 1,080 design points, roughly a fourfold increase in compute cost. Reducing r increases sampling noise on each µ\*/σ estimate without biasing it, since the Morris method [[46]](#References) is unbiased at any r ≥ 1 and only its variance improves with more trajectories. The ranking below should therefore be read as indicative of relative influence across the full parameter surface, with a wider re-run at r = 20 remaining a worthwhile follow-up (see Further Development).
...
A change in the screened factor count is not a small perturbation of the previous ranking. `morris()` regenerates its pseudorandom one-at-a-time trajectories as a function of the factor count, so adding or removing a parameter changes the sampling for every remaining parameter as well, and each ranking should be read on its own terms.

µ\* ranges from 0.1203 (`ot_hours`) to 9.6285 (`pri1_evac_prob`), an 80-fold span, with the top parameter roughly 11× the median (0.8707). 51 of 53 parameters (96%) have σ exceeding µ\* (ratio range 0.72–2.16), indicating nonlinear or interaction-dominated effects remain the norm across this parameter set. The top 10 parameters by µ\* span all three categories: seven Context (`pri1_evac_prob` 1st, `pri1_surg_prob` 2nd, `r2e_dcs2_factor` 3rd, `pri3_other_surg_prob` 6th, `wia_spt_mean` 8th, `pri3_dnbi_surg_prob` 9th, `mass_casualty_rate` 10th), two Capacity (`r2b_hold_mode` 5th, `short_resus_mode` 7th), and one Policy (`fr_demand_interval_days`, 4th), with the reinforcement demand cycle's high ranking notable given it ships disabled by default (`demand_interval_days = 0`) in the baseline configuration; its influence here reflects the screen's own bound (0–14 days) exercising the mechanism away from that default, not the shipped baseline's own behaviour. `mass_casualty_max_cas` and `mass_casualty_rate` both rank in the top 12 (12th and 10th), while their sibling `mass_casualty_min_cas` ranks at the exact median (27th), a smaller but still-present asymmetry within the mass-casualty sizing trio. The reinforcement pipeline's three parameters split widely: `fr_demand_interval_days` (4th) and `fr_fill_mode_frac` (20th) rank well inside the top half, while `fr_fulfillment_lag_days` ranks 52nd of 53, a materially larger spread within one mechanism's own parameters than either mass-casualty trio shows, though not itself evidence for or against the fill/lag-vs-Policy classification question (see "Reading the plots: colour coding" below). `post_surgery_prob`, classified Context rather than Policy, ranks 41st (µ\* = 0.3662), a below-median but not bottom-tier influence on this KPI, a classification that affects only how the parameter is labelled and coloured, not its screened bounds.

The lowest-ranked parameters (`ot_hours` 53rd, `fr_fulfillment_lag_days` 52nd, `p2_t_mid` 51st, `r2e_postop_hold_penalty` 50th, and `post_op_hold_mode` 49th) span all three categories rather than clustering in any one of them. A parameter's rank position in a given Morris design is not a stable, portable fact about that parameter in isolation, only about its relative influence within that specific trajectory sample. This is expected behaviour for a screening method rather than a defect, but it is what makes the wider r = 20 re-run valuable as a follow-up, since more trajectories narrow the sampling variance that drives rank movement of this kind.

**Reading the plots: colour coding.** Each point below is coloured by `morris_params$category`, which splits into three levels rather than two, because a simple Context-vs-Design split would conflate two kinds of design parameter with very different practical implications for a planner deciding whether to act on a highly-ranked result:

- **Orange, "Scenario / Casualty Context"** (36 parameters): a fact about the operational environment or the casualty population itself: casualty generation rates, the DOW logistic curve, clinical-need composition, treatment efficacy, and (see below) inter-echelon transport time. A planner does not choose these; a finding sensitive to one says that the conclusions depend on how bad the scenario turns out to be, not something a health system decision can change.
...
Upon arrival, casualties are triaged; those identified as DOW by the time-dependent conditional increment check (see [Died of Wounds](#died-of-wounds)) are transferred for mortuary handling. By the time casualties reach R2E, elapsed time since injury is typically 60–180 minutes; the P1 logistic function produces substantially higher incremental mortality risk in this window, reflecting the clinical criticality of the R2E reception phase for casualties who have not yet undergone surgery. Surviving casualties queue directly for a resuscitation bay, where they undergo a resuscitation phase. Where previous resuscitation has not been completed (at the R2B) a long duration resuscitation is completed, otherwise a short resuscitation is completed. The R2E long duration resuscitation follows the triangular distribution estimated for R2B resuscitations (`min = 25`, `max = 70`, and `mode = 45` (min)). The short duration resuscitation is modelled based on task estimate durations. These times are outlined in the table below. The duration uses a triangular distribution with ``min = 13``, ``max = 55``, and ``mode = 28``.
...
**Wait-time mortality (Issue #23 third follow-up).** Casualties queued in either pool are periodically re-assessed for DOW risk while waiting, closing what was previously a structural gap: the R2E post-operative check is the model's last fixed checkpoint, so an unbounded AME wait carried zero mortality risk before this follow-up. See [AME Wait Checkpoint](#ame-wait-checkpoint) (Died of Wounds) for the mechanism and the seed-42 finding.
