...
| `scripts/check_holding_occupancy_split.R` | Regression check asserting that R2E holding bed occupancy splits into in-theatre recovery and strategic evacuation wait and that the two account for the pool exactly: the components sum to the measured occupancy at the shipped configuration and under a cancellation rate that forms a backlog, the evacuation component is bounded by the pool and rises when sorties are lost, it counts the standard route and not the critical one (asserted against a run carrying both), it agrees with a casualty-by-casualty recount taken independently of the function under test, and a wait still running when the window closes is charged to the window's end rather than dropped; exits non-zero on failure |
