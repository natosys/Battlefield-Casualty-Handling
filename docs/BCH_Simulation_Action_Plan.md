...
| 348 | The Morris screen cannot rank the resource that saturates, and adding a response to the cache silently produces an all-NA column | Medium | Medium | Open |
...
UNBLOCKED (start now):
  ...
  #348 The Morris screen has no holding bed queue response and never varies
       ...
       saturates; and adding a response to the populated cache would leave
       it permanently NA, every point reading as cached. Sequencing against
       #335 and #339 is the substance: a response added to a re-screen that
       is happening anyway is nearly free. Raised from the #312 audit.
