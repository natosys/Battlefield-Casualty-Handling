...
**L17 — R2E holding beds carry recovery and evacuation on one pool.** A single pool of 30 beds serves two unrelated demands, and the split between them is now measured rather than assumed. A casualty awaiting the standard airlift pool holds a holding bed for the whole of its wait; a casualty awaiting the critical pool holds an intensive care bed instead, so only the first appears here, and the distinction is large: at a 40% cancellation rate the standard-route waits account for 91.1 holding bed-days against the 340.4 the critical-route waits would add if charged to the same pool. Across 50 replications the evacuation wait holds 10% of the pool's occupancy (95% CI [8%, 12%]) at moderate intensity and 29% ([27%, 32%]) at high, so in-theatre recovery accounts for most of it at the shipped configuration but by no means all of it under load. Every lever that delays a sortie raises that share, reaching 39% ([34%, 43%]) at a 14-day sortie interval. What the coupling does is push back into intensive care: a ventilated casualty on the critical route cannot step down from its bounded pre-flight period while the holding pool is full, and that hold runs 24.7 hours at a 3-day interval against 91.3 hours at 14. Any R2E intensive care or theatre-gating capacity finding must therefore be read alongside the holding pool's occupancy rather than in isolation. Separating the staging pool from the clinical holding beds, or expanding the pool, would each reduce the coupling; the defaults were deliberately not tuned to hide it.
