...
| 389 | The mass casualty stress test has no tracked evidence set, and its replication count may not be worth backing as it stands | Low | Medium | Open |
...
UNBLOCKED (start now):
  #389 The mass casualty stress test, the weakest of the three cases: its own
       table labels the finding direction only at 13 deaths per arm, so
       backing it at 10 replications would satisfy the rule and change
       nothing. Derive the replication count that would separate the two
       mortality rates first, then decide. Unblocked by the merge of #384.
