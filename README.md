...
R2E has no surgical or general ward beds, only ICU and holding, so both non-ICU categories share the R2E holding beds while awaiting evacuation.
...
The shipped defaults are a sortie every 7 days, a 15% cancellation probability, and configurations of 2 critical with 8 standard, or 0 critical with 20 standard. These are informed estimates and uncertainty is high: AJP-4.10(B) [[33]](#References) establishes strategic evacuation, casualty staging and critical care augmentation as planning functions without prescribing a cadence, cancellation rate or per-sortie load, all of which are platform, theatre and campaign specific. The 7-day interval matches the cadence the force reinforcement mechanism is built around, on the reasoning that inbound reinforcement and outbound evacuation are likely to share lift on the same resupply cycle, though no source confirms that coupling and the two intervals stay independently configurable. Critical capacity is set at a quarter of standard, reflecting that a critical care team is a scarce augmentation rather than like-for-like capacity, but no source quantifies the ratio. Together the interval and cancellation rate set the scale of both backlogs, and the two configurations decide how much of any backlog sits in the critical pool.
