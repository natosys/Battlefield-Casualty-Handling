...
> **Provenance note (Issue #151 Okinawa died-of-wounds calibration):** Issue #151 gives the `high_intensity` scenario profile its own died-of-wounds ceilings and treatment efficacy factors, where it previously inherited both from the Falklands-calibrated base. It touches no trajectory, no base-configuration parameter and no random draw outside that profile, so **no seed-42 single-run row in the table below changes** and no tracked artifact in `data/`, `logs/` or `images/` was regenerated apart from `images/scenario_comparison.png`, which the comparative scenario runner rewrites alongside the tables re-measured for it. As the validation step this project applies as standard, the seed-42 single run was reproduced first and matched the tracked baseline console log byte for byte, which is both the evidence that this sandbox is faithful and the confirmation that neither `default` nor `moderate_intensity` moves; the 50-replication comparative run reproduced every published `moderate_intensity` figure exactly, including each queue group to the precision published (total casualties 437.8, WIA 188.7, DOW 0.78, DOW/WIA 0.42%; R2B Hold 0.593, R2E OT 1.063, R2E ICU 0.131, R2E Hold 0.622, transport 0.00386).

> The base ceiling was fitted to a Falklands mortality target jointly with OIF/OEF-era efficacy multipliers, so the two are entangled and neither transfers on its own; running an Okinawa casualty stream through that pairing applied a mortality target and a standard of care drawn from two unrelated contexts, neither of them the profile's own. The calibration target is now Okinawa's own reported rate, three point four percent of casualties who reached a hospital alive dying there (Marble, 2025, *Joint Force Quarterly* 117), measured on the same treated cohort the model reports and bracketed by the war-wide figures of 3.5% (Holcomb et al., 2006) and 4.5% (Marble). The treatment efficacy factors are informed estimates for 1945 Pacific-theatre care, disclosed as such per Source Prioritisation level 5, and against them the ceilings re-calibrate to `p1_p_max` = 0.052 and `p2_p_max` = 0.042, up from the base 0.020 and 0.016. The ceilings rise here where `moderate_intensity`'s fell, because this profile weakens the same factors against a target roughly seven times higher. Pooling three independent 50-replication measurements gives a treated-cohort rate of 3.471% (95% CI [3.360%, 3.583%]) against individual measurements of 3.592%, 3.463% and 3.359%, an interval spanning the target; DOW/WIA measures 3.164% (95% CI [3.066%, 3.262%]).

> `scripts/check_dow_calibration.R` now holds a target per configuration rather than one shared bound, and tests `high_intensity` two-sided at a tolerance of two percentage points, a reported rate against a stated denominator being something to reach rather than merely to stay under; a full run is 450 replications rather than 300. The 50-replication comparative tables in `docs/Multi_Run_Analysis.md` were re-measured, the profile's own rows moving as the calibration intends (DOW/run 5.80 to 23.58, DOW/WIA 0.88% to 3.43%) and its queue rows moving a little with them, the theatre and intensive care queues shortening (39.8 to 38.2, 0.618 to 0.564) because a casualty who dies of wounds leaves the queue for the resource that would have treated them, while the queues further from the point of death lengthen alongside a casualty count reading 1,021.0 against 992.3. The README's 30-replication profile comparison table was refreshed at the same time and its `moderate_intensity` column corrected: it had stood at 151.1 WIA and 57.1 KIA per run since before the arrival generators were rebuilt around per-day rate draws, and measures 184.8 and 68.7 now. The two `high_intensity` force-regeneration rows in `docs/Single_Run_Analysis.md` were **not** re-measured and are flagged in place, the calibration steepening the depletion those rows demonstrate rather than working against it. The `Welch ICU-queue CMA` row, the transport and forward ICU share sweeps, the R2B pre-open hold window comparison and the published Morris ranking were **not** re-run and remain flagged as such below and in those documents; the Morris set is unchanged, no parameter being added or removed by this issue. The measurements were made in an **unpinned R 4.3.3 sandbox** (no Docker access in this session to build `rocker/rstudio:4.4.2`), under the same caveat as the Issue #18, #23, #161, #154, #156, #160, #159, #173, #178, #189, #148, #146, #203, #208, #206, #150 and #149 work.
