...
Against that bound the base configuration measures 0.416%, with a 95% confidence interval of 0.346% to 0.485% that spans it, and the ceilings are deliberately left as they are. Reaching that conclusion required abandoning the single 50-replication measurement this project had used as its calibration test. Died of wounds averages about one death per replication, and three independent 50-replication measurements of the same shipped configuration returned 0.348%, 0.400% and 0.499%: a spread wider than the confidence interval any one of them reported, which is why the figure above pools all three over 150 replications. An earlier single measurement of 0.617% had been read as an overshoot requiring the ceilings to be re-fitted; it is one draw from this spread and lies outside the pooled interval. The spread is understated in turn because replications are antithetically paired and every interval in this project divides by the replication count as though they were independent, which is recorded as a defect in its own right rather than absorbed here.

Two further consequences follow for how the bound is used. A test asking whether the model's interval spans the target is stricter than an upper bound supports, since it fails a model for sitting comfortably below a ceiling; the `moderate_intensity` profile does sit below, at 0.274% (95% confidence interval 0.216% to 0.331%), and is left there rather than having its ceilings raised, because lifting modelled mortality to meet a bound would add deaths the historical record does not evidence. And the ceilings remain entangled with the treatment efficacy factors, so agreement with the bound confirms the pair jointly and not either alone.
