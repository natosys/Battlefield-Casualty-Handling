...
| `scripts/check_replication_independence.R` | Regression check asserting that replications carry no positive correlation with one another, which is what would narrow every interval computed over them; exits non-zero on failure |
