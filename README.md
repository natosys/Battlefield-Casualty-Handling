...
µ\* spans 80-fold, from 0.1203 (`ot_hours`) to 9.6285 (`pri1_evac_prob`), with the top parameter about 11 times the median of 0.8707. σ exceeds µ\* for 51 of the 53, with σ/µ\* ratios running from 0.72 to 2.16 across the set, so for almost every parameter the size of its effect depends on what the others are set to.

The top ten by µ\* cover all three categories: seven Context, led by `pri1_evac_prob` and `pri1_surg_prob`; two Capacity (`r2b_hold_mode`, `short_resus_mode`); and one Policy, `fr_demand_interval_days`, 4th. That Policy result needs care. The reinforcement demand cycle ships disabled (`demand_interval_days = 0`), so its rank comes from the screen exercising it across 0 to 14 days, not from anything the shipped baseline does.

The reinforcement pipeline's three parameters spread further than any other mechanism's: `fr_demand_interval_days` 4th and `fr_fill_mode_frac` 20th against `fr_fulfillment_lag_days` 52nd of 53. The mass-casualty sizing trio spreads less but still unevenly, `mass_casualty_rate` 10th and `mass_casualty_max_cas` 12th against `mass_casualty_min_cas` at the median, 27th. Neither spread settles whether fill rate and lag belong under Policy (see "Reading the plots: colour coding" below). `post_surgery_prob` ranks 41st (µ\* 0.3662); classifying it Context rather than Policy changes only its label and plot colour, not its bounds.

The five lowest (`ot_hours` 53rd, `fr_fulfillment_lag_days` 52nd, `p2_t_mid` 51st, `r2e_postop_hold_penalty` 50th, and `post_op_hold_mode` 49th) also cover all three categories rather than clustering in one.
