...
> **Provenance (sustained-campaign defaults, Issue #312).** Every seed-42 figure below was re-measured after two shipped defaults changed: `evacuation_policy_days` from 30 to 21 and `force_regeneration.reinforcement.demand_interval_days` from 0 to 7. README [Return to Duty](README.md#return-to-duty) and [Force Regeneration and the Endogenous Feedback Loop](README.md#6-force-regeneration-and-the-endogenous-feedback-loop) record why each was changed.
>
...
|---|---|
| Total casualties (30 days) | 535 |
| WIA (combat + support) | 207 (combat 154 / support 53) |
| KIA (combat + support) | 107 (combat 81 / support 26) |
| DNBI (combat + support) | 221 (combat 196 / support 25) |
| Effective force size, end of run (seed 42) | combat: 2342 of 2500 initial (−6.3%); support: 1236 of 1250 initial (−1.1%), under the shipped 7-day reinforcement cycle. The pool is held far nearer establishment than an unreinforced run leaves it, which is the mechanism's purpose |
...
| Priority 1 share (seed 42) | 286 of 535 classified (53.5%) — P1 286 / P2 85 / P3 57 / KIA 107. Every generated casualty carries an assigned priority in this run |
...
| Mean DOW/run (150 reps pooled) | Treated-cohort DOW rate (casualties reaching R2B or R2E) 0.464% (95% CI [0.397%, 0.531%]) for the shipped `default` configuration, pooled across three independent 50-replication measurements returning 0.403%, 0.483% and 0.507%. This is the quantity the historical Ajax Bay bound of ~0.46% applies to, and the interval spans it. `moderate_intensity` measures 0.353% ([0.294%, 0.411%]) across 0.377%, 0.315% and 0.366%, and `high_intensity` 3.498% ([3.378%, 3.618%]) against the Okinawa target of 3.40%. `scripts/check_dow_calibration.R` passes for all three, so no ceiling was re-fitted for the defaults change. See README Further Development L22 |
...
| DNBI sub-types (seed 42) | battle_fatigue=60, disease=116, nbi=45 |
| bf_rtd (seed 42) | 57 |
| clinical_rtd (seed 42) | 115 (r1: 60, r2b: 40, r2e: 15) |
| total_rtd (seed 42) | 172, which is 32.1% of arrivals |
| Realised in-theatre share (seed 42) | 14.2% of 211 R2E dispositions retained in theatre under the shipped 21-day `evacuation_policy_days`. An output of the policy, not an input. The 50-replication figure is 11.6% (95% CI [10.9%, 12.2%]) over 179.4 dispositions per run, inside the 7.6%-42.1% historical range cited in README Return to Duty |
| Evacuation is severity-conditioned (seed 42) | Sorting the 211 dispositions into quartiles by drawn `recovery_to_duty_days` gives evacuation rates of 43.4% / 100% / 100% / 100% from shortest to longest recovery. An unconditioned draw would give the same rate in every quartile. The shorter 21-day policy lifts the first quartile's rate, more of the shortest-recovery casualties now falling beyond the threshold |
| Pre-flight critical ICU hold (seed 42) | 13 critical-route evacuees drew the ventilated pre-flight path (`critical_hold.ventilated_share = 0.15`), holding intensive care a mean of 69.2 h, median 25.8 h, p90 163 h. Pooled across 50 replications the hold measures 36.1 h mean, 24.6 h median, 38.6 h p90 over 631 ventilated holds. The mean sits above the median because a hold stretches only when the holding pool is full, which the 21-day policy makes rarer (README Further Development L17) |
| Welch ICU-queue CMA (10 reps × 90 days) | Final cross-replication CMA 0.342 with 14.2% of 2,159 increments decreasing, instantaneous mean queue peaking at 1.0. The queue roughly halves against the previous defaults, the 21-day evacuation policy freeing the holding beds that were blocking intensive care step-down. `WARM_UP_DAYS` remains 0; the basis for that classification at a sustained-campaign horizon is open work under Issue #312 |
| Surgical pathway split (seed 42) | 163 casualties operated on: 81 damage control (49.7%), 82 single-stage. By priority, Priority 1 130 operated / 76 damage control (58.5%, configured rate 55%), Priority 2 33 operated / 5 damage control (15.2%, configured 20%); no Priority 3 casualty reached theatre in this run. The 50-replication damage control share is 43.1% (95% CI [42.1%, 44.0%]) |
...
| R2B surgical decision point (seed 42) | 245 casualties reached it (69 operated at R2B, 176 bypassed to R2E) |
| R2B surgeries (seed 42) | 69 |
| R2E surgeries — first op (seed 42) | 124 |
| R2E surgeries — second op (seed 42) | 41. Only a damage control casualty whose abbreviated operation was performed at R2E returns for a second procedure |
| R2B pre-transport bypass (seed 42) | at-R2B hold-full bypass: 3 — an independent count from the R2B surgical decision point above |
| R2B OT bypass reason (seed 42) | at-R2B subset: team off-shift 79, OT busy/queued 25 |
| R2B pre-open hold (seed 42) | 4 casualties held forward for a section about to reopen, mean hold 41.3 minutes, longest 52.1, against the shipped 60-minute `r2b.surgery.pre_open_window_min`. The 50-replication figure is 6.0 held per run (95% CI [5.3, 6.6]) |
...
| R2E post-definitive care pathway (seed 42) | icu=46, hold=105 — 30% of operated casualties receive post-definitive care in an ICU bed, the remainder taking the degraded holding-bed fallback. The 50-replication ICU share is 39.3% (95% CI [36.3%, 42.3%]) |
| R2E post-op DOW rate — icu vs hold (seed 42) | 0/64 vs 0/48 (single-run). The saturated-ICU 90-day stress test from Issue #43, which confirmed the mechanism fires with the hold pathway riskier than the icu pathway, has not been re-run since |
| Role 4 demand (seed 42) | 181 strategic evacuation decisions (100 critical route, 81 standard); 177 boarded and reached Role 4 by day 30, 4 still queued at R2E; Role 4 peak occupancy 106.0 concurrent patients on day 30, the campaign's last day |
| Strategic AME actual performance (seed 42) | C-17A Globemaster III at 36 critical / 54 standard places; 177 evacuated at a mean wait of 2.3 days (p10-p90 0.0-6.0). The wait falls sharply against an unreinforced 30-day policy because the R2E holding pool is no longer saturated, so a casualty reaches the boarding queue rather than waiting upstream for a bed |
