...
for (team in 1:team_count) {
  env %>%
    add_resource(paste0("c_medic_1_t", team), 1) %>%
    add_resource(paste0("c_medic_2_t", team), 1) %>%
    add_resource(paste0("c_medic_3_t", team), 1) %>%
    add_resource(paste0("c_nurse_t", team), 1) %>%
    add_resource(paste0("c_doctor_t", team), 1)
}
  ...
  branch(
    ...
    trajectory("KIA Branch") %>%
      branch(
        option = function() get_attribute(env, "team"),
        continue = TRUE,
        treat_kia(1) %>% join(transport_kia()),
        treat_kia(2) %>% join(transport_kia())
      )
  )
...
# Automatically plot resource usage for each team
...
for (team in 1:team_count) {
  ...
  team_plot <- plot(resources, metric = "usage", clinical_resources_team) +
