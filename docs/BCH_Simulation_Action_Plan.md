...
| 314 | Make Role 4 demand configurable and reportable against a stated capacity | Medium | Medium | Open — partially advanced (PR #366) |
...
16a. **Issue 314** — Make Role 4 demand configurable and reportable against a stated capacity, without modelling the echelon. Partially advanced: the ward mapping is configured, a stay steps down from intensive care to a ward on the conservation contract the model already applies forward, and the step-down is calibrated to a Role 4 source rather than assumed, moving the seed-42 intensive care share from 75.6% to 25.4%. The `role4.capacity` block and its validation exist. Three tasks remain: wiring the establishment comparison into the analysis pipeline, which computes but never reports; the capacity line on the multi-run census plot; and re-measuring the issue's evidence table at the current defaults. Stage 2, Role 4 as a seized resource with back-pressure into theatre, stays out of scope on model-boundary grounds. — **Advanced by PR #366.**
...
UNBLOCKED (start now):
  ...
  #314 Role 4 demand against a stated capacity, partially advanced by PR #366.
       The ward composition is configured, calibrated and shipped; the
       establishment block exists and is validated. What remains is reporting:
       the shortfall function is computed and tested but called by nothing, so
       a planner never sees the comparison, and the multi-run census plot
       carries no capacity line. The issue's evidence table is also due a
       re-measurement at the current defaults.
